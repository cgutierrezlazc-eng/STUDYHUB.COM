...
export default function Messages({ conversationId, onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    const unsubConnection = wsService.onConnection((connected) => {
      ...
      if (activeConv) {
        ...
        pollRef.current = setInterval(() => loadMessages(activeConv), connected ? 30000 : 5000);
      }
    });
  ...
  const handleSend = async () => {
    ...
    if (textareaRef2.current) {
      textareaRef2.current.style.height = '36px';
    }
  ...
  const deleteMsg = async (msgId: string) => {
    if (!activeConv) return;
    try {
      await api.deleteMessage(activeConv, msgId);
      await loadMessages(activeConv);
    } catch (err: any) {
  ...
  const handleBlockUser = async () => {
    ...
    if (
      confirm(
        t('msg.confirmBlock').replace('{name}', `${otherUser.firstName} ${otherUser.lastName}`)
      )
    ) {
      try {
  ...
  const messageRequests = conversations.filter(
    (c) => c.type === 'message_request' && c.participants.some((p) => p.id !== user?.id)
  );
  ...
  const filteredFriends = friendsFilter
    ? friendsWithChatStatus.filter((f) =>
        `${f.firstName} ${f.lastName} ${f.username}`
          .toLowerCase()
          .includes(friendsFilter.toLowerCase())
      )
            ...
            <div className="msg-tab-bar">
              ...
              >
                ...
                {incomingRequests.length > 0 && (
                  <span className="msg-tab-badge">{incomingRequests.length}</span>
                )}
              </button>
            </div>
              ...
              <>
                ...
                {folders.length > 0 && (
                  <div className="msg-folders">
                    {folders.map((f) => (
                      <div key={f.id} className="msg-folder-label">
                        ...
                        >
                          <div className="msg-conv-avatar wa-avatar">
                            {conv.type === 'group_study' ? (
                              ...
                              <span>
                                {(other?.firstName?.[0] || conv.name?.[0] || '?').toUpperCase()}
                              </span>
                            )}
                          </div>
                          <div className="msg-conv-info">
                            <div className="msg-conv-name">
                              ...
                              {conv.name}
                              {conv.unreadCount > 0 && (
                                <span className="msg-badge">{conv.unreadCount}</span>
                              )}
                            </div>
                            <div className="msg-conv-preview">
                              {conv.lastMessage ? (
                                <>
                                  <span className="msg-conv-sender">
                          ...
                          <div className="msg-conv-preview">
                            ...
                            {conv.lastMessage && (
                              <>
                                {' '}
                                · {conv.lastMessage.sender?.firstName}:{' '}
                                {conv.lastMessage.content?.slice(0, 25)}
                              </>
                            )}
                          </div>
                        </div>
                  ...
                  <>
                    ...
                    {incomingRequests.map((conv) => {
                      ...
                      return (
                        <div
                          key={conv.id}
                          className="msg-conv-item msg-request-item"
                          style={{
                            flexDirection: 'column',
                            alignItems: 'stretch',
                            gap: 8,
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                                @{other?.username} {t('msg.wantsToMessage')}
                              </div>
                            </div>
                          </div>
                          {conv.lastMessage && (
                          ...
                          return (
                            <div key={conv.id} className="msg-conv-item" style={{ opacity: 0.7 }}>
                              ...
                              <div className="msg-conv-info">
                                ...
                                <div className="msg-conv-preview">
                                  {Hourglass({ size: 12 })} {t('msg.waitingResponse')}
                                </div>
                              </div>
                            </div>
                          );
              ...
              <>
                ...
                <div className="msg-chat-header wa-header">
                  <div
                    ...
                    onClick={() => otherParticipant && onNavigate(`/user/${otherParticipant.id}`)}
                  >
                    ...
                    <div>
                      ...
                      {activeConversation.type === 'group_study' ? (
                        <small>
                          {activeConversation.participants.length} {t('msg.members')}
                        </small>
                      ) : (
                  ...
                  <div className="wa-header-actions">
                    <button className="wa-icon-btn" title={t('msg.voiceCall')} disabled>
                      {Mic({ size: 16 })}
                    </button>
                    <button
                    ...
                    {showMsgSearch ? (
                      <input
                        ...
                        onKeyDown={(e) =>
                          e.key === 'Escape' && (setShowMsgSearch(false), setMsgSearch(''))
                        }
                      />
                    ) : null}
                    <button
                        ...
                        <div className="wa-dropdown-menu">
                          {otherParticipant && (
                            <>
                              <button
                                onClick={() => {
                                  onNavigate(`/user/${otherParticipant.id}`);
                                  setShowContactMenu(false);
                                }}
                              >
                ...
                <div
                  className="msg-messages wa-messages"
                ...
                >
                  {filteredMessages.map((msg, index) => {
                    ...
                    const isAudio =
                      (msg.documentName?.endsWith('.webm') && !msg.documentName?.match(/video/i)) ||
                      msg.content === '🎤 Mensaje de voz';
                    const isPhoto =
                      msg.documentName?.match(/\.(jpg|jpeg|png|gif)$/i) ||
                      msg.content === '📷 Foto';
                    const isVideo =
                      (msg.documentName?.match(/\.(webm|mp4|ogg)$/i) &&
                        msg.content !== '🎤 Mensaje de voz') ||
                      msg.content === 'Video';
                          ...
                          {msg.messageType === 'system' ? (
                            ...
                            <>
                              ...
                              {hoveredMsgId === msg.id && !msg.isDeleted && (
                                <div
                                  style={{
                                    ...
                                    zIndex: 10,
                                    marginRight: isMine ? 0 : 4,
                                    marginLeft: isMine ? 4 : 0,
                                  }}
                                >
                                  <button
                                    onClick={() =>
                              ...
                              {!isMine &&
                                activeConversation?.type === 'group_study' &&
                                !isGrouped && (
                                  <div className="msg-sender wa-sender">
                                ...
                                <button
                                  onClick={async (e) => {
                                    ...
                                    if (btn.dataset.translated) {
                                      ...
                                      const contentEl = btn.parentElement?.querySelector(
                                        '.msg-content'
                                      ) as HTMLElement;
                ...
                {activeConversation.type === 'message_request' && (
                  ...
                  >
                    <div style={{ flex: 1, fontSize: 13 }}>
                      {activeConversation.participants.find((p) => p.role === 'admin')?.id ===
                      user?.id ? (
                        <span style={{ color: 'var(--text-muted)' }}>
                          {Hourglass({ size: 14 })} {t('msg.waitingAccept')}
                        </span>
                      ) : (
                        <span>
                          <strong>{otherParticipant?.firstName}</strong> {t('msg.wantsToMessage')}
                        </span>
                      )}
                    </div>
                    {activeConversation.participants.find((p) => p.role === 'admin')?.id !==
                      ...
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button
                          className="btn btn-primary btn-sm"
                          onClick={() => acceptMessageRequest(activeConversation.id)}
                        >
                          {t('msg.accept')}
                        </button>
                        <button
                          className="btn btn-secondary btn-sm"
                          onClick={() => rejectMessageRequest(activeConversation.id)}
                        >
                          {t('msg.reject')}
                        </button>
                      </div>
                  ...
                  <div className="wa-photo-preview" style={{ flexDirection: 'column', gap: 10 }}>
                    ...
                    >
                      ...
                      <span style={{ fontSize: 12, color: '#92400e', lineHeight: 1.4 }}>
                        Los videos se eliminan automáticamente del servidor a las{' '}
                        <strong>72 horas</strong> de ser enviados. Descarga el video si quieres
                        conservarlo.
                      </span>
                    </div>
                    <div className="wa-photo-preview-actions">
                      <button className="btn btn-secondary btn-sm" onClick={cancelVideoRecording}>
                        Cancelar
                      </button>
                      <button
                        className="btn btn-primary btn-sm"
                ...
                {isRecordingVideo && (
                  <div className="wa-recording-bar">
                    <button
                      className="wa-icon-btn wa-cancel-rec"
                    ...
                    <div className="recording-indicator" style={{ flex: 1 }}>
                      ...
                      <span style={{ fontWeight: 600 }}>
                        🎥 {formatRecTime(videoRecordingTime)} / 0:30
                      </span>
                    </div>
                    <button
                      className="wa-icon-btn wa-send-rec"
                  ...
                  {isRecordingAudio ? (
                    ...
                    <>
                      ...
                      <div className="wa-actions-group">
                        ...
                        <button
                          className="wa-icon-btn wa-mic-btn"
                          onClick={startAudioRecording}
                          title={t('msg.voiceMessage')}
                        >
                          <svg
                            width="20"
                            height="20"
                ...
                {searchResults.map((u) => (
                  <div key={u.id} className="msg-user-result" onClick={() => startDirectChat(u)}>
                    <div className="msg-user-avatar">
                      {u.avatar ? <img src={u.avatar} alt="" /> : u.firstName?.[0]}
                    </div>
                    <div>
                      <div className="msg-user-name">
                        {u.firstName} {u.lastName}
                      </div>
                      <div className="msg-user-username">
                        @{u.username} #{String(u.userNumber).padStart(4, '0')}
                      </div>
                    </div>
                  </div>
                ))}
              </div>
              <button className="btn btn-secondary btn-sm" onClick={() => setShowNewChat(false)}>
                {t('newProject.cancel')}
              </button>
            </div>
          </div>
          ...
          <div className="modal-overlay" onClick={() => setShowNewGroup(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="auth-field">
                ...
                <input
                  placeholder={t('msg.groupNameExample')}
                  value={groupName}
        ...
        {showNewFolder && (
          <div className="modal-overlay" onClick={() => setShowNewFolder(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="auth-field">
                <input
                  placeholder={t('msg.folderNamePlaceholder')}
                  value={newFolderName}
