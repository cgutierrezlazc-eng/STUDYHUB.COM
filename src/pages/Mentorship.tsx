...
export default function Mentorship({ onNavigate }: Props) {
  ...
  const loadMentors = async () => {
    setLoading(true);
    try {
      setMentors(await api.getMentors(search || undefined));
    } catch (err: any) {
    ...
    <>
      <div className="page-header page-enter">
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
          ...
          <button
            className={`tab ${tab === 'become' ? 'active' : ''}`}
      ...
      <div className="page-body">
        {tab === 'find' && (
          <>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              ...
              />
              <button className="btn btn-primary" onClick={loadMentors}>
                Buscar
              </button>
            </div>
            {loading ? (
              ...
              <div className="empty-state" style={{ padding: 40 }}>
                <div className="empty-state-icon">{'\u{1F9ED}'}</div>
                <h3>No hay mentores disponibles</h3>
                <p>¡Sé el primero en ofrecer mentoría!</p>
              </div>
            ) : (
              <div
                style={{
              ...
              >
                {mentors.map((m: any) => (
                  <div key={m.id} className="u-card" style={{ padding: 20 }}>
                    ...
                    >
                      <div
                        onClick={() => onNavigate(`/user/${m.mentor?.id}`)}
                        style={{
                          width: 48,
                          height: 48,
                          borderRadius: '50%',
                      ...
                      >
                        {m.mentor?.avatar ? (
                          ...
                          m.mentor?.firstName?.[0] || '?'
                        )}
                      </div>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 15 }}>
                          {m.mentor?.firstName} {m.mentor?.lastName}{' '}
                          {m.mentor?.isGraduated && '\u{1F393}'}
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                          {m.mentor?.career} · {m.mentor?.university}
                        </div>
                      </div>
                    </div>
                    {m.bio && (
                      <p
                        style={{ fontSize: 13, color: 'var(--text-secondary)', margin: '0 0 8px' }}
                      >
                        {m.bio.slice(0, 120)}
                      </p>
                    )}
                    ...
                    >
                      <span>
                        {m.ratingCount > 0
                          ? `\u2B50 ${m.rating}/5 (${m.ratingCount})`
                          : 'Sin reseñas'}
                      </span>
                      <span>
                        {m.currentMentees}/{m.maxMentees} mentoreados
                      </span>
                    </div>
                    <button
                      className="btn btn-primary btn-sm"
                      style={{ width: '100%', marginTop: 12 }}
                      onClick={() => handleRequest(m.mentor?.id)}
                      disabled={m.currentMentees >= m.maxMentees}
                    >
        ...
        {tab === 'my' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {myMentors.length > 0 && (
              <div>
                <h3 style={{ fontSize: 15, marginBottom: 8 }}>Mis Mentores</h3>
                {myMentors.map((r: any) => (
                  <div
                    key={r.id}
                    className="u-card"
                  ...
                  >
                    ...
                    <span
                      style={{
                        ...
                        borderRadius: 12,
                        background:
                          r.status === 'active' ? 'rgba(5,150,105,0.08)' : 'var(--bg-tertiary)',
                        color: r.status === 'active' ? 'var(--accent-green)' : 'var(--text-muted)',
                      }}
                    >
                      {r.status === 'pending'
                        ? '\u23F3 Pendiente'
            ...
            {myMentees.length > 0 && (
              <div>
                <h3 style={{ fontSize: 15, marginBottom: 8 }}>Mis Mentoreados</h3>
                {myMentees.map((r: any) => (
                  <div
                    key={r.id}
                    className="u-card"
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                        {r.subject || 'General'}
                        {r.message ? ` — "${r.message.slice(0, 50)}"` : ''}
                      </div>
                    </div>
                    {r.status === 'pending' && (
                      <button className="btn btn-primary btn-xs" onClick={() => handleAccept(r.id)}>
                        Aceptar
                      </button>
                    )}
                    {r.status === 'active' && (
            ...
            {myMentors.length === 0 && myMentees.length === 0 && (
              <div className="empty-state" style={{ padding: 40 }}>
                <div className="empty-state-icon">{'\u{1F9ED}'}</div>
                <h3>No tienes mentorías activas</h3>
              </div>
            )}
          </div>
        )}
        {tab === 'become' && (
          <div className="u-card" style={{ padding: 24, maxWidth: 600 }}>
            ...
            <p style={{ color: 'var(--text-muted)', marginBottom: 16 }}>
              Comparte tu experiencia y ayuda a otros estudiantes. Ganas +50 XP por cada mentoría
              completada.
            </p>
            <div className="auth-field">
              <label>Materias que puedes mentorar *</label>
              <input
                value={subjects}
                onChange={(e) => setSubjects(e.target.value)}
                placeholder="Separadas por coma: Cálculo, Física, Programación"
              />
            </div>
            <div className="auth-field">
              ...
              <input
                ...
                placeholder="Ej: Lunes y Miércoles 3-5pm"
              />
            </div>
            <div className="auth-field">
              <label>Sobre ti como mentor</label>
              <textarea
                value={bio}
                onChange={(e) => setBio(e.target.value)}
                placeholder="¿Por qué eres buen mentor? ¿Cuál es tu enfoque?"
                style={{
                  width: '100%',
                  minHeight: 80,
                  resize: 'vertical',
