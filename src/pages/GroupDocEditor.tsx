    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', gap: 16 }}>
        ...
        <button
          ...
          style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#2D62C8', color: '#fff', cursor: 'pointer' }}
        >
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {/* Color dot + title */}
        <div style={{
          width: 10, height: 10, borderRadius: '50%', background: doc?.color || '#2D62C8', flexShrink: 0,
        }} />
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, overflow: 'auto', background: 'var(--bg-primary)' }}>
          <Suspense fallback={<div style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>Cargando editor...</div>}>
            <CollabEditor
              content={doc?.content || ''}
            ...
            {showMembers && (
              <div style={{ flex: 1, overflow: 'auto', padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  ...
                  <button onClick={() => setShowMembers(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 2 }}>
                    {X({ size: 16 })}
                  </button>
                </div>
                    ...
                    {searchResults.length > 0 && (
                      <div style={{
                        ...
                        {searchResults.map((r: any) => (
                          ...
                          >
                            <div style={{
                              width: 28, height: 28, borderRadius: '50%', background: '#2D62C8',
                              display: 'flex', alignItems: 'center', justifyContent: 'center',
                              color: '#fff', fontSize: 11, fontWeight: 700, flexShrink: 0, overflow: 'hidden',
                            }}>
                              {r.avatar ? <img src={r.avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (r.firstName?.[0] || '?').toUpperCase()}
                            </div>
                            <div>
                              <div style={{ fontWeight: 600 }}>{r.firstName} {r.lastName}</div>
                              <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>@{r.username}</div>
                            </div>
                            <span style={{ marginLeft: 'auto', color: '#38A169' }}>{Plus({ size: 14 })}</span>
                          </button>
                        ))}
                      </div>
                    )}
                  ...
                  {doc?.members?.map((m: any) => (
                    <div key={m.id} style={{
                      ...
                      <div style={{
                        ...
                        {m.user?.avatar ? <img src={m.user.avatar} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (m.user?.firstName?.[0] || '?').toUpperCase()}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          {m.role === 'owner' && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2 }}>{Crown({ size: 10 })} Creador</span>}
            ...
            {showVersions && !showMembers && (
              <div style={{ flex: 1, overflow: 'auto', padding: 16 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                  ...
                  <button onClick={() => setShowVersions(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', padding: 2 }}>
                    {X({ size: 16 })}
                  </button>
                </div>
                ...
                {versions.length === 0 ? (
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', textAlign: 'center', padding: 20 }}>
                    Sin versiones guardadas. Usa el boton "Version" para crear un punto de restauracion.
                  </p>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {versions.map(v => (
                      <div key={v.id} style={{
                        padding: '10px 12px', borderRadius: 8, background: 'var(--bg-primary)',
                          ...
                          <button
                            onClick={async () => {
                              try {
                                ...
                                if (confirm('Restaurar esta version? El contenido actual se reemplazara.')) {
                        ...
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
                          {v.createdBy?.firstName} {v.createdBy?.lastName} &middot; {new Date(v.createdAt).toLocaleString('es-CL')}
                        </div>
                      </div>
                    ))}
                  </div>
