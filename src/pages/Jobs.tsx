...
const LANG_LEVELS_KEYS = [
  'jobs.langBasic',
  'jobs.langIntermediate',
  'jobs.langAdvanced',
  'jobs.langNative',
];
const CV_LANGUAGES = [
...
const EXP_LEVEL_KEYS: Record<string, string> = {
  entry: 'jobs.expJunior',
  mid: 'jobs.expMid',
  senior: 'jobs.expSenior',
  any: 'jobs.expAny',
};
...
const cvStyles: Record<string, React.CSSProperties> = {
  ...
  addBtn: {
    ...
    color: 'var(--accent, #2D62C8)',
    border: '1px dashed var(--accent, #2D62C8)',
    borderRadius: 10,
    fontSize: 13,
    fontWeight: 600,
    cursor: 'pointer',
...
export default function Jobs({ onNavigate }: Props) {
  ...
  const [localApps, setLocalApps] = useState<
    Record<string, { jobTitle: string; companyName: string; date: string; status: string }>
  >(() => {
    try {
      return JSON.parse(localStorage.getItem('conniku_applications') || '{}');
    } catch {
      return {};
    }
  });
  ...
  const getMatchScore = (job: any): { pct: number; label: string; icon: string } => {
    const userKeywords = [
      user?.career,
      ...(cv.competencies || []),
      ...(cv.skillGroups?.flatMap((g) => g.skills.map((s) => s.name)) || []),
    ]
      .filter(Boolean)
      .map((k) => (k as string).toLowerCase());
    if (userKeywords.length === 0) return { pct: 0, label: '', icon: '' };
    const jobText =
      `${job.jobTitle} ${job.description || ''} ${job.requirements || ''} ${job.careerField || ''}`.toLowerCase();
  ...
  const loadMyApps = async () => {
    try {
      setMyApps(await api.getMyApplications());
    } catch (err: any) {
  ...
  const loadMyMatches = async () => {
    ...
    try {
      setMyMatches(await api.getMyJobMatches());
    } catch (err: any) {
  ...
  const loadTutorings = async () => {
    try {
      ...
      console.error('Failed to load tutorings:', err);
    }
  };
  ...
  const loadPublicCVs = async (q?: string) => {
    ...
    try {
      ...
      console.error('Failed to load public CVs:', err);
    }
    setCvLoading(false);
  };
  ...
  const handleDownloadCVPDF = () => {
    const name =
      `${user?.firstName || ''} ${user?.lastName || ''}`.trim() || t('jobs.defaultProfessional');
...
${cv.competencies.length ? `<h2>${t('jobs.pdfCompetencies')}</h2><div class="skills-list">${cv.competencies.map((c) => `<span class="skill-tag">${c}</span>`).join('')}</div>` : ''}
  ...
  function addSkillGroup() {
    updateCV('skillGroups', [...cv.skillGroups, { category: '', skills: [] }]);
  }
  ...
  function addLanguage() {
    updateCV('languages', [...cv.languages, { name: '', level: 'Intermedio' }]);
  }
  ...
  function addDifferentiator() {
    updateCV('differentiators', [...cv.differentiators, '']);
  }
  ...
  const handleCreateTutoring = async () => {
    if (!tutoringForm.subject) {
      alert(t('jobs.subjectRequired2'));
      return;
    }
    try {
  ...
  const handleRegisterRecruiter = async () => {
    if (
      ...
      !recruiterForm.corporate_email ||
      !recruiterForm.recruiter_title
    ) {
      alert(t('jobs.fillRequired'));
      return;
    }
    try {
  ...
  const handleApply = async (jobId: string) => {
    try {
      await api.applyToJob(jobId, {
        ...
        resume_url: careerStatus?.resumeUrl,
      });
      setShowApply(false);
      setCoverLetter('');
  ...
  const runCvCoach = async () => {
    if (coachText.trim().length < 30) {
      setCoachError(t('jobs.cvMinChars'));
      return;
    }
    ...
    try {
      ...
      setCoachError(err.message || t('jobs.errorAnalyzing'));
    } finally {
      ...
      <div>
            ...
            <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              {totalExp > 0 && (
                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)' }}>
                  💼 {totalExp} experiencia{totalExp !== 1 ? 's' : ''}
                </span>
              )}
              {totalEdu > 0 && (
                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)' }}>
        ...
        {cvUploadMsg && (
          <div
            style={{
              padding: '10px 16px',
              borderRadius: 10,
              background: 'rgba(45,98,200,0.08)',
              color: '#2D62C8',
              fontSize: 13,
              marginBottom: 16,
            }}
        ...
        <div className="card" style={{ padding: 20, marginBottom: 16 }}>
            ...
            {(
              ...
              <label
                ...
                style={{
                  ...
                  borderRadius: 10,
                  background:
                    cv.visibility === opt.value ? 'rgba(45,98,200,0.1)' : 'var(--bg-secondary)',
                  border:
                    cv.visibility === opt.value
                      ? '1px solid rgba(45,98,200,0.3)'
              ...
              >
                <input
                  type="radio"
                  name="cv-visibility"
                  checked={cv.visibility === opt.value}
                  onChange={() => {
                    setCv((prev) => ({ ...prev, visibility: opt.value }));
                    api.updateCV({ visibility: opt.value }).catch(() => {});
                  }}
                  style={{ accentColor: '#2D62C8' }}
                />
                {opt.label}
              </label>
            ))}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <button
              onClick={() => {
                const next = !cv.openToOffers;
                setCv((prev) => ({ ...prev, openToOffers: next }));
                api.updateCV({ open_to_offers: next }).catch(() => {});
              }}
              style={{
                width: 48,
                height: 26,
          ...
          <div className="card" style={{ padding: 20, marginBottom: 16 }}>
              ...
              <div>
                ...
                {cv.experience.slice(0, 2).map((exp: any, i: number) => (
                  <div
                    ...
                    style={{
                      marginBottom: 10,
                      paddingLeft: 10,
                      borderLeft: '3px solid rgba(45,98,200,0.3)',
                    }}
                  >
                ...
                {cv.experience.length > 2 && (
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
                    +{cv.experience.length - 2} experiencias más
                  </div>
                )}
              </div>
            )}
            ...
            <button
              ...
              style={{
                ...
                padding: '8px 18px',
                borderRadius: 8,
                border: '1px solid rgba(45,98,200,0.3)',
                background: 'transparent',
                color: '#2D62C8',
                fontWeight: 600,
                fontSize: 13,
        ...
        {showRecruiterModal && (
          <div className="modal-overlay" onClick={() => setShowRecruiterModal(false)}>
            <div
              ...
              style={{ maxWidth: 550, maxHeight: '80vh', overflowY: 'auto' }}
            >
                  ...
                  {recruiterList.map((r: any, idx: number) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {(r.headline || r.recruiterTitle) && (
                          <div style={{ fontSize: 12, color: '#2D62C8' }}>
                            {r.headline || r.recruiterTitle}
                          </div>
                        )}
                      </div>
                      <button
                        ...
                        onClick={() => {
                          ...
                          handleSendCVToRecruiter(r.userId || r.id);
                        }}
                      >
                        {t('jobs.send')}
                      </button>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 16 }}>
                <button className="btn btn-secondary" onClick={() => setShowRecruiterModal(false)}>
                  {t('jobs.close')}
                </button>
              </div>
            </div>
      ...
      <div className="page-header page-enter">
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
            ...
            >
              {careerStatus?.isOpenToOpportunities ? (
                <>
                  {Sparkles()} {t('jobs.exploringOpportunities')}
                </>
              ) : (
                <>
                  {SearchIcon()} {t('jobs.activateVisibility')}
                </>
              )}
            </button>
        ...
        >
          {TAB_CONFIG_KEYS.map((tc) => (
            <button
              key={tc.key}
              className={`tab ${tab === tc.key ? 'active' : ''}`}
              onClick={() => handleTabChange(tc.key)}
              style={{ flexShrink: 0 }}
            >
              {tc.icon({ size: 14 })} {t(tc.labelKey)}
              {tc.key === 'my-apps' && localAppCount > 0 ? ` (${localAppCount})` : ''}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="page-body">
        ...
        {(tab === 'listings' || tab === 'candidates') && (
          <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
            <input
              ...
              onKeyDown={(e) =>
                e.key === 'Enter' && (tab === 'listings' ? loadJobs() : loadCandidates())
              }
            ...
            <button
              ...
              onClick={() => (tab === 'listings' ? loadJobs() : loadCandidates())}
            >
              {t('jobs.searchBtn')}
            </button>
          </div>
        ...
        {tab === 'cvs' && (
          <div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
              ...
              />
              <button className="btn btn-primary" onClick={() => loadPublicCVs(cvSearch)}>
                {SearchIcon({ size: 14 })} {t('jobs.searchBtn')}
              </button>
            </div>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
                    ...
                    <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start' }}>
                      <div
                        style={{
                          ...
                          borderRadius: '50%',
                          flexShrink: 0,
                          background: cv.avatar
                            ? `url(${cv.avatar}) center/cover`
                            : 'linear-gradient(135deg, #2D62C8, #5B8DEF)',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {cv.experience && cv.experience.length > 0 && (
                          ...
                          >
                            {Briefcase({ size: 12 })} {cv.experience[0].title} en{' '}
                            {cv.experience[0].company}
                            {cv.experience.length > 1 && (
                              <span> · +{cv.experience.length - 1} mas</span>
                            )}
                          </div>
                        )}
                      </div>
        ...
        {tab === 'cv-coach' && (
          <div style={{ maxWidth: 720 }}>
            <div className="u-card" style={{ marginBottom: 16 }}>
              ...
              {coachError && (
                <p style={{ color: 'var(--accent-red, #dc2626)', fontSize: 13, marginBottom: 12 }}>
                  {coachError}
                </p>
              )}
              <button
              ...
              >
                {coachLoading ? (
                  <>
                    {RefreshCw({ size: 14 })} {t('jobs.analyzingCv')}
                  </>
                ) : (
                  <>
                    {Zap({ size: 14 })} {t('jobs.analyzeCv')}
                  </>
                )}
              </button>
            </div>
                ...
                <div className="u-card" style={{ textAlign: 'center' }}>
                  <div
                    style={{
                      ...
                      lineHeight: 1,
                      color:
                        (coachResult.score || 0) >= 70
                          ? '#16a34a'
                          : (coachResult.score || 0) >= 40
                            ? '#f59e0b'
                            : '#dc2626',
                    }}
                  >
                  ...
                  <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      >
                        ...
                        >
                          {t(
                            JOB_TYPE_KEYS.find((jt) => jt.value === job.jobType)?.labelKey || ''
                          ) || job.jobType}
                        </span>
                        {job.location && (
                          <span style={{ color: 'var(--text-muted)' }}>
                            {Map({ size: 12 })} {job.location}
                          </span>
                        )}
                        {job.isRemote && (
                          <span style={{ color: 'var(--accent-green)' }}>
                            {Globe({ size: 12 })} {t('jobs.remote')}
                          </span>
                        )}
                        ...
                        <span style={{ color: 'var(--text-muted)' }}>
                          {t(EXP_LEVEL_KEYS[job.experienceLevel] || '')}
                        </span>
                        {(() => {
                          const m = getMatchScore(job);
                          return m.pct > 0 ? (
                            <span
                              style={{
                                color:
                    ...
                    {job.applied || localApps[job.id] ? (
                      ...
                      >
                        {CheckCircle({ size: 12 })} Postulacion enviada
                      </span>
                    ) : (
                      <button
                        className="btn btn-primary btn-sm"
                      ...
                      >
                        {Zap({ size: 12 })} Postulacion Rapida
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>
                      ...
                      {c.user?.isGraduated && (
                        <span title="Graduado" style={{ marginLeft: 4 }}>
                          {GraduationCap({ size: 14 })}
                        </span>
                      )}
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                      {c.user?.career} — {c.user?.university}
                    </div>
                  </div>
                </div>
                {c.headline && (
                    ...
                    {myApps.map((a: any) => (
                      ...
                      >
                        ...
                        >
                          {a.status === 'pending' ? (
                            <>
                              {Hourglass({ size: 12 })} {t('jobs.statusPending')}
                            </>
                          ) : a.status === 'reviewed' ? (
                            <>
                              {Eye({ size: 12 })} {t('jobs.statusReviewed')}
                            </>
                          ) : a.status === 'interview' ? (
                            <>
                              {Target({ size: 12 })} {t('jobs.statusInterview')}
                            </>
                          ) : a.status === 'accepted' ? (
                            <>
                              {CheckCircle({ size: 12 })} {t('jobs.statusAccepted')}
                            </>
                          ) : (
                            <>
                              {XCircle({ size: 12 })} {t('jobs.statusRejected')}
                            </>
                          )}
                        </span>
                      </div>
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            borderRadius: 12,
                            fontWeight: 600,
                            background: (STATUS_COLORS[app.status] || STATUS_COLORS.Enviada).bg,
                            color: (STATUS_COLORS[app.status] || STATUS_COLORS.Enviada).color,
                          }}
                        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div
              style={{
                ...
                border: '1.5px solid rgba(45,98,200,0.2)',
                borderRadius: 16,
                padding: 20,
                display: 'flex',
                alignItems: 'flex-start',
            ...
            >
              ...
              <div>
                <div
                  style={{ fontWeight: 700, fontSize: 15, color: 'var(--accent)', marginBottom: 4 }}
                >
                ...
                >
                  Cuando una empresa publica una oferta, el sistema analiza tu perfil académico y
                  profesional y, si hay compatibilidad, te presenta automáticamente al reclutador.
                  Aquí puedes ver las oportunidades donde ya fuiste sugerido. Para aparecer en más
                  matchings, activa <strong>«Abierto a oportunidades»</strong> en tu perfil
                  profesional.
                </p>
              </div>
            </div>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      position: 'relative',
                      border:
                        m.status === 'interested' ? '1.5px solid rgba(5,150,105,0.3)' : undefined,
                    }}
                  >
                    {/* Conniku badge */}
                    <div
                      ...
                      >
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setMyMatches((prev) =>
                              prev.map((x) =>
                                x.matchId === m.matchId ? { ...x, status: 'interested' } : x
                              )
                            );
                          }}
                        >
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            setMyMatches((prev) =>
                              prev.map((x) =>
                                x.matchId === m.matchId ? { ...x, status: 'declined' } : x
                              )
                            );
                          }}
                        >
        ...
        {tab === 'tutoring' && (
          <>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  ...
                  <h3 style={{ margin: 0, fontSize: 18, color: '#92400E' }}>
                    {t('jobs.verifiedTutors')}
                  </h3>
                </div>
                <p style={{ margin: 0, fontSize: 14, color: '#78350F' }}>
                  {t('jobs.verifiedTutorsDesc')}
                </p>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
              <button className="btn btn-primary btn-sm" onClick={() => setShowTutoringForm(true)}>
                {t('jobs.offerTutoring')}
              </button>
            </div>
            {tutorings.length === 0 ? (
              <div className="empty-state" style={{ padding: 40 }}>
                  ...
                  <div key={tut.id} className="u-card hover-lift" style={{ padding: 20 }}>
                    ...
                    >
                      ...
                      >
                        {tut.tutor?.avatar ? (
                          ...
                          tut.tutor?.firstName?.[0] || '?'
                        )}
                      </div>
                      <div>
                        <div style={{ fontWeight: 600, fontSize: 14 }}>
                          {tut.tutor?.firstName} {tut.tutor?.lastName}
                          {tut.tutor?.isGraduated && (
                            <span title="Graduado" style={{ marginLeft: 4 }}>
                              {GraduationCap({ size: 14 })}
                            </span>
                          )}
                          {tut.tutor?.isSeniorYear && !tut.tutor?.isGraduated && (
                            <span title="Ultimo ano" style={{ marginLeft: 4 }}>
                              {BookOpen({ size: 14 })}
                            </span>
                          )}
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                          {tut.tutor?.career} · {tut.tutor?.university}
                        </div>
                      </div>
                    </div>
                    ...
                    >
                      ...
                      >
                        ...
                        ) : tut.modality === 'presencial' ? (
                          <>
                            {Map({ size: 12 })} {t('jobs.presencial')}
                          </>
                        ) : (
                          <>
                            {RefreshCw({ size: 12 })} {t('jobs.hybrid')}
                          </>
                        )}
                      </span>
                      <span
                      ...
                      {tut.isFree ? (
                        ...
                        <span style={{ color: 'var(--accent-green)', fontWeight: 600 }}>
                          {tut.currency} {tut.pricePerHour}/hr
                        </span>
                      ) : null}
                      {tut.freeTrial && (
                        <span style={{ color: 'var(--accent-blue)' }}>
                          {t('jobs.freeTrialClass')}
                        </span>
                      )}
                    </div>
                    {tut.description && (
                    ...
                    {tut.ratingCount > 0 && (
                      <div style={{ fontSize: 12, color: 'var(--accent-orange)', marginBottom: 8 }}>
                        {Star({ size: 12 })} {tut.rating}/5 ({tut.ratingCount} {t('jobs.reviews')})
                        · {tut.totalSessions} {t('jobs.sessions')}
                      </div>
                    )}
                    <button
                      className="btn btn-primary btn-sm"
                      style={{ width: '100%' }}
                      onClick={() => handleRequestTutoring(tut.id)}
                    >
            ...
            {recruiterProfile ? (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 16 }}>
                  ...
                  <div>
                    ...
                    <p style={{ margin: 0, color: 'var(--text-muted)', fontSize: 13 }}>
                      {recruiterProfile.recruiterTitle}
                    </p>
                  </div>
                  <span
                    style={{
                  ...
                  >
                    {recruiterProfile.verificationStatus === 'verified' ? (
                      <>
                        {CheckCircle({ size: 12 })} {t('jobs.verified')}
                      </>
                    ) : recruiterProfile.verificationStatus === 'rejected' ? (
                      <>
                        {XCircle({ size: 12 })} {t('jobs.rejected')}
                      </>
                    ) : (
                      <>
                        {Hourglass({ size: 12 })} {t('jobs.underReview')}
                      </>
                    )}
                  </span>
                </div>
                ...
                >
                  {recruiterProfile.industry && (
                    <div>
                      ...
                      <div>{recruiterProfile.industry}</div>
                    </div>
                  )}
                  {recruiterProfile.companySize && (
                    <div>
                      ...
                      <div>
                        {recruiterProfile.companySize} {t('jobs.employees')}
                      </div>
                    </div>
                  )}
                  {recruiterProfile.city && (
                    <div>
                      ...
                      <div>
                        {recruiterProfile.city}, {recruiterProfile.country}
                      </div>
                    </div>
                  )}
                  {recruiterProfile.companyWebsite && (
                    <div>
                      ...
                      <div>{recruiterProfile.companyWebsite}</div>
                    </div>
                  )}
                </div>
                {recruiterProfile.verificationStatus === 'verified' && (
                  <div style={{ marginTop: 16 }}>
                    <button className="btn btn-primary" onClick={() => setTab('candidates')}>
                      {t('jobs.searchCandidates')}
                    </button>
                  </div>
                )}
              </div>
              ...
              <div className="u-card hover-lift" style={{ padding: 24 }}>
                <h3 style={{ marginTop: 0 }}>
                  {BuildingIcon()} {t('jobs.registerAsRecruiter')}
                </h3>
                <p style={{ color: 'var(--text-muted)', marginBottom: 16 }}>
                  {t('jobs.registerRecruiterDesc')}
                </p>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div className="auth-field">
                    <label>{t('jobs.companyRequired')}</label>
                    <input
                      value={recruiterForm.company_name}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, company_name: e.target.value })
                      }
                      placeholder={t('jobs.companyPlaceholder')}
                    />
                  </div>
                  <div className="auth-field">
                    <label>{t('jobs.corporateEmail')}</label>
                    <input
                      value={recruiterForm.corporate_email}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, corporate_email: e.target.value })
                      }
                      placeholder="rh@empresa.com"
                    />
                  </div>
                  <div className="auth-field">
                    <label>{t('jobs.yourTitle')}</label>
                    <input
                      value={recruiterForm.recruiter_title}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, recruiter_title: e.target.value })
                      }
                      placeholder="Ej: HR Manager"
                    />
                  </div>
                  <div className="auth-field">
                    <label>{t('jobs.website')}</label>
                    <input
                      value={recruiterForm.company_website}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, company_website: e.target.value })
                      }
                      placeholder="https://empresa.com"
                    />
                  </div>
                  <div className="auth-field">
                    <label>{t('jobs.industryLabel')}</label>
                    <input
                      value={recruiterForm.industry}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, industry: e.target.value })
                      }
                      placeholder={t('jobs.industryLabel')}
                    />
                  </div>
                  <div className="auth-field">
                    <label>{t('jobs.companySizeLabel')}</label>
                    <select
                      value={recruiterForm.company_size}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, company_size: e.target.value })
                      }
                      style={{
                        width: '100%',
                        padding: 10,
                  ...
                  <div className="auth-field">
                    <label>{t('jobs.taxId')}</label>
                    <input
                      value={recruiterForm.tax_id}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, tax_id: e.target.value })
                      }
                      placeholder={t('jobs.taxIdPlaceholder')}
                    />
                  </div>
                  <div className="auth-field">
                  ...
                  <div className="auth-field">
                    <label>{t('jobs.country')}</label>
                    <input
                      value={recruiterForm.country}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, country: e.target.value })
                      }
                  ...
                  <div className="auth-field">
                    <label>{t('jobs.linkedin')}</label>
                    <input
                      value={recruiterForm.linkedin_url}
                      onChange={(e) =>
                        setRecruiterForm({ ...recruiterForm, linkedin_url: e.target.value })
                      }
                ...
                <div className="auth-field">
                  <label>{t('jobs.companyDescription')}</label>
                  <textarea
                    value={recruiterForm.company_description}
                    onChange={(e) =>
                      setRecruiterForm({ ...recruiterForm, company_description: e.target.value })
                    }
                    placeholder={t('jobs.companyDescPlaceholder')}
                    style={{
                      width: '100%',
                      minHeight: 60,
                      resize: 'vertical',
                ...
                <button
                  className="btn btn-primary"
                  style={{ marginTop: 16 }}
                  onClick={handleRegisterRecruiter}
                >
          ...
          <div className="modal-overlay" onClick={() => setShowQuickApply(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <p style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>
                Postular a {selectedJob.jobTitle} en {selectedJob.companyName}?
              </p>
              <div
                style={{
                  background: 'var(--bg-secondary)',
              ...
              >
                ...
                {user?.university && (
                  <div>
                    <strong>Universidad:</strong> {user.university}
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                <button className="btn btn-secondary" onClick={() => setShowQuickApply(false)}>
                  Cancelar
                </button>
                <button
                  className="btn btn-primary"
                  onClick={() => {
                    ...
                    saveLocalApp(selectedJob);
                    setShowQuickApply(false);
                  }}
                >
                  {Zap({ size: 14 })} Confirmar Postulacion
                </button>
              </div>
            </div>
          </div>
        ...
        {showApply && selectedJob && (
          <div className="modal-overlay" onClick={() => setShowApply(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              <h3>
                {t('jobs.applyTo')} {selectedJob.jobTitle}
              </h3>
              <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
        ...
        {showPostForm && (
          <div className="modal-overlay" onClick={() => setShowPostForm(false)}>
            ...
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div className="auth-field">
                  <label>{t('jobs.companyLabel2')}</label>
                  <input
                ...
                <div className="auth-field">
                  <label>{t('jobs.positionRequired')}</label>
                  <input
                    value={form.job_title}
                    onChange={(e) => setForm({ ...form, job_title: e.target.value })}
                    placeholder={t('jobs.positionPlaceholder2')}
                  />
                </div>
                <div className="auth-field">
                  ...
                  >
                    {JOB_TYPE_KEYS.filter((jt) => jt.value).map((jt) => (
                      <option key={jt.value} value={jt.value}>
                        {t(jt.labelKey)}
                      </option>
                    ))}
                ...
                <div className="auth-field">
                  <label>{t('jobs.locationField')}</label>
                  <input
                    value={form.location}
                ...
                <div className="auth-field">
                  ...
                  <input
                    ...
                    placeholder={t('jobs.salaryMinPlaceholder')}
                  />
                </div>
                <div className="auth-field">
                  ...
                  <input
                    ...
                    placeholder={t('jobs.salaryMaxPlaceholder')}
                  />
                </div>
                <div className="auth-field">
                  <label>{t('jobs.careerField')}</label>
                  <input
                    value={form.career_field}
                    onChange={(e) => setForm({ ...form, career_field: e.target.value })}
                    placeholder={t('jobs.careerFieldPlaceholder')}
                  />
                </div>
                <div className="auth-field">
                  <label>{t('jobs.contactEmailField')}</label>
                  <input
              ...
              <div className="auth-field">
                <label>{t('jobs.jobDescription')}</label>
                <textarea
              ...
              >
                <input
                  type="checkbox"
                  checked={form.konni_broadcast}
                  onChange={(e) => setForm({ ...form, konni_broadcast: e.target.checked })}
                  style={{
                    marginTop: 3,
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>
                    Todos los usuarios recibirán esta oferta como mensaje en su chat de Konni
                    Soporte.
                  </div>
                </div>
              </label>
          ...
          <div className="modal-overlay" onClick={() => setSelectedJob(null)}>
            ...
            >
              ...
              >
                ...
                >
                  {t(JOB_TYPE_KEYS.find((jt) => jt.value === selectedJob.jobType)?.labelKey || '')}
                </span>
                {selectedJob.location && (
                  <span>
                    {Map({ size: 13 })} {selectedJob.location}
                  </span>
                )}
                {selectedJob.isRemote && (
                  <span style={{ color: 'var(--accent-green)' }}>
                    {Globe({ size: 13 })} {t('jobs.remote')}
                  </span>
                )}
                {formatSalary(
              ...
              <div
                style={{ fontSize: 14, lineHeight: 1.6, whiteSpace: 'pre-wrap', marginBottom: 16 }}
              >
                {selectedJob.description}
              </div>
              {selectedJob.requirements && (
                <div style={{ marginBottom: 16 }}>
                  <h4 style={{ fontSize: 14, marginBottom: 4 }}>{t('jobs.requirements')}</h4>
                  <div
                    style={{ fontSize: 13, whiteSpace: 'pre-wrap', color: 'var(--text-secondary)' }}
                  >
                    {selectedJob.requirements}
                  </div>
                </div>
              ...
              {selectedJob.benefits && (
                <div style={{ marginBottom: 16 }}>
                  <h4 style={{ fontSize: 14, marginBottom: 4 }}>{t('jobs.benefits')}</h4>
                  <div
                    style={{ fontSize: 13, whiteSpace: 'pre-wrap', color: 'var(--text-secondary)' }}
                  >
                    {selectedJob.benefits}
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                ...
                {!selectedJob.applied && !localApps[selectedJob.id] ? (
                  <button className="btn btn-primary" onClick={() => setShowQuickApply(true)}>
                    {Zap({ size: 14 })} Postulacion Rapida
                  </button>
                ) : (
                  <span style={{ fontSize: 13, color: 'var(--accent-green)', fontWeight: 600 }}>
                    {CheckCircle({ size: 14 })} Postulacion enviada
                  </span>
                )}
              </div>
            </div>
        ...
        {showTutoringForm && (
          <div className="modal-overlay" onClick={() => setShowTutoringForm(false)}>
            ...
            >
              ...
              <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
                {t('jobs.shareKnowledge')}
              </p>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div className="auth-field">
                  <label>{t('jobs.subjectRequired')}</label>
                  <input
                    value={tutoringForm.subject}
                    onChange={(e) => setTutoringForm({ ...tutoringForm, subject: e.target.value })}
                    placeholder={t('jobs.subjectPlaceholder')}
                  />
                </div>
                <div className="auth-field">
                  <label>{t('jobs.categoryLabel')}</label>
                  <select
                    value={tutoringForm.category}
                    onChange={(e) => setTutoringForm({ ...tutoringForm, category: e.target.value })}
                    style={{
                      width: '100%',
                      padding: 10,
                      borderRadius: 8,
                  ...
                  >
                    <option value="">{t('jobs.selectOption')}</option>
                    <option value="ciencias">{t('jobs.catSciences')}</option>
                    <option value="ingenieria">{t('jobs.catEngineering')}</option>
                    <option value="humanidades">{t('jobs.catHumanities')}</option>
                    <option value="tecnologia">{t('jobs.catTech')}</option>
                    <option value="idiomas">{t('jobs.catLanguages')}</option>
                    <option value="negocios">{t('jobs.catBusiness')}</option>
                    <option value="artes">{t('jobs.catArts')}</option>
                    <option value="salud">{t('jobs.catHealth')}</option>
                  </select>
                </div>
                <div className="auth-field">
                  <label>{t('jobs.modality')}</label>
                  <select
                    value={tutoringForm.modality}
                    onChange={(e) => setTutoringForm({ ...tutoringForm, modality: e.target.value })}
                    style={{
                      width: '100%',
                      padding: 10,
                      borderRadius: 8,
                  ...
                  >
                    ...
                    <option value="presencial">{t('jobs.presencial')}</option>
                    <option value="hybrid">{t('jobs.hybrid')}</option>
                  </select>
                </div>
                <div className="auth-field">
                  <label>{t('jobs.sessionDuration')}</label>
                  <select
                    value={tutoringForm.session_duration}
                    onChange={(e) =>
                      setTutoringForm({ ...tutoringForm, session_duration: Number(e.target.value) })
                    }
                    style={{
                      width: '100%',
                      padding: 10,
                ...
                <div className="auth-field">
                  <label>{t('jobs.pricePerHour')}</label>
                  <input
                    type="number"
                    value={tutoringForm.price_per_hour}
                    onChange={(e) =>
                      setTutoringForm({ ...tutoringForm, price_per_hour: e.target.value })
                    }
                    placeholder={t('jobs.freePricePlaceholder')}
                  />
                </div>
                <div
                  className="auth-field"
                ...
                >
                  <input
                    type="checkbox"
                    checked={tutoringForm.is_free}
                    onChange={(e) =>
                      setTutoringForm({ ...tutoringForm, is_free: e.target.checked })
                    }
                  />
              ...
              <div className="auth-field">
                <label>{t('jobs.descAndMethodology')}</label>
                <textarea
                  value={tutoringForm.description}
                  onChange={(e) =>
                    setTutoringForm({ ...tutoringForm, description: e.target.value })
                  }
                  placeholder={t('jobs.descAndMethodPlaceholder')}
                  style={{
                    width: '100%',
                    minHeight: 80,
                    resize: 'vertical',
              ...
              <div className="auth-field">
                <label>{t('jobs.tagsLabel')}</label>
                <input
                  value={tutoringForm.tags}
                  onChange={(e) => setTutoringForm({ ...tutoringForm, tags: e.target.value })}
                  placeholder={t('jobs.tagsPlaceholder')}
                />
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                <button className="btn btn-secondary" onClick={() => setShowTutoringForm(false)}>
                  {t('jobs.cancel')}
                </button>
                <button className="btn btn-primary" onClick={handleCreateTutoring}>
                  {t('jobs.publishTutoring')}
                </button>
              </div>
            </div>
          </div>
