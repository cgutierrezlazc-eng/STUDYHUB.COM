...
export default function CeoMail({ onNavigate, defaultAccount = 'all' }: Props) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      alert(e.message || t('ceomail.errorSending'));
    }
  ...
  const handleBroadcast = async () => {
    ...
    try {
      const result = await api.ceoBroadcastEmail(
        broadcastSubject,
        broadcastBody + getSignatureHtml(),
        broadcastFilter,
        broadcastCta,
        broadcastCtaUrl
      );
  ...
  const handleDelete = async (emailId: string) => {
    ...
    try {
      ...
      alert(e.message || t('ceomail.errorDeleting'));
    }
  };

  const handleBulkDelete = async () => {
    if (selectedIds.size === 0) return;
    if (!confirm(t('ceomail.bulkDeleteConfirm').replace('{count}', String(selectedIds.size))))
      return;
    try {
          ...
          {(['all', 'unread', 'starred'] as FilterMode[]).map((f) => (
            ...
            >
              {f === 'all'
                ? t('ceomail.filterAll')
                : f === 'unread'
                  ? t('ceomail.filterUnread')
                  : t('ceomail.filterStarred')}
            </button>
          ))}
        </div>
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                alert(
                  `Prueba enviada a ${dest} desde ${acct}@conniku.com. Revisa tu bandeja en 1-2 min.`
                );
              } catch (e: any) {
          ...
          {stats && (
            <div style={{ padding: '4px 16px', fontSize: 12, color: 'var(--text-secondary)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span>{t('ceomail.totalSent')}</span>
                <strong>{stats.sent || 0}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                <span>{t('ceomail.unread')}</span>
                <strong style={{ color: '#0078d4' }}>{stats.unread || 0}</strong>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <span>{t('ceomail.total')}</span>
                  ...
                  <div className="mail-item-content">
                    <div className="mail-item-top">
                      <span className="mail-item-sender">
                        {email.email_type && (
                          <span
                            className={`mail-item-badge ${email.email_type === 'broadcast' ? 'broadcast' : email.email_type === 'system' ? 'system' : 'direct'}`}
                          >
                            {email.email_type === 'broadcast'
                              ? t('ceomail.typeBroadcast')
                              : email.email_type === 'system'
                                ? t('ceomail.typeSystem')
                                : t('ceomail.typeDirect')}
                          </span>
                        )}
                    ...
                    <div className="mail-item-subject">
                      {email.subject || t('ceomail.noSubject')}
                    </div>
                    <div className="mail-item-preview">
                      {email.preview || stripHtml(email.body || '').slice(0, 100)}
                    </div>
                  </div>
                  <div
              ...
              <div className="mail-reading-header">
                ...
                <div className="mail-reading-meta">
                  <div className="mail-reading-avatar">
                    {(selectedEmail.to_email || selectedEmail.from_name || 'C')
                      .charAt(0)
                      .toUpperCase()}
                  </div>
                  <div>
                    <div style={{ fontWeight: 600 }}>
                      {selectedEmail.to_email || selectedEmail.from_name || 'Conniku'}
                    </div>
                  ...
                  {selectedEmail.email_type && (
                    <span
                      className={`mail-item-badge ${selectedEmail.email_type === 'broadcast' ? 'broadcast' : selectedEmail.email_type === 'system' ? 'system' : 'direct'}`}
                    >
                      {selectedEmail.email_type === 'broadcast'
                        ? 'Masivo'
                  ...
                  {selectedEmail.status && (
                    <span style={{ fontSize: 11, display: 'flex', alignItems: 'center', gap: 4 }}>
                      {selectedEmail.status === 'delivered' ? (
                        <>
                          <CheckCircle size={12} /> {t('ceomail.delivered')}
                        </>
                      ) : selectedEmail.status === 'pending' ? (
                        <>
                          <Clock size={12} /> {t('ceomail.pending')}
                        </>
                      ) : selectedEmail.status === 'failed' ? (
                        <>
                          <AlertTriangle size={12} /> {t('ceomail.error')}
                        </>
                      ) : (
                        selectedEmail.status
                      )}
        ...
        <div className="mail-compose-overlay" onClick={() => setShowCompose(false)}>
          <div className="mail-compose" onClick={(e) => e.stopPropagation()}>
            <div className="mail-compose-header">
              <span>
                {composeMode === 'reply'
                  ? t('ceomail.composeReply')
                  : composeMode === 'forward'
                    ? t('ceomail.composeForward')
                    : t('ceomail.composeNew')}
              </span>
            ...
            <div className="mail-compose-fields">
              <div className="mail-compose-field">
                ...
                <select
                  value={composeFrom}
                  onChange={(e) => setComposeFrom(e.target.value as 'ceo' | 'contacto')}
                  style={{
                    flex: 1,
                    padding: '6px 10px',
                    borderRadius: 4,
              ...
              <div className="mail-compose-field">
                ...
                <input
                  ...
                  placeholder={t('ceomail.toPlaceholder')}
                />
              </div>
              <div className="mail-compose-field">
                ...
                <input
                  ...
                  placeholder={t('ceomail.optional')}
                />
              </div>
              <div className="mail-compose-field">
              ...
              <div className="mail-compose-field">
                ...
                <input
                  ...
                  placeholder={t('ceomail.subjectPlaceholder')}
                />
              </div>
            </div>
            <div className="mail-compose-toolbar">
              <button onClick={() => insertFormatting('bold')} title={t('ceomail.bold')}>
                <strong>N</strong>
              </button>
              <button onClick={() => insertFormatting('italic')} title={t('ceomail.italic')}>
                <em>C</em>
              </button>
              <button onClick={() => insertFormatting('underline')} title={t('ceomail.underline')}>
                <u>S</u>
              </button>
              <span
                style={{ width: 1, height: 16, background: 'var(--border, #ddd)', margin: '0 4px' }}
              />
              <button onClick={() => insertFormatting('list')} title={t('ceomail.list')}>
                • {t('ceomail.list')}
              </button>
              <span
                style={{ width: 1, height: 16, background: 'var(--border, #ddd)', margin: '0 4px' }}
              />
              <button title={t('ceomail.attach')}>
                <Paperclip size={13} />
              </button>
            </div>
            ...
            <div className="mail-compose-cta">
              <span style={{ fontWeight: 600, fontSize: 11, color: 'var(--text-secondary)' }}>
                {t('ceomail.ctaOptional')}
              </span>
              <input
                value={composeCta}
                onChange={(e) => setComposeCta(e.target.value)}
                placeholder={t('ceomail.ctaTextPlaceholder')}
              />
              <input
                value={composeCtaUrl}
                onChange={(e) => setComposeCtaUrl(e.target.value)}
                placeholder={t('ceomail.ctaUrlPlaceholder')}
              />
            </div>
            <div className="mail-compose-footer">
              <div className="left">
                ...
                <button className="mail-btn secondary" onClick={() => setShowCompose(false)}>
                  {t('ceomail.saveDraft')}
                </button>
              </div>
              <div className="right">
                <button
                  className="mail-btn danger"
                  onClick={() => {
                    setShowCompose(false);
                    resetCompose();
                  }}
            ...
            <div className="mail-compose-fields">
              ...
              <div className="mail-compose-field">
                ...
                <input
                  ...
                  placeholder={t('ceomail.broadcastSubjectPlaceholder')}
                />
              </div>
            </div>
            <div className="mail-compose-body">
            ...
            <div className="mail-compose-footer">
              <div className="left">
                <button
                  ...
                  disabled={broadcastSending || !broadcastSubject || !broadcastBody}
        ...
        <div className="sig-overlay" onClick={() => setShowSignatureSettings(false)}>
          <div className="sig-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="sig-body">
              <div className="sig-field">
                ...
                <input
                  ...
                  onChange={(e) => setSignature((s) => ({ ...s, department: e.target.value }))}
                  placeholder={t('ceomail.departmentPlaceholder')}
                />
              </div>
              <div className="sig-field">
                ...
                <input
                  ...
                  onChange={(e) => setSignature((s) => ({ ...s, tagline: e.target.value }))}
                />
              </div>
              <div className="sig-field">
                ...
                <input
                  ...
                  onChange={(e) => setSignature((s) => ({ ...s, location: e.target.value }))}
                />
              </div>
              <div className="sig-field">
                ...
                <input
                  ...
                  onChange={(e) => setSignature((s) => ({ ...s, website: e.target.value }))}
                />
              </div>
              <div className="sig-field">
                <label>{t('ceomail.motto')}</label>
                <input
                  value={signature.motto}
                  onChange={(e) => setSignature((s) => ({ ...s, motto: e.target.value }))}
                />
              </div>
              <div className="sig-field">
                <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                  <input
                    type="checkbox"
                    checked={signature.showLogo}
                    onChange={(e) => setSignature((s) => ({ ...s, showLogo: e.target.checked }))}
              ...
              <div className="sig-preview">
                ...
                >
                  —<br />
                  <strong style={{ color: '#333', fontSize: 14 }}>{signature.department}</strong>
                  <br />
                  <span style={{ color: '#0078d4' }}>{signature.tagline}</span>
                  <br />
                  ...
                  {signature.showLogo && (
                    <>
                      <img src="/logo.png" alt="Conniku" style={{ height: 28, marginTop: 8 }} />
                      <br />
                    </>
                  )}
                  <em style={{ color: '#888' }}>{signature.motto}</em>
            ...
            <div className="sig-footer">
              ...
              <button className="mail-btn primary" onClick={saveSignature}>
                {t('ceomail.saveSignature')}
              </button>
            </div>
          </div>
        </div>
