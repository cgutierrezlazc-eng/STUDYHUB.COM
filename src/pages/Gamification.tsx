...
const BADGE_DEFS: Record<
  string,
  { emoji: string; name: string; description: string; hint: string }
> = {
  first_login: {
    emoji: '🎉',
    name: 'Bienvenido',
    description: 'Primer inicio de sesión',
    hint: 'Inicia sesión por primera vez',
  },
  streak_7: {
    emoji: '🔥',
    name: 'Racha Semanal',
    description: '7 días de racha',
    hint: 'Necesitas 7 días de racha',
  },
  streak_30: {
    emoji: '💎',
    name: 'Racha Legendaria',
    description: '30 días consecutivos',
    hint: 'Necesitas 30 días consecutivos',
  },
  study_1h: {
    emoji: '📖',
    name: 'Primer Paso',
    description: '1 hora de estudio',
    hint: 'Estudia durante 1 hora',
  },
  study_10h: {
    emoji: '📚',
    name: 'Estudioso',
    description: '10 horas de estudio',
    hint: 'Acumula 10 horas de estudio',
  },
  study_50h: {
    emoji: '🎓',
    name: 'Dedicado',
    description: '50 horas de estudio',
    hint: 'Acumula 50 horas de estudio',
  },
  study_100h: {
    emoji: '🏆',
    name: 'Maestro',
    description: '100 horas de estudio',
    hint: 'Acumula 100 horas de estudio',
  },
  quiz_master: {
    emoji: '🧠',
    name: 'Quiz Master',
    description: 'Aprueba 10 quizzes',
    hint: 'Aprueba 10 quizzes',
  },
  social_butterfly: {
    emoji: '🦋',
    name: 'Mariposa Social',
    description: '10 amigos',
    hint: 'Agrega 10 amigos',
  },
  helper: {
    emoji: '🤝',
    name: 'Ayudante',
    description: 'Ayuda a 5 compañeros',
    hint: 'Ayuda a 5 compañeros',
  },
  league_gold: {
    emoji: '🥇',
    name: 'Liga de Oro',
    description: 'Liga de Oro',
    hint: 'Alcanza la Liga de Oro',
  },
  league_diamond: {
    emoji: '💠',
    name: 'Liga Diamante',
    description: 'Liga Diamante',
    hint: 'Alcanza la Liga Diamante',
  },
  early_bird: {
    emoji: '🌅',
    name: 'Madrugador',
    description: 'Estudio antes de las 8am',
    hint: 'Estudia antes de las 8am',
  },
};
...
function getWeeklyChallenge() {
  ...
  const challenge = {
    id: 'weekly_5days',
    label: 'Estudia 5 días esta semana',
    target: 5,
    xp: 200,
    progress: 0,
    claimed: false,
...
export default function Gamification({ onNavigate }: Props) {
    ...
    if (studyTime?.todaySeconds) {
      ...
      setDailyChallenges((prev) => {
        const updated = prev.map((c) =>
          c.id === 'study_30' ? { ...c, progress: Math.min(mins, c.target) } : c
        );
        const today = new Date().toISOString().slice(0, 10);
  ...
  const claimWeekly = () => {
    ...
    localStorage.setItem(
      `conniku_weekly_challenge_${monday.toISOString().slice(0, 10)}`,
      ...
      <div style={{ display: 'grid', gap: 20 }}>
        ...
        <div className="u-card" style={{ padding: 32, textAlign: 'center' }}>
          <div style={{ position: 'relative', width: 160, height: 160, margin: '0 auto 16px' }}>
            ...
            >
              <span
                style={{ fontSize: 42, fontWeight: 800, color: 'var(--accent)', lineHeight: 1 }}
              >
                {level}
              </span>
              <span
                style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600, marginTop: 4 }}
              >
                NIVEL
              </span>
            </div>
            ...
            >
              <div
                style={{
                  ...
                  background:
                    'linear-gradient(90deg, var(--accent), var(--accent-purple, #8b5cf6))',
                  borderRadius: 4,
        ...
        <div className="u-card" style={{ padding: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 120 }}>
              <div style={{ fontSize: 28, fontWeight: 800, color: '#ff6b35', lineHeight: 1 }}>
                {streakDays}
              </div>
    ...
    return (
      <div>
        <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 20 }}>
          {earnedBadges.filter((b: any) => b.earned).length} de {Object.keys(BADGE_DEFS).length}{' '}
          ...
          {Object.entries(BADGE_DEFS).map(([id, def]) => {
            ...
            return (
              ...
              >
                ...
                {isEarned ? (
                  <div
                    style={{ fontSize: 11, color: 'var(--accent-green, #22c55e)', fontWeight: 600 }}
                  >
                    ✅ Desbloqueada
                    {earned.earnedAt
                      ? ` · ${new Date(earned.earnedAt).toLocaleDateString('es-CL')}`
                      : ''}
                  </div>
                ) : (
  ...
  const renderLeague = () => {
    ...
    return (
      <div style={{ display: 'grid', gap: 20 }}>
        ...
        <div className="u-card" style={{ padding: 32, textAlign: 'center' }}>
          ...
          {league && (
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 8 }}>
              Posición #{league.userRank} · {league.weeklyXp} XP esta semana · {league.daysLeft}d
              restantes
            </div>
          )}
        </div>
        ...
        >
          📅 La liga se reinicia cada lunes. Los 3 primeros ascienden de liga, los 3 últimos
          descienden.
        </div>
                ...
                .map((h, i) => (
                  ...
                  >
                    <span style={{ color: 'var(--text-muted)' }}>
                      Semana del{' '}
                      {new Date(h.week).toLocaleDateString('es-CL', {
                        day: 'numeric',
                        month: 'short',
                      })}
                    </span>
      ...
      <div style={{ display: 'grid', gap: 20 }}>
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          width: `${pct}%`,
                          background: isComplete ? 'var(--accent-green, #22c55e)' : 'var(--accent)',
                          borderRadius: 3,
        ...
        <div className="u-card" style={{ padding: 20 }}>
          ...
          >
            ...
            {weeklyChal.claimed ? (
              <span
                style={{ fontSize: 12, color: 'var(--accent-green, #22c55e)', fontWeight: 700 }}
              >
                ✅ Reclamado
              </span>
            ) : weeklyChal.progress >= weeklyChal.target ? (
