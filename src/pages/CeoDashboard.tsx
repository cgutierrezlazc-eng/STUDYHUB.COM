...
export default function CeoDashboard({ onNavigate }: Props) {
  ...
  const [modFilter, setModFilter] = useState<'pending' | 'approved' | 'rejected' | 'all'>(
    'pending'
  );
  ...
  const loadF129 = async (month?: number, year?: number) => {
    try {
      setF129(await api.generateF129(month, year));
    } catch (e: any) {
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      alert(e.message || t('ceo.errorSending'));
    }
  ...
  const handleBroadcast = async () => {
    ...
    try {
      const result = await api.ceoBroadcastEmail(
        broadcastSubject,
        broadcastBody,
        broadcastFilter,
        broadcastCta,
  ...
  const handleDeleteEmail = async (emailId: string) => {
    ...
    try {
      ...
      alert(e.message || t('ceo.errorDeleting'));
    }
  ...
  const handleDeleteSelectedEmails = async () => {
    ...
    if (!confirm(t('ceo.deleteEmailsConfirm').replace('{count}', String(selectedEmails.size))))
      return;
    setDeletingEmail(true);
    try {
        ...
        >
          ...
          {(
            ...
            <button
              ...
              onClick={() => {
                setTab(tb as any);
                if (tb === 'f129' && !f129) loadF129();
                if (tb === 'certifications' && !progressData) loadProgressOverview();
                if (tb === 'moderation') loadModeration(modFilter);
              }}
            >
              {tb === 'overview' ? (
                <>
                  {BarChart3({ size: 14 })} {t('ceo.tabOverview')}
                </>
              ) : tb === 'push' ? (
                <>
                  {Megaphone({ size: 14 })} {t('ceo.tabPush')}
                </>
              ) : tb === 'certifications' ? (
                <>
                  {GraduationCap({ size: 14 })} {t('ceo.tabCertifications')}
                </>
              ) : tb === 'financial' ? (
                <>
                  {Gem({ size: 14 })} {t('ceo.tabFinancial')}
                </>
              ) : tb === 'f129' ? (
                <>
                  {ClipboardList({ size: 14 })} {t('ceo.tabF129')}
                </>
              ) : tb === 'fraud' ? (
                <>
                  {Shield({ size: 14 })} {t('ceo.tabFraud')}
                </>
              ) : tb === 'moderation' ? (
                <>
                  {Shield({ size: 14 })} Moderación{' '}
                  {modStats?.pending > 0 ? (
                    <span
                      style={{
                        background: '#ef4444',
                ...
                <>
                  {CheckCircle({ size: 14 })} {t('ceo.tabCompliance')}
                </>
              )}
            </button>
          ))}
                ...
                <div className="stats-grid" style={{ marginBottom: 24 }}>
                  ...
                  >
                    ...
                    <div className="stat-label">
                      {t('ceo.weekRevenue')} {weeklyReport.revenue?.trend}
                    </div>
                  </div>
                  <div className="stat-card" style={{ borderLeft: '4px solid var(--accent-blue)' }}>
                    ...
                    <div className="stat-label">
                      {t('ceo.newUsers')} ({weeklyReport.users?.growthPercent > 0 ? '+' : ''}
                      {weeklyReport.users?.growthPercent}%)
                    </div>
                  </div>
                  <div
                    className="stat-card"
                  ...
                  >
                    ...
                    <div className="stat-label">
                      {t('ceo.activeUsers')} ({weeklyReport.users?.activeRate}%)
                    </div>
                  </div>
                  <div
                    className="stat-card"
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div className="u-card" style={{ padding: 20 }}>
                    <h4 style={{ marginTop: 0 }}>
                      {Gem({ size: 16 })} {t('ceo.netProfit')}
                    </h4>
                    <div style={{ fontSize: 28, fontWeight: 700 }}>
                      ${fmt(weeklyReport.revenue?.gananciaNetaClp)} CLP
                    </div>
                    <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                      {t('ceo.afterCommissions')}
                    </p>
                  </div>
                  <div className="u-card" style={{ padding: 20 }}>
                    <h4 style={{ marginTop: 0 }}>
                      {BarChart3({ size: 16 })} {t('ceo.engagement')}
                    </h4>
                    <div style={{ fontSize: 13 }}>
                      <div>
                        {t('ceo.posts')}: {weeklyReport.engagement?.wallPosts} |{' '}
                        {t('ceo.community')}: {weeklyReport.engagement?.communityPosts}
                      </div>
                      <div>
                        {t('ceo.messages')}: {weeklyReport.engagement?.messages} |{' '}
                        {t('ceo.quizzes')}: {weeklyReport.engagement?.quizzesTaken}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 11, color: '#009ee3' }}>
                        {t('ceo.mercadoPagoStatus')}
                      </div>
                    </div>
                  </div>
                  <div
                  ...
                  <>
                    <div className="stats-grid" style={{ marginBottom: 24 }}>
                      ...
                      >
                        ...
                        <div className="stat-label">
                          {t('ceo.subscribers')} ({financials.proSubscribers} Pro +{' '}
                          {financials.maxSubscribers} Max)
                        </div>
                      </div>
                      <div
                        className="stat-card"
                    ...
                    <div className="u-card" style={{ padding: 20, marginBottom: 16 }}>
                      ...
                      <table style={{ width: '100%', fontSize: 14 }}>
                        <tbody>
                          <tr>
                            <td>{t('ceo.grossTotal')}</td>
                            <td style={{ textAlign: 'right', fontWeight: 600 }}>
                              ${fmt(financials.grossRevenueClp)}
                            </td>
                          </tr>
                          <tr style={{ color: 'var(--accent-red)' }}>
                            <td>{t('ceo.platformCommission')}</td>
                            <td style={{ textAlign: 'right' }}>
                              -${fmt(financials.grossRevenueClp - financials.netAfterStripeClp)}
                            </td>
                          </tr>
                          <tr>
                            <td>{t('ceo.netAfterCommissions')}</td>
                            <td style={{ textAlign: 'right', fontWeight: 600 }}>
                              ${fmt(financials.netAfterStripeClp)}
                            </td>
                          </tr>
                          <tr style={{ color: 'var(--accent-red)' }}>
                            <td>{t('ceo.iva19')}</td>
                            <td style={{ textAlign: 'right' }}>-${fmt(financials.totalIvaClp)}</td>
                          </tr>
                          <tr
                            style={{
                          ...
                          >
                            <td>{t('ceo.netProfitLabel')}</td>
                            <td style={{ textAlign: 'right', color: 'var(--accent-green)' }}>
                              ${fmt(financials.gananciaNetaTotalClp)}
                            </td>
                          </tr>
                        </tbody>
                      </table>
                      <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8 }}>
                        {t('ceo.exchangeRate')} ${fmt(financials.exchangeRateUsdClp)} CLP
                      </p>
                    </div>
                        ...
                        <div style={{ overflowX: 'auto' }}>
                          ...
                          >
                            <thead>
                              ...
                              >
                                <th style={{ padding: 8 }}>{t('ceo.user')}</th>
                                <th>{t('ceo.country')}</th>
                                <th>USD</th>
                                <th>CLP</th>
                                <th>IVA</th>
              ...
              <div>
                {!f129 ? (
                  <div className="u-card" style={{ padding: 24, textAlign: 'center' }}>
                    <h3>
                      {ClipboardList({ size: 18 })} {t('ceo.generateF129')}
                    </h3>
                    <p style={{ color: 'var(--text-muted)' }}>{t('ceo.generateF129Desc')}</p>
                    <button className="btn btn-primary" onClick={() => loadF129()}>
                      {t('ceo.generateCurrentPeriod')}
                    </button>
                  </div>
                ) : (
                  <div>
                    <div className="u-card" style={{ padding: 24, marginBottom: 16 }}>
                      ...
                      >
                        <h3 style={{ margin: 0 }}>
                          {ClipboardList({ size: 18 })} F129 — {f129.periodoTexto}
                        </h3>
                        <span
                          style={{
                            fontSize: 12,
                        ...
                        >
                          {f129.estado === 'generado' ? (
                            <>
                              {Hourglass({ size: 12 })} {t('ceo.pendingSubmission')}
                            </>
                          ) : (
                            <>
                              {CheckCircle({ size: 12 })} {t('ceo.submitted')}
                            </>
                          )}
                        </span>
                      </div>
                      <table style={{ width: '100%', fontSize: 14, borderCollapse: 'collapse' }}>
                        <tbody>
                          <tr>
                            <td style={{ padding: 8 }}>{t('ceo.rutIssuer')}</td>
                            <td style={{ textAlign: 'right' }}>{f129.rutEmisor}</td>
                          </tr>
                          <tr>
                            <td style={{ padding: 8 }}>{t('ceo.businessName')}</td>
                            <td style={{ textAlign: 'right' }}>{f129.razonSocial}</td>
                          </tr>
                          <tr style={{ borderTop: '1px solid var(--border)' }}>
                            <td style={{ padding: 8 }}>{t('ceo.netTaxableSales')}</td>
                            <td style={{ textAlign: 'right', fontWeight: 600 }}>
                              ${fmt(f129.ventasNetasAfectas)} CLP
                            </td>
                          </tr>
                          <tr>
                            <td style={{ padding: 8 }}>{t('ceo.ivaRate')}</td>
                            <td style={{ textAlign: 'right' }}>{f129.tasaIva}</td>
                          </tr>
                          <tr
                            style={{
                              borderTop: '2px solid var(--border)',
                          ...
                          >
                            <td style={{ padding: 8 }}>{t('ceo.ivaToPay')}</td>
                            <td style={{ textAlign: 'right', color: 'var(--accent-red)' }}>
                              ${fmt(f129.ivaAPagar)} CLP
                            </td>
                          </tr>
                        </tbody>
                      </table>
                      ...
                      <p style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                        {t('ceo.deadline')}: {f129.plazoDeclaracion}
                      </p>
                    </div>
                    <div style={{ display: 'flex', gap: 8 }}>
                      <button className="btn btn-primary" onClick={submitF129}>
                        {t('ceo.submitToSii')}
                      </button>
                      <a
                        href={f129.urlSii}
                      ...
                      >
                        {Link({ size: 14 })} {t('ceo.siiPortal')}
                      </a>
                      <button className="btn btn-secondary" onClick={() => loadF129()}>
                        {RefreshCw({ size: 14 })} {t('ceo.regenerate')}
                      </button>
                    </div>
                  </div>
                )}
                ...
                {compliance.pending?.length > 0 && (
                  <div style={{ marginTop: 16 }}>
                    ...
                    {compliance.pending.map((p: string, i: number) => (
                      <div
                        key={i}
                        style={{ fontSize: 13, padding: 8, color: 'var(--accent-orange)' }}
                      >
                        • {p}
                      </div>
                    ))}
                    ...
                    {(progressData.users as any[])
                      ...
                      .map((userData: any) => {
                        ...
                        const incompleteCourses = progressData.courses.filter(
                          (c: any) =>
                            !userData.courses.some(
                              (uc: any) => uc.courseId === c.id && uc.completed
                            )
                        );
                            ...
                            >
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                                <span style={{ fontSize: 12, color: '#059669', fontWeight: 600 }}>
                                  {Medal({ size: 14 })} {userData.completedCount}{' '}
                                  {t('ceo.completed')}
                                </span>
                                <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                                  {BookOpen({ size: 14 })} {userData.totalStarted}{' '}
                                  {t('ceo.started')}
                                </span>
                              ...
                              {incompleteCourses.length > 0 && (
                                <div>
                                  ...
                                  >
                                    ...
                                    >
                                      {incompleteCourses.every((c: any) =>
                                        userSelected.includes(c.id)
                                      )
                                        ? t('ceo.deselectAll')
                                        : t('ceo.selectAll')}
                                    </button>
                                    ...
                                    {incompleteCourses.map((c: any) => {
                                      ...
                                      return (
                                        <button
                                          key={c.id}
                                          onClick={() =>
                                            toggleCourseSelection(userData.userId, c.id)
                                          }
                                          style={{
                                            display: 'flex',
                                            alignItems: 'center',
                  ...
                  {modStats && (
                    <div style={{ display: 'flex', gap: 8, marginLeft: 'auto' }}>
                      {(['pending', 'approved', 'rejected', 'all'] as const).map((f) => (
                        ...
                        >
                          {f === 'pending'
                            ? `Pendiente (${modStats.pending})`
                            : f === 'approved'
                              ? `Aprobado (${modStats.approved})`
                              : f === 'rejected'
                                ? `Rechazado (${modStats.rejected})`
                                : 'Todos'}
                        </button>
                      ))}
                      <button
                        className="btn btn-secondary btn-sm"
                        onClick={() => loadModeration(modFilter)}
                      >
                        Refrescar
                      </button>
                    </div>
                ...
                {modLoading ? (
                  ...
                  <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>
                    ...
                    <div style={{ fontSize: 13, marginTop: 4 }}>
                      Todo el contenido está revisado.
                    </div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                          ...
                          <div style={{ flex: 1, minWidth: 200 }}>
                            ...
                            >
                              {item.originalContent?.startsWith('data:image') ? (
                                <img
                                  src={item.originalContent}
                                  alt="Contenido flaggeado"
