...
const CHILE_LABOR = {
  ...
  CONNIKU_PAYROLL: {
    ...
    anticipoMaxPct: 0.40,    // Max anticipo: 40% del sueldo liquido estimado
  ...
  INDEMNIZACIONES: {
    ...
    recargos: {
      art161: 0.30,          // 30% necesidades empresa
      art159_4_6: 0.50,      // 50% causales objetivas
      art160: 0.80,          // 80% causales imputables al trabajador (injustificado)
    },
  },
...
function validateSalary(gross: number, weeklyHours: number = 40): { valid: boolean; min: number; message: string } {
          ...
          {activeTab === 'jornada' && (
            <div>
              <div style={{ padding: 12, background: 'linear-gradient(135deg, #EC489920, #F4385E20)', borderRadius: 8, marginBottom: 16, fontSize: 12, color: 'var(--text-muted)' }}>
                <strong>Art. 10 N°5, Art. 22-40 bis — Codigo del Trabajo:</strong> Duracion y distribucion de la jornada. Ley 21.561: reduccion gradual a 40 hrs semanales. Art. 22 inc. 2: excluye de limitacion a gerentes, teletrabajadores sin fiscalizacion y otros.
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                <div style={{ ...fieldStyle, gridColumn: 'span 2' }}>
      ...
      <div className="card" style={{ padding: 20, marginBottom: 16 }}>
        ...
        <div style={{ padding: 16, background: 'var(--bg-secondary)', borderRadius: 8, marginBottom: 12 }}>
          ...
          {employees.filter(e => e.status === 'active').length > 0 && (
            <div style={{ display: 'flex', gap: 8 }}>
              <select style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: 13, fontFamily: 'inherit' }}>
                <option value="">Seleccionar trabajador...</option>
                {employees.filter(e => e.status === 'active').map(e => (
                  <option key={e.id} value={e.id}>{e.firstName} {e.lastName}</option>
                ))}
              </select>
              <button style={btnPrimary}><Download size={14} /> Generar Finiquito</button>
            </div>
          )}
        </div>
        ...
        <div style={{ padding: 16, background: 'var(--bg-secondary)', borderRadius: 8, marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button style={btnSecondary}><FileText size={14} /> Generar Carta Art. 159 (Vencimiento/Mutuo Acuerdo)</button>
            <button style={btnSecondary}><FileText size={14} /> Generar Carta Art. 161 (Necesidades Empresa)</button>
          </div>
        </div>
        ...
        <div style={{ padding: 16, background: 'var(--bg-secondary)', borderRadius: 8, marginBottom: 12 }}>
          ...
          <button style={btnSecondary}><FileText size={14} /> Generar Carta de Amonestacion</button>
        </div>
        ...
        <div style={{ padding: 16, background: 'var(--bg-secondary)', borderRadius: 8 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 8 }}>
            <button style={btnSmall}><FileText size={14} /> Certificado de Antiguedad</button>
            <button style={btnSmall}><FileText size={14} /> Certificado de Remuneraciones</button>
            <button style={btnSmall}><FileText size={14} /> Constancia de Vacaciones</button>
