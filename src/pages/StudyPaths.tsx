...
function getProgress(projectId: string, docs: unknown[]): PathProgress {
  ...
  return {
    ...
    flashcardsMastered: lsGet<FlashcardItem[]>(`${prefix}_flashcards_items`, []).filter(
      (f) => f.mastered
    ).length,
...
function stepStatus(p: PathProgress, idx: number): 'completed' | 'active' | 'locked' {
  switch (idx) {
    case 0:
      return p.hasDocuments ? 'completed' : 'active';
    case 1:
    ...
    case 2:
      ...
      if (
        p.flashcardsGenerated &&
        p.flashcardsMastered >= p.flashcardsTotal &&
        p.flashcardsTotal > 0
      )
        return 'completed';
    ...
    case 4:
      ...
      if (p.planGenerated && p.planChecks.length > 0 && p.planChecks.every(Boolean))
        return 'completed';
...
const S = {
  ...
  flipFront: {
    background: 'var(--bg-secondary, #fff)',
    border: '2px solid var(--accent, #2D62C8)',
    color: 'var(--text-primary)',
  } as React.CSSProperties,
  flipBack: {
  ...
  checkbox: (checked: boolean) =>
    ({
      ...
      height: 22,
      borderRadius: 6,
      border: `2px solid ${checked ? '#10b981' : 'var(--border-color, #cbd5e1)'}`,
...
export default function StudyPaths({ projects, onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    if (savedGuide) {
      try {
        setGuideContent(JSON.parse(savedGuide));
      } catch {
  ...
  const progress = activeProject
    ? getProgress(activeProject.id, activeProject.documents || [])
    : null;
  ...
  const handleGenerateFlashcards = useCallback(async () => {
    ...
    try {
      ...
      const cards: FlashcardItem[] = (res.flashcards || res.cards || []).map(
        (c: any, i: number) => ({
          id: `fc-${i}`,
  ...
  const toggleFlashcardMastered = useCallback(
    (idx: number) => {
      setFlashcards((prev) => {
  ...
  const handleSubmitQuiz = useCallback(() => {
    ...
    const weakTopics = withAnswers
      .filter((q, i) => quizAnswers[i] !== q.correctAnswer)
  ...
  const togglePlanCheck = useCallback(
    (idx: number) => {
      setPlanChecks((prev) => {
        const next = [...prev];
        next[idx] = !next[idx];
    ...
    if (projects.length === 0) {
      return (
        <div style={S.empty}>
          ...
          <p style={{ fontSize: 14 }}>
            Sube documentos a una asignatura para crear tu ruta de estudio
          </p>
        </div>
      );
    }
          ...
          return (
            ...
            >
              ...
              <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 14 }}>
                ...
                {(project.documents || []).length !== 1 ? 's' : ''} subido
              ...
              >
                <span style={S.tag}>
                  {done}/5 pasos \u2022 {Math.round((done / 5) * 100)}%
                </span>
                <span
                  style={{
                    ...S.tag,
                    background: done > 0 ? 'rgba(45,98,200,0.1)' : undefined,
                    color: done > 0 ? 'var(--accent, #2D62C8)' : undefined,
                  }}
                >
    ...
    const getPlanDays = (): { title: string; description: string }[] => {
      ...
      if (typeof planData === 'string') {
        return planData
          ...
          .map((l: string, i: number) => ({ title: `D\u00eda ${i + 1}`, description: l }));
      }
      return Array.from({ length: 7 }, (_, i) => ({
        title: `D\u00eda ${i + 1}`,
        description: 'Meta de estudio pendiente',
      }));
    };
                  ...
                  {(activeProject.documents || []).map((doc: any, i: number) => (
                    <div key={i} style={S.docItem}>
                      ...
                      <span style={{ flex: 1, fontWeight: 500, color: 'var(--text-primary)' }}>
                        {doc.name || doc.filename || `Documento ${i + 1}`}
                      </span>
              ...
              {!guideContent ? (
                <div>
                  <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 12 }}>
                    Genera una guía de estudio basada en tus documentos. Conniku analizará el
                    contenido y creará un resumen estructurado.
                  </p>
                  <button
                ...
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    <span style={S.tag}>
                      {'\u23F1\uFE0F'} ~{Math.max(5, Math.round(guideContent.length / 1200))} min de
                      lectura
                    </span>
                    {progress.guideRead && (
                      <span style={{ ...S.tag, background: '#d1fae5', color: '#065f46' }}>
                        {'\u2705'} Le\u00edda
                      </span>
                    )}
                  </div>
              ...
              {flashcards.length === 0 ? (
                <div>
                  ...
                  <button
                    style={{
                      ...S.btn,
                      ...S.btnPrimary,
                      ...(flashcardsLoading ? S.btnDisabled : {}),
                    }}
                ...
                <div>
                  ...
                  >
                    ...
                    <span style={S.tag}>
                      {'\uD83C\uDCCF'} Tarjeta {currentFlashcard + 1} de {flashcards.length}
                    </span>
                  </div>
                  ...
                  >
                    ...
                    <button
                      style={{
                        ...S.btn,
                        background: flashcards[currentFlashcard]?.mastered
                          ? '#10b981'
                          : 'var(--bg-tertiary, #f1f5f9)',
                        color: flashcards[currentFlashcard]?.mastered
                          ? '#fff'
                          : 'var(--text-primary)',
                      }}
          ...
          return (
            <div>
              {quizQuestions.length === 0 && !quizSubmitted ? (
                <div>
                  <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 12 }}>
                    Evalúa tu conocimiento con un quiz de 10 preguntas. Conniku identificará tus
                    temas débiles.
                  </p>
                  <button
                ...
                <div>
                  ...
                  {(() => {
                    const correct = quizQuestions.filter(
                      (q, i) => (q.userAnswer ?? quizAnswers[i]) === q.correctAnswer
                    ).length;
                      ...
                      <div style={{ textAlign: 'center', marginBottom: 20 }}>
                        ...
                        {score < 70 && (
                          ...
                          >
                            ...
                            <ul
                              style={{ fontSize: 12, color: '#92400e', margin: 0, paddingLeft: 16 }}
                            >
                              {quizQuestions
                                .filter(
                                  (q, i) => (q.userAnswer ?? quizAnswers[i]) !== q.correctAnswer
                                ...
                                .slice(0, 5)
                                .map((q, i) => (
                                  <li key={i}>{q.question.slice(0, 80)}</li>
                                ))}
                            </ul>
                          </div>
                        )}
                ...
                <div>
                  ...
                  >
                    <span style={S.tag}>
                      Pregunta {quizCurrent + 1} de {quizQuestions.length}
                    </span>
                    <span style={S.tag}>
        ...
        <div style={S.topProgress}>
          <span
            style={{
              fontSize: 14,
              fontWeight: 700,
              color: pct === 100 ? '#10b981' : 'var(--accent, #2D62C8)',
            }}
          >
            {pct}%
          </span>
        ...
        {pct < 100 && (activeProject.documents || []).length > 0 && (
          <div style={{ marginBottom: 24 }}>
            ...
            >
              {marathonLoading
                ? '\u23F3 Generando todo...'
                : '\uD83C\uDFC3 Modo marat\u00f3n \u2014 generar todo de una vez'}
            </button>
          </div>
        )}
            ...
            return (
              <div key={step.key} style={S.stepContainer}>
                ...
                <div style={S.stepContent(status)}>
                  ...
                  {isExpanded && <div style={S.stepBody}>{renderPhaseContent(idx, status)}</div>}
                </div>
              </div>
            );
          })}
    ...
    <div style={S.page}>
      ...
      <div style={S.header}>
        <div style={S.headerRow}>
          <div>
            <h1 style={S.h1}>{'\uD83D\uDEA9'} Rutas de Estudio</h1>
            <p style={S.subtitle}>
      ...
      {/* Body */}
      <div style={S.body}>{view === 'list' ? renderList() : renderActive()}</div>
    </div>
  );
}
