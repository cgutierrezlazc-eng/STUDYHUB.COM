...
const STATUS_META: Record<string, { label: string; color: string; icon: string; desc: string }> = {
  pending_review: {
    ...
    color: '#f59e0b',
    icon: '⏳',
    desc: 'Tu postulación está siendo revisada por el equipo Conniku. Te notificaremos por email.',
  },
  approved: {
    label: 'Aprobado',
    color: '#22c55e',
    icon: '✅',
    desc: 'Tu perfil está activo y visible para los estudiantes.',
  },
  rejected: {
    label: 'Rechazado',
    color: '#ef4444',
    icon: '❌',
    desc: 'Tu postulación fue rechazada. Puedes apelar con nueva información.',
  },
  suspended: {
    label: 'Suspendido',
    color: '#ef4444',
    icon: '🚫',
    desc: 'Tu cuenta está suspendida. Contacta a soporte para más información.',
  },
  appealing: {
    label: 'Apelando',
    color: '#8b5cf6',
    icon: '📝',
    desc: 'Tu apelación está en revisión. El equipo te responderá pronto.',
  },
};
...
export default function MyTutorDashboard({ onNavigate, subPath }: Props) {
  ...
  const [tab, setTab] = useState<'overview' | 'clases' | 'pagos' | 'materias' | 'boletas'>(
    subPath || 'overview'
  );
  ...
  const handleRateStudents = async () => {
    ...
    try {
      await api.rateStudentInClass(ratingModal.classId, {
        rating: ratingValue,
  ...
  return (
    <div style={{ padding: '24px 28px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      >
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 13, opacity: 0.85 }}>
            {profile.professional_title || profile.career}
          </div>
          <div style={{ fontSize: 11, opacity: 0.6, fontFamily: 'monospace', marginTop: 4 }}>
            {profile.tutor_role_number}
          </div>
        </div>
      ...
      {profile.status === 'rejected' && !appealSent && !profile.appeal_text && (
        <div
          className="card"
          style={{ padding: 20, marginBottom: 20, borderLeft: '4px solid #8b5cf6' }}
        >
          <h4 style={{ margin: '0 0 10px', color: '#8b5cf6' }}>Apelar Decisión</h4>
          <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>
            Si crees que la decisión fue incorrecta, puedes enviar nueva información para ser
            revisada.
          </p>
          <textarea
            value={appealText}
        ...
        {[
          ...
          ...(isApproved
            ? [
                {
                  id: 'materias',
                  label: `📚 Mis Materias${subjects.length > 0 ? ` (${subjects.length})` : ''}`,
                },
          ...
          {isApproved && (
            ...
            >
              {[
                {
                  label: 'Rating',
                  value: (profile.rating_average || 0).toFixed(2),
                  icon: '⭐',
                  color: '#f59e0b',
                },
                {
                  label: 'Clases Dadas',
                  value: profile.total_classes,
                  icon: '📚',
                  color: '#3b82f6',
                },
                {
                ...
                {
                  label: 'Tarifa/Hora',
                  value: fmtClp(profile.individual_rate),
                  icon: '💰',
                  color: '#8b5cf6',
                },
              ].map((kpi) => (
          ...
          {isApproved &&
            ...
            (() => {
              ...
              const handleShare = () => {
                if (navigator.share) {
                  navigator.share({
                    title: `${user.firstName} ${user.lastName} — Tutor Conniku`,
                    text: `¡Reserva clases conmigo en Conniku!`,
                    url: publicUrl,
                  });
                } else {
                  handleCopy();
              ...
              return (
                ...
                >
                  ...
                  >
                    <h4 style={{ margin: 0, fontSize: 14, color: '#1e40af' }}>
                      🌐 Mi Página Pública
                    </h4>
                  ...
                  >
                    ...
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 6,
                        border: 'none',
                        background: urlCopied ? '#22c55e' : '#dbeafe',
                        color: urlCopied ? '#fff' : '#1e40af',
                        fontWeight: 700,
                        fontSize: 11,
                        cursor: 'pointer',
                        whiteSpace: 'nowrap',
                  ...
                  {showQR && (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 200 }}>
                        ...
                        >
                          Comparte este código QR con tus estudiantes
                        </p>
                        <p
                          style={{
                            margin: '0 0 12px',
                        ...
                        >
                          Pueden escanearlo para llegar directo a tu página y reservar clases
                          contigo.
                        </p>
                        <a
                  ...
                  <div style={{ marginTop: 10, fontSize: 11, color: '#64748b' }}>
                    Esta es tu página pública. Los estudiantes pueden ver tu perfil, reservar clases
                    y dejar reseñas sin necesitar cuenta en Conniku.
                  </div>
                </div>
              );
            })()}
              ...
              <div>
                ...
                {profile.bank_name ? (
                  <div style={{ fontSize: 12, lineHeight: 1.8 }}>
                    <div>
                      {profile.bank_name} — {profile.bank_account_type}
                    </div>
                    <div>Cta: {profile.bank_account_number}</div>
                    <div>RUT: {profile.rut}</div>
                    <div>
                      Frecuencia:{' '}
                      {profile.payment_frequency === 'per_class'
                        ? 'Por clase'
                        : profile.payment_frequency === 'biweekly'
                          ? 'Quincenal'
                          : 'Mensual'}
                    </div>
                  </div>
                  ...
                  <span style={{ color: '#ef4444', fontSize: 12 }}>
                    ⚠️ Sin datos bancarios — actualiza tu perfil
                  </span>
                )}
              </div>
              <div>
                ...
                <div style={{ fontSize: 12 }}>
                  {profile.contract_signed ? (
                    <span style={{ color: '#22c55e' }}>
                      ✓ Firmado el {fmtDate(profile.contract_signed_at)}
                    </span>
                  ) : (
                    <div>
                      <span style={{ color: '#ef4444' }}>✗ Sin firmar</span>
                      <button
                        onClick={() => api.signTutorContract()}
                        style={{
                          marginLeft: 10,
                          padding: '4px 12px',
                          borderRadius: 6,
          ...
          {classes.length === 0 ? (
            <div className="card" style={{ padding: 40, textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>
                Tus clases programadas y pasadas aparecerán aquí.
              </p>
            </div>
          ) : (
            <div style={{ display: 'grid', gap: 10 }}>
              {classes.map((cls: any) => {
                ...
                const unrated = enrollments.filter(
                  (e: any) => e.payment_status === 'paid' && !e.tutor_rating_of_student
                );
                  ...
                  >
                    <div style={{ flex: 1, minWidth: 140 }}>
                      <div style={{ fontWeight: 700, marginBottom: 4 }}>
                        {cls.subject || cls.title || 'Clase'}
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                        {fmtDate(cls.scheduled_at)} ·{' '}
                        {enrollments.length || cls.enrolled_count || 0} estudiante
                        {(enrollments.length || cls.enrolled_count || 0) !== 1 ? 's' : ''}
                      </div>
                      {needsBoleta && (
                        <div
                    ...
                    {canEnter && (
                      <button
                        ...
                        style={{
                          ...
                          borderRadius: 8,
                          border: 'none',
                          background: cls.status === 'in_progress' ? '#22c55e' : 'var(--accent)',
                          color: '#fff',
                          fontWeight: 700,
                          fontSize: 12,
                          cursor: 'pointer',
                    ...
                    {canRate && (
                      <button
                        onClick={() => {
                          setRatingModal({
                            classId: cls.id,
                            className: cls.title || cls.subject || 'Clase',
                      ...
                      <button
                        onClick={() => {
                          setBoletaModal({
                            ...
                            tutorAmount: pendingPayment.tutor_amount,
                            classTitle: cls.title || cls.subject || 'Clase',
                          });
                          setBoletaNum('');
                          setBoletaAmount(String(Math.round(pendingPayment.tutor_amount)));
                          setBoletaFile(null);
                          setBoletaSuccess(false);
                        }}
                        style={{
      ...
      {tab === 'materias' && (
        <div>
          ...
          {showSubjectForm && (
            <div
              className="card"
              style={{ padding: 20, marginBottom: 20, borderLeft: '4px solid #f59e0b' }}
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                <div>
                  ...
                  <input
                    ...
                    max={2}
                    step={0.5}
                    value={subjectForm.duration_hours}
                    onChange={(e) =>
                      setSubjectForm((p) => ({
                        ...p,
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setSubjectForm((p) => ({ ...p, max_students: parseInt(e.target.value) || 1 }))
                    }
                    style={{
                      width: '100%',
                      padding: '8px 10px',
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  <label style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)' }}>
                    Descripción * (mínimo requerido para enviar a revisión)
                  </label>
                  <textarea
                    value={subjectForm.description}
                    onChange={(e) => setSubjectForm((p) => ({ ...p, description: e.target.value }))}
                    rows={3}
                    placeholder="Describe qué aprenderá el estudiante en esta asignatura..."
                    style={{
                      width: '100%',
                      padding: '8px 10px',
                      borderRadius: 8,
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <textarea
                    value={subjectForm.learning_objectives}
                    onChange={(e) =>
                      setSubjectForm((p) => ({ ...p, learning_objectives: e.target.value }))
                    }
                    rows={2}
                    placeholder="Al finalizar la clase, el estudiante podrá..."
                    style={{
                      width: '100%',
                      padding: '8px 10px',
                      borderRadius: 8,
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setSubjectForm((p) => ({ ...p, prerequisites: e.target.value }))
                    }
                    rows={2}
                    placeholder="El estudiante debe saber..."
                    style={{
                      width: '100%',
                      padding: '8px 10px',
                      borderRadius: 8,
              ...
              <div style={{ marginTop: 14, display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                ...
                <button
                  ...
                  disabled={
                    subjectSaving || !subjectForm.name.trim() || !subjectForm.description.trim()
                  }
                  style={{
                    padding: '8px 20px',
                    borderRadius: 8,
                    ...
                    fontSize: 13,
                    cursor: 'pointer',
                    opacity: subjectSaving || !subjectForm.name.trim() ? 0.6 : 1,
                  }}
                >
              ...
              <div style={{ marginTop: 10, fontSize: 11, color: 'var(--text-muted)' }}>
                💡 La asignatura se guarda como borrador. Deberás enviarla a revisión para que
                CEO/UTP la apruebe antes de poder usarla en clases.
              </div>
            </div>
          )}
          ...
          {subjects.length === 0 && !showSubjectForm && (
            <div className="card" style={{ padding: 40, textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>
                Crea tu primera asignatura. El equipo Conniku la revisará y aprobará.
              </p>
            </div>
          )}
          ...
          {subjects.map((s: any) => {
            ...
            return (
              <div
                key={s.id}
                className="card"
                style={{ padding: 16, marginBottom: 10, borderLeft: `4px solid ${subStatusColor}` }}
              >
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 16 }}>
                  <div style={{ flex: 1 }}>
                    ...
                    >
                      ...
                      <span style={{ fontSize: 11, fontWeight: 700, color: subStatusColor }}>
                        {subStatusLabel}
                      </span>
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
                      {s.category && `${s.category} · `}
                      {s.level && `${s.level} · `}⏱ {s.duration_hours}h · 👥 Máx {s.max_students}
                    </div>
                    {s.description && (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                    {(s.status === 'draft' || s.status === 'rejected') && (
                      <button
                        onClick={() => handleSubmitSubject(s.id)}
                        style={{
                          padding: '6px 12px',
                          borderRadius: 8,
                          border: 'none',
        ...
        <div>
          {payments.length === 0 ? (
            <div className="card" style={{ padding: 40, textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>
                Tus pagos aparecen después de que una clase sea confirmada.
              </p>
            </div>
          ) : (
            <div style={{ display: 'grid', gap: 10 }}>
              {payments.map((p: any) => {
                ...
                const psm = payStatusMeta[p.payment_status] || {
                ...
                return (
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, marginBottom: 4 }}>
                        Pago · {fmtDate(p.created_at)}
                      </div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                        Bruto: {fmtClp(p.gross_amount)} · Comisión: {fmtClp(p.conniku_commission)} ·{' '}
                        <strong>Neto: {fmtClp(p.tutor_amount)}</strong>
                      </div>
                    </div>
                    <span
                      style={{
                    ...
                    {(p.payment_status === 'pending_boleta' || p.payment_status === 'rejected') && (
                      <button
                        onClick={() => {
                          setBoletaModal({
                            paymentId: p.id,
                            tutorAmount: p.tutor_amount,
                            classTitle: `Pago ${fmtDate(p.created_at)}`,
                          });
                          setBoletaNum('');
                          setBoletaAmount(String(Math.round(p.tutor_amount)));
                          setBoletaFile(null);
                          setBoletaSuccess(false);
                        }}
                        style={{
      ...
      {tab === 'boletas' && (
        <div>
          ...
          >
            🧾 Aquí aparecen todas las boletas de honorarios que has enviado a Conniku. Cada vez que
            envías una boleta, una copia llega automáticamente al equipo para procesar tu pago en
            máx. <strong>7 días hábiles</strong>.
          </div>
          {boletas.length === 0 ? (
            <div className="card" style={{ padding: 40, textAlign: 'center' }}>
              <div style={{ fontSize: 40, marginBottom: 12 }}>🧾</div>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>
                Cuando termines una clase y envíes tu boleta, aparecerá aquí.
              </p>
            </div>
          ) : (
            <div style={{ display: 'grid', gap: 10 }}>
              {boletas.map((b: any) => {
                ...
                return (
                  ...
                  >
                      ...
                      <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                        ...
                        {pmt && (
                          <>
                            {' '}
                            · Monto:{' '}
                            <strong>{fmtClp(pmt.tutor_amount || pmt.boleta_amount)}</strong>
                          </>
                        )}
                      </div>
                    </div>
                    ...
                    {b.has_file && (
                      <button
                        onClick={() => downloadBoleta(b.id, b.name + '.pdf')}
                        style={{
                          padding: '6px 14px',
                          borderRadius: 8,
                          border: '1px solid var(--border)',
      ...
      {boletaModal && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget && !boletaSending) closeBoletaModal();
          }}
        >
            ...
            {boletaSuccess ? (
              ...
              <>
                <h3 style={{ margin: '0 0 4px', fontSize: 18, fontWeight: 700 }}>
                  🧾 Enviar Boleta de Honorarios
                </h3>
                <p style={{ margin: '0 0 20px', fontSize: 13, color: 'var(--text-muted)' }}>
                  {boletaModal.classTitle}
                </p>
                ...
                >
                  💡 Emite la boleta electrónica en <strong>sii.cl</strong> a nombre de{' '}
                  <strong>Conniku SpA, RUT 78.395.702-7</strong>
                  por el monto de <strong>{fmtClp(boletaModal.tutorAmount)}</strong>. Luego sube el
                  PDF aquí.
                </div>
                ...
                <div style={{ display: 'grid', gap: 14 }}>
                  <div>
                    ...
                    <input
                      value={boletaNum}
                      onChange={(e) => setBoletaNum(e.target.value)}
                      placeholder="ej. 123"
                      style={{
                        width: '100%',
                        padding: '9px 12px',
                  ...
                  <div>
                    ...
                    <input
                      type="number"
                      value={boletaAmount}
                      onChange={(e) => setBoletaAmount(e.target.value)}
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>
                      Monto a pagar: {fmtClp(boletaModal.tutorAmount)} (tolerancia máx. $100)
                    </div>
                  </div>
                  <div>
                    <label
                    ...
                    <label
                      style={{
                        ...
                        padding: '10px 14px',
                        borderRadius: 8,
                        border: `2px dashed ${boletaFile ? '#8b5cf6' : 'var(--border)'}`,
                        background: boletaFile ? '#8b5cf611' : 'var(--bg-secondary)',
                        cursor: 'pointer',
                        fontSize: 13,
                        color: boletaFile ? '#8b5cf6' : 'var(--text-muted)',
                      }}
                    >
                      <span>
                        {boletaFile ? `✓ ${boletaFile.name}` : '📎 Haz clic para seleccionar PDF'}
                      </span>
                      <input
                        type="file"
                ...
                >
                  ...
                  <button
                    onClick={handleUploadBoleta}
                    disabled={boletaSending || !boletaNum.trim() || !boletaAmount || !boletaFile}
                    style={{
                      padding: '9px 20px',
                      borderRadius: 8,
                      border: 'none',
                      ...
                      cursor: 'pointer',
                      opacity:
                        !boletaNum.trim() || !boletaAmount || !boletaFile || boletaSending
                          ? 0.6
                          : 1,
                    }}
                  >
      ...
      {ratingModal && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget) setRatingModal(null);
          }}
        >
          <div className="card" style={{ width: '100%', maxWidth: 440, padding: 28 }}>
            ...
            <p style={{ margin: '0 0 20px', fontSize: 12, color: 'var(--text-muted)' }}>
              Esta calificación aplica a los {ratingModal.unratedCount} estudiante
              {ratingModal.unratedCount !== 1 ? 's' : ''} sin calificar de esta clase.
            </p>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  fontSize: 13,
                  cursor: ratingSaving ? 'wait' : 'pointer',
