...
const ClassRoom: React.FC<ClassRoomProps> = ({ classId, onNavigate }) => {
  ...
  const handleSubmitObjection = async () => {
    if (!termsAccepted) { setObjectionError('Debes aceptar los términos y condiciones.'); return }
    if (!wordCountOk) { setObjectionError(`Escribe entre 30 y 500 palabras (tienes ${wordCount}).`); return }
    setObjectionError('')
  ...
  const handleSubmitRating = async () => {
    if (!ratingValue) { setRatingError('Selecciona una calificación.'); return }
  ...
  const statusLabel = (s: string) => {
    const m: Record<string, string> = {
      published: 'Publicada', confirmed: 'Confirmada', in_progress: 'En curso',
      completed: 'Completada', cancelled: 'Cancelada', disputed: 'Objetada',
    }
    return m[s] || s
  }
  ...
  const statusColor = (s: string) => {
    const m: Record<string, string> = {
      in_progress: '#22c55e', confirmed: '#3b82f6', published: '#f59e0b',
      completed: '#8b5cf6', disputed: '#ef4444', cancelled: '#6b7280',
    }
    return m[s] || '#6b7280'
  }
  ...
  if (error || !classData) {
    return (
      <div style={{ padding: 40, textAlign: 'center' }}>
        ...
        <button onClick={() => onNavigate('/my-tutor/clases')}
          style={{ marginTop: 16, padding: '8px 20px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer' }}>
          Volver
        </button>
      </div>
    )
  ...
  const canObject = !isTutor && ['in_progress', 'confirmed', 'completed', 'disputed'].includes(status) && !classEnded && !objectionSubmitted
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 60px)', overflow: 'hidden' }}>
      ...
      {/* ── Header ──────────────────────────────────────────── */}
      <div className="card" style={{ borderRadius: 0, borderLeft: 'none', borderRight: 'none', borderTop: 'none', padding: '10px 20px', display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0, zIndex: 10 }}>
        <button onClick={() => onNavigate('/my-tutor/clases')}
          style={{ padding: '6px 12px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-primary)', cursor: 'pointer', fontSize: 13 }}>
          ← Salir
        </button>
        ...
        {isTutor && status === 'confirmed' && (
          <button onClick={handleStartClass} disabled={startingClass}
            style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#22c55e', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer', opacity: startingClass ? 0.7 : 1 }}>
        ...
        {canEnd && (
          <button onClick={() => setConfirmingEnd(true)}
            style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: '#3b82f6', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            ✓ Terminar Clase
          </button>
        )}
        {classEnded && (
          <span style={{ padding: '6px 14px', borderRadius: 8, background: '#22c55e22', color: '#22c55e', fontWeight: 700, fontSize: 12 }}>
            ✓ Clase terminada
          </span>
        )}
        {canObject && (
          <button onClick={() => setShowObjection(true)}
            style={{ padding: '8px 16px', borderRadius: 8, border: '1px solid #ef4444', background: 'transparent', color: '#ef4444', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            ⚠ Objetar
          </button>
        )}
        {objectionSubmitted && (
          <span style={{ padding: '6px 14px', borderRadius: 8, background: '#ef444422', color: '#ef4444', fontWeight: 700, fontSize: 12 }}>
            Objeción enviada
          </span>
        )}
      </div>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden', gap: 0 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100%', color: '#fff', gap: 16 }}>
              ...
              {status === 'completed' ? (
                <>
                  ...
                  <div style={{ fontSize: 14, color: '#aaa' }}>La sesión de video ha finalizado.</div>
                  <button onClick={() => onNavigate('/my-tutor/clases')}
                    style={{ padding: '10px 24px', borderRadius: 8, border: 'none', background: '#3b82f6', color: '#fff', fontWeight: 700, cursor: 'pointer' }}>
                    Ver mis clases
                  </button>
                </>
              ) : status === 'disputed' ? (
                <>
                  ...
                  <div style={{ fontSize: 14, color: '#aaa' }}>Se ha enviado una objeción. El equipo de Conniku resolverá en máx. 7 días hábiles.</div>
                </>
              ) : (
                <>
        ...
        {/* Chat panel */}
        <div style={{ width: 300, display: 'flex', flexDirection: 'column', borderLeft: '1px solid var(--border)', background: 'var(--bg-primary)', flexShrink: 0 }}>
          <div style={{ padding: '10px 14px', borderBottom: '1px solid var(--border)', fontWeight: 700, fontSize: 13 }}>
            💬 Chat de clase
          </div>
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px 12px', display: 'flex', flexDirection: 'column', gap: 8 }}>
            {messages.length === 0 ? (
              <p style={{ color: 'var(--text-muted)', fontSize: 12, textAlign: 'center', marginTop: 40 }}>
                Sin mensajes aún. Sé el primero en escribir.
              </p>
            ) : (
              messages.map((msg: any) => (
                <div key={msg.id} style={{ maxWidth: '90%', alignSelf: msg.is_own ? 'flex-end' : 'flex-start' }}>
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 2, textAlign: msg.is_own ? 'right' : 'left' }}>
                    {msg.sender_name} · {fmtTime(msg.created_at)}
                  </div>
                  <div style={{ padding: '7px 11px', borderRadius: 12, fontSize: 13, background: msg.is_own ? 'var(--accent)' : 'var(--bg-secondary)', color: msg.is_own ? '#fff' : 'var(--text-primary)', wordBreak: 'break-word' }}>
                    {msg.content}
                  </div>
                </div>
              ))
          ...
          <form onSubmit={handleSendMessage} style={{ padding: '8px 10px', borderTop: '1px solid var(--border)', display: 'flex', gap: 6 }}>
            <input
              value={newMessage}
              onChange={e => setNewMessage(e.target.value)}
              placeholder="Escribe un mensaje…"
              disabled={sendingMsg}
              style={{ flex: 1, padding: '7px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 13, outline: 'none' }}
            />
            <button type="submit" disabled={sendingMsg || !newMessage.trim()}
              style={{ padding: '7px 12px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontSize: 13, opacity: sendingMsg || !newMessage.trim() ? 0.5 : 1 }}>
              ➤
            </button>
          </form>
        </div>
          ...
          <div className="card" style={{ maxWidth: 420, width: '100%', padding: 28, textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button onClick={() => setConfirmingEnd(false)}
                style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-primary)', cursor: 'pointer' }}>
                Cancelar
              </button>
              <button onClick={handleEndClass} disabled={endingClass}
                style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: '#22c55e', color: '#fff', fontWeight: 700, cursor: 'pointer', opacity: endingClass ? 0.7 : 1 }}>
      ...
      {/* ── Objection Modal ─────────────────────────────────────── */}
      {showObjection && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1000, display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '20px 16px', overflowY: 'auto' }}>
          <div className="card" style={{ maxWidth: 560, width: '100%', padding: 28, marginTop: 20 }}>
            ...
            {objectionSubmitted ? (
              ...
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <p style={{ color: 'var(--text-primary)', fontSize: 14, fontWeight: 700, marginBottom: 24 }}>
                  En un máximo de <span style={{ color: 'var(--accent)' }}>7 días hábiles</span> recibirás una resolución por correo y notificación en la plataforma.
                </p>
                <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 20 }}>
                  El pago queda retenido hasta la resolución final. Puedes cerrar esta ventana.
                </p>
                <button onClick={() => setShowObjection(false)}
                  style={{ padding: '10px 24px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff', fontWeight: 700, cursor: 'pointer' }}>
                  Cerrar
                </button>
              </div>
            ) : (
              ...
              <>
                ...
                <div style={{ background: '#ef444411', border: '1px solid #ef444433', borderRadius: 8, padding: '10px 14px', marginBottom: 18, fontSize: 13 }}>
                  <strong>Importante:</strong> Esta objeción será enviada inmediatamente al tutor y al equipo de Conniku. El pago quedará retenido hasta la resolución (máx. 7 días hábiles).
                </div>
                ...
                <textarea
                  value={objectionText}
                  onChange={e => { setObjectionText(e.target.value); setObjectionError('') }}
                  ...
                  rows={7}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: 8, border: `1px solid ${wordCount > 0 && !wordCountOk ? '#ef4444' : 'var(--border)'}`, background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 13, resize: 'vertical', boxSizing: 'border-box', fontFamily: 'inherit', lineHeight: 1.5 }}
                />
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: wordCount > 500 ? '#ef4444' : wordCount < 30 && wordCount > 0 ? '#f59e0b' : 'var(--text-muted)', marginBottom: 18, marginTop: 4 }}>
                  <span>{wordCount} palabras</span>
                  <span>{wordCount < 30 ? `Mínimo 30 palabras (faltan ${30 - wordCount})` : wordCount > 500 ? `Máximo 500 palabras (excedes por ${wordCount - 500})` : '✓ Longitud correcta'}</span>
                </div>
                ...
                <div style={{ border: '1px solid var(--border)', borderRadius: 8, marginBottom: 16 }}>
                  <button
                    ...
                    style={{ width: '100%', padding: '10px 14px', background: 'none', border: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', cursor: 'pointer', color: 'var(--text-primary)', fontSize: 13, fontWeight: 700 }}>
                    <span>📄 Términos y Condiciones de Objeción</span>
                    <span>{showTerms ? '▲' : '▼'}</span>
                  </button>
                  {showTerms && (
                    <div style={{ padding: '0 14px 14px', maxHeight: 220, overflowY: 'auto' }}>
                      <pre style={{ fontSize: 11, lineHeight: 1.6, color: 'var(--text-muted)', whiteSpace: 'pre-wrap', fontFamily: 'inherit', margin: 0 }}>
                        {OBJECTION_TERMS}
                      </pre>
                    </div>
                  )}
                ...
                <label style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer', fontSize: 13, marginBottom: 20 }}>
                  <input
                    type="checkbox"
                    checked={termsAccepted}
                    onChange={e => { setTermsAccepted(e.target.checked); setObjectionError('') }}
                    style={{ marginTop: 2, accentColor: 'var(--accent)', width: 16, height: 16, flexShrink: 0 }}
                  />
                  <span>
                    He leído y acepto los{' '}
                    <button onClick={() => setShowTerms(true)} style={{ background: 'none', border: 'none', padding: 0, color: 'var(--accent)', cursor: 'pointer', fontSize: 13, textDecoration: 'underline' }}>
                ...
                {objectionError && (
                  <div style={{ padding: '8px 12px', borderRadius: 8, background: '#ef444422', color: '#ef4444', fontSize: 13, marginBottom: 16 }}>
                    {objectionError}
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
                  <button onClick={() => setShowObjection(false)}
                    style={{ padding: '10px 20px', borderRadius: 8, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text-primary)', cursor: 'pointer', fontSize: 13 }}>
                    Cancelar
                  </button>
                  <button
                    onClick={handleSubmitObjection}
                    disabled={submittingObjection || !termsAccepted || !wordCountOk}
                    style={{ padding: '10px 20px', borderRadius: 8, border: 'none', background: '#ef4444', color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer', opacity: (submittingObjection || !termsAccepted || !wordCountOk) ? 0.5 : 1 }}>
          ...
          <div className="card" style={{ maxWidth: 480, width: '100%', padding: 28 }}>
            <div style={{ textAlign: 'center', marginBottom: 24 }}>
              ...
              <p style={{ color: 'var(--text-muted)', fontSize: 13, margin: 0 }}>
                {isTutor ? 'Gracias por tu clase. Sube tu boleta para recibir el pago.' : '¡Bien hecho! Completa las siguientes acciones.'}
              </p>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              {!isTutor && (
                <div className="card" style={{ padding: 16, background: 'var(--bg-secondary)' }}>
                  <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 10 }}>⭐ Califica al tutor</div>
                  {ratingSubmitted ? (
                    <p style={{ color: '#22c55e', fontSize: 13, margin: 0 }}>✓ Calificación enviada. ¡Gracias!</p>
                  ) : (
                    <>
                      <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
                        {[1, 2, 3, 4, 5].map(n => (
                          <button key={n} onClick={() => setRatingValue(n)}
                            style={{ fontSize: 24, background: 'none', border: 'none', cursor: 'pointer', opacity: n <= ratingValue ? 1 : 0.3, transition: 'opacity 0.15s' }}>
                            ★
                          </button>
                        ))}
                      </div>
                      <textarea value={ratingComment} onChange={e => setRatingComment(e.target.value)}
                        ...
                        style={{ width: '100%', padding: '8px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontSize: 12, resize: 'none', boxSizing: 'border-box', fontFamily: 'inherit', marginBottom: 8 }} />
                      {ratingError && <p style={{ color: '#ef4444', fontSize: 12, marginBottom: 6 }}>{ratingError}</p>}
                      <button onClick={handleSubmitRating} disabled={ratingSubmitting || !ratingValue}
                        style={{ padding: '8px 16px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff', fontWeight: 700, fontSize: 12, cursor: 'pointer', opacity: (ratingSubmitting || !ratingValue) ? 0.5 : 1 }}>
              ...
              {!isTutor && (
                <button onClick={() => setShowExam(true)}
                  style={{ padding: '14px 16px', borderRadius: 10, border: '2px solid var(--accent)', background: 'transparent', color: 'var(--accent)', fontWeight: 700, fontSize: 13, cursor: 'pointer', textAlign: 'left' }}>
                  📝 Rendir examen post-clase
                  <span style={{ display: 'block', fontSize: 11, fontWeight: 400, color: 'var(--text-muted)', marginTop: 2 }}>Máx. 3 intentos. El tercero muestra advertencia de garantía.</span>
                </button>
              )}

              {/* AI Summary */}
              <div className="card" style={{ padding: 16, background: 'var(--bg-secondary)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: summaryText ? 10 : 0 }}>
                  <div style={{ fontWeight: 700, fontSize: 13 }}>🤖 Resumen IA de la clase</div>
                  <button onClick={handleGenerateSummary} disabled={summaryLoading}
                    style={{ padding: '6px 12px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#fff', fontSize: 11, fontWeight: 700, cursor: 'pointer', opacity: summaryLoading ? 0.6 : 1 }}>
                    {summaryLoading ? 'Generando…' : summaryText ? 'Regenerar' : 'Generar'}
                  </button>
                </div>
                {summaryText && (
                  <div style={{ fontSize: 12, lineHeight: 1.7, color: 'var(--text-primary)', maxHeight: 180, overflowY: 'auto', whiteSpace: 'pre-wrap', marginTop: 8, padding: '8px 10px', background: 'var(--bg-primary)', borderRadius: 6 }}>
                    {summaryText}
                  </div>
                )}
              </div>
            ...
            <div style={{ marginTop: 20, textAlign: 'center' }}>
              <button onClick={() => { setShowPostClass(false); onNavigate('/my-tutor/clases') }}
                style={{ padding: '10px 24px', borderRadius: 8, border: 'none', background: '#6b7280', color: '#fff', fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>
                Volver a Mis Clases
              </button>
            </div>
          </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1001, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div className="card" style={{ maxWidth: 560, width: '100%', maxHeight: '90vh', overflowY: 'auto' }}>
            <div style={{ padding: '12px 20px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => setShowExam(false)} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: 'var(--text-muted)' }}>×</button>
            </div>
