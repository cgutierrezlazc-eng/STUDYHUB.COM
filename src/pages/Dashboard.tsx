...
const PHILOSOPHY_QUOTES = [
  {
    quote: 'Educar la mente sin educar el corazón no es educación en absoluto.',
    author: 'Aristóteles',
  },
  {
    quote: 'La mente no es un recipiente que se llena, sino un fuego que se enciende.',
    author: 'Plutarco',
  },
  ...
  {
    quote: 'Solo hay un bien: el conocimiento. Solo hay un mal: la ignorancia.',
    author: 'Sócrates',
  },
  ...
  {
    quote: 'Toda la educación viene de dentro; tú no puedes obtenerla de nadie más.',
    author: 'Ralph Waldo Emerson',
  },
  { quote: 'La raíz del aprendizaje es amarga, pero su fruto es dulce.', author: 'Aristóteles' },
  {
    quote: 'El hombre sabio no da las respuestas correctas, hace las preguntas correctas.',
...
function OnboardingSection({ user, projects, onNavigate, onNewProject }: OnboardingProps) {
  const [visited, setVisited] = useState<Record<string, boolean>>(() => {
    try {
      return JSON.parse(localStorage.getItem('ob_visited') || '{}');
    } catch {
      return {};
    }
  });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div className="ob-hero">
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <button
            className="ob-hero-btn"
            onClick={() => (onNewProject ? onNewProject() : onNavigate('/dashboard'))}
          >
        ...
        <div className="ob-feature-grid">
          {features.map((f) => (
            <button
              ...
              onClick={() =>
                f.path === '__new_project__' && onNewProject ? onNewProject() : onNavigate(f.path)
              }
            >
              <div className="ob-feature-icon" style={{ background: f.accentColor + '18' }}>
                {f.emoji}
              </div>
...
export default function Dashboard({ projects, onNavigate, onNewProject }: Props) {
  ...
  useEffect(() => {
        ...
        if (data.milestones?.length > 0) {
          ...
          if (m.type === 'level_up')
            setMilestonePopup({
              type: 'level_up',
              title: '¡Nuevo nivel!',
              description: `Has alcanzado el nivel ${m.level}`,
              icon: '⬆️',
            });
            ...
            setMilestonePopup({
              ...
              description: `${m.days} días consecutivos de estudio`,
              icon: '🔥',
            });
            ...
            setMilestonePopup({
              ...
              description: `${m.emoji} ${m.name}: ${m.description}`,
              icon: m.emoji || '🏅',
            });
        }
      })
    ...
    api
      .getActivityFeed(1)
      .then((data) => setFriendActivity((data.items || data).slice(0, 5)))
      .catch(() => {});
    api
      .getCalendarEvents()
      .then((d) => {
        ...
        {isNewUser ? (
          ...
          <>
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  <div
                    style={{
                      ...
                      flexShrink: 0,
                      background:
                        dailySummary.mood === 'positive'
                          ? 'rgba(5,150,105,0.08)'
                          : dailySummary.mood === 'alert'
                            ? 'rgba(217,119,6,0.08)'
                            : 'rgba(37,99,235,0.08)',
                      display: 'flex',
                      alignItems: 'center',
              ...
              <div className="u-card" style={{ padding: 0, overflow: 'hidden' }}>
                ...
                {friendActivity.length > 0 ? (
                  friendActivity.map((act: any, i: number) => {
                    ...
                    const meta = activityLabel[typeKey] || {
                      label: act.activityType || 'publicó',
                      color: '#3b82f6',
                      emoji: '📝',
                    };
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          padding: '10px 16px',
                          borderBottom:
                            i < friendActivity.length - 1 ? '1px solid var(--border)' : 'none',
                      ...
                      {league.leaderboard.slice(0, 5).map((entry: any) => (
                        <div
                          ...
                          style={{
                            ...
                            color:
                              entry.rank <= league.promotionZone
                                ? 'var(--accent-green)'
                                : 'var(--text-primary)',
                          }}
                        >
                          <span style={{ width: 24, fontSize: 13, color: 'var(--text-muted)' }}>
                            #{entry.rank}
                          </span>
                          <div
                            style={{
            ...
            {totalDocs === 0 && projects.length > 0 && (
              <div
                className="u-card"
                style={{ marginTop: 20, padding: 16, borderLeft: '4px solid var(--accent-orange)' }}
              >
                <strong style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                  {Lightbulb({ size: 16, color: 'var(--accent-orange)' })} Consejo:
                </strong>{' '}
                Sube documentos a una asignatura para que Conniku genere guías, quizzes y flashcards
                automáticamente.
              </div>
            )}
                  ...
                  >
                    ...
                    <div className="project-info">
                      ...
                      <span className="project-doc-count">
                        {project.documents.length} documentos
                      </span>
                    </div>
                  </div>
                ))}
