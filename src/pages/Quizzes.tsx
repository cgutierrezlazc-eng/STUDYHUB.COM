...
function getQuizHistory(): QuizHistoryEntry[] {
  try { return JSON.parse(localStorage.getItem(QUIZ_HISTORY_KEY) || '[]') }
  catch { return [] }
}
...
export default function Quizzes({ projects, onNavigate }: Props) {
  ...
  const diffLabels: Record<Difficulty, string> = { easy: 'Fácil', medium: 'Medio', hard: 'Difícil' }
    ...
    if (projects.length === 0) {
      return (
        <div className="u-card" style={{ padding: 48, textAlign: 'center', color: 'var(--text-muted)' }}>
          ...
          <p>Crea una asignatura para empezar a generar {tab === 'quiz' ? 'quizzes' : 'flashcards'}.</p>
        </div>
      )
    }
    return (
          ...
          {projects.map(p => {
            ...
            return (
              <div key={p.id} className="u-card" style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
                      ...
                      {tab === 'quiz' && ph.length > 0 && <span> · {ph.length} quiz{ph.length !== 1 ? 'zes' : ''}</span>}
                      {tab === 'quiz' && avgPct !== null && <span style={{ color: scoreColor(avgPct), fontWeight: 600 }}> · avg {avgPct}%</span>}
                    </div>
                  </div>
                </div>
                ...
                >
                  {hasDocs ? (tab === 'quiz' ? 'Ir al Quiz →' : 'Ir a Flashcards →') : 'Sin documentos'}
                </button>
              </div>
            )
          })}
            ...
            <>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 24 }}>
                  {bestSubject && (
                    <div className="u-card" style={{ padding: 18 }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: '#10B981', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: 0.5 }}>Mejor materia</p>
                      <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 2px' }}>{bestSubject.name}</p>
                      <p style={{ fontSize: 14, color: '#10B981', margin: 0, fontWeight: 600 }}>{bestSubject.avg}% promedio ({bestSubject.count} quizzes)</p>
                    </div>
                  )}
                  {worstSubject && subjectAvgs.length > 1 && (
                    <div className="u-card" style={{ padding: 18 }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: '#EF4444', margin: '0 0 6px', textTransform: 'uppercase', letterSpacing: 0.5 }}>Para repasar</p>
                      <p style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-primary)', margin: '0 0 2px' }}>{worstSubject.name}</p>
                      <p style={{ fontSize: 14, color: '#EF4444', margin: 0, fontWeight: 600 }}>{worstSubject.avg}% promedio ({worstSubject.count} quizzes)</p>
                    </div>
                  )}
                </div>
              )}
              ...
              {chartData.length > 0 && (
                <div className="u-card" style={{ padding: 24, marginBottom: 24 }}>
                  <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 20px' }}>Puntajes recientes</h3>
                  <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 160 }}>
                    {chartData.map((d, i) => (
                      <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, height: '100%', justifyContent: 'flex-end' }}>
                        ...
                        <span style={{ fontSize: 10, color: 'var(--text-secondary)', textAlign: 'center', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 60 }}>
                          {d.label}
                        </span>
                      </div>
                    ))}
              ...
              <div className="u-card" style={{ padding: 24 }}>
                <h3 style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)', margin: '0 0 16px' }}>Rendimiento por materia</h3>
                  ...
                  {subjectAvgs.map((s, i) => (
                    <div key={i}>
                      ...
                      <div style={{ height: 8, borderRadius: 4, background: 'var(--bg-secondary)', overflow: 'hidden' }}>
                        <div style={{ height: '100%', borderRadius: 4, background: scoreColor(s.avg), width: `${s.avg}%`, transition: 'width 0.5s ease' }} />
                      </div>
                    </div>
                  ))}
                </div>
