...
import { BookOpen, Target, MessageSquare, Users, Video, Timer, PlayCircle, RotateCcw, Send, Clock, Trash2, Pencil, Check, X, Save, BarChart3, Crown, Shield, Lock } from '../components/Icons'
...
function loadChat(roomId: string): ChatMessage[] {
  try { return JSON.parse(localStorage.getItem(LS_CHAT_PREFIX + roomId) || '[]') } catch { return [] }
}
...
function loadTimerState(): TimerState | null {
  try {
    ...
    if (Date.now() - state.startedAt > 4 * 60 * 60 * 1000) { localStorage.removeItem(LS_TIMER); return null }
    return state
  } catch { return null }
}
function saveTimerState(state: TimerState | null) {
  if (!state) { localStorage.removeItem(LS_TIMER); return }
...
export default function StudyRooms({ onNavigate }: Props) {
  ...
  const [summaryData, setSummaryData] = useState<{ study: number; breaks: number; score: number; roomName: string } | null>(null)
  ...
  const [participants, setParticipants] = useState<{ name: string; avatar: string; studyMin: number; isHost: boolean }[]>([])
  ...
  useEffect(() => { loadRooms(); setSessions(loadSessions()) }, [])
  ...
  const enterRoom = (room: any, isCreator = false) => {
    ...
    // Set participants
    const me = { name: userName, avatar: userName[0]?.toUpperCase() || '?', studyMin: 0, isHost: isCreator }
    const hostEntry = room.host && !isCreator
      ? { name: room.host.firstName || 'Host', avatar: (room.host.firstName || 'H')[0].toUpperCase(), studyMin: 0, isHost: true }
      : null
  ...
  const handleLeave = async () => {
    ...
    setSummaryData({ study: totalStudyMin, breaks: totalBreakMin, score, roomName: activeRoom.name })
    setShowSummary(true)
  ...
  const handleSaveSettings = () => {
    if (activeRoom) {
      setActiveRoom({ ...activeRoom, name: editName, subject: editSubject, isPrivate: editPrivate })
    }
  ...
  const fmtChatTime = (ts: number) => new Date(ts).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })

  const isRoomCreator = activeRoom?.host?.id === user?.id || participants.some(p => p.isHost && p.name === (user?.firstName || ''))
    ...
    return (
      <>
        ...
        <div className="page-body" style={{ display: 'flex', justifyContent: 'center', paddingTop: 32 }}>
          <div className="u-card" style={{ padding: 32, maxWidth: 420, width: '100%', textAlign: 'center' }}>
            <h3 style={{ margin: '0 0 4px' }}>{summaryData.roomName}</h3>
            <p style={{ color: 'var(--text-muted)', fontSize: 13, margin: '0 0 24px' }}>Sesión finalizada</p>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, marginBottom: 24 }}>
              <div>
                <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--accent)' }}>{summaryData.study}</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>min estudio</div>
              </div>
              <div>
                <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--accent-green, #22c55e)' }}>{summaryData.breaks}</div>
                <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>min descanso</div>
              </div>
              <div>
                <div style={{ fontSize: 28, fontWeight: 700, color: summaryData.score >= 70 ? 'var(--accent-green, #22c55e)' : 'var(--accent-orange, #f59e0b)' }}>{summaryData.score}%</div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
              ...
              <button className="btn btn-secondary" onClick={() => { setShowSummary(false); setSummaryData(null) }}>
                Descartar
              </button>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              ...
              <p>
                ...
                {pomodoroRunning && <span style={{ marginLeft: 8, color: 'var(--accent)', fontWeight: 600, fontSize: 12 }}>{Timer({ size: 12 })} Sesión en curso</span>}
              </p>
            </div>
        ...
        <div className="page-body" style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: '1 1 340px', display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="u-card" style={{ padding: 24, textAlign: 'center' }}>
              <div style={{ fontSize: 12, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.1em', color: pomodoroPhase === 'work' ? 'var(--accent)' : 'var(--accent-green, #22c55e)', marginBottom: 8 }}>
                {pomodoroPhase === 'work' ? <>{Target({ size: 14 })} Enfoque</> : <>{Timer({ size: 14 })} Descanso</>}
              </div>
              ...
              <div style={{ display: 'flex', gap: 12, justifyContent: 'center', marginTop: 16 }}>
                ...
                <button className="btn btn-secondary" onClick={() => {
                  ...
                  setPomodoroPhase('work')
                  setPomodoroRunning(false)
                }}>{RotateCcw({ size: 14 })} Reiniciar</button>
              </div>
            ...
            {activeRoom.meetingUrl && (
              <div className="u-card" style={{ padding: 16, textAlign: 'center' }}>
                ...
                <a href={activeRoom.meetingUrl} target="_blank" rel="noopener noreferrer" className="btn btn-primary btn-sm" style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
            ...
            <div className="u-card" style={{ padding: 16 }}>
              <h4 style={{ margin: '0 0 12px', fontSize: 14 }}>{Users()} Participantes ({participants.length})</h4>
                  ...
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 13 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 500 }}>
                        {p.name}
                        {p.isHost && <span style={{ marginLeft: 6, fontSize: 11, color: 'var(--accent)', fontWeight: 600 }}>{Crown({ size: 11 })} Anfitrión</span>}
                      </div>
                      <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{p.studyMin} min estudiados</div>
                    </div>
                    {isRoomCreator && !p.isHost && (
                      <button
                        onClick={() => handleKickParticipant(p.name)}
                        style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: 4, fontSize: 11 }}
          ...
          {/* Right column: Chat */}
          <div style={{ flex: '1 1 340px', display: 'flex', flexDirection: 'column', minHeight: 400 }}>
            <div className="u-card" style={{ padding: 0, display: 'flex', flexDirection: 'column', height: '100%', minHeight: 400 }}>
              ...
              {/* Messages */}
              <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 360 }}>
                ...
                {chatMessages.map(msg => (
                  <div key={msg.id} style={{
                    fontSize: 13,
                    ...(msg.system ? { textAlign: 'center', color: 'var(--text-muted)', fontStyle: 'italic', fontSize: 12, padding: '4px 0' } : {}),
                  }}>
                    {msg.system ? (
                      <span>{msg.text}</span>
                    ) : (
                      <>
                        <span style={{ fontWeight: 600, color: 'var(--accent)' }}>{msg.sender}</span>
                        <span style={{ color: 'var(--text-muted)', fontSize: 11, marginLeft: 6 }}>{fmtChatTime(msg.time)}</span>
          ...
          <div className="modal-overlay" onClick={() => setShowSettings(false)}>
            <div className="modal" onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, margin: '12px 0' }}>
                <input type="checkbox" checked={editPrivate} onChange={e => setEditPrivate(e.target.checked)} id="room-private" />
                <label htmlFor="room-private" style={{ fontSize: 13, cursor: 'pointer' }}>{Lock({ size: 13 })} Sala privada</label>
              </div>
              <div style={{ display: 'flex', gap: 8, marginTop: 16, flexWrap: 'wrap' }}>
                <button className="btn btn-primary" onClick={handleSaveSettings}>{Check({ size: 14 })} Guardar</button>
                ...
                >{Trash2({ size: 14 })} Cerrar Sala</button>
              </div>
            </div>
          </div>
        )}
      ...
      <>
        <div className="page-header page-enter">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button className="btn btn-secondary" onClick={() => setShowHistory(false)}>Volver a Salas</button>
          </div>
        </div>
        <div className="page-body">
          {/* Weekly summary */}
          <div className="u-card" style={{ padding: 20, marginBottom: 16, display: 'flex', alignItems: 'center', gap: 16 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 4 }}>Total esta semana</div>
              <div style={{ fontSize: 28, fontWeight: 700, color: 'var(--accent)' }}>
                {weeklyTotal} <span style={{ fontSize: 14, fontWeight: 400, color: 'var(--text-muted)' }}>minutos</span>
              </div>
            </div>
          ...
          {sessions.length === 0 ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {sessions.map(s => (
                <div key={s.id} className="u-card hover-lift" style={{ padding: 16, display: 'flex', alignItems: 'center', gap: 16 }}>
                  <div style={{ flex: 1 }}>
                  ...
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--accent)' }}>{s.studyMinutes}</div>
                    <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>min</div>
                  </div>
                  <div style={{ textAlign: 'center' }}>
                    <div style={{ fontSize: 18, fontWeight: 700, color: s.focusScore >= 70 ? 'var(--accent-green, #22c55e)' : 'var(--accent-orange, #f59e0b)' }}>{s.focusScore}%</div>
    ...
    <>
      <div className="page-header page-enter">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-secondary" onClick={() => { setSessions(loadSessions()); setShowHistory(true) }}>
      ...
      <div className="page-body">
        ...
        {loadTimerState() && (
          <div className="u-card" style={{ padding: '12px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 12, borderLeft: '3px solid var(--accent)' }}>
              ...
              <div key={room.id} className="u-card hover-lift" style={{ padding: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                    {room.isPrivate && <span title="Sala privada" style={{ color: 'var(--text-muted)' }}>{Lock({ size: 12 })}</span>}
                    <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 12, background: room.roomType === 'pomodoro' ? 'rgba(37,99,235,0.08)' : 'var(--bg-tertiary)', color: room.roomType === 'pomodoro' ? 'var(--accent)' : 'var(--text-muted)' }}>
                      {room.roomType === 'pomodoro' ? <>{Timer({ size: 12 })} Pomodoro</> : room.roomType === 'focus' ? <>{Target({ size: 12 })} Enfoque</> : <>{MessageSquare({ size: 12 })} Libre</>}
                    </span>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, color: 'var(--text-muted)' }}>
                  <span>{Users({ size: 12 })} {room.currentParticipants}/{room.maxParticipants}</span>
                  {room.host && <span>Host: {room.host.firstName}</span>}
                  {room.roomType === 'pomodoro' && <span>{Timer({ size: 12 })} {room.pomodoroWorkMin}/{room.pomodoroBreakMin} min</span>}
                </div>
