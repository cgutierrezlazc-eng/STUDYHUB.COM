...
export default function UserProfile({ userId, onNavigate }: Props) {
  ...
  const [postVisibility, setPostVisibility] = useState<
    'friends' | 'university' | 'private' | 'specific'
  >('friends');
  ...
  useEffect(() => {
    ...
    api
      .lmsGetConnections()
      .then((d: any) => setLmsConnections(d?.connections || d || []))
      .catch(() => {});
  }, [userId, user]);
  ...
  const handleCVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    setCvUploading(true);
    setCvUploadMsg('⏳ Analizando tu CV...');
    try {
      ...
      setCvUploadMsg(
        `❌ ${err.message || 'No se pudo importar. Asegúrate de subir un PDF o Word válido.'}`
      );
  ...
  const handleCVVisibility = async (vis: string) => {
    try {
      ...
      console.error('Failed to update CV visibility:', err);
    }
  };
  ...
  const loadFriends = async () => {
    try {
      setFriendsList(await api.getUserFriends(userId));
    } catch (err: any) {
  ...
  const loadActivityFeed = async () => {
    ...
    try {
      setActivityFeed(await api.getActivityFeed(1, userId));
    } catch (err: any) {
  ...
  const loadStudentTutoringData = async () => {
    ...
    try {
      ...
      console.error('Failed to load student tutoring data:', err);
    }
  ...
  const handleBookSession = async () => {
    ...
    try {
      ...
      alert(err.message || 'Error al reservar sesion');
    }
  ...
  const scheduleClassReminders = (subject: string, dateStr: string) => {
    ...
    if (oneHourBefore > now) {
      setTimeout(() => {
        if ('Notification' in window && Notification.permission === 'granted') {
          new Notification('Tutoria en 1 hora', {
            body: `Tu clase de ${subject} comienza en 1 hora`,
            icon: '/logo.png',
          });
        }
      }, oneHourBefore - now);
    ...
    if (fifteenMinBefore > now) {
      setTimeout(() => {
        if ('Notification' in window && Notification.permission === 'granted') {
          new Notification('Tutoria en 15 minutos', {
            body: `Tu clase de ${subject} comienza en 15 minutos`,
            icon: '/logo.png',
          });
        }
  ...
  const handleSubmitExam = async () => {
    ...
    try {
      ...
      alert(err.message || 'Error al enviar examen');
    }
  ...
  const handleCreateClass = async () => {
    ...
    try {
      ...
      const data: any = {
        ...
        category: ccCategory,
        materials_description: ccMaterials,
        zoom_link: ccZoom,
        scheduled_at,
        duration_minutes: ccDuration,
        max_students: ccMaxStudents,
        class_mode: ccMode,
      };
  ...
  const handleSendTutoringRequest = async () => {
    ...
    try {
      await api.sendMentoringRequest({
        tutor_id: userId,
  ...
  const handleBlock = async () => {
    if (!confirm(`¿Bloquear a ${profile?.firstName}? No podrá ver tu perfil ni contactarte.`))
      return;
    try {
  ...
  return (
    <div className="fb-profile-page">
      ...
      <div
        className="li-profile-card card"
      ...
      >
        ...
        <div className="fb-profile-photo-section">
          <div
            className="fb-profile-photo"
            onClick={() => isOwn && profilePhotoRef.current?.click()}
          >
            {profile.avatar ? (
          ...
          <div className="fb-profile-name-section">
            <div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <h1
                  className="fb-profile-identity-name"
                  style={{
                    fontSize: 28,
                    fontWeight: 800,
              ...
              <div
                style={{ height: 1, background: 'var(--border-subtle)', margin: '10px 0 8px' }}
              />
              {editingBio ? (
                <div style={{ marginBottom: 8 }}>
                  ...
                  />
                  <div
                    style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', marginTop: 6 }}
                  >
                    <button
                    ...
                    <button className="btn btn-primary btn-xs" onClick={handleSaveBio}>
                      Guardar
                    </button>
                  </div>
                </div>
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: 11,
                      fontWeight: 700,
                      color: profile.mentoringPriceType === 'free' ? '#22c55e' : '#2D62C8',
                      background:
                  ...
                  >
                    {profile.mentoringPriceType === 'free'
                      ? 'Gratis'
                      : `$${profile.mentoringPricePerHour || 0}/hr`}
                  </span>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: 14, marginTop: 10, alignItems: 'center' }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                  ...
                  >
                    {(profile.friendCount || 0).toLocaleString()}
                  </span>
                  <span
                    style={{
                      fontSize: 10,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start' }}>
                  <span
                    style={{ fontSize: 17, fontWeight: 700, color: 'var(--accent)', lineHeight: 1 }}
                  >
                    {((profile as any).xp || 0).toLocaleString()}
                  </span>
                  <span
                    style={{
                      fontSize: 10,
                  ...
                  <>
                    ...
                    {lmsConnections.length > 0 ? (
                      ...
                      >
                        ...
                        >
                          {lmsConnections[0]?.university_name ||
                            lmsConnections[0]?.base_url?.replace(/https?:\/\//, '').split('/')[0] ||
              ...
              {!isOwn && (
                <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
                  {profile.isFriend ? (
                    ...
                    <>
                      ...
                      <button
                        ...
                        style={{ borderRadius: 24 }}
                        onClick={() => {
                          api.rejectFriendRequest(profile.friendshipId);
                          loadProfile();
                        }}
                      >
                    ...
                    <button
                      className="btn btn-secondary btn-sm"
                      style={{ borderRadius: 24, cursor: 'default', opacity: 0.8 }}
                      disabled
                    >
                  ...
                  <button
                    className="btn btn-secondary btn-sm"
                    style={{ borderRadius: 24 }}
                    onClick={() => onNavigate(`/messages?new=${userId}`)}
                  >
                    {MessageSquare({ size: 14 })} {t('userprofile.message')}
                  </button>
                  <div style={{ position: 'relative' }}>
                    <button
                      className="btn btn-secondary btn-sm"
                      onClick={() => setShowMoreMenu(!showMoreMenu)}
                      style={{ borderRadius: 24, padding: '6px 14px' }}
                    >
                      {t('userprofile.more')} ···
                    </button>
                    {showMoreMenu && (
                      ...
                      >
                        ...
                        >
                          {XCircle({ size: 14 })} {t('userprofile.blockUser')}
                        </button>
                        <button
                          onClick={() => {
                          ...
                          onMouseLeave={(e) => (e.currentTarget.style.background = 'none')}
                        >
                          {AlertTriangle({ size: 14 })} {t('userprofile.reportUser')}
                        </button>
                        <button
                          onClick={() => {
                            alert(t('userprofile.userMuted'));
                            setShowMoreMenu(false);
                          }}
                          style={{
                            width: '100%',
                          ...
                          onMouseLeave={(e) => (e.currentTarget.style.background = 'none')}
                        >
                          {EyeOff({ size: 14 })} {t('userprofile.muteUser')}
                        </button>
                        <div style={{ height: 1, background: 'var(--border)' }} />
                        <button
                          onClick={() => {
                            alert(t('userprofile.reportSentShort'));
                            setShowMoreMenu(false);
                          }}
                          style={{
                            width: '100%',
                          ...
                          onMouseLeave={(e) => (e.currentTarget.style.background = 'none')}
                        >
                          {AlertTriangle({ size: 14 })} {t('userprofile.noCollaborate')}
                        </button>
                      </div>
                    )}
                  </div>
              ...
              {isOwn && (
                <div style={{ display: 'flex', gap: 8, marginTop: 14, flexWrap: 'wrap' }}>
                  <button
                    className="btn btn-primary btn-sm"
                    style={{ borderRadius: 24, display: 'flex', alignItems: 'center', gap: 6 }}
                    onClick={() => setConfigSection('profile')}
                  >
              ...
              {!isOwn && profile && (
                ...
                >
                  {mutualFriends.length > 0 && (
                    <div style={{ position: 'relative' }}>
                      <button
                        onClick={() => setShowMutualList(!showMutualList)}
                        style={{
                          display: 'flex',
                          alignItems: 'center',
                          gap: 6,
                      ...
                      >
                        ...
                        👥 {mutualFriends.length} amigo{mutualFriends.length !== 1 ? 's' : ''} en
                        común
                      </button>
                      {showMutualList && (
                        <div
                        ...
                        >
                          {mutualFriends.map((f: any) => (
                            <div
                              key={f.id}
                              onClick={() => {
                                setShowMutualList(false);
                                onNavigate(`/profile/${f.id}`);
                              }}
                              style={{
                                display: 'flex',
                                alignItems: 'center',
                  ...
                  {user?.university &&
                    ...
                    user.university === profile.university && (
                      ...
                      >
                        🏛️ Estudia en {profile.university}
                      </span>
                    )}
                  ...
                  <button
                    ...
                    onClick={() => onNavigate(`/study-room?invite=${userId}`)}
                  >
                    📚 Estudiar juntos
                  </button>
                </div>
            ...
            <div
              className="fb-profile-identifiers"
              style={{
                display: 'flex',
      ...
      >
        <button
          className={`fb-tab ${!configSection && activeTab === 'wall' ? 'active' : ''}`}
          onClick={() => {
            setActiveTab('wall');
        ...
        <button
          className={`fb-tab ${!configSection && activeTab === 'tutorias' ? 'active' : ''}`}
          onClick={() => {
            setActiveTab('tutorias');
            loadStudentTutoringData();
            loadTutorData();
            setConfigSection(null);
          }}
        ...
        <button
          className={`fb-tab ${!configSection && activeTab === 'courses' ? 'active' : ''}`}
          onClick={() => {
        ...
        <button
          className={`fb-tab ${!configSection && activeTab === 'cv' ? 'active' : ''}`}
          onClick={() => {
            setActiveTab('cv');
            loadCV();
            setConfigSection(null);
        ...
        <button
          className={`fb-tab ${!configSection && activeTab === 'friends' ? 'active' : ''}`}
          onClick={() => {
        ...
        <button
          className={`fb-tab ${!configSection && activeTab === 'photos' ? 'active' : ''}`}
          onClick={() => {
            setActiveTab('photos');
            setConfigSection(null);
          }}
          ...
          <div>
              ...
              <>
                  ...
                  <div className="card fb-post-composer">
                    <div className="fb-composer-header">
                      ...
                      <textarea
                        ...
                        placeholder={
                          isOwn
                            ? t('userprofile.thinking')
                            : `${t('userprofile.writeOnProfile')} ${profile.firstName}...`
                        }
                        value={newPost}
                    ...
                    {postImage && (
                      <div className="fb-composer-preview">
                        ...
                        <button className="fb-remove-image" onClick={() => setPostImage(null)}>
                          ✕
                        </button>
                      </div>
                    )}
                    ...
                    >
                      ...
                      />
                      <button
                        className="fb-composer-action-btn"
                      ...
                      <div style={{ position: 'relative' }}>
                        ...
                        >
                          {postVisibility === 'friends' ? (
                            <>
                              {Globe({ size: 14 })} {t('userprofile.visibilityFriends')}
                            </>
                          ) : postVisibility === 'university' ? (
                            <>
                              {GraduationCap({ size: 14 })} {t('userprofile.visibilityUniversity')}
                            </>
                          ) : postVisibility === 'private' ? (
                            <>
                              {Lock({ size: 14 })} {t('userprofile.visibilityPrivate')}
                            </>
                          ) : (
                            <>
                              {Users({ size: 14 })} {t('userprofile.visibilitySpecific')}
                            </>
                          )}
                            ...
                            {[
                              {
                                value: 'friends',
                                icon: Globe({ size: 16 }),
                                label: t('userprofile.allFriends'),
                                desc: t('userprofile.allFriendsDesc'),
                              },
                              {
                                value: 'university',
                                icon: GraduationCap({ size: 16 }),
                                label: t('userprofile.myUniversity'),
                                desc: t('userprofile.myUniversityDesc'),
                              },
                              {
                                value: 'private',
                                icon: Lock({ size: 16 }),
                                label: t('userprofile.onlyProfile'),
                                desc: t('userprofile.onlyProfileDesc'),
                              },
                            ].map((opt) => (
                              <button
                                key={opt.value}
                                type="button"
                                onClick={() => {
                                  setPostVisibility(opt.value as any);
                ...
                {(() => {
                  ...
                  return merged.map((item) => {
                    ...
                    const mc = post.isMilestone
                      ? milestoneConfig[post.milestoneType] || {
                          emoji: '✨',
                          color: 'var(--accent)',
                    ...
                    return (
                      <div
                        key={post.id}
                        className="card fb-post"
                        style={
                          mc ? { border: `1.5px solid ${mc.color}30`, overflow: 'hidden' } : {}
                        }
                      >
                        {/* Milestone banner */}
                        {mc && (
                          <div
                            style={{
                              background: mc.bg,
                              borderBottom: `1px solid ${mc.color}20`,
                              padding: '10px 16px',
                              display: 'flex',
                              alignItems: 'center',
                              gap: 8,
                        ...
                        <div className="fb-post-header" style={mc ? { paddingTop: 12 } : {}}>
                          <div
                            ...
                            onClick={() => post.author && onNavigate(`/user/${post.author.id}`)}
                          >
                            {post.author?.avatar ? (
                        ...
                        <p
                          ...
                          style={mc ? { fontSize: 15, fontWeight: 600, color: mc.color } : {}}
                        >
                          {post.content}
                        </p>
                        ...
                        <div className="fb-post-stats">
                          {post.likes > 0 && (
                            <span>
                              {Heart({ size: 14, color: '#ef4444' })} {post.likes}
                            </span>
                          )}
                          {post.commentCount > 0 && (
                            <span
                              className="fb-comment-count"
                        ...
                        <div className="fb-post-actions">
                          ...
                          >
                            {Heart({ size: 14, color: post.liked ? '#ef4444' : undefined })}{' '}
                            {t('userprofile.like')}
                          </button>
                            ...
                            {(comments[post.id] || []).map((c) => (
                              <div key={c.id} className="fb-comment">
                                <div
                                  ...
                                  onClick={() => c.author && onNavigate(`/user/${c.author.id}`)}
                                >
                                  {c.author?.avatar ? (
            ...
            {activeTab === 'about' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div className="card fb-info-card">
                  ...
                  <div className="fb-about-grid">
                    ...
                    <div className="fb-about-item">
                      ...
                      <span>
                        {t('userprofile.semester')} {profile.semester}
                      </span>
                    </div>
                    {profile.academicStatus && profile.academicStatus !== 'estudiante' && (
                      <div className="fb-about-item">
                        <span className="fb-about-label">
                          {t('userprofile.academicStatusLabel')}
                        </span>
                        <span
                          style={{ textTransform: 'capitalize', color: '#7c3aed', fontWeight: 600 }}
                        >
                ...
                <div className="card fb-info-card">
                  ...
                  <div className="fb-about-grid">
                    <div className="fb-about-item">
                      ...
                      <span style={{ fontWeight: 600, color: '#2D62C8' }}>
                        {profile.friendCount} {t('userprofile.connections')}
                      </span>
                    </div>
                    {profile.studyDays > 0 && (
                      <div className="fb-about-item">
                        ...
                        <span style={{ fontWeight: 600, color: '#059669' }}>
                          {profile.studyDays.toLocaleString()} días
                        </span>
                      </div>
                    )}
                  </div>
                    ...
                    {profile.studentRatingCount > 0 ? (
                      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                        <div style={{ display: 'flex', gap: 3 }}>
                          {[1, 2, 3, 4, 5].map((s) => (
                            <svg
                              ...
                              viewBox="0 0 24 24"
                              fill={
                                s <= Math.round(profile.studentRatingAvg || 0) ? '#f59e0b' : 'none'
                              }
                              stroke="#f59e0b"
                              strokeWidth="2"
                            >
                        ...
                        <div>
                          ...
                          >
                            {(profile.studentRatingAvg || 0).toFixed(1)}
                          </span>
                          <span style={{ fontSize: 13, color: 'var(--text-muted)', marginLeft: 6 }}>
                            ({profile.studentRatingCount}{' '}
                            {profile.studentRatingCount === 1 ? 'evaluación' : 'evaluaciones'} de
                            tutores)
                          </span>
                        </div>
                      </div>
                      ...
                      >
                        Aún no tienes evaluaciones de tutores. Aparecerán luego de completar
                        sesiones de tutoría.
                      </p>
                    )}
                  </div>
                ...
                {profile.offersMentoring && (
                  <div className="card fb-info-card">
                    <h4 style={{ marginTop: 0, marginBottom: 14 }}>
                      {t('userprofile.tutoringServices')}
                    </h4>
                    <div className="fb-about-grid">
                      ...
                      {profile.mentoringDescription && (
                        <div className="fb-about-item">
                          <span className="fb-about-label">
                            {t('userprofile.descriptionLabel')}
                          </span>
                      ...
                      <div className="fb-about-item">
                        ...
                        <div>
                          ...
                          >
                            {profile.mentoringPriceType === 'free'
                              ? t('userprofile.freeVolunteer')
                              : `$${profile.mentoringPricePerHour || 0} USD por hora`}
                          </span>
                          {profile.mentoringPriceType === 'paid' &&
                            profile.mentoringPricePerHour &&
                            user?.country &&
                            (() => {
                              const prices = formatPriceDisplay(
                                profile.mentoringPricePerHour,
                                user.country
                              );
                              return prices.localText ? (
                                ...
                                >
                                  ≈ {prices.localText} (conversión aprox.)
                                </span>
                              ) : null;
                            })()}
                        </div>
                ...
                {((profile as any).subscriptionTier === 'max' || isOwn) && (
                  <div className="card fb-info-card">
                    <ExecutiveShowcase userId={profile.id} isOwner={!!isOwn} />
                  </div>
                )}
              </div>
            )}
                ...
                {coursesLoading ? (
                  <div className="u-card" style={{ textAlign: 'center', padding: 40 }}>
                    ...
                    <p style={{ color: 'var(--text-muted)', marginTop: 12 }}>
                      {t('userprofile.loadingCourses')}
                    </p>
                  </div>
                ) : completedCourses.length === 0 ? (
                  <div className="u-card" style={{ textAlign: 'center', padding: 40 }}>
                    <div style={{ marginBottom: 12 }}>
                      {GraduationCap({ size: 48, color: 'var(--text-muted)' })}
                    </div>
                  ...
                  <>
                    ...
                    >
                      ...
                      <div>
                        ...
                        >
                          {completedCourses.length} {t('userprofile.coursesCompleted')}
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                          {t('userprofile.clickToSeeCert')}
                        </div>
                      </div>
                    </div>
                    <div
                          ...
                          >
                            ...
                            <div style={{ padding: '16px 16px 14px' }}>
                              ...
                              >
                                ...
                                <span>
                                  {t('userprofile.score')}: {cert.score}%
                                </span>
                                <span style={{ margin: '0 2px' }}>·</span>
                                <span>
                                  {cert.completedAt
                                    ? new Date(cert.completedAt).toLocaleDateString('es', {
                                        month: 'short',
                                        year: 'numeric',
                                      })
                                    : ''}
                ...
                {cvLoading ? (
                  ...
                  isOwn ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      <div className="u-card" style={{ padding: '36px 32px', textAlign: 'center' }}>
                        <div style={{ marginBottom: 16 }}>
                          {FileUp({ size: 52, color: 'var(--accent)' })}
                        </div>
                        ...
                        >
                          Sube tu CV en PDF y se extraen automáticamente experiencias, habilidades y
                          formación. También puedes completarlo manualmente.
                        </p>
                        {cvUploadMsg && (
                          <p
                            style={{
                              ...
                              marginTop: 12,
                              fontWeight: 500,
                              color: cvUploadMsg.startsWith('✅')
                                ? '#22c55e'
                                : cvUploadMsg.startsWith('❌')
                                  ? '#ef4444'
                                  : 'var(--text-muted)',
                            }}
                          >
                        ...
                        >
                          ...
                          >
                            {Upload({ size: 16 })}{' '}
                            {cvUploading ? 'Analizando...' : 'Subir CV en PDF'}
                            <input
                              type="file"
                              accept=".pdf,.doc,.docx"
                        ...
                        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 20 }}>
                          Formatos aceptados: PDF, DOC, DOCX · Los datos se extraen automáticamente
                        </p>
                      </div>
                          ...
                          {[
                            {
                              ...
                              icon: '🌐',
                              label: 'Todos',
                              desc: 'Visible en Bolsa de Empleo para cualquier usuario',
                            },
                            {
                              val: 'recruiters',
                              icon: '💼',
                              label: 'Solo Reclutadores',
                              desc: 'Visible únicamente para reclutadores verificados',
                            },
                            {
                              val: 'private',
                              icon: '🔒',
                              label: 'Solo yo',
                              desc: 'Privado — nadie más puede verlo',
                            },
                          ].map((opt) => (
                            <div
                              key={opt.val}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    ...
                    {cvUploadMsg && isOwn && (
                      <div
                        style={{
                          ...
                          fontSize: 13,
                          fontWeight: 500,
                          background: cvUploadMsg.startsWith('✅')
                            ? 'rgba(34,197,94,0.08)'
                            : 'rgba(239,68,68,0.08)',
                          border: `1px solid ${cvUploadMsg.startsWith('✅') ? 'rgba(34,197,94,0.3)' : 'rgba(239,68,68,0.3)'}`,
                          color: cvUploadMsg.startsWith('✅') ? '#22c55e' : '#ef4444',
                        }}
                      >
                        {cvUploadMsg}
                      </div>
                      ...
                      <div className="u-card" style={{ padding: 20 }}>
                        ...
                        {cvData.experience.map((exp: any, i: number) => (
                          <div
                            key={i}
                            style={{
                              marginBottom: i < cvData.experience.length - 1 ? 16 : 0,
                              paddingBottom: i < cvData.experience.length - 1 ? 16 : 0,
                              borderBottom:
                          ...
                          >
                            ...
                            <div style={{ fontSize: 13, color: '#2D62C8', marginTop: 2 }}>
                              {exp.company}
                            </div>
                      ...
                      <div className="u-card" style={{ padding: 20 }}>
                        ...
                        {cvData.projectsPortfolio.map((proj: any, i: number) => (
                          <div
                            key={i}
                            style={{
                              marginBottom: i < cvData.projectsPortfolio.length - 1 ? 16 : 0,
                              paddingBottom: i < cvData.projectsPortfolio.length - 1 ? 16 : 0,
                              borderBottom:
                                i < cvData.projectsPortfolio.length - 1
                                  ? '1px solid var(--border-subtle)'
                                  : 'none',
                            }}
                          >
                          ...
                          <>
                            ...
                            <div
                              style={{
                                ...
                                flexWrap: 'wrap',
                                gap: 8,
                                marginBottom: cvData.tools?.length ? 18 : 0,
                              }}
                            >
                    ...
                    {cvData.volunteering && cvData.volunteering.length > 0 && (
                      <div className="u-card" style={{ padding: 20 }}>
                        ...
                        {cvData.volunteering.map((v: any, i: number) => (
                          <div
                            key={i}
                            style={{ marginBottom: i < cvData.volunteering.length - 1 ? 12 : 0 }}
                          >
                            <div
                              style={{
                                fontWeight: 600,
                    ...
                    {cvData.interests && cvData.interests.length > 0 && (
                      <div className="u-card" style={{ padding: 20 }}>
                        <h4
                          style={{ margin: '0 0 10px', fontSize: 15, color: 'var(--text-primary)' }}
                        >
                    ...
                    {cvData.testimonials && cvData.testimonials.length > 0 && (
                      <div className="u-card" style={{ padding: 20 }}>
                        <h4
                          style={{ margin: '0 0 14px', fontSize: 15, color: 'var(--text-primary)' }}
                        >
                          {t('userprofile.recommendations')}
                        </h4>
                        {cvData.testimonials.map((t: any, i: number) => (
                          <div
                            key={i}
                            style={{
                              marginBottom: i < cvData.testimonials.length - 1 ? 14 : 0,
                              padding: 14,
                              background: 'var(--bg-secondary)',
                              borderRadius: 10,
            ...
            {activeTab === 'friends' && (
              <div className="fb-friends-section">
                <h3>
                  {t('userprofile.friends')} ({profile.friendCount})
                </h3>
                <div className="friends-grid">
                  {friendsList.map((friend) => (
                    <div
                      ...
                      onClick={() => onNavigate(`/user/${friend.id}`)}
                    >
            ...
            {activeTab === 'servicios' && tutorProfile && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  ...
                  >
                    <div>
                      ...
                      <p style={{ margin: '4px 0 0', fontSize: 13, color: '#a16207' }}>
                        {t('userprofile.roleNumber')}{' '}
                        {String(tutorProfile.roleNumber || tutorProfile.id || 0).padStart(4, '0')}
                      </p>
                    </div>
                    <span
                      style={{
                    ...
                    >
                      {tutorProfile.status === 'approved'
                        ...
                        : tutorProfile.status === 'suspended'
                          ? t('userprofile.suspended')
                          : t('userprofile.pendingStatus')}
                    </span>
                  </div>
                </div>
                ...
                >
                  <div
                    className="u-card"
                    style={{ padding: 16, textAlign: 'center', borderTop: '3px solid #d97706' }}
                  >
                    <div style={{ fontSize: 28, fontWeight: 700, color: '#d97706' }}>
                      {tutorProfile.totalStudents || 0}
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>
                      {t('userprofile.totalStudents')}
                    </div>
                  </div>
                  <div
                    className="u-card"
                  ...
                  >
                    <div style={{ fontSize: 28, fontWeight: 700, color: '#d97706' }}>
                      {tutorProfile.averageRating
                        ? Number(tutorProfile.averageRating).toFixed(1)
                        : '—'}
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>
                      {t('userprofile.avgRating')}
                    </div>
                  </div>
                  <div
                    className="u-card"
                  ...
                  >
                    <div style={{ fontSize: 28, fontWeight: 700, color: '#d97706' }}>
                      {tutorProfile.totalHours || 0}h
                    </div>
                    <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>
                      {t('userprofile.hoursGiven')}
                    </div>
                  </div>
                </div>
                ...
                <div className="u-card" style={{ padding: 20 }}>
                  <h4 style={{ margin: '0 0 12px', color: '#92400e' }}>
                    {Medal({ size: 16 })} {t('userprofile.ratingAndClasses')}
                  </h4>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    <div style={{ display: 'flex', gap: 2 }}>
                      {[1, 2, 3, 4, 5].map((star) => (
                        <span
                          ...
                          style={{
                            color:
                              star <= Math.round(tutorProfile.averageRating || 0)
                                ? '#f59e0b'
                                : '#d1d5db',
                            fontSize: 20,
                          }}
                    ...
                    <span style={{ fontWeight: 600, fontSize: 15 }}>
                      {tutorProfile.averageRating
                        ? Number(tutorProfile.averageRating).toFixed(1)
                        : t('userprofile.noRatings')}
                    </span>
                    <span style={{ color: 'var(--text-muted)', fontSize: 13 }}>
                      ({tutorProfile.totalRatings || 0} evaluaciones)
                    </span>
                  </div>
                  <p style={{ margin: 0, fontSize: 13, color: 'var(--text-muted)' }}>
                    {t('userprofile.totalClasses')}:{' '}
                    <strong>{tutorProfile.totalClasses || 0}</strong>
                  </p>
                </div>
                ...
                <div className="u-card" style={{ padding: 20 }}>
                  <div
                    style={{
                      ...
                      justifyContent: 'space-between',
                      alignItems: 'center',
                      marginBottom: showCreateClass ? 16 : 0,
                    }}
                  >
                    <h4 style={{ margin: 0, color: '#92400e' }}>
                      {t('userprofile.createClassOrProgram')}
                    </h4>
                    <button
                      onClick={() => {
                      ...
                      style={{
                        padding: '6px 16px',
                        background: showCreateClass ? 'var(--bg-secondary)' : '#d97706',
                        color: showCreateClass ? 'var(--text-primary)' : '#fff',
                        border: '1px solid ' + (showCreateClass ? 'var(--border)' : '#d97706'),
                        borderRadius: 8,
                        cursor: 'pointer',
                        fontWeight: 600,
                        fontSize: 13,
                  ...
                  {showCreateClass && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {ccSuccess && (
                        ...
                        >
                          {`✓ ${t('userprofile.classCreated')}`}
                          {ccMode === 'program'
                            ? ` (Sesion ${ccSessionNumber - 1} de ${ccProgramSessions})`
                            : ''}
                        </div>
                      )}
                      {/* Mode selector */}
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          onClick={() => {
                            setCcMode('individual');
                            setCcProgramId('');
                            setCcSessionNumber(1);
                          }}
                          style={{
                            flex: 1,
                            padding: '10px 16px',
                            borderRadius: 8,
                            border: '2px solid',
                            borderColor: ccMode === 'individual' ? '#d97706' : 'var(--border)',
                            background: ccMode === 'individual' ? '#fffbeb' : 'var(--bg-card)',
                            cursor: 'pointer',
                            fontWeight: 600,
                            fontSize: 13,
                            color: ccMode === 'individual' ? '#92400e' : 'var(--text-secondary)',
                          }}
                        >
                          {t('userprofile.individualClass')}
                        </button>
                        <button
                          onClick={() => setCcMode('program')}
                          style={{
                            flex: 1,
                            padding: '10px 16px',
                            borderRadius: 8,
                            border: '2px solid',
                            borderColor: ccMode === 'program' ? '#d97706' : 'var(--border)',
                            background: ccMode === 'program' ? '#fffbeb' : 'var(--bg-card)',
                            cursor: 'pointer',
                            fontWeight: 600,
                            fontSize: 13,
                            color: ccMode === 'program' ? '#92400e' : 'var(--text-secondary)',
                          }}
                        >
                        ...
                        >
                          ...
                          >
                            <div>
                              ...
                              <input
                                value={ccProgramTitle}
                                onChange={(e) => setCcProgramTitle(e.target.value)}
                                placeholder="Ej: Curso de Calculo I"
                                style={{
                                  width: '100%',
                                  padding: '8px 12px',
                                  borderRadius: 6,
                            ...
                            <div>
                              ...
                              <input
                                ...
                                min={2}
                                max={30}
                                value={ccProgramSessions}
                                onChange={(e) =>
                                  setCcProgramSessions(parseInt(e.target.value) || 2)
                                }
                                style={{
                                  width: '100%',
                                  padding: '8px 12px',
                          ...
                          <div>
                            ...
                            <textarea
                              value={ccProgramDesc}
                              onChange={(e) => setCcProgramDesc(e.target.value)}
                              placeholder="Describe el programa completo..."
                              rows={2}
                              style={{
                                width: '100%',
                                padding: '8px 12px',
                          ...
                          {ccProgramId && (
                            ...
                            >
                              Creando sesion {ccSessionNumber} de {ccProgramSessions} — Programa ID:{' '}
                              {ccProgramId.slice(0, 8)}...
                            </p>
                          )}
                        </div>
                      )}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          placeholder={
                            ccMode === 'program'
                              ? `Sesion ${ccSessionNumber}: Tema`
                              : 'Ej: Introduccion a Derivadas'
                          }
                          style={{
                            width: '100%',
                            padding: '8px 12px',
                      ...
                      <div>
                        ...
                        <p style={{ fontSize: 11, color: '#d97706', margin: '4px 0 0' }}>
                          ⚠️ Eres responsable de proporcionar informacion completa. Cualquier
                          malentendido por informacion insuficiente sera tu responsabilidad.
                        </p>
                      </div>
                      <div>
                        <label
                        ...
                        <input
                          value={ccMaterials}
                          onChange={(e) => setCcMaterials(e.target.value)}
                          placeholder="Ej: Calculadora, cuaderno, libro de texto..."
                          style={{
                            width: '100%',
                            padding: '8px 12px',
                            borderRadius: 6,
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                        ...
                        <div>
                          ...
                          <select
                            value={ccDuration}
                            onChange={(e) => setCcDuration(parseInt(e.target.value))}
                            style={{
                              width: '100%',
                              padding: '8px 12px',
                              borderRadius: 6,
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
                        <div>
                          ...
                          <select
                            value={ccMaxStudents}
                            onChange={(e) => setCcMaxStudents(parseInt(e.target.value))}
                            style={{
                              width: '100%',
                              padding: '8px 12px',
                              borderRadius: 6,
                        ...
                        <div>
                          ...
                          >
                            Precio por estudiante (CLP)
                          </label>
                          <input
                            type="number"
                            value={ccPrice}
                            onChange={(e) => setCcPrice(e.target.value)}
                            placeholder="Auto (segun tarifas)"
                            style={{
                              width: '100%',
                              padding: '8px 12px',
                              borderRadius: 6,
                        ...
                        <div>
                          ...
                          <input
                            value={ccZoom}
                            onChange={(e) => setCcZoom(e.target.value)}
                      ...
                      <button
                        ...
                        disabled={
                          ccSubmitting || !ccTitle.trim() || !ccDate || !ccTime || !ccCategory
                        }
                        style={{
                          padding: '10px 24px',
                          background: '#d97706',
                ...
                <div className="u-card" style={{ padding: 20 }}>
                  <h4 style={{ margin: '0 0 12px', color: '#92400e' }}>
                    {Calendar({ size: 16 })} {t('userprofile.upcomingClasses')}
                  </h4>
                  {tutorLoading ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {[1, 2, 3].map((i) => (
                          ...
                          >
                            <div>
                              <div style={{ fontWeight: 600, fontSize: 14 }}>
                                {cls.subject || cls.topic || 'Clase'}
                              </div>
                              <div
                                style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}
                              >
                                {cls.studentName || cls.student?.firstName || 'Estudiante'} —{' '}
                                {cls.scheduledAt
                                  ? new Date(cls.scheduledAt).toLocaleDateString('es-CL', {
                                      weekday: 'short',
                                      day: 'numeric',
                                      month: 'short',
                                      hour: '2-digit',
                    ...
                    <p style={{ color: 'var(--text-muted)', fontSize: 13, margin: 0 }}>
                      {t('userprofile.noUpcomingClasses')}
                    </p>
                  )}
                </div>
                ...
                <div className="u-card" style={{ padding: 20 }}>
                  <h4 style={{ margin: '0 0 12px', color: '#92400e' }}>
                    {FileText({ size: 16 })} {t('userprofile.paymentSummary')}
                  </h4>
                  {tutorPayments ? (
                    <>
                      <div
                      ...
                      >
                        ...
                        >
                          <div style={{ fontSize: 12, color: '#166534', fontWeight: 600 }}>
                            {t('userprofile.totalEarned')}
                          </div>
                          <div style={{ fontSize: 20, fontWeight: 700, color: '#15803d' }}>
                            $
                            {(
                              tutorPayments.totalEarned ||
                              tutorPayments.total ||
                              0
                            ).toLocaleString()}
                          </div>
                        ...
                        >
                          ...
                          >
                            {t('userprofile.lastPayment')}
                          </div>
                          <div style={{ fontSize: 14, fontWeight: 600 }}>
                            {tutorPayments.lastPaymentDate
                              ? new Date(tutorPayments.lastPaymentDate).toLocaleDateString(
                                  'es-CL',
                      ...
                      {(tutorPayments.pendingAmount > 0 ||
                        tutorPayments.pendingPayments?.length > 0) && (
                        <button
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  {(['all', 'upcoming', 'past'] as const).map((f) => (
                    <button
                      key={f}
                      onClick={() => setStudentClassFilter(f)}
                      style={{
                        padding: '6px 16px',
                        borderRadius: 20,
                        border: '1px solid',
                    ...
                    >
                      {f === 'all'
                        ? t('userprofile.filterAll')
                        : f === 'upcoming'
                          ? t('userprofile.filterUpcoming')
                          : t('userprofile.filterPast')}
                    </button>
                  ))}
                </div>
                ...
                {/* Enrolled Classes */}
                <div className="u-card" style={{ padding: 20 }}>
                  <h4 style={{ margin: '0 0 14px', color: '#5b21b6' }}>
                    {Calendar({ size: 16 })} {t('userprofile.enrolledClasses')}
                  </h4>
                  {studentClassesLoading ? (
                    ...
                    (() => {
                      ...
                      const filtered = studentClasses.filter((cls: any) => {
                        ...
                        if (studentClassFilter === 'upcoming')
                          return (
                            clsDate >= now ||
                            cls.status === 'upcoming' ||
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                          {filtered.map((cls: any) => {
                            ...
                            const countdown = isUpcoming
                              ? Math.max(0, clsDate.getTime() - now.getTime())
                              : 0;
                            ...
                            const examCompleted =
                              cls.exam_status === 'completed' || cls.examScore !== undefined;
                            return (
                              <div
                                key={cls.id}
                                style={{
                              ...
                              >
                                ...
                                >
                                    ...
                                    <div>
                                      ...
                                      >
                                        {clsDate && (
                                          <span>
                                            {Calendar({ size: 12 })}{' '}
                                            {clsDate.toLocaleDateString('es-CL', {
                                              weekday: 'short',
                                              day: 'numeric',
                                              month: 'short',
                                              hour: '2-digit',
                                        ...
                                        <span style={{ textTransform: 'capitalize' }}>
                                          {cls.classType || cls.class_type || 'individual'}
                                        </span>
                                      </div>
                                          ...
                                          style={{
                                            ...
                                            background:
                                              (cls.examScore || 0) >= 60
                                                ...
                                                : 'rgba(239,68,68,0.1)',
                                            color:
                                              (cls.examScore || 0) >= 60 ? '#15803d' : '#dc2626',
                                          }}
                                        >
                                          {(cls.examScore || 0) >= 60
                                            ? CheckCircle({ size: 12 })
                                            : XCircle({ size: 12 })}{' '}
                                          Examen: {cls.examScore || 0}% —{' '}
                                          {(cls.examScore || 0) >= 60 ? 'Aprobado' : 'Reprobado'}
                                        </div>
                                      )}
                                    </div>
                                  </div>
                                  ...
                                  >
                                    ...
                                    {hasExam && !examCompleted && (
                                      <button
                                        onClick={() => handleStartExam(cls.id)}
                                        style={{
                                          padding: '5px 14px',
                                          background: '#ef4444',
                                          color: '#fff',
                ...
                <div className="u-card" style={{ padding: 20 }}>
                  ...
                  >
                    <h4 style={{ margin: 0, color: '#5b21b6' }}>
                      {t('userprofile.availableClasses')}
                    </h4>
                    <button
                  ...
                  {tutorClassCategories.length > 0 && (
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                      ...
                      {tutorClassCategories.length > 8 && (
                        <span
                          style={{ fontSize: 11, color: 'var(--text-muted)', alignSelf: 'center' }}
                        >
                          +{tutorClassCategories.length - 8} mas
                        </span>
                      )}
                    </div>
                  )}
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {availableTutorClasses.slice(0, 6).map((cls: any) => (
                        <div
                          key={cls.id}
                          onClick={() =>
                            cls.tutor_user_id ? onNavigate(`/user/${cls.tutor_user_id}`) : null
                          }
                          style={{
                            padding: 12,
                            borderRadius: 8,
                          ...
                          >
                            <div style={{ flex: 1 }}>
                              ...
                              >
                                ...
                                {cls.class_mode === 'program' && (
                                  <span style={{ color: '#7c3aed', fontWeight: 600 }}>
                                    {' '}
                                    • Programa ({cls.program_total_sessions} sesiones)
                                  </span>
                                )}
                              </div>
                              <div
                              ...
                              >
                                {cls.scheduled_at
                                  ...
                                  : ''}{' '}
                                • {cls.duration_minutes} min • {cls.spots_available} cupos
                              </div>
                            </div>
                            <div style={{ textAlign: 'right', flexShrink: 0 }}>
                              <div style={{ fontWeight: 700, fontSize: 15, color: '#5b21b6' }}>
                                {cls.price_per_student === 0
                                  ? 'Gratis'
                                  : `$${(cls.price_per_student || 0).toLocaleString()}`}
                              </div>
                              {cls.tutor_rating > 0 && (
                                <div style={{ fontSize: 11, color: '#f59e0b' }}>
                                  ★ {Number(cls.tutor_rating).toFixed(1)}
                                </div>
                              )}
                            </div>
                          </div>
                ...
                <div className="u-card" style={{ padding: 20 }}>
                  <h4 style={{ margin: '0 0 14px', color: '#5b21b6' }}>
                    {FileText({ size: 16 })} {t('userprofile.paymentHistory')}
                  </h4>
                        ...
                        <tbody>
                          {studentPayments.map((p: any, idx: number) => (
                            ...
                            >
                              ...
                              >
                                {p.paidAt || p.created_at
                                  ? new Date(p.paidAt || p.created_at).toLocaleDateString('es-CL', {
                                      day: 'numeric',
                                      month: 'short',
                                      year: 'numeric',
                                    })
        ...
        <div className="modal-overlay" onClick={() => setShowBookingForm(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 500 }}>
            <h3 style={{ margin: '0 0 4px', color: '#5b21b6' }}>
              {Calendar({ size: 20 })} Reservar Clase de Tutoria
            </h3>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <select
                ...
                onChange={(e) => setBookingTutorId(e.target.value)}
                style={{
                  width: '100%',
                  padding: '10px 12px',
                  borderRadius: 8,
              ...
              >
                ...
                {tutorDirectoryList.map((t: any) => (
                  <option key={t.id || t.userId} value={t.id || t.userId}>
                    {t.firstName || t.name || 'Tutor'} {t.lastName || ''} —{' '}
                    {(t.subjects || t.mentoringSubjects || []).join(', ') || 'General'}
                  </option>
                ))}
              </select>
            </div>
                ...
                {(['individual', 'group'] as const).map((type) => (
                  <button
                    ...
                    style={{
                      ...
                      background:
                        bookingType === type ? 'rgba(124,58,237,0.08)' : 'var(--bg-secondary)',
            ...
            >
              <p style={{ fontSize: 11, color: '#7c3aed', margin: 0 }}>
                La clase se agregara automaticamente a tu calendario con alertas 1 hora y 15 minutos
                antes.
              </p>
            </div>
            ...
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                disabled={
                  bookingSaving || !bookingTutorId || !bookingSubject.trim() || !bookingDate
                }
                style={{
                  padding: '8px 20px',
                  background: '#7c3aed',
            ...
            {examLoading ? (
                ...
                >
                  <span
                    style={{
                      fontSize: 28,
                      fontWeight: 700,
                      color: (examResult.score || 0) >= 60 ? '#15803d' : '#dc2626',
                    }}
                  >
                    {examResult.score || 0}%
                  </span>
              ...
              <div>
                ...
                >
                  ...
                  >
                    {Hourglass({ size: 16 })}
                    {String(Math.floor(examTimeLeft / 60)).padStart(2, '0')}:
                    ...
                    >
                      ...
                      {q.type === 'multiple_choice' || q.options ? (
                          ...
                          {(q.options || []).map((opt: any, oi: number) => {
                            ...
                            return (
                              <button
                                key={oi}
                                onClick={() =>
                                  setExamAnswers((prev) => ({ ...prev, [q.id || idx]: optKey }))
                                }
                                style={{
                                  padding: '10px 14px',
                                  borderRadius: 8,
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            setExamAnswers((prev) => ({ ...prev, [q.id || idx]: e.target.value }))
                          }
                          rows={3}
                          className="form-input"
                          style={{ fontSize: 13 }}
                ...
                >
                  <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>
                    {Object.keys(examAnswers).length} de {examData.questions?.length || 0}{' '}
                    respondidas
                  </span>
                  <button
        ...
        <div className="modal-overlay" onClick={() => setShowEditInfoModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 480 }}>
            <div className="modal-header">
              ...
              <button className="modal-close" onClick={() => setShowEditInfoModal(false)}>
                ✕
              </button>
            </div>
            <div
              style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '4px 0 8px' }}
            >
              <div>
                <label
                  style={{
                ...
                <input
                  className="form-input"
                  style={{ marginTop: 4 }}
                  value={editInfoForm.career}
                  onChange={(e) => setEditInfoForm((f) => ({ ...f, career: e.target.value }))}
              ...
              <div>
                ...
                <input
                  className="form-input"
                  style={{ marginTop: 4 }}
                  value={editInfoForm.university}
                  onChange={(e) => setEditInfoForm((f) => ({ ...f, university: e.target.value }))}
              ...
              <div>
                ...
                <input
                  ...
                  min={1}
                  max={20}
                  value={editInfoForm.semester}
                  onChange={(e) =>
                    setEditInfoForm((f) => ({ ...f, semester: Number(e.target.value) }))
                  }
                />
              </div>
              <div>
                ...
                <select
                  className="form-input"
                  style={{ marginTop: 4 }}
                  value={editInfoForm.academicStatus}
                  onChange={(e) =>
                    setEditInfoForm((f) => ({ ...f, academicStatus: e.target.value }))
                  }
                >
              ...
              <div>
                ...
                <input
                  className="form-input"
                  style={{ marginTop: 4 }}
                  value={editInfoForm.professionalTitle}
                  onChange={(e) =>
                    setEditInfoForm((f) => ({ ...f, professionalTitle: e.target.value }))
                  }
                  placeholder="Ej: Ingeniero Civil Industrial"
                />
              </div>
              <div>
                <label
                ...
                <textarea
                  ...
                  onChange={(e) => setEditInfoForm((f) => ({ ...f, bio: e.target.value }))}
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end', marginTop: 8 }}>
              <button className="btn btn-secondary" onClick={() => setShowEditInfoModal(false)}>
                Cancelar
              </button>
              <button
                className="btn btn-primary"
            ...
            {tutoringSuccess ? (
              ...
              <>
                <h3 style={{ margin: '0 0 4px' }}>
                  Solicitar tutoría a {profile?.firstName} {profile?.lastName}
                </h3>
                <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 16 }}>
                  {profile?.mentoringPriceType === 'free'
                ...
                >
                  <p style={{ fontSize: 11, color: '#2D62C8', margin: 0 }}>
                    Una vez aceptada tu solicitud, podrán coordinar todo a través del chat de
                    Conniku.
                  </p>
                </div>
                <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                  ...
                  <button
                    style={{
                      ...
                      fontWeight: 600,
                      fontSize: 13,
                      opacity: tutoringLoading || !tutoringSubject.trim() ? 0.6 : 1,
                    }}
      ...
      {showReportModal && (
        <div className="modal-overlay" onClick={() => setShowReportModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420 }}>
            <h3>
              {t('userprofile.reportTitle')} {profile?.firstName} {profile?.lastName}
            </h3>
            <p style={{ fontSize: 13, color: 'var(--text-muted)', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 12 }}>
              {[
                t('userprofile.reportInappropriate'),
                t('userprofile.reportHarassment'),
                t('userprofile.reportSpam'),
                t('userprofile.reportFakeAccount'),
                t('userprofile.reportNoCollab'),
              ].map((reason) => (
                <button
                  key={reason}
                  ...
                  style={{
                    padding: '10px 14px',
                    background:
                      reportReason === reason ? 'var(--accent-blue)' : 'var(--bg-primary)',
                    border: '1px solid var(--border-color)',
                    borderRadius: 8,
                    color: reportReason === reason ? '#fff' : 'var(--text-primary)',
                    cursor: 'pointer',
                    textAlign: 'left',
                    fontSize: 13,
                  }}
              ...
              >
                ...
                <div
                  style={{
                    position: 'absolute',
                    inset: 8,
                    border: `2px solid ${selectedCert.accentColor}`,
                    pointerEvents: 'none',
                  }}
                />
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: '#8E99A4', marginBottom: 24 }}>
                    {selectedCert.estimatedMinutes
                      ? `Horas: ${Math.round(selectedCert.estimatedMinutes / 60)} · `
                      : ''}
                    Nota: {selectedCert.score} · Fecha:{' '}
                    {selectedCert.completedAt
                      ? new Date(selectedCert.completedAt).toLocaleDateString('es-CL')
                      : ''}
                  </div>
                  ...
                  {selectedCert.certificateId && (
                    <div style={{ marginTop: 16, fontSize: 9, color: '#8E99A4' }}>
                      Codigo de verificacion: {selectedCert.certificateId}
                      <br />
