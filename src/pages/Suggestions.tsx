...
import { Lightbulb, MessageSquare, AlertTriangle, Settings, CheckCircle } from '../components/Icons'
...
export default function Suggestions() {
  ...
  const typeOptions = [
    { value: 'feature', label: 'Nueva funcion', icon: Lightbulb({ size: 16, color: 'var(--accent-orange)' }) },
    { value: 'bug', label: 'Reportar error', icon: AlertTriangle({ size: 16, color: 'var(--accent-red)' }) },
    { value: 'improvement', label: 'Mejora', icon: Settings({ size: 16, color: 'var(--accent-blue)' }) },
    { value: 'other', label: 'Otro', icon: MessageSquare({ size: 16, color: 'var(--accent-purple)' }) },
  ]
      ...
      <div className="page-body page-enter">
        {sent && (
          <div style={{
            ...
            color: 'var(--accent-green)', fontSize: 13, fontWeight: 500,
            marginBottom: 16,
          }} className="toast">
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 24, maxWidth: 960 }}>
          ...
          <div className="u-card-flat" style={{ padding: 28 }}>
            <form onSubmit={handleSubmit}>
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, display: 'block' }}>Tipo de sugerencia</label>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {typeOptions.map(opt => (
                    <button
                      key={opt.value}
                      type="button"
                      className={type === opt.value ? 'btn btn-primary btn-sm' : 'btn btn-secondary btn-sm'}
              ...
              <div style={{ marginBottom: 16 }}>
                <label style={{ fontSize: 13, fontWeight: 600, marginBottom: 6, display: 'block' }}>Asunto</label>
                <input
              ...
              <div style={{ marginBottom: 20 }}>
                <label style={{ fontSize: 13, fontWeight: 600, marginBottom: 6, display: 'block' }}>Mensaje</label>
                <textarea
                  className="form-input"
              ...
              <button type="submit" className="btn btn-primary btn-glow press-feedback"
          ...
          <div>
            ...
            <div className="u-card-flat" style={{ padding: 20, marginBottom: 16 }}>
              ...
              <a href="mailto:contacto@conniku.com" style={{
            ...
            {history.length > 0 && (
              <div className="u-card-flat" style={{ padding: 20 }}>
                <h4 style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>Enviadas anteriormente</h4>
                {history.slice(0, 5).map(item => (
                  <div key={item.id} style={{
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      <span className={`u-card-tag${item.type === 'bug' ? '-red' : item.type === 'improvement' ? '' : item.type === 'other' ? '-purple' : '-orange'}`}>
                        {item.type === 'feature' ? 'Funcion' : item.type === 'bug' ? 'Error' : item.type === 'improvement' ? 'Mejora' : 'Otro'}
                      </span>
