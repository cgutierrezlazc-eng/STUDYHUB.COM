...
export default function Marketplace({ onNavigate }: Props) {
  ...
  const loadDocuments = async () => {
    try {
      const data = await api.getMarketplaceDocuments(
        search || undefined,
  ...
  const handleRate = async (docId: string, rating: number) => {
    try {
      ...
      setDocuments((prev) =>
        prev.map((d) =>
          d.id === docId ? { ...d, rating: result.rating, ratingCount: result.count } : d
        )
      );
    } catch (err: any) {
    ...
    <>
      <div className="page-header page-enter">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <h2>
              {BookOpen({ size: 20 })} {t('marketplace.title')}
            </h2>
      ...
      <div className="page-body">
        {loading ? (
          ...
          <div className="empty-state" style={{ padding: 48, textAlign: 'center' }}>
            ...
            >
              Sube tus mejores apuntes para ayudar a otros y encuentra material de calidad para tus
              ramos
            </p>
                ...
                <div key={doc.id} className="u-card" style={{ padding: 20 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>
                    ...
                    {doc.career && (
                      <span>
                        {' '}
                        · {GraduationCap({ size: 14 })} {doc.career}
                      </span>
                    )}
                  </div>
                  <div
                  ...
                  >
                    ...
                    <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                      {doc.rating > 0 ? `${doc.rating}/5` : t('marketplace.unrated')} (
                      {doc.ratingCount})
                    </span>
                  </div>
                  <div
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {doc.author && (
                        <button
                          onClick={() => onNavigate(`/user/${doc.author!.id}`)}
                          style={{
                            background: 'none',
                            border: 'none',
                            cursor: 'pointer',
            ...
            {total > 20 && (
              <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 20 }}>
                ...
                <span style={{ padding: '8px 16px', fontSize: 13 }}>
                  {t('marketplace.page')} {page} {t('marketplace.of')} {Math.ceil(total / 20)}
                </span>
                <button
                  className="btn btn-secondary btn-sm"
          ...
          <div className="modal-overlay" onClick={() => setShowShare(false)}>
            <div className="modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="auth-field">
                ...
                <input
                  value={shareCourse}
                  onChange={(e) => setShareCourse(e.target.value)}
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                ...
                <button
                  ...
                  onClick={handleShare}
                  disabled={!shareTitle.trim() || !shareFile}
                >
