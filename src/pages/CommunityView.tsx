...
export default function CommunityView({ onNavigate }: Props) {
  ...
  const loadCommunity = async () => {
    try {
      setCommunity(await api.getCommunity(id!));
    } catch (err: any) {
  ...
  const handlePin = async (postId: string) => {
    try {
      ...
      setPosts((prev) =>
        prev.map((p) => (p.id === postId ? { ...p, isPinned: result.isPinned } : p))
      );
    } catch (err: any) {
  ...
  const handleKickMember = async (userId: string, name: string) => {
    ...
    try {
      ...
      alert(err.message || 'Error al expulsar');
    }
  };
    ...
    return (
      <div className="page-body">
        <div className="empty-state">
          <h3>{t('communityview.notfound')}</h3>
        </div>
      </div>
    );
          ...
          <div
            style={{
              ...
              background: community.avatar
                ? `url(${community.avatar}) center/cover`
                : categoryColor + '22',
          ...
          <div style={{ flex: 1, paddingBottom: 4 }}>
            ...
            >
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                {Users({ size: 13 })} {community.memberCount} miembros
              </span>
              <span
                style={{
                  width: 1,
            ...
            {resources.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
                ...
                <div style={{ fontSize: 13, marginTop: 4 }}>
                  Comparte links, apuntes y materiales con la comunidad.
                </div>
              </div>
            ) : (
                    ...
                    <div style={{ display: 'flex', gap: 8 }}>
                      ...
                      {(r.uploaderId === user?.id || isMod) && (
                        <button
                          ...
                          onClick={async () => {
                            try {
                              await api.deleteCommunityResource(id!, r.id);
                              loadResources();
                            } catch {}
                          }}
                        >
            ...
            {showAddResource && (
              <div className="modal-overlay" onClick={() => setShowAddResource(false)}>
                <div className="modal" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div className="auth-field">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setNewResource((p) => ({ ...p, resource_type: e.target.value }))
                        }
                        style={{
                          padding: '8px 12px',
                          borderRadius: 8,
                  ...
                  >
                    <button className="btn btn-secondary" onClick={() => setShowAddResource(false)}>
                      Cancelar
                    </button>
                    <button
          ...
          <div>
            ...
            {events.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '40px 0', color: 'var(--text-muted)' }}>
                ...
                <div style={{ fontSize: 13, marginTop: 4 }}>
                  Los administradores pueden crear sesiones de estudio y reuniones.
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {events.map((ev) => {
                  ...
                  return (
                    <div
                      key={ev.id}
                      className="u-card"
                      style={{ padding: 16, opacity: isPast ? 0.7 : 1 }}
                    >
                                ...
                                onClick={async () => {
                                  try {
                                    ...
                                    setEvents((prev) =>
                                      prev.map((e2) =>
                                        e2.id === ev.id
                                          ? { ...e2, myRsvp: s, rsvpCount: res.rsvpCount }
                                          : e2
                                      )
                                    );
                                  } catch {}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div className="auth-field">
                      ...
                      <input
                        ...
                        placeholder="Ej: Sesión de estudio — Parcial 2"
                      />
                    </div>
                    <div className="auth-field">
                      <label>Descripción</label>
                    ...
                    <div className="auth-field">
                      ...
                      <input
                        ...
                        placeholder="Ej: Sala A-203 o Online"
                      />
                    </div>
                    <div className="auth-field">
                      <label>Link de videollamada (opcional)</label>
                      <input
                        value={newEvent.meet_url}
                        onChange={(e) => setNewEvent((p) => ({ ...p, meet_url: e.target.value }))}
                  ...
                  >
                    ...
                    <button
                      ...
                      onClick={async () => {
                        ...
                        try {
                          await api.createCommunityEvent(id!, {
                            ...newEvent,
                  ...
                  >
                    ...
                    {isAdmin && m.user?.id !== user?.id && m.role !== 'admin' && (
                      <div style={{ display: 'flex', gap: 2 }}>
                        ...
                        <button
                          onClick={() =>
                            handleKickMember(m.user.id, `${m.user.firstName} ${m.user.lastName}`)
                          }
                          style={{
                            fontSize: 10,
                            padding: '2px 6px',
              ...
              >
                <div>
                  <strong style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    {t('communityview.category')}
                  </strong>
                  <div>{community.category}</div>
                </div>
                <div>
                  <strong style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    {t('communityview.type')}
                  </strong>
                ...
                <div>
                  <strong style={{ fontSize: 12, color: 'var(--text-muted)' }}>
                    {t('communityview.created')}
                  </strong>
                  <div>
                    {community.createdAt
                      ? new Date(community.createdAt).toLocaleDateString('es')
                      : ''}
                  </div>
                </div>
              </div>
      ...
      {reportPostId && (
        ...
        >
          ...
          >
            ...
            />
            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
              <button className="btn btn-secondary" onClick={() => setReportPostId(null)}>
                Cancelar
              </button>
              <button
                className="btn btn-primary"
