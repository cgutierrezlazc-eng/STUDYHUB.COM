...
export default function Friends({ onNavigate }: Props) {
  ...
  const loadRequests = async () => {
    try {
      setRequests(await api.getFriendRequests());
    } catch (err: any) {
  ...
  const loadSentRequests = async () => {
    try {
      setSentRequests(await api.getSentFriendRequests());
    } catch (err: any) {
    ...
    <>
      ...
      <div className="page-body">
        <div className="friends-tabs">
          ...
          <button
            className={`friends-tab ${tab === 'search' ? 'active' : ''}`}
          ...
          <div className="friends-grid">
            {friends.length === 0 ? (
              <div className="friends-empty" style={{ padding: 48, textAlign: 'center' }}>
                ...
                >
                  Agrega amigos para estudiar juntos, compartir apuntes y competir en las ligas
                  semanales
                </p>
                ...
                <div
                  ...
                  style={{ marginTop: 0, marginBottom: 12, maxWidth: 400, margin: '0 auto' }}
                >
                  <input
        ...
        {tab === 'requests' && (
          <div className="friends-grid">
            {requests.length === 0 ? (
              <div className="friends-empty" style={{ padding: 48, textAlign: 'center' }}>
                <div style={{ marginBottom: 16, opacity: 0.6 }}>
                  {Inbox({ size: 56, color: 'var(--accent)' })}
                </div>
                <h3 style={{ fontSize: 18, fontWeight: 700, marginBottom: 8 }}>
              ...
              requests.map((req) => (
                <div key={req.id} className="friend-card card">
                  ...
                  <div className="friend-actions">
                    <button className="btn btn-primary btn-xs" onClick={() => handleAccept(req.id)}>
                      {t('friends.accept')}
                    </button>
                    <button
                      className="btn btn-secondary btn-xs"
            ...
            {sentRequests.length === 0 ? (
              ...
              sentRequests.map((req) => (
                <div
                  key={req.id}
                  className="friend-card card"
                  onClick={() => onNavigate(`/user/${req.user.id}`)}
                >
                  <div className="friend-avatar">
                    {req.user.avatar ? (
                      <img src={req.user.avatar} alt="" />
