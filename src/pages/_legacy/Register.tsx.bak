  ...
  const validateStep = (): boolean => {
    ...
    if (step === 1) {
      ...
      if (calculateAge(form.birthDate) < 18) {
        setError(t('err.under18'));
        return false;
      }
    }
    if (step === 2) {
      if (!selectedUni) {
        setError('Debes seleccionar tu institución de estudios de la lista oficial');
        return false;
      }
      if (!form.username.trim()) {
        setError(t('err.usernameRequired'));
        return false;
      }
      if (form.username.length < 5 || form.username.length > 30) {
        setError('El nombre de usuario debe tener entre 5 y 30 caracteres en total');
        return false;
      }
      ...
      if (usernameAvailable === false) {
        setError(t('err.usernameTaken'));
        return false;
      }
      if (!form.career.trim()) {
        setError(t('err.enterCareer'));
        return false;
      }
      if (form.academicStatus === 'titulado' && !form.professionalTitle.trim()) {
        setError(t('err.enterProfTitle'));
        return false;
      }
      if (
        (form.academicStatus === 'egresado' || form.academicStatus === 'titulado') &&
        !form.graduationStatusYear
      ) {
        setError(t('err.selectGradYear'));
        return false;
      }
      if (form.academicStatus === 'titulado' && !form.titleYear) {
        setError(t('err.selectTitleYear'));
        return false;
      }
    }
    if (step === 3) {
        ...
        <div className={rgStyles.formCard}>
          ...
          <div className="auth-steps">
            {STEP_TITLES.map((title, i) => (
              <div
                key={i}
                className={`auth-step ${i === step ? 'active' : ''} ${i < step ? 'done' : ''}`}
              >
          ...
          {step === 1 && (
            <>
              ...
              <div className="auth-row">
                <div className="auth-field">
                  <label>
                    {t('reg.phone')} <span className="auth-optional">{t('reg.optional')}</span>
                  </label>
                  <input
                    type="tel"
                ...
                <div className="auth-field">
                  <label>
                    {t('reg.birthDate')} <span style={{ color: '#ef4444' }}>*</span>
                  </label>
                  <input
                    type="date"
            ...
            <>
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          <span
                            style={{
                              background: 'rgba(45,98,200,0.12)',
                              color: 'var(--accent)',
                              fontWeight: 700,
                              padding: '1px 6px',
                              borderRadius: 4,
                          ...
                          <span>
                            {selectedUni.type === 'cft'
                              ? 'CFT'
                              : selectedUni.type === 'instituto'
                                ? 'IP'
                                : 'Universidad'}
                          </span>
                        </div>
                          ...
                          {uniResults.length === 0 ? (
                            ...
                            >
                              <div style={{ fontWeight: 600, marginBottom: 4 }}>
                                Sin resultados para "{uniSearch}"
                              </div>
                              <div style={{ fontSize: 12 }}>
                                Solo se pueden seleccionar instituciones de la lista oficial. Si tu
                                institución no aparece, contáctanos a{' '}
              ...
              <div className="auth-field">
                ...
                {institutionPrefix && (
                  ...
                  >
                    🔒 El prefijo <strong>{institutionPrefix}</strong> está asignado a tu
                    institución y no puede cambiarse
                  </small>
                )}
                <div
                  style={{
                ...
                >
                  ...
                  <input
                    placeholder={
                      institutionPrefix ? 'Nombre_Apellido' : t('reg.usernamePlaceholderShort')
                    }
                    value={
                      institutionPrefix
                        ? form.username.slice(institutionPrefix.length)
                        : form.username
                    }
                    onChange={(e) => {
              ...
              <div className="auth-field">
                ...
                <input
                  placeholder={t('reg.careerPlaceholder')}
                  value={form.career}
                ...
                <div className="auth-semester-picker">
                  {(
                    [
                      {
                        value: 'estudiante',
                        label: t('reg.statusStudent'),
                        desc: t('reg.statusStudentDesc'),
                      },
                      {
                        value: 'egresado',
                        label: t('reg.statusGraduate'),
                        desc: t('reg.statusGraduateDesc'),
                      },
                      {
                        value: 'titulado',
                        label: t('reg.statusTitled'),
                        desc: t('reg.statusTitledDesc'),
                      },
                    ] as const
                  ).map((opt) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (opt.value === 'estudiante') {
                          setForm((prev) => ({
                            ...
                            mentoringSubjects: [],
                            mentoringDescription: '',
                            mentoringPriceType: 'free' as const,
                            mentoringPricePerHour: null,
                            graduationStatusYear: null,
                            titleYear: null,
                            professionalTitle: '',
                          }));
                        }
              ...
              {(form.academicStatus === 'egresado' || form.academicStatus === 'titulado') && (
                <div className="auth-row">
                  <div className="auth-field">
                    ...
                    <select
                      value={form.graduationStatusYear || ''}
                      onChange={(e) =>
                        setForm((prev) => ({
                          ...prev,
                          graduationStatusYear: parseInt(e.target.value) || null,
                        }))
                      }
                      style={{
                        width: '100%',
                  ...
                  {form.academicStatus === 'titulado' && (
                    <div className="auth-field">
                      ...
                      <select
                        value={form.titleYear || ''}
                        onChange={(e) =>
                          setForm((prev) => ({
                            ...prev,
                            titleYear: parseInt(e.target.value) || null,
                          }))
                        }
                        style={{
                          width: '100%',
              ...
              {form.academicStatus === 'titulado' && (
                <div className="auth-field">
                  ...
                  <input
                    placeholder={t('reg.professionalTitlePlaceholder')}
            ...
            <>
              ...
              {(form.academicStatus === 'titulado' || form.academicStatus === 'egresado') && (
                <div
                  ...
                  style={{
                    background: 'var(--bg-tertiary, rgba(45,98,200,0.04))',
                    borderRadius: 12,
                    padding: 16,
                    border: '1px solid var(--border)',
                  }}
                >
                  ...
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                    {[
                      {
                        id: 'ayudantias',
                        label: t('reg.svcTutoring'),
                        desc: t('reg.svcTutoringDesc'),
                      },
                      { id: 'cursos', label: t('reg.svcCourses'), desc: t('reg.svcCoursesDesc') },
                      {
                        id: 'clases_particulares',
                        label: t('reg.svcPrivate'),
                        desc: t('reg.svcPrivateDesc'),
                      },
                    ].map((svc) => {
                      ...
                      return (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setForm((prev) => ({
                              ...prev,
                              mentoringServices: services,
                              offersMentoring: services.length > 0,
                            }));
                          }}
                        >
                  ...
                  {form.mentoringServices.length > 0 && (
                    <>
                      ...
                      >
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                          ...
                          <div>
                            ...
                            <p
                              style={{ fontSize: 12, color: '#A16207', margin: 0, lineHeight: 1.5 }}
                            >
                      ...
                      <div className="auth-field" style={{ marginBottom: 12 }}>
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setForm((prev) => ({
                              ...prev,
                              mentoringSubjects: e.target.value
                                .split(',')
                                .map((s) => s.trim())
                                .filter(Boolean),
                            }))
                      ...
                      <div className="auth-field" style={{ marginBottom: 12 }}>
                        <label style={{ fontSize: 13, fontWeight: 600 }}>
                          {t('reg.offerDescription')}
                        </label>
                        <textarea
                          placeholder={t('reg.offerDescPlaceholder')}
                          value={form.mentoringDescription}
                          onChange={(e) =>
                            setForm((prev) => ({ ...prev, mentoringDescription: e.target.value }))
                          }
                          rows={2}
                          style={{ fontSize: 13 }}
                        />
                      ...
                      <div className="auth-field" style={{ marginBottom: 12 }}>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button
                            ...
                            onClick={() =>
                              setForm((prev) => ({ ...prev, mentoringPriceType: 'paid' as const }))
                            }
                          >
                            <div style={{ fontSize: 14, fontWeight: 700, color: '#2D62C8' }}>
                              {t('reg.pricePaid')}
                            </div>
                            <div style={{ fontSize: 10, opacity: 0.7 }}>
                              {t('reg.pricePaidDesc')}
                      ...
                      {form.mentoringPriceType === 'paid' && (
                        <div className="auth-field" style={{ marginBottom: 12 }}>
                          ...
                          <div style={{ position: 'relative' }}>
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setForm((prev) => ({
                                  ...prev,
                                  mentoringPricePerHour: parseFloat(e.target.value) || null,
                                }))
                              }
                              min={1}
                              step={0.5}
                          ...
                          {form.mentoringPricePerHour && localCurrency.code !== 'USD' && (
                            <div
                              style={{
                                background: 'rgba(45,98,200,0.05)',
                                borderRadius: 6,
                                padding: '6px 10px',
                                marginTop: 6,
                                border: '1px solid rgba(45,98,200,0.1)',
                              }}
                            >
                ...
                <div className="auth-semester-picker">
                  {[
                    ...
                    <button
                      key={r.value}
                      type="button"
                      className={`auth-semester-btn ${form.academicRegime === r.value ? 'active' : ''}`}
                      style={{ flex: 1 }}
                      onClick={() => {
                        update('academicRegime', r.value);
                        update('semester', 1);
                      }}
                    >
                      {r.label}
              ...
              <div className="auth-field">
                <label>
                  {form.academicRegime === 'semestral'
                    ? t('reg.currentSemester')
                    : form.academicRegime === 'trimestral'
                      ? t('reg.currentTrimester')
                      : form.academicRegime === 'cuatrimestral'
                        ? t('reg.currentQuarter')
                        : t('reg.currentYear')}
                </label>
                <div className="auth-semester-picker">
                  {Array.from(
                    ...
                    <button
                      key={s}
                      type="button"
                      className={`auth-semester-btn ${form.semester === s ? 'active' : ''}`}
              ...
              <div className="auth-row">
                <div className="auth-field">
                  ...
                  <select
                    ...
                    onChange={(e) => update('entryYear', parseInt(e.target.value))}
                    style={{
                      width: '100%',
                      padding: '10px 12px',
                      borderRadius: 8,
                ...
                <div className="auth-field">
                  ...
                  <select
                    value={form.careerDuration}
                    onChange={(e) => update('careerDuration', parseInt(e.target.value))}
                    style={{
                      width: '100%',
                      padding: '10px 12px',
                      borderRadius: 8,
              ...
              <div className="auth-field">
                <label>
                  {t('reg.bio')} <span className="auth-optional">{t('reg.optional')}</span>
                </label>
                <textarea
                  placeholder={t('reg.bioPlaceholder')}
                  value={form.bio}
          ...
          {step < 4 && (
            <div className="auth-actions">
              {step > 0 && (
                <button className="btn-auth-secondary" onClick={handleBack}>
                  {t('auth.back')}
                </button>
              )}
              {step < 3 ? (
