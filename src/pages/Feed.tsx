...
export default function Feed({ onNavigate }: Props) {
  ...
  const [feedSort, setFeedSort] = useState<'recent' | 'smart'>(
    () => (localStorage.getItem('conniku_feed_sort') as any) || 'smart'
  );
  ...
  const loadFeed = async (
    p: number = 1,
    sort?: 'recent' | 'smart',
    filter?: 'all' | 'career' | 'university' | 'friends'
  ) => {
    try {
        ...
        prev.map((p) =>
          p.id === postId
            ? {
                ...
                likes: Object.values(result.reactions as Record<string, number>).reduce(
    ...
    try {
      ...
      setComments((prev) => ({
        ...
        [postId]: (prev[postId] || []).map((c: any) =>
          c.id === commentId ? { ...c, content: updated.content, editedAt: updated.editedAt } : c
        ),
      }));
    } catch (err: any) {
  ...
  const handleSaveEdit = async (postId: string) => {
    ...
    try {
      ...
      setPosts((prev) =>
        prev.map((p) =>
          p.id === postId ? { ...p, content: editContent, updatedAt: new Date().toISOString() } : p
        )
      );
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          >
            <button
              ...
              onClick={() => {
                setFeedSort('smart');
                localStorage.setItem('conniku_feed_sort', 'smart');
                setLoading(true);
                loadFeed(1, 'smart');
              }}
            >
            ...
            <button
              ...
              onClick={() => {
                setFeedSort('recent');
                localStorage.setItem('conniku_feed_sort', 'recent');
                setLoading(true);
                loadFeed(1, 'recent');
              }}
            >
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                background:
                  feedFilter === f.key
                    ? 'var(--accent-alpha, rgba(79,140,255,0.12))'
                    : 'var(--bg-secondary)',
            ...
            {feedSort === 'smart' && trendingPosts.length > 0 && (
              <div
                style={{
                  ...
                  borderRadius: 12,
                  background:
                    'linear-gradient(135deg, var(--accent-alpha, rgba(79,140,255,0.08)), var(--bg-secondary))',
                  ...
                  {trendingPosts.map((tp) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        >
                          {tp.content?.slice(0, 80)}
                          {tp.content?.length > 80 ? '...' : ''}
                        </div>
                        <div
                          style={{
                            fontSize: 11,
                      ...
                      <button
                        ...
                        style={{
                          background: showPollCreator
                            ? 'var(--accent-alpha, rgba(79,140,255,0.15))'
                            : 'none',
                              ...
                              key !== 'list' ? (
                                <button
                                  ...
                                  style={{
                                    ...
                                    background:
                                      postVisibility === key
                                        ? 'var(--accent-alpha, rgba(79,140,255,0.1))'
                                        : 'transparent',
                                    color: 'var(--text-primary)',
                                    cursor: 'pointer',
                                    fontSize: 13,
                                      ...
                                      >
                                        <div style={{ display: 'flex', gap: 6 }}>
                                          <input
                                            ...
                                            onKeyDown={(e) =>
                                              e.key === 'Enter' && handleCreateFriendList()
                                            }
                                          ...
                                          <button
                                            ...
                                            style={{
                                              ...
                                              whiteSpace: 'nowrap',
                                              opacity:
                                                creatingList || !newListName.trim() ? 0.5 : 1,
                                            }}
                                          >
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {suggestedTags.length > 0 && (
                        <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                          {suggestedTags.map((tag, i) => (
                            <span
                              key={i}
                              className="u-card-tag"
                              style={{ fontSize: 10, cursor: 'pointer' }}
                              onClick={() =>
                                setNewPostContent((prev) => prev + ` #${tag.replace(/\s/g, '')}`)
                              }
                            >
                              #{tag}
                            </span>
                      ...
                      {tagsLoading && (
                        <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          {t('feed.suggestingTags')}
                        </span>
                      )}
                      <button
                        className="btn btn-primary btn-sm press-feedback"
                        onClick={handlePost}
                        disabled={posting || (!newPostContent.trim() && !postImage)}
                      >
                    ...
                    >
                      ...
                      >
                        <span style={{ fontSize: 14, fontWeight: 600 }}>
                          {BarChart3({ size: 16 })} Crear encuesta
                        </span>
                        <button
                ...
                {posts.map((post, postIndex) => (
                  <React.Fragment key={post.id}>
                    <div id={`post-${post.id}`} className="u-card" style={{ padding: 16 }}>
                      ...
                      >
                        <div
                          onClick={() => onNavigate(`/user/${post.authorId || post.author?.id}`)}
                          style={{
                            width: 40,
                            height: 40,
                            borderRadius: '50%',
                              ...
                              <span
                                title={
                                  post.visibility === 'friends'
                                    ? t('feed.visibilityFriends')
                                    : post.visibility === 'private'
                                      ? t('feed.visibilityPrivate')
                                      : post.visibility === 'list'
                                        ? t('feed.visibilityList')
                                        : post.visibility
                                }
                                  ...
                                  (() => {
                                    ...
                                    if (exp > now) {
                                      const hrs = Math.round(
                                        (exp.getTime() - now.getTime()) / 3600000
                                      );
                      ...
                      {post.reactions && Object.keys(post.reactions).length > 0 && (
                        ...
                        >
                          {Object.entries(post.reactions as Record<string, number>).map(
                            ([type, count]) => (
                              <span key={type}>
                                {REACTION_EMOJIS[type] || '\u{1F44D}'} {count}
                              </span>
                            )
                          )}
                        </div>
                      ...
                      >
                        ...
                        >
                          <button
                            onClick={() => handleQuickLike(post.id, post.userReaction)}
                            style={{
                              background: 'none',
                              border: 'none',
                              cursor: 'pointer',
                              padding: '6px 12px',
                              fontSize: 13,
                              color: post.userReaction ? 'var(--accent)' : 'var(--text-muted)',
                              fontWeight: post.userReaction ? 700 : 400,
                              borderRadius: 8,
                            }}
                          >
                            {post.userReaction ? REACTION_EMOJIS[post.userReaction] : '\u{1F44D}'}{' '}
                            {post.userReaction ? t('feed.reacted') : t('feed.like')}
                          </button>
                        ...
                        <button
                          onClick={async () => {
                            try {
                              ...
                              setPosts((prev) =>
                                prev.map((p) =>
                                  p.id === post.id ? { ...p, bookmarked: result.bookmarked } : p
                                )
                              );
                            } catch (err: any) {
                          ...
                          style={{
                            ...
                            padding: '6px 12px',
                            fontSize: 13,
                            color: post.bookmarked ? 'var(--accent)' : 'var(--text-muted)',
                            borderRadius: 8,
                          }}
                        >
                      ...
                      {expandedComments.has(post.id) && (
                        ...
                        >
                          {(comments[post.id] || []).map((c: any) => (
                            <div
                              key={c.id}
                              style={{ display: 'flex', gap: 8, marginBottom: 8, fontSize: 13 }}
                            >
                              <div
                                style={{
                                  width: 28,
                              ...
                              >
                                ...
                                >
                                  <strong
                                    ...
                                    onClick={() => onNavigate(`/user/${c.author?.id}`)}
                                  >
