  ...
  return (
    <div style={{ padding: '24px 20px', maxWidth: 900, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        padding: '4px', background: 'var(--bg-secondary)', borderRadius: 12,
      }}>
        {TABS.map(t => (
          <button
            key={t.id}
...
function MarketingTab() {
  ...
  const generate = async () => {
    ...
    try {
      const res = await api.aiMarketing({ type, topic, tone, audience, extra_instructions: extra || undefined })
      setResult(res)
    } catch (e: any) {
      ...
      <div className="u-card" style={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div style={{ gridColumn: '1 / -1' }}>
            ...
            <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="Ej: Lanzamiento de nueva funcion de comunidades..." style={inputStyle} />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={extra} onChange={e => setExtra(e.target.value)} placeholder="Incluir CTA, mencionar descuento..." style={inputStyle} />
          </div>
        </div>
        <button onClick={generate} disabled={loading || !topic.trim()} style={{ ...btnPrimary, marginTop: 16, opacity: loading || !topic.trim() ? 0.5 : 1 }}>
          Generar Contenido
        </button>
      </div>
        ...
        <div className="u-card" style={cardStyle}>
          ...
          {result.suggestions?.length > 0 && (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 8 }}>Ideas de seguimiento:</div>
              {result.suggestions.map((s: string, i: number) => (
                <div key={i} style={{
                  padding: '8px 12px', background: 'var(--bg-tertiary)', borderRadius: 8,
                  fontSize: 12, marginBottom: 4, color: 'var(--text-secondary)',
                  cursor: 'pointer',
                }} onClick={() => { setTopic(s); setResult(null) }}>
                  {i + 1}. {s}
                </div>
              ))}
            </div>
          )}
          <button onClick={() => { navigator.clipboard.writeText(result.content) }} style={{ ...btnPrimary, marginTop: 12, background: 'var(--bg-tertiary)', color: 'var(--text-primary)' }}>
...
function CalendarTab() {
  ...
  const generate = async () => {
    ...
    try {
      const res = await api.aiMarketingCalendar({ month, year, platforms, posts_per_week: postsPerWeek })
  ...
  const PLATFORM_COLORS: Record<string, string> = {
    instagram: '#E4405F', linkedin: '#0A66C2', tiktok: '#000000',
      ...
      <div className="u-card" style={cardStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12 }}>
          ...
          <div>
            ...
            <select value={postsPerWeek} onChange={e => setPostsPerWeek(Number(e.target.value))} style={selectStyle}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['instagram', 'linkedin', 'tiktok', 'twitter', 'facebook', 'youtube'].map(p => (
              <button key={p} onClick={() => togglePlatform(p)} style={{
                ...
                background: platforms.includes(p) ? (PLATFORM_COLORS[p] || 'var(--accent)') : 'var(--bg-primary)',
        ...
        <button onClick={generate} disabled={loading || platforms.length === 0} style={{ ...btnPrimary, marginTop: 16, opacity: loading ? 0.5 : 1 }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    <span style={{
                      ...
                      background: PLATFORM_COLORS[post.platform] || 'var(--accent)', color: '#fff',
                  ...
                  <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)' }}>{post.date}</span>
                </div>
                <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>{post.topic}</div>
                <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>{post.caption}</div>
  ...
  const run = async () => {
    ...
    try {
      if (mode === 'moderate') {
        ...
        const res = await api.aiEngage({ post_content: content, post_type: context, community_name: communityName || undefined })
    ...
    <div>
      <div className="u-card" style={cardStyle}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          ...
          <button onClick={() => { setMode('engage'); setResult(null) }} style={{
            ...btnPrimary,
        ...
        <div style={{ marginBottom: 12 }}>
          <label style={labelStyle}>{mode === 'moderate' ? 'Contenido a Moderar' : 'Post para Responder'}</label>
          <textarea value={content} onChange={e => setContent(e.target.value)} placeholder={mode === 'moderate' ? 'Pega el contenido a revisar...' : 'Pega el post al que quieres generar engagement...'} style={textareaStyle} />
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          {mode === 'engage' && (
            <div>
              ...
              <input value={communityName} onChange={e => setCommunityName(e.target.value)} placeholder="Ej: Ingenieria Civil UChile" style={inputStyle} />
            </div>
          )}
        </div>

        <button onClick={run} disabled={loading || !content.trim()} style={{ ...btnPrimary, marginTop: 16, opacity: loading || !content.trim() ? 0.5 : 1 }}>
        ...
        <div className="u-card" style={{ ...cardStyle, borderLeft: `4px solid ${result.data.approved ? '#22c55e' : '#ef4444'}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
            <span style={{
              ...
              background: result.data.approved ? '#22c55e' : '#ef4444', color: '#fff',
            }}>
              {result.data.approved ? 'APROBADO' : 'RECHAZADO'}
            </span>
          </div>
          <div style={{ fontSize: 13, marginBottom: 8 }}><strong>Razon:</strong> {result.data.reason}</div>
            ...
            <div style={{ marginBottom: 8 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 4 }}>
                {result.data.flags.map((f: string, i: number) => (
                  <span key={i} style={{ padding: '2px 10px', borderRadius: 12, fontSize: 11, background: 'rgba(239,68,68,0.1)', color: '#ef4444' }}>{f}</span>
                ))}
              </div>
            </div>
          )}
          {result.data.suggestion && (
            <div style={{ fontSize: 13, color: 'var(--text-muted)' }}><strong>Sugerencia:</strong> {result.data.suggestion}</div>
          )}
        </div>
      )}
      ...
      {result?.type === 'engage' && (
        <div className="u-card" style={cardStyle}>
          ...
          {result.data.questions?.length > 0 && (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6 }}>Preguntas de seguimiento:</div>
              {result.data.questions.map((q: string, i: number) => (
                <div key={i} style={{ padding: '6px 12px', background: 'var(--bg-tertiary)', borderRadius: 8, fontSize: 12, marginBottom: 4 }}>
                  {i + 1}. {q}
                </div>
              ))}
            </div>
          ...
          {result.data.related_topics?.length > 0 && (
            <div style={{ marginTop: 12 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 6 }}>Temas relacionados:</div>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {result.data.related_topics.map((t: string, i: number) => (
                  <span key={i} style={{ padding: '4px 12px', borderRadius: 20, fontSize: 11, background: 'var(--bg-tertiary)', color: 'var(--accent)', fontWeight: 600 }}>{t}</span>
                ))}
              </div>
            </div>
          )}
          <button onClick={() => { navigator.clipboard.writeText(result.data.reply) }} style={{ ...btnPrimary, marginTop: 12, background: 'var(--bg-tertiary)', color: 'var(--text-primary)' }}>
            Copiar Respuesta
          </button>
        </div>
      )}
    ...
    <div>
      <div className="u-card" style={cardStyle}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
          ...
          <button onClick={() => { setMode('testplan'); setResult(null) }} style={{
            ...btnPrimary,
        ...
        {mode === 'review' ? (
          <>
            <div style={{ marginBottom: 12 }}>
              ...
              <textarea value={code} onChange={e => setCode(e.target.value)} placeholder="Pega tu codigo aqui..." style={{ ...textareaStyle, minHeight: 200, fontFamily: 'monospace' }} />
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              <div>
              ...
              <div>
                ...
                <input value={codeContext} onChange={e => setCodeContext(e.target.value)} placeholder="Ej: componente de login..." style={inputStyle} />
              </div>
            </div>
          </>
        ) : (
          <>
            <div style={{ marginBottom: 12 }}>
              ...
              <textarea value={featureDesc} onChange={e => setFeatureDesc(e.target.value)} placeholder="Describe la funcionalidad que necesitas testear..." style={textareaStyle} />
            </div>
            <div>
        ...
        <button onClick={run} disabled={loading} style={{ ...btnPrimary, marginTop: 16, opacity: loading ? 0.5 : 1 }}>
      ...
      {result?.type === 'review' && (
        <div className="u-card" style={cardStyle}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <div style={{
              padding: '6px 16px', borderRadius: 20, fontSize: 18, fontWeight: 800,
              background: result.data.score >= 80 ? '#22c55e' : result.data.score >= 50 ? '#f59e0b' : '#ef4444',
              color: '#fff',
            }}>
              {result.data.score}/100
            </div>
          </div>
          <div style={{ fontSize: 13, marginBottom: 16, color: 'var(--text-secondary)' }}>{result.data.summary}</div>
          {result.data.issues?.length > 0 && (
            <div>
              {result.data.issues.map((issue: any, i: number) => (
                <div key={i} style={{
                  padding: 12, background: 'var(--bg-secondary)', borderRadius: 8, marginBottom: 8,
                  borderLeft: `4px solid ${severityColors[issue.severity] || '#999'}`,
                }}>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                    <span style={{
                      padding: '2px 8px', borderRadius: 10, fontSize: 10, fontWeight: 700,
                      background: severityColors[issue.severity] || '#999', color: '#fff',
                      textTransform: 'uppercase' as const,
                    }}>
                      {issue.severity}
                    </span>
                    {issue.line && <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Linea {issue.line}</span>}
                  </div>
      ...
      {result?.type === 'testplan' && (
        <div className="u-card" style={cardStyle}>
          ...
          {result.data.test_cases?.map((tc: any, i: number) => (
            <div key={i} style={{
              padding: 14, background: 'var(--bg-secondary)', borderRadius: 10, marginBottom: 8,
              borderLeft: `4px solid ${priorityColors[tc.priority] || '#999'}`,
            }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                <div style={{ fontSize: 13, fontWeight: 700 }}>{tc.id} — {tc.name}</div>
                <span style={{
                  padding: '2px 8px', borderRadius: 10, fontSize: 10, fontWeight: 600,
                  background: priorityColors[tc.priority] || '#999', color: '#fff',
                }}>
                  {tc.priority}
                </span>
              </div>
          ...
          {result.data.coverage_notes && (
            <div style={{ ...resultBox, marginTop: 12 }}>
              <strong>Notas de Cobertura:</strong><br />{result.data.coverage_notes}
            </div>
          )}
        </div>
      )}
    ...
    <div>
      <div className="u-card" style={cardStyle}>
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe lo que necesitas disenar..." style={textareaStyle} />
        </div>
        <button onClick={generate} disabled={loading || !description.trim()} style={{ ...btnPrimary, marginTop: 16, opacity: loading || !description.trim() ? 0.5 : 1 }}>
      ...
      {result && (
        <div className="u-card" style={cardStyle}>
            ...
            <div style={{ marginTop: 16, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              {result.specifications.dimensions && (
                <div style={{ padding: 12, background: 'var(--bg-secondary)', borderRadius: 8 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 4 }}>DIMENSIONES</div>
                  <div style={{ fontSize: 13 }}>{result.specifications.dimensions}</div>
                </div>
              )}
              {result.specifications.typography && (
                <div style={{ padding: 12, background: 'var(--bg-secondary)', borderRadius: 8 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 4 }}>TIPOGRAFIA</div>
                  <div style={{ fontSize: 13 }}>{result.specifications.typography}</div>
                </div>
              )}
              {result.specifications.colors?.length > 0 && (
                <div style={{ padding: 12, background: 'var(--bg-secondary)', borderRadius: 8 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 4 }}>COLORES</div>
              ...
              {result.specifications.layout && (
                <div style={{ padding: 12, background: 'var(--bg-secondary)', borderRadius: 8 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 4 }}>LAYOUT</div>
          ...
          {result.copy_suggestions?.length > 0 && (
            <div style={{ marginTop: 16 }}>
              <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', marginBottom: 8 }}>Copy sugerido:</div>
