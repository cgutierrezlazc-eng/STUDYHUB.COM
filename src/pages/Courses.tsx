...
export default function Courses({ onNavigate }: Props) {
  ...
  const saveProgress = (courseId: string, completedLessons: string[]) => {
    try {
      localStorage.setItem(`conniku_course_progress_${courseId}`, JSON.stringify(completedLessons));
    } catch {}
  };
  const loadProgress = (courseId: string): string[] | null => {
    try {
      const d = localStorage.getItem(`conniku_course_progress_${courseId}`);
  ...
  const saveQuizResult = (courseId: string, result: any) => {
    try {
      localStorage.setItem(`conniku_course_quiz_${courseId}`, JSON.stringify(result));
    } catch {}
  };
  const loadQuizResult = (courseId: string): any => {
    try {
      const d = localStorage.getItem(`conniku_course_quiz_${courseId}`);
      return d ? JSON.parse(d) : null;
    } catch {
      return null;
    }
  ...
  const saveExerciseResult = (courseId: string, result: any, stats: any) => {
    try {
      localStorage.setItem(
        `conniku_course_exercises_${courseId}`,
  ...
  const loadExerciseResult = (courseId: string): { result: any; stats: any } | null => {
    try {
      const d = localStorage.getItem(`conniku_course_exercises_${courseId}`);
      return d ? JSON.parse(d) : null;
    } catch {
      return null;
    }
...
<div style="text-align:center;padding:60px;border:3px double #2563EB;margin:40px;min-height:60vh;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      ...
      if (savedProgress && data.lessons) {
        ...
        if (data.progress) {
          const merged = Array.from(
            new Set([...(data.progress.completedLessons || []), ...savedProgress])
          );
    ...
    try {
      ...
      setCourseDetail((prev: any) => ({
        ...
        lessons: prev.lessons.map((l: any) => ({
          ...l,
          completed: result.completedLessons.includes(l.id),
        })),
      }));
  ...
  const loadCertificates = async () => {
    try {
      setCertificates(await api.getMyCertificates());
    } catch (err: any) {
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>
                      ~{courseDetail.estimatedMinutes} min ·{' '}
                      {courseDetail.difficulty === 'intermediate' ? 'Intermedio' : 'Principiante'}
                    </div>
                  </div>
                </div>
                {/* Progress bar */}
                ...
                >
                  ...
                  >
                    ...
                    <span
                      style={{
                        fontWeight: 600,
                        color: progressPercent === 100 ? '#059669' : catColor,
                      }}
                    >
                      {progressPercent}%
                    </span>
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: '100%',
                        borderRadius: 3,
                        background: progressPercent === 100 ? '#059669' : catColor,
                        transition: 'width 0.5s ease',
                      }}
                    />
                  </div>
                  ...
                  <>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setExerciseMode(false);
                        setSidebarOpen(false);
                        if (quizQuestions.length === 0 && !quizQuestionsLoading)
                          loadQuizQuestions(courseDetail.id);
                      }}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    >
                      <div
                        style={{
                          ...
                          justifyContent: 'center',
                          fontSize: 13,
                          background: courseDetail.progress?.quizPassed
                            ? '#059669'
                            : quizMode
                              ? catColor
                              : 'transparent',
                          color:
                            courseDetail.progress?.quizPassed || quizMode
                              ? '#fff'
                              : 'var(--text-muted)',
                          border:
                            courseDetail.progress?.quizPassed || quizMode
                      ...
                      <div>
                        <div
                          style={{
                            fontSize: 13,
                            fontWeight: quizMode ? 600 : 400,
                            color: quizMode ? catColor : 'var(--text-primary)',
                          }}
                        >
                          Examen Final
                        </div>
                  ...
                  <>
                    ...
                    <button
                      onClick={() => {
                        ...
                        setQuizMode(false);
                        setSidebarOpen(false);
                        if (exercises.length === 0 && !exerciseLoading) loadExercises();
                      }}
                      style={{
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    >
                      ...
                      <div>
                        <div
                          style={{
                            fontSize: 13,
                            fontWeight: exerciseMode ? 600 : 400,
                            color: exerciseMode ? catColor : 'var(--text-primary)',
                          }}
                        >
                          Ejercicios
                        </div>
                  ...
                  {exerciseLoading ? (
                    ...
                    <div style={{ textAlign: 'center', padding: '40px 20px' }}>
                      ...
                      >
                        <div>
                          <div
                            style={{
                              fontSize: 36,
                              fontWeight: 700,
                              color: exerciseResult.score >= 70 ? '#059669' : '#DC2626',
                            }}
                          >
                        ...
                        <div>
                          ...
                          >
                            {exerciseResult.correct}/{exerciseResult.total}
                          </div>
                      ...
                      {exerciseStats && (
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            ...
                            <strong
                              style={{
                                color: exerciseStats.accuracy >= 70 ? '#059669' : '#DC2626',
                              }}
                            >
                    ...
                    <div>
                      ...
                      <div style={{ marginBottom: 28 }}>
                        ...
                        <h2
                          style={{ margin: '0 0 6px', fontSize: 22, color: 'var(--text-primary)' }}
                        >
                          {courseDetail.title}
                        </h2>
                        <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: 14 }}>
                          Cada vez que practiques recibiras preguntas diferentes. Nuestro sistema
                          avanzado se asegura de que nunca veas la misma pregunta dos veces.
                        </p>
                        {exerciseStats && exerciseStats.totalAnswered > 0 && (
                          ...
                          >
                            <span>
                              {Target({ size: 12 })} {exerciseStats.totalAnswered} ejercicios
                              resueltos
                            </span>
                            <span>
                              {CheckCircle({ size: 12 })} {exerciseStats.accuracy}% precision
                            </span>
                          </div>
                        )}
                        <div
                        ...
                        >
                          <span>
                            {FileText({ size: 12 })} {exercises.length} preguntas
                          </span>
                          <span>
                            {CheckCircle({ size: 12 })} {Object.keys(exerciseAnswers).length}{' '}
                            respondidas
                          </span>
                        </div>
                      </div>
                            ...
                            {q.options.map((opt: string, oi: number) => {
                              ...
                              return (
                                <button
                                  key={oi}
                                  onClick={() =>
                                    setExerciseAnswers((prev) => ({ ...prev, [String(qi)]: oi }))
                                  }
                                  style={{
                                    padding: '10px 14px',
                                    borderRadius: 8,
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            fontWeight: 600,
                            opacity:
                              Object.keys(exerciseAnswers).length < exercises.length ? 0.6 : 1,
                          }}
                        >
                          Enviar respuestas
                        </button>
                  ...
                  {quizResult ? (
                    ...
                    <div>
                      <div style={{ marginBottom: 28 }}>
                        ...
                        <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: 14 }}>
                          Responde todas las preguntas. Necesitas <strong>80%</strong> para aprobar
                        ...
                        >
                          <span>
                            {FileText({ size: 12 })} {quizQuestions.length} preguntas
                          </span>
                          <span>
                            {CheckCircle({ size: 12 })} {Object.keys(quizAnswers).length}{' '}
                            respondidas
                          </span>
                            ...
                            {q.options.map((opt: string, oi: number) => {
                              ...
                              return (
                                <button
                                  key={oi}
                                  onClick={() =>
                                    setQuizAnswers((prev) => ({ ...prev, [String(q.qid)]: oi }))
                                  }
                                  style={{
                                    padding: '10px 14px',
                                    borderRadius: 8,
                      ...
                      >
                        ...
                        <button
                          ...
                          style={{
                            ...
                            fontWeight: 600,
                            opacity:
                              Object.keys(quizAnswers).length < quizQuestions.length ? 0.6 : 1,
                          }}
                        >
                          Enviar examen
                        </button>
