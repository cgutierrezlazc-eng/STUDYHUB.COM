...
async function request(endpoint: string, options?: RequestInit) {
  ...
  if (res.status === 401 && endpoint !== '/auth/refresh') {
    ...
    throw new Error(
      (errData as any).detail || 'Sesión expirada. Por favor inicia sesión nuevamente.'
    );
  }
...
export const api = {
  ...
  checkUsername: (q: string) => request(`/auth/check-username?q=${encodeURIComponent(q)}`),
  ...
  getMyMentoringRequests: () => request('/auth/tutoring-requests'),

  respondMentoringRequest: (requestId: string, action: 'accepted' | 'rejected') =>
    request(`/auth/tutoring-request/${requestId}/respond`, {
      method: 'POST',
  ...
  uploadDocument: async (projectId: string, file: File) => {
    ...
    if (!res.ok) {
      ...
      try {
        ...
        if (e.detail || e.message) msg = e.detail || e.message;
      } catch {}
      throw new Error(msg);
    }
    return res.json();
  ...
  supportChat: (
    ...
    request('/support/chat', {
      method: 'POST',
      body: JSON.stringify({ message, history, page_context }),
    }),
  ...
  chat: async (
    ...
    languageSkill: string = 'intermediate',
  ...
  // ─── Study Tools ───────────────────────────────────────────
  generateGuide: (projectId: string) => request(`/projects/${projectId}/guide`, { method: 'POST' }),
  ...
  getStudyPlan: (projectId: string) => request(`/projects/${projectId}/study-plan`),

  generateFlashcards: (projectId: string) =>
    request(`/projects/${projectId}/flashcards`, { method: 'POST' }),

  // ─── Summary & Herramientas Avanzadas ────────────────────────────────
  generateSummary: (
    projectId: string,
    detailLevel: string = 'comprehensive',
    exportFormat: string = ''
  ) =>
  ...
  exportSummaryDocx: async (
    projectId: string,
    summaryData: any,
    title: string = 'Resumen de Estudio'
  ) => {
    const token = getToken();
    const headers: Record<string, string> = { 'Content-Type': 'application/json' };
    if (token) headers['Authorization'] = `Bearer ${token}`;
    const res = await fetch(`${API_BASE}/projects/${projectId}/summary/export-docx`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ ...summaryData, title }),
    });
  ...
  exportChatPdf: async (projectId: string, messages: any[], title: string = 'Chat de Estudio') => {
    ...
    const res = await fetch(`${API_BASE}/projects/${projectId}/chat/export-pdf`, {
      method: 'POST',
      headers,
      body: JSON.stringify({ content: '', messages, title }),
    });
    if (!res.ok) throw new Error('Export Error');
    const blob = await res.blob();
    const url = URL.createObjectURL(blob);
  ...
  scanAndSolve: (imageBase64: string, language?: string) =>
    request('/math/scan', {
      method: 'POST',
      body: JSON.stringify({ image_base64: imageBase64, language: language || 'es' }),
    }),

  // ─── Messaging ─────────────────────────────────────────────
  searchUsers: (q: string) => request(`/messaging/users/search?q=${encodeURIComponent(q)}`),

  getConversations: () => request('/messaging/conversations'),

  getUnreadMessageCount: () => request('/messaging/unread-count'),
  ...
  getConversation: (id: string) => request(`/messaging/conversations/${id}`),
  ...
  sendMessage: (
    ...
    request(`/messaging/conversations/${convId}/messages`, {
      method: 'POST',
  ...
  editMessage: (convId: string, msgId: string, content: string) =>
    request(`/messaging/conversations/${convId}/messages/${msgId}`, {
      method: 'PUT',
  ...
  addParticipant: (convId: string, userId: string) =>
    request(`/messaging/conversations/${convId}/participants?user_id=${userId}`, {
      method: 'POST',
    }),
  ...
  deleteFolder: (id: string) => request(`/messaging/folders/${id}`, { method: 'DELETE' }),
  addToFolder: (folderId: string, conversationId: string) =>
    request(`/messaging/folders/${folderId}/conversations`, {
      method: 'POST',
  ...
  adminGetFlaggedMessages: (page: number = 1) => request(`/admin/messages/flagged?page=${page}`),
  adminDeleteMessage: (id: string) => request(`/admin/messages/${id}`, { method: 'DELETE' }),
  adminGetStats: () => request('/admin/stats'),
  getOnlineUsers: () => request('/admin/online-users'),
  adminGetModerationLogs: (page: number = 1) => request(`/admin/moderation-logs?page=${page}`),
  ...
  adminGetBlockedUsers: (page: number = 1) => request(`/admin/blocked-users?page=${page}`),
  ...
  adminGetPayments: (page: number = 1) => request(`/admin/payments?page=${page}`),
  adminGetFinancialSummary: () => request('/admin/financial-summary'),
  ...
  sendClosureFeedback: (reason: string, feedback: string) =>
    request('/auth/me/closure-feedback', {
      method: 'POST',
      body: JSON.stringify({ reason, feedback }),
    }),

  sendSuggestion: (type: string, subject: string, message: string) =>
    request('/auth/me/suggestion', {
      method: 'POST',
      body: JSON.stringify({ type, subject, message }),
    }),

  // ─── Bio automática ─────────────────────────────────────────
  generateBio: () => request('/auth/me/bio/generate', { method: 'POST' }),
  ...
  cvCoach: (cvText: string, targetRole: string = '') =>
    request('/ai/cv-coach', {
      method: 'POST',
      body: JSON.stringify({ cv_text: cvText, target_role: targetRole }),
    }),
  ...
  unfriend: (userId: string) => request(`/social/friend/${userId}`, { method: 'DELETE' }),
  searchSocialUsers: (q: string) => request(`/social/users/search?q=${encodeURIComponent(q)}`),
  getUserProfile: (userId: string) => request(`/social/users/${userId}/profile`),
  getFeed: (
    page: number = 1,
    sort: 'recent' | 'smart' = 'recent',
    filter: 'all' | 'career' | 'university' | 'friends' = 'all'
  ) => request(`/social/feed?page=${page}&sort=${sort}&filter=${filter}`),
  getTrendingPosts: () => request(`/social/feed/trending`),
  ...
  createWallPost: (
    ...
    request(`/social/wall/${wallOwnerId}/posts`, {
      ...
      body: JSON.stringify({
        ...
        visibility_list_id: visibilityListId || null,
      }),
    }),
  ...
  deleteWallPost: (postId: string) => request(`/social/posts/${postId}`, { method: 'DELETE' }),
  toggleLike: (postId: string) => request(`/social/posts/${postId}/like`, { method: 'POST' }),
  reactToPost: (postId: string, reactionType: string) =>
    request(`/social/posts/${postId}/react`, {
      method: 'POST',
      body: JSON.stringify({ reaction_type: reactionType }),
    }),
  getComments: (postId: string) => request(`/social/posts/${postId}/comments`),
  ...
  blockUser: (userId: string) => request(`/social/users/${userId}/block`, { method: 'POST' }),
  unblockUser: (userId: string) => request(`/social/users/${userId}/unblock`, { method: 'POST' }),
  getBlockedUsers: () => request('/social/blocked-users'),
  reportUser: (userId: string, reason: string) =>
    request(`/social/users/${userId}/report`, { method: 'POST', body: JSON.stringify({ reason }) }),
  getFriendSuggestions: () => request('/social/friend-suggestions'),
  ...
  // ─── Friend Lists ─────────────────────────────────────────
  getFriendLists: () => request('/social/friend-lists'),
  ...
  addToFriendList: (listId: string, friendId: string) =>
    request(`/social/friend-lists/${listId}/members`, {
      method: 'POST',
  ...
  // ─── News & Announcements ─────────────────────────────────
  getUniversityNews: () => request('/news/university'),

  getAnnouncements: () => request('/news/announcements'),
  ...
  addYoutubeVideo: (projectId: string, url: string, title?: string) =>
    request(`/projects/${projectId}/video/youtube`, {
      method: 'POST',
  ...
  getVideos: (projectId: string) => request(`/projects/${projectId}/videos`),
  ...
  deleteCalendarEvent: (id: string) => request(`/calendar/events/${id}`, { method: 'DELETE' }),

  // ─── Marketplace ──────────────────────────────────────────
  getMarketplaceDocuments: (
    search?: string,
    university?: string,
    career?: string,
    page?: number
  ...
  rateDocument: (docId: string, rating: number) =>
    request(`/marketplace/documents/${docId}/rate`, {
      method: 'POST',
  ...
  markNotificationRead: (id: string) =>
    request(`/notifications/in-app/${id}/read`, { method: 'POST' }),
  ...
  deleteCommunity: (id: string) => request(`/communities/${id}`, { method: 'DELETE' }),
  ...
  createCommunityPost: (
    id: string,
    content: string,
    opts?: string | { image_url?: string; is_announcement?: boolean }
  ) => {
    ...
    return request(`/communities/${id}/posts`, {
      method: 'POST',
      body: JSON.stringify({ content, image_url, is_announcement }),
    });
  },
  likeCommunityPost: (postId: string, reactionType?: string) =>
    request(`/communities/posts/${postId}/like`, {
      method: 'POST',
      body: JSON.stringify({ reaction_type: reactionType || 'like' }),
    }),
  getCommunityPostComments: (postId: string) => request(`/communities/posts/${postId}/comments`),
  ...
  reportCommunityPost: (postId: string, reason: string) =>
    request(`/communities/posts/${postId}/report`, {
      method: 'POST',
      body: JSON.stringify({ reason }),
    }),
  getCommunitySuggestions: () => request('/communities/suggestions'),
  getCommunityResources: (communityId: string) => request(`/communities/${communityId}/resources`),
  addCommunityResource: (
    communityId: string,
    data: { resource_type: string; title: string; url: string; description?: string }
  ) =>
  ...
  getCommunityEvents: (communityId: string) => request(`/communities/${communityId}/events`),
  ...
  rsvpCommunityEvent: (communityId: string, eventId: string, status: string) =>
    request(`/communities/${communityId}/events/${eventId}/rsvp?status=${status}`, {
      method: 'POST',
    }),
  ...
  votePoll: (pollId: string, optionId: string) =>
    request(`/social/polls/${pollId}/vote`, {
      method: 'POST',
  ...
  autocompleteUsers: (q: string) =>
    request(`/social/users/autocomplete?q=${encodeURIComponent(q)}`),
  ...
  deleteJobListing: (id: string) => request(`/jobs/listings/${id}`, { method: 'DELETE' }),
  ...
  updateApplicationStatus: (appId: string, status: string, notes?: string) =>
    request(`/jobs/applications/${appId}/status`, {
      method: 'PUT',
  ...
  // ─── Courses ──────────────────────────────────────────────
  getCourses: (category?: string) => request(`/courses${category ? `?category=${category}` : ''}`),
  getCourse: (id: string) => request(`/courses/${id}`),
  generateCourseContent: (id: string) => request(`/courses/${id}/generate`, { method: 'POST' }),
  completeLesson: (courseId: string, lessonId: string) =>
    request(`/courses/${courseId}/lessons/${lessonId}/complete`, { method: 'POST' }),
  getCourseQuizQuestions: (courseId: string) => request(`/courses/${courseId}/quiz/questions`),

  submitCourseQuiz: (courseId: string, answers: Record<string, number>) =>
    request(`/courses/${courseId}/quiz/submit`, {
      method: 'POST',
  ...
  submitExercises: (courseId: string, answers: Record<string, number>, questions: any[]) =>
    request(`/courses/${courseId}/exercises/submit`, {
      method: 'POST',
  ...
  // Admin course management (manual, sin automatizacion)
  ...
  adminCertifyUser: (userId: string, courseIds: string[], scoreOverride?: number) =>
    request('/courses/admin/certify', {
      method: 'POST',
      body: JSON.stringify({
        user_id: userId,
        course_ids: courseIds,
        score_override: scoreOverride || 100,
      }),
    }),
  adminRevokeCertificate: (userId: string, courseId: string) =>
    request(`/courses/admin/revoke-certificate?user_id=${userId}&course_id=${courseId}`, {
      method: 'POST',
    }),
  ...
  updateCV: (data: any) => request('/cv/me', { method: 'PUT', body: JSON.stringify(data) }),
  getUserCV: (username: string) => request(`/cv/${username}`),
  getPublicCVs: (search?: string) =>
    request(`/courses/cv-public/all${search ? `?search=${encodeURIComponent(search)}` : ''}`),
  ...
  retryClassPayment: (classId: string) =>
    request(`/tutors/classes/${classId}/payment-retry`, { method: 'PUT' }),
  ...
  requestTutoring: (listingId: string, data: { message?: string; scheduled_at?: string }) =>
    request(`/jobs/tutoring/listings/${listingId}/request`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  ...
  updateTutoringRequestStatus: (
    ...
    request(`/jobs/tutoring/requests/${requestId}/status`, {
      method: 'PUT',
  ...
  addSkill: (userId: string, skillName: string) =>
    request(`/social/users/${userId}/skills`, {
      method: 'POST',
      body: JSON.stringify({ skill_name: skillName }),
    }),
  removeSkill: (skillId: string) => request(`/social/skills/${skillId}`, { method: 'DELETE' }),
  ...
  getBookmarks: (page?: number) => request(`/social/bookmarks${page ? `?page=${page}` : ''}`),
  ...
  completeMentorship: (relationId: string, rating?: number, review?: string) =>
    request(`/mentorship/${relationId}/complete`, {
      method: 'POST',
  ...
  cancelMpSubscription: (termsAccepted = true) =>
    request('/payments/mp/cancel-subscription', {
      method: 'POST',
  ...
  createPaypalSubscription: (plan_id: string) =>
    request('/payments/paypal/create-subscription', {
      method: 'POST',
      body: JSON.stringify({ plan_id }),
    }),
  cancelPaypalSubscription: (subscription_id: string, termsAccepted = true) =>
    request('/payments/paypal/cancel-subscription', {
      method: 'POST',
      body: JSON.stringify({ subscription_id, terms_accepted: termsAccepted }),
    }),
  ...
  joinStudyRoom: (roomId: string) => request(`/study-rooms/${roomId}/join`, { method: 'POST' }),
  leaveStudyRoom: (roomId: string, studyMinutes?: number) =>
    request(`/study-rooms/${roomId}/leave`, {
      method: 'POST',
      body: JSON.stringify({ study_minutes: studyMinutes || 0 }),
    }),
  closeStudyRoom: (roomId: string) => request(`/study-rooms/${roomId}`, { method: 'DELETE' }),
  ...
  generateDiagnostic: (
    ...
    request(`/quiz-system/diagnostic/${projectId}`, {
      method: 'POST',
      body: JSON.stringify({
  ...
  submitDiagnostic: (projectId: string, answers: Record<string, number>, questions: any[]) =>
    request(`/quiz-system/diagnostic/${projectId}/submit`, {
      method: 'POST',
      body: JSON.stringify({ answers, questions }),
    }),
  getScheduledQuizzes: (projectId: string) => request(`/quiz-system/scheduled/${projectId}`),
  ...
  submitScheduledQuiz: (quizId: string, answers: Record<string, number>, questions: any[]) =>
    request(`/quiz-system/scheduled/${quizId}/submit`, {
      method: 'POST',
      body: JSON.stringify({ answers, questions }),
    }),
  getSubjectAverage: (projectId: string) => request(`/quiz-system/average/${projectId}`),
  checkQuizAlerts: () => request('/quiz-system/check-alerts', { method: 'POST' }),

  // ─── Pomodoro ─────────────────────────────────────────────
  completePomodoroSession: (durationMinutes: number, subjectId?: string) =>
    request('/pomodoro/complete', {
      method: 'POST',
      body: JSON.stringify({ duration_minutes: durationMinutes, subject_id: subjectId }),
    }),
  ...
  getMoodHistory: (days?: number) =>
    request(`/wellness/mood/history${days ? `?days=${days}` : ''}`),
  ...
  setStudyGoal: (goalHours: number) =>
    request('/auth/me', {
      method: 'PUT',
      body: JSON.stringify({ weekly_study_goal_hours: goalHours }),
    }),

  // ─── Finance / Multi-currency ─────────────────────────────
  getFinancePrices: (countryCode?: string) => request(`/finance/prices/${countryCode || 'CL'}`),
  ...
  // ─── Fiscal Alerts ───────────────────────────────────────
  checkFiscalAlerts: () => request('/email/fiscal-alerts/check', { method: 'POST' }),
  ...
  rejectModerationItem: (itemId: string, note?: string) =>
    request(`/ceo/moderation/${itemId}/reject${note ? `?note=${encodeURIComponent(note)}` : ''}`, {
      method: 'POST',
    }),
  ...
  getCeoEmailInbox: (page?: number, emailType?: string, account?: string) =>
    request(
      `/email/ceo/inbox?page=${page || 1}${emailType ? `&email_type=${emailType}` : ''}${account ? `&account=${account}` : ''}`
    ),
  ...
  ceoDeleteEmail: (emailId: string) => request(`/email/ceo/email/${emailId}`, { method: 'DELETE' }),
  ...
  logAttendance: (
    projectId: string,
    data: { title?: string; duration_minutes?: number; recorded?: boolean; transcribed?: boolean }
  ...
  generateExamNightPlan: (projectId: string, hours: number) =>
    request(`/projects/${projectId}/exam-night-mode`, {
      method: 'POST',
  ...
  sendContactFromProfile: (subject: string, message: string) =>
    request('/email/contact/from-profile', {
      method: 'POST',
  ...
  downloadToConniku: (url: string, filename?: string) =>
    request('/search/download-to-conniku', {
      method: 'POST',
  ...
  getConferences: (status?: string) => request(`/conferences/${status ? `?status=${status}` : ''}`),
  getConference: (id: string) => request(`/conferences/${id}`),
  joinConference: (id: string) => request(`/conferences/${id}/join`, { method: 'POST' }),
  leaveConference: (id: string) => request(`/conferences/${id}/leave`, { method: 'POST' }),
  endConference: (id: string) => request(`/conferences/${id}/end`, { method: 'POST' }),
  getConferenceTranscription: (id: string) => request(`/conferences/${id}/transcription`),
  deleteConference: (id: string) => request(`/conferences/${id}`, { method: 'DELETE' }),
  ...
  uploadEmployeeDocument: (id: string, data: any) =>
    request(`/hr/employees/${id}/documents`, { method: 'POST', body: JSON.stringify(data) }),
  ...
  generateContractPdf: (employeeId: string, data: { html: string; worker_name: string }) =>
    request(`/hr/employees/${employeeId}/contract/generate`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  ...
  calculatePayroll: (month: number, year: number, overrides?: Record<string, any>) =>
    request('/hr/payroll/calculate', {
      method: 'POST',
  ...
  createExpense: (data: any) =>
    request('/hr/expenses', { method: 'POST', body: JSON.stringify(data) }),
  updateExpense: (id: string, data: any) =>
    request(`/hr/expenses/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  ...
  rejectLeaveRequest: (requestId: string, reason?: string) =>
    request(`/hr/leave/requests/${requestId}/reject`, {
      method: 'PUT',
  ...
  applyAsTutor: (data: any) =>
    request('/tutors/apply', { method: 'POST', body: JSON.stringify(data) }),
  getMyTutorProfile: () => request('/tutors/my-profile'),
  updateMyTutorProfile: (data: any) =>
    request('/tutors/my-profile', { method: 'PUT', body: JSON.stringify(data) }),
  uploadTutorDocument: (data: any) =>
    request('/tutors/my-profile/documents', { method: 'POST', body: JSON.stringify(data) }),
  getMyTutorDocuments: () => request('/tutors/my-profile/documents'),
  getTutorApplications: (params?: string) =>
    request(`/tutors/admin/applications${params ? `?${params}` : ''}`),
  ...
  rejectTutor: (id: string, reason: string) =>
    request(`/tutors/admin/${id}/reject?reason=${encodeURIComponent(reason)}`, { method: 'PUT' }),
  suspendTutor: (id: string, reason: string) =>
    request(`/tutors/admin/${id}/suspend?reason=${encodeURIComponent(reason)}`, { method: 'PUT' }),
  getTutorAdmin: (id: string) => request(`/tutors/admin/${id}`),
  createTutorClass: (data: any) =>
    request('/tutors/classes', { method: 'POST', body: JSON.stringify(data) }),
  getTutorClasses: (params?: string) => request(`/tutors/classes${params ? `?${params}` : ''}`),
  getTutorClass: (id: string) => request(`/tutors/classes/${id}`),
  enrollInClass: (id: string, data: any) =>
    request(`/tutors/classes/${id}/enroll`, { method: 'POST', body: JSON.stringify(data) }),
  confirmClassCompletion: (id: string) =>
    request(`/tutors/classes/${id}/confirm`, { method: 'PUT' }),
  rateTutorClass: (id: string, data: any) =>
    request(`/tutors/classes/${id}/rate`, { method: 'POST', body: JSON.stringify(data) }),
  rateStudentInClass: (classId: string, data: { rating: number; comment?: string }) =>
    request(`/tutors/classes/${classId}/rate-student`, {
      method: 'POST',
      body: JSON.stringify(data),
    }),
  getMyOwnClasses: () => request('/tutors/my-own-classes'),
  reportClassNoshow: (id: string, data: any) =>
    request(`/tutors/classes/${id}/report-noshow`, { method: 'POST', body: JSON.stringify(data) }),
  getMyTutorPayments: (params?: string) =>
    request(`/tutors/my-payments${params ? `?${params}` : ''}`),
  getMyTutorPayslips: (params?: string) =>
    request(`/tutors/my-payslips${params ? `?${params}` : ''}`),
  ...
  getAdminTutorPayments: (params?: string) =>
    request(`/tutors/admin/payments${params ? `?${params}` : ''}`),
  processAdminTutorPayment: (id: string) =>
    request(`/tutors/admin/payments/${id}/process`, { method: 'PUT' }),
  markAdminTutorPaymentPaid: (id: string, data: any) =>
    request(`/tutors/admin/payments/${id}/paid`, { method: 'PUT', body: JSON.stringify(data) }),
  generateTutorPayslips: (year: number, month: number) =>
    request(`/tutors/admin/payslips/generate/${year}/${month}`),
  ...
  getMyTutoringClasses: (params?: string) =>
    request(`/tutors/my-enrolled-classes${params ? `?${params}` : ''}`),
  bookTutoringSession: (data: {
    tutor_id: string;
    subject: string;
  ...
  getMyTutoringPayments: (params?: string) =>
    request(`/tutors/my-student-payments${params ? `?${params}` : ''}`),
  ...
  submitTutoringExam: (classId: string, answers: Record<string, any>) =>
    request(`/tutors/classes/${classId}/exam/submit`, {
      method: 'POST',
      body: JSON.stringify({ answers }),
    }),

  // ─── Tutor Exam Management (tutor side) ────────────────────
  createTutorExam: (classId: string, data: any) =>
    request(`/tutors/classes/${classId}/exam`, { method: 'POST', body: JSON.stringify(data) }),
  enableTutorExam: (classId: string) =>
    request(`/tutors/classes/${classId}/exam/enable`, { method: 'PUT' }),
  ...
  setMyAvailability: (slots: any[]) =>
    request('/tutors/my-availability', { method: 'PUT', body: JSON.stringify({ slots }) }),
  addBlockedDates: (data: any) =>
    request('/tutors/my-blocked-dates', { method: 'POST', body: JSON.stringify(data) }),
  removeBlockedDate: (id: string) =>
    request(`/tutors/my-blocked-dates/${id}`, { method: 'DELETE' }),
  getTutorAvailability: (tutorId: string) => request(`/tutors/${tutorId}/availability`),

  // ─── Tutor Programs ────────────────────────────────────────
  createTutorProgram: (data: any) =>
    request('/tutors/programs', { method: 'POST', body: JSON.stringify(data) }),
  getTutorCategories: () => request('/tutors/categories'),

  // ─── Class Chat ───────────────────────────────────────────
  getClassMessages: (classId: string, page?: number) =>
    request(`/tutors/classes/${classId}/messages?page=${page || 1}`),
  sendClassMessage: (classId: string, message: string) =>
    request(`/tutors/classes/${classId}/messages`, {
      method: 'POST',
      body: JSON.stringify({ message }),
    }),

  // ─── Tutor Subjects (Asignaturas) ────────────────────────
  createTutorSubject: (data: any) =>
    request('/tutors/subjects', { method: 'POST', body: JSON.stringify(data) }),
  getMyTutorSubjects: () => request('/tutors/my-subjects'),
  updateTutorSubject: (id: string, data: any) =>
    request(`/tutors/subjects/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteTutorSubject: (id: string) => request(`/tutors/subjects/${id}`, { method: 'DELETE' }),
  submitTutorSubjectForApproval: (id: string) =>
    request(`/tutors/subjects/${id}/submit`, { method: 'POST' }),
  getAdminSubjects: (params?: string) =>
    request(`/tutors/admin/subjects${params ? `?${params}` : ''}`),
  approveSubject: (id: string) =>
    request(`/tutors/admin/subjects/${id}/approve`, { method: 'PUT' }),
  rejectSubject: (id: string, reason: string) =>
    request(`/tutors/admin/subjects/${id}/reject?reason=${encodeURIComponent(reason)}`, {
      method: 'PUT',
    }),
  ...
  updateMyExecutiveShowcase: (items: any[]) =>
    request('/auth/me/executive-showcase', { method: 'PUT', body: JSON.stringify(items) }),
  getUserExecutiveShowcase: (userId: string) => request(`/auth/users/${userId}/executive-showcase`),

  // ─── Class Session / FASE 3 ──────────────────────────────
  startClassSession: (classId: string) =>
    request(`/tutors/classes/${classId}/start`, { method: 'PUT' }),
  studentConfirmClass: (classId: string) =>
    request(`/tutors/classes/${classId}/student-confirm`, { method: 'PUT' }),
  objectToClass: (classId: string, reason: string, termsAccepted: boolean) =>
    request(`/tutors/classes/${classId}/object`, {
      method: 'POST',
      body: JSON.stringify({ reason, terms_accepted: termsAccepted }),
    }),
  getClassJitsiRoom: (classId: string) => request(`/tutors/classes/${classId}/jitsi-room`),

  // ─── FASE 4: Post-class exam (3-attempt) + Resumen automatico ────
  ...
  submitExamAttempt: (classId: string, answers: Record<string, any>, timeSpentSeconds?: number) =>
    request(`/tutors/classes/${classId}/exam/attempt`, {
      method: 'POST',
      body: JSON.stringify({ answers, time_spent_seconds: timeSpentSeconds || 0 }),
    }),
  generateClassSummary: (classId: string) =>
    request(`/tutors/classes/${classId}/generate-summary`, { method: 'POST' }),
  ...
  getAllAttendance: (date?: string) => request(`/hr/attendance/all${date ? `?date=${date}` : ''}`),
  ...
  likeBlogPost: (postId: string) => request(`/blog/posts/${postId}/like`, { method: 'POST' }),
  ...
  lmsSyncItem: (itemId: string, conniku_project_id: string) =>
    request(`/lms/sync/${itemId}`, {
      method: 'POST',
      body: JSON.stringify({ item_id: itemId, conniku_project_id }),
    }),
  ...
  lmsLinkCourse: (courseId: string, conniku_project_id: string) =>
    request('/lms/link-course', {
      method: 'POST',
      body: JSON.stringify({ course_id: courseId, conniku_project_id }),
    }),
  lmsGetCourses: () => request('/lms/courses'),
  lmsActivateCourses: (connectionId: string, courseIds: string[]) =>
    request('/lms/activate-courses', {
      method: 'POST',
      body: JSON.stringify({ connection_id: connectionId, course_ids: courseIds }),
    }),
  ...
  lmsRenameCourse: (courseId: string, displayName: string) =>
    request(`/lms/courses/${courseId}/rename`, {
      method: 'PATCH',
  ...
  lmsDownloadUrl: (itemId: string) => `${getApiBase()}/lms/download/${itemId}`,
  ...
  cloneBibliotecaDoc: (docId: string, projectId: string) =>
    request(`/biblioteca/${docId}/clone`, {
      method: 'POST',
  ...
  collabUpdate: (id: string, data: any) =>
    request(`/collab/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  collabDelete: (id: string) => request(`/collab/${id}`, { method: 'DELETE' }),
  collabAddMember: (docId: string, data: any) =>
    request(`/collab/${docId}/members`, { method: 'POST', body: JSON.stringify(data) }),
  collabRemoveMember: (docId: string, userId: string) =>
    request(`/collab/${docId}/members/${userId}`, { method: 'DELETE' }),
  collabUpdateRole: (docId: string, userId: string, data: any) =>
    request(`/collab/${docId}/members/${userId}/role`, {
      method: 'PUT',
      body: JSON.stringify(data),
    }),
  ...
  collabGetVersion: (docId: string, versionId: string) =>
    request(`/collab/${docId}/versions/${versionId}`),
  ...
  collabSendChat: (docId: string, content: string) =>
    request(`/collab/${docId}/chat`, { method: 'POST', body: JSON.stringify({ content }) }),
  collabExportPdf: (docId: string) => {
    const token = localStorage.getItem('conniku_token');
    return fetch(`${getApiBase()}/collab/${docId}/export/pdf`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  },
  collabExportDocx: (docId: string) => {
    const token = localStorage.getItem('conniku_token');
    return fetch(`${getApiBase()}/collab/${docId}/export/docx`, {
      headers: { Authorization: `Bearer ${token}` },
    });
  },
};
