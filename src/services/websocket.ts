...
class WebSocketService {
  ...
  connect(token: string): void {
    ...
    const apiBase = window.location.hostname === 'localhost'
      ? 'ws://localhost:8000'
      : `wss://${window.location.hostname === 'conniku.com' ? 'studyhub-api-bpco.onrender.com' : window.location.hostname}`
  ...
  private _dispatch(type: string, data: any): void {
    ...
    if (handlers) {
      handlers.forEach(h => {
        try { h(data) } catch (e) { console.error(`[WS] Handler error for ${type}:`, e) }
      })
    }
    ...
    if (wildcardHandlers) {
      wildcardHandlers.forEach(h => {
        try { h(data) } catch (e) { console.error('[WS] Wildcard handler error:', e) }
      })
    }
  }
  ...
  private _notifyConnection(connected: boolean): void {
    this.connectionHandlers.forEach(h => {
      try { h(connected) } catch (e) { console.error('[WS] Connection handler error:', e) }
    })
  }
