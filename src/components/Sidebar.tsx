      ...
      <div style={{ padding: '10px 10px 4px' }}>
        <button
          ...
          style={{
            ...
            alignItems: 'center',
            gap: 9,
            boxShadow: isActive('/mi-universidad')
        ...
        >
          ...
          <div style={{ textAlign: 'left', flex: 1 }}>
            ...
            {projects.length > 0 && (
              ...
              >
                {projects.length} asignatura{projects.length !== 1 ? 's' : ''} activa
                {projects.length !== 1 ? 's' : ''}
              </div>
            )}
          </div>
        ...
        <div style={{ paddingTop: 4 }}>
          ...
          <div
            style={{
              height: 2,
              background:
                'linear-gradient(90deg, transparent 0%, #1a56db 40%, #3b82f6 60%, transparent 100%)',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ textAlign: 'left' }}>
                  ...
                  >
                    {user?.role === 'owner'
                      ? 'CEO'
                      : user?.username || (user as any)?.first_name || 'Mi Panel'}
                  </div>
                  <div
                    style={{
                      fontSize: 10,
            ...
            {adminPanelOpen && (
              <div style={{ marginTop: 10 }}>
                {Object.entries(CATEGORY_LABELS).map(([catKey, cat]) => {
                  const mods = ADMIN_MODULES.filter(
                    (m) =>
                      m.category === catKey &&
                          ...
                          {mods.map((mod) => (
                            <button
                              ...
                              style={{
                                ...
                                width: '100%',
                                textAlign: 'left',
                                background: currentPath.startsWith(mod.route)
                                  ? 'rgba(26,86,219,0.1)'
                                  : 'transparent',
                                border: 'none',
                                borderLeft: currentPath.startsWith(mod.route)
                                  ? '2px solid var(--accent, #1a56db)'
                                ...
                                color: currentPath.startsWith(mod.route)
                                  ? 'var(--accent, #1a56db)'
                                  : 'var(--text-secondary)',
                                gap: 6,
                              }}
