...
export default function RightPanel({ currentPath, onNavigate }: Props) {
  ...
  if (['/messages', '/profile', '/admin', '/feed'].some((p) => currentPath.startsWith(p)))
    return null;
    ...
    <aside className="right-panel">
          ...
          {stats.badges?.filter((b: any) => b.earned).length > 0 && (
            <div style={{ display: 'flex', gap: 4, marginTop: 8, flexWrap: 'wrap' }}>
              {stats.badges
                ...
                .slice(0, 6)
                .map((b: any) => (
                  <span key={b.id} title={b.name} style={{ fontSize: 16 }}>
                    {b.emoji}
                  </span>
                ))}
            </div>
      ...
      {suggestions.length > 0 && currentPath.startsWith('/communities') && (
        <div className="rp-widget">
          ...
          <button className="rp-see-more" onClick={() => onNavigate('/friends')}>
          ...
          {communities.map((c) => (
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div className="rp-label" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                  {Users({ size: 14 })} {c.memberCount} {t('rightpanel.members')}
                </div>
              </div>
            </div>
          ))}
          <button className="rp-see-more" onClick={() => onNavigate('/communities')}>
            {t('rightpanel.explore')}
          </button>
        </div>
      )}
        ...
        <div className="rp-widget">
          <div
            style={{
              ...
              marginBottom: showcaseItems.length > 0 ? 12 : 8,
            }}
          >
            <h4 className="rp-widget-title" style={{ margin: 0 }}>
              🏅 Showcase
            </h4>
