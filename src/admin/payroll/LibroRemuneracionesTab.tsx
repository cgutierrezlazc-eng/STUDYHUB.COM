...
import { BookOpen, Download, AlertTriangle, CheckCircle, Calendar, Users, FileText, RefreshCw } from 'lucide-react'
...
const LRE_COLUMNS = [
  'RUT', 'Nombre', 'Cargo', 'Fecha Ingreso', 'Días Trabajados',
  'Sueldo Base', 'Gratificación', 'Horas Extra', 'Monto HE',
  'Bonos/Comisiones', 'Total Haberes Imponibles',
  'Colación', 'Movilización', 'Total Haberes No Imponibles',
  'AFP', '% AFP', 'Monto AFP', 'Salud', 'Monto Salud',
  'AFC Trabajador', 'Impuesto Único', 'Total Descuentos Legales',
  'Otros Descuentos', 'Total Descuentos',
  'Líquido a Pagar',
  'AFC Empleador', 'SIS', 'Mutual', 'Total Costo Empleador',
]
...
export default function LibroRemuneracionesTab() {
  ...
  const lreRows: LRERow[] = useMemo(() => {
    ...
    return activeEmps.map(emp => {
      ...
      // Employer costs
      const afcEmployerRate = emp.contractType === 'indefinido' ? CHILE_LABOR.AFC.employerIndefinido : CHILE_LABOR.AFC.employerPlazoFijo
      const afcEmployer = Math.round(Math.min(totalImponible, CHILE_LABOR.TOPES.afcCLP) * afcEmployerRate)
      ...
      const totalEmployerCost = totalImponible + emp.colacion + emp.movilizacion + afcEmployer + sisAmount + mutualAmount
  ...
  const totals = useMemo(() => {
    const t = {
      baseSalary: 0, gratificacion: 0, overtimeAmount: 0, bonuses: 0,
      totalImponible: 0, colacion: 0, movilizacion: 0, totalNoImponible: 0,
      afpAmount: 0, healthAmount: 0, afcEmployee: 0, taxAmount: 0,
      totalLegalDeductions: 0, otherDeductions: 0, totalDeductions: 0,
      netSalary: 0, afcEmployer: 0, sisAmount: 0, mutualAmount: 0, totalEmployerCost: 0,
    }
    lreRows.forEach(r => {
  ...
  const generateCSV = () => {
    ...
    const rows = lreRows.map(r => [
      r.rut, r.name, r.position, r.hireDate, r.daysWorked,
      r.baseSalary, r.gratificacion, r.overtimeHours, r.overtimeAmount,
      r.bonuses, r.totalImponible,
      r.colacion, r.movilizacion, r.totalNoImponible,
      r.afpName, (r.afpRate * 100).toFixed(2), r.afpAmount,
      r.healthSystem, r.healthAmount,
      r.afcEmployee, r.taxAmount, r.totalLegalDeductions,
      r.otherDeductions, r.totalDeductions,
      r.netSalary,
      r.afcEmployer, r.sisAmount, r.mutualAmount, r.totalEmployerCost,
    ].join(separator))

    const totalsRow = [
      'TOTALES', '', '', '', '',
      totals.baseSalary, totals.gratificacion, '', totals.overtimeAmount,
      totals.bonuses, totals.totalImponible,
      totals.colacion, totals.movilizacion, totals.totalNoImponible,
      '', '', totals.afpAmount,
      '', totals.healthAmount,
      totals.afcEmployee, totals.taxAmount, totals.totalLegalDeductions,
      totals.otherDeductions, totals.totalDeductions,
      totals.netSalary,
      totals.afcEmployer, totals.sisAmount, totals.mutualAmount, totals.totalEmployerCost,
    ].join(separator)
        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12, marginBottom: 20 }}>
            {[
              ...
              { label: 'Total Imponibles', value: `$${fmt(totals.totalImponible)}`, color: '#10b981' },
              { label: 'Total Descuentos', value: `$${fmt(totals.totalDeductions)}`, color: '#ef4444' },
              { label: 'Total Líquido', value: `$${fmt(totals.netSalary)}`, color: '#3b82f6' },
              { label: 'Costo Empleador', value: `$${fmt(totals.totalEmployerCost)}`, color: '#8b5cf6' },
            ].map((card, i) => (
              <div key={i} style={{
          ...
          <div style={{
            ...
            <span>
              <strong>Obligación legal:</strong> Las empresas con 5 o más trabajadores deben llevar LRE y tenerlo disponible para fiscalización de la Dirección del Trabajo.
              El archivo CSV generado es compatible con el formato requerido por la DT.
            </span>
          </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 2000 }}>
              <thead>
                <tr style={{ background: 'var(--bg-secondary)' }}>
                  <th style={{ ...thStyle, position: 'sticky', left: 0, background: 'var(--bg-secondary)', zIndex: 2, minWidth: 100 }}>RUT</th>
                  <th style={{ ...thStyle, position: 'sticky', left: 100, background: 'var(--bg-secondary)', zIndex: 2, minWidth: 160 }}>Nombre</th>
                  ...
                  <th style={{ ...thStyle, background: '#d1fae544', fontWeight: 800 }}>Total Imp.</th>
                  ...
                  <th style={{ ...thStyle, background: '#fee2e233', fontWeight: 800 }}>Total Desc.</th>
                  ...
                  <th style={{ ...thStyle, background: '#ede9fe44', fontWeight: 800 }}>Costo Total</th>
                </tr>
              </thead>
              <tbody>
                {lreRows.map((r, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ ...tdStyle, position: 'sticky', left: 0, background: 'var(--bg-primary)', zIndex: 1, fontFamily: 'monospace', fontSize: 11 }}>{r.rut}</td>
                    <td style={{ ...tdStyle, position: 'sticky', left: 100, background: 'var(--bg-primary)', zIndex: 1, fontWeight: 600 }}>{r.name}</td>
                    <td style={{ ...tdStyle, fontSize: 11, color: 'var(--text-muted)' }}>{r.position}</td>
                    ...
                    <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: '#10b981' }}>{fmt(r.totalImponible)}</td>
                    <td style={{ ...tdStyle, textAlign: 'right' }}>{fmt(r.colacion)}</td>
                    <td style={{ ...tdStyle, textAlign: 'right' }}>{fmt(r.movilizacion)}</td>
                    <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 600 }}>{fmt(r.totalNoImponible)}</td>
                    <td style={{ ...tdStyle, fontSize: 11 }}>{r.afpName}</td>
                    <td style={{ ...tdStyle, textAlign: 'right', fontSize: 11 }}>{(r.afpRate * 100).toFixed(2)}%</td>
                    ...
                    <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: '#ef4444' }}>{fmt(r.totalDeductions)}</td>
                    <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 800, color: '#3b82f6', fontSize: 13 }}>{fmt(r.netSalary)}</td>
                    ...
                    <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: '#8b5cf6' }}>{fmt(r.totalEmployerCost)}</td>
                  </tr>
                ))}
                {/* Totals row */}
                <tr style={{ background: 'var(--bg-secondary)', borderTop: '3px solid var(--border)' }}>
                  <td colSpan={4} style={{ ...tdStyle, fontWeight: 800, fontSize: 13, position: 'sticky', left: 0, background: 'var(--bg-secondary)', zIndex: 1 }}>
                    TOTALES ({lreRows.length} trabajadores)
                  </td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.baseSalary)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.gratificacion)}</td>
                  <td style={tdStyle}></td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.overtimeAmount)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.bonuses)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 800, color: '#10b981', fontSize: 13 }}>{fmt(totals.totalImponible)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.colacion)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.movilizacion)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.totalNoImponible)}</td>
                  <td colSpan={2} style={tdStyle}></td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.afpAmount)}</td>
                  <td style={tdStyle}></td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.healthAmount)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.afcEmployee)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.taxAmount)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 800, color: '#ef4444', fontSize: 13 }}>{fmt(totals.totalDeductions)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 800, color: '#3b82f6', fontSize: 14 }}>{fmt(totals.netSalary)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.afcEmployer)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.sisAmount)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>{fmt(totals.mutualAmount)}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 800, color: '#8b5cf6', fontSize: 13 }}>{fmt(totals.totalEmployerCost)}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {activeView === 'historial' && (
        <div>
          <div style={{
            ...
            <h3 style={{ fontSize: 18, fontWeight: 700, margin: '0 0 8px' }}>Historial de Períodos</h3>
            ...
            <div style={{ marginTop: 20, display: 'flex', justifyContent: 'center', gap: 12 }}>
              {['2026-03', '2026-02', '2026-01', '2025-12'].map(period => (
                <div key={period} style={{
                  padding: '12px 20px', borderRadius: 10, background: 'var(--bg-primary)',
      ...
      {activeView === 'config' && (
        <div style={{ maxWidth: 600 }}>
          ...
          <div style={{
            ...
            <h3 style={{ fontSize: 16, fontWeight: 700, margin: '0 0 16px' }}>Formato de Exportación</h3>
            <div style={{ display: 'grid', gap: 12 }}>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
                <input type="radio" name="format" value="csv" defaultChecked /> CSV (Dirección del Trabajo)
              </label>
              <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, cursor: 'pointer' }}>
            ...
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 12 }}>
              El formato CSV con separador punto y coma (;) es el requerido por la Dirección del Trabajo para la carga del LRE.
            </p>
          </div>
        </div>
      )}
