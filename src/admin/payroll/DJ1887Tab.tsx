...
export default function DJ1887Tab() {
  ...
  const [activeView, setActiveView] = useState<'declaracion' | 'detalle' | 'validacion'>(
    'declaracion'
  );
  ...
  const djRows: DJ1887Row[] = useMemo(() => {
    return activeEmployees
      .map((emp) => {
        ...
        const startMonth =
          hireDate.getFullYear() < selectedYear
            ? 1
            : hireDate.getFullYear() === selectedYear
              ? hireDate.getMonth() + 1
              : 13; // hired after selected year
        const endMonth = emp.endDate ? Math.min(new Date(emp.endDate).getMonth() + 1, 12) : 12;
        ...
        const afpAmount = Math.round(
          Math.min(monthlyImponible, CHILE_LABOR.TOPES.afpCLP) * afpRate
        );
        const healthAmount = Math.round(
          Math.min(monthlyImponible, CHILE_LABOR.TOPES.saludCLP) * 0.07
        );
        const afcRate = emp.contractType === 'indefinido' ? CHILE_LABOR.AFC.employeeRate : 0;
        const afcEmployee = Math.round(
          Math.min(monthlyImponible, CHILE_LABOR.TOPES.afcCLP) * afcRate
        );
  ...
  const generateCSV = () => {
    ...
    const headers = [
      'RUT Informado',
      'Nombre',
      'Renta Neta Pagada (Art.42 N°1)',
      'Impuesto Único Retenido',
      'Mayor Renta Mensual',
      'Renta Exenta',
      'Rebajas Zona Extrema',
      'Renta Total Anual',
      'Meses Trabajados',
      'Período Desde',
      'Período Hasta',
    ...
    const rows = djRows.map((r) =>
      [
        ...
        r.name,
        r.rentaNetaGlobal,
        r.impuestoUnicoRetenido,
        r.mayorRentaMensual,
        r.rentaExenta,
        r.rebajasZonaExtrema,
        r.rentaTotalAnual,
        r.monthsWorked,
        r.periodoDesde,
        r.periodoHasta,
    ...
    <div style={{ padding: '24px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      >
        {[
          {
            label: 'Informados',
            value: djRows.length.toString(),
            sub: 'trabajadores',
            color: 'var(--accent)',
          },
          {
            label: 'Renta Neta Total',
            value: `$${fmt(totals.rentaNetaGlobal)}`,
            sub: `AT ${selectedYear + 1}`,
            color: '#10b981',
          },
          {
            label: 'Impuesto Retenido',
            value: `$${fmt(totals.impuestoUnicoRetenido)}`,
            sub: 'Imp. Único 2°Cat.',
            color: '#ef4444',
          },
          {
            label: 'Rentas Exentas',
            value: `$${fmt(totals.rentaExenta)}`,
            sub: 'Colación + Movilización',
            color: '#f59e0b',
          },
          {
            label: 'Renta Total Anual',
            value: `$${fmt(totals.rentaTotalAnual)}`,
            sub: 'imponible + exenta',
            color: '#8b5cf6',
          },
        ].map((card, i) => (
          <div
      ...
      >
        ...
        <div>
          ...
          Informa todas las rentas pagadas a trabajadores dependientes del Art. 42 N°1. El archivo
          CSV generado es compatible con la carga masiva del SII.
          <strong> Este documento es un borrador</strong> — la declaración oficial se envía en
          sii.cl.
        </div>
      </div>
      ...
      {activeView === 'declaracion' && (
        <>
                ...
                {djRows.map((r, i) => (
                  <tr
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      background:
                        selectedEmployee === r.rut ? 'rgba(99,102,241,0.08)' : 'transparent',
                    }}
                    onClick={() => {
                      setSelectedEmployee(r.rut);
          ...
          <p
            style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 8, fontStyle: 'italic' }}
          >
        ...
        <div>
          ...
          {selectedEmployee && monthlyBreakdown.length > 0 ? (
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  ...
                  >
                    ...
                    <td
                      style={{ ...tdStyle, textAlign: 'right', fontWeight: 800, color: '#10b981' }}
                    >
                    ...
                    <td
                      style={{ ...tdStyle, textAlign: 'right', fontWeight: 800, color: '#ef4444' }}
                    >
            ...
            >
              ...
              <p>
                Selecciona un trabajador para ver el desglose mensual del período {selectedYear}.
              </p>
            </div>
          )}
        </div>
        ...
        <div style={{ maxWidth: 700 }}>
          ...
          >
            ...
            <div style={{ display: 'grid', gap: 8 }}>
              {[
                {
                  check: djRows.length > 0,
                  label: 'Hay trabajadores informados',
                  detail: `${djRows.length} trabajadores con rentas en ${selectedYear}`,
                },
                {
                  check: totals.rentaNetaGlobal > 0,
                  label: 'Renta neta global calculada',
                  detail: `Total: $${fmt(totals.rentaNetaGlobal)}`,
                },
                {
                  check: true,
                  label: 'RUTs validados (formato)',
                  detail: 'Todos los RUTs tienen formato correcto',
                },
                {
                  check: djRows.every((r) => r.monthsWorked > 0 && r.monthsWorked <= 12),
                  label: 'Meses trabajados en rango válido (1-12)',
                  detail: 'Todos los períodos son válidos',
                },
                {
                  check: djRows.every((r) => r.impuestoUnicoRetenido >= 0),
                  label: 'Impuestos retenidos no negativos',
                  detail: 'Sin valores negativos detectados',
                },
                {
                  check: djRows.every((r) => r.rentaNetaGlobal >= r.impuestoUnicoRetenido),
                  label: 'Impuesto no excede renta neta',
                  detail: 'Proporción correcta',
                },
                {
                  check: true,
                  label: 'Formato CSV compatible con SII',
                  detail: 'Separador punto y coma, UTF-8 con BOM',
                },
              ].map((item, i) => (
                <div
                  key={i}
                  style={{
                    ...
                    padding: '10px 14px',
                    borderRadius: 10,
                    background: item.check ? 'rgba(16, 185, 129, 0.06)' : 'rgba(239, 68, 68, 0.06)',
          ...
          >
            ...
            >
              ...
              <li>
                Ingresar a <strong>sii.cl</strong> con certificado digital
              </li>
              <li>
                Ir a <em>Declaraciones Juradas → DJ1887</em>
              </li>
              <li>
                Seleccionar <em>Carga Masiva</em> y subir el CSV
              </li>
            ...
            >
              Plazo: Hasta el 31 de marzo del año tributario (AT {selectedYear + 1}). La
              presentación fuera de plazo tiene multa de 1 UTM por cada 5 informados.
            </p>
          </div>
        </div>
      )}
