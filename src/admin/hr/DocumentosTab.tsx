...
const DOC_DEFINITIONS: DocDefinition[] = [
  ...
  },
  {
    id: 'cert-salud',
    category: 'previsional',
    label: 'Certificado salud (Fonasa/Isapre)',
    required: true,
    expires: true,
  },
  {
  ...
  },
  {
    id: 'finiquitos-ant',
    category: 'legal',
    label: 'Finiquitos anteriores',
    required: false,
    expires: false,
  },
  {
    id: 'reglamento-firmado',
    category: 'legal',
  ...
  // Formación
  {
    id: 'titulo-prof',
    category: 'formacion',
    label: 'Títulos/certificados profesionales',
    required: false,
    expires: false,
  },
  {
  ...
  // Salud
  {
    ...
    category: 'salud',
    label: 'Examen pre-ocupacional',
    required: true,
    expires: true,
  },
  {
    id: 'licencias-medicas',
    category: 'salud',
      ...
      <div style={sidebar}>
        ...
        {SEED_EMPLOYEES.map((emp) => {
          ...
          return (
            <div
              key={emp.id}
              style={empItem(emp.id === selectedEmpId)}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
        {selectedEmp ? (
          <>
            ...
            <div
              style={{ ...card, display: 'flex', gap: 20, alignItems: 'center', flexWrap: 'wrap' }}
            >
              <div style={{ flex: 1, minWidth: 200 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                    {requiredUploaded.length} de {requiredDefs.length} documentos requeridos
                    cargados
                  </span>
                </div>
                {/* Progress bar */}
                ...
                <div style={{ minWidth: 200 }}>
                  ...
                  {alertDocs.slice(0, 3).map((def) => {
                    ...
                    return (
                      <div
                        key={def.id}
                        style={{ fontSize: 11, color: STATUS_COLORS[st], marginBottom: 2 }}
                      >
                            ...
                            <tr key={def.id} style={{ borderBottom: '1px solid var(--border)' }}>
                              <td style={{ ...tdStyle, paddingLeft: 14 }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                  {isMissingRequired && (
                                    <span
                                      style={{ color: '#ef4444', fontSize: 14, fontWeight: 800 }}
                                      title="Documento requerido faltante"
                                    >
                                      ●
                                    </span>
                                  )}
                              ...
                              <td style={{ ...tdStyle, textAlign: 'center', fontSize: 11 }}>
                                {uploaded && def.expires
                                  ? formatDate(uploaded.expirationDate)
                                  : '—'}
                              </td>
                              <td style={{ ...tdStyle, textAlign: 'center' }}>
                                <div
                                ...
                                >
                                  ...
                                  >
                                    ...
                                    <input
                                      ...
                                      onChange={(e) =>
                                        handleFileInput(def.id, e.target.files?.[0] || null)
                                      }
                                    />
                                  </label>
                                  {/* Download (simulated) */}
                  ...
                  {ALL_CATEGORIES.map((cat) => {
                    ...
                    return (
                      <tr key={cat} style={{ borderBottom: '1px solid var(--border)' }}>
                        ...
                        <td style={{ ...tdStyle, textAlign: 'center', color: '#22c55e' }}>
                          {counts.vigente}
                        </td>
                        <td style={{ ...tdStyle, textAlign: 'center', color: '#f59e0b' }}>
                          {counts.por_vencer}
                        </td>
                        <td style={{ ...tdStyle, textAlign: 'center', color: '#ef4444' }}>
                          {counts.vencido}
                        </td>
        ...
        <div style={modalOverlay} onClick={() => setUploadModal(null)}>
          <div style={modalBox} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
              <button style={btnSecondary} onClick={() => setUploadModal(null)}>
                Cancelar
              </button>
              <button style={btnPrimary} onClick={confirmUploadModal}>
                Confirmar
              </button>
            </div>
          </div>
