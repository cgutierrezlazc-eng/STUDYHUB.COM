...
const RECORD_CARD_CONFIG: {
  key: RecordCardCategory;
  label: string;
  icon: any;
  color: string;
  severe?: boolean;
...
const PERFORMANCE_DIMENSIONS = [
  ...
  },
  {
    ...
    desc: 'Colaboracion, apoyo mutuo y contribucion al ambiente laboral',
  },
  {
    key: 'initiative',
    name: 'Iniciativa y Proactividad',
    desc: 'Capacidad de anticipar problemas y proponer soluciones sin esperar instrucciones',
  },
  {
    key: 'attendance',
  ...
  {
    key: 'values',
    name: 'Adherencia a Valores Conniku',
...
const CONVERSATION_LIBRARY = [
  {
    title: 'Feedback de Desempeno',
    desc: 'Conversacion sobre rendimiento reciente del trabajador',
  },
  ...
  {
    title: 'Retroalimentacion de Proyecto',
    desc: 'Revision de resultados de un proyecto especifico',
  },
  {
    title: 'Expectativas Trimestrales',
    desc: 'Alineamiento de expectativas para el proximo trimestre',
  },
...
function loadSignatures(): FESSignature[] {
  return lsGet(KEYS.fes);
}
function saveSignature(sig: FESSignature) {
  const all = loadSignatures();
  const idx = all.findIndex(
    (s) => s.documentType === sig.documentType && s.employeeId === sig.employeeId
  );
...
function getSignature(employeeId: string, docType: string): FESSignature | null {
  return (
    loadSignatures().find((s) => s.employeeId === employeeId && s.documentType === docType) || null
  );
}
...
function getRecordCardStatus(
  ...
  const disciplines = lsGetForEmployee<DisciplineRecord>(KEYS.discipline, empId).filter(
  ...
  if (category === 'incumplimiento') {
    if (disciplines.some((d) => d.level === 'termination'))
      return { status: 'red', label: 'Terminacion' };
    if (disciplines.some((d) => d.level === 'written_2'))
      return { status: 'orange', label: 'Escrita 2 activa' };
    if (disciplines.some((d) => d.level === 'written_1'))
      return { status: 'yellow', label: 'Escrita 1 activa' };
  ...
  if (category === 'documentos') {
    ...
    const existing = requiredDocs.filter(
      (dt) => documents.some((d: any) => d.documentType === dt) || getSignature(empId, dt)
    );
    const pct = (existing.length / requiredDocs.length) * 100;
    if (pct >= 100) return { status: 'green', label: 'Carpeta completa' };
    if (pct >= 60)
      return { status: 'yellow', label: `${existing.length}/${requiredDocs.length} documentos` };
...
function getPositionPriority(position: string): number {
  ...
  if (
    lower.includes('ceo') ||
    lower.includes('gerente general') ||
    lower.includes('director general')
  )
    return 0;
...
function ERCDashboard({
  ...
  for (const emp of employees) {
    ...
    if (emp.contractType === 'plazo_fijo' && emp.endDate) {
      ...
      if (days <= 15)
        scheduledItems.push({
          emp,
          label: `Contrato vence en ${days} dias`,
          type: 'contrato',
    ...
    const hasFES = ['contrato', 'job_description', 'expectation_memo'].every((dt) =>
      getSignature(emp.id, dt)
    );
    if (!hasFES)
      scheduledItems.push({
        emp,
        label: 'Documentos ERC pendientes de firma',
        type: 'docs',
        urgency: 'medium',
      });
    ...
    if (daysSinceHire >= 45 && !reviews.some((r) => r.type === 'mid_contract')) {
      scheduledItems.push({
        emp,
        label: 'Evaluacion Mid-Contract pendiente',
        type: 'review',
        urgency: daysSinceHire >= 60 ? 'high' : 'low',
      });
    }
    if (daysSinceHire >= 80 && !reviews.some((r) => r.type === 'end_contract')) {
      scheduledItems.push({
        emp,
        label: 'Evaluacion End-of-Contract pendiente',
        type: 'review',
        urgency: 'medium',
      });
    }
  ...
  const recentActivity = [
    ...allDisciplines.map((d) => ({
      date: d.date,
      label: `Disciplina: ${d.explanation.slice(0, 60)}`,
      empId: d.employeeId,
    })),
    ...allCoaching.map((c) => ({
      date: c.date,
      label: `Coaching: ${c.topic}`,
      empId: c.employeeId,
    })),
    ...allConversations.map((c) => ({
      date: c.date,
      label: `Conversacion: ${c.title}`,
      empId: c.employeeId,
    })),
    ...allAcknowledgements.map((a) => ({
      date: a.date,
      label: `Reconocimiento: ${a.title}`,
      empId: a.employeeId,
    })),
    ...allReviews.map((r) => ({
      date: r.date,
      label: `Evaluacion: ${r.type === 'mid_contract' ? 'Mid-Contract' : 'End-of-Contract'} (${r.overallRating.toFixed(1)})`,
      empId: r.employeeId,
    })),
  ]
    ...
    <div style={{ padding: 24, maxWidth: 1100, margin: '0 auto' }}>
      ...
      >
        ...
        <StatCard
          icon={Clock}
          label="En Prueba"
          value={employees.filter((e) => e.contractType === 'plazo_fijo').length}
          color="#f59e0b"
        />
        <StatCard
          icon={AlertTriangle}
          label="Alertas"
          value={scheduledItems.filter((s) => s.urgency === 'high').length}
          color="#ef4444"
        />
      </div>
                  ...
                  >
                    <div
                      style={{
                        ...
                        height: 36,
                        borderRadius: 3,
                        background: urgColors[item.urgency],
                        flexShrink: 0,
                      }}
                    />
                    <div style={{ flex: 1 }}>
                    ...
                    <span
                      style={{
                        ...
                        fontSize: 10,
                        fontWeight: 700,
                        background: urgColors[item.urgency] + '20',
                        color: urgColors[item.urgency],
                      }}
                    >
                      {item.urgency === 'high'
                        ? 'URGENTE'
            ...
            <div style={{ display: 'grid', gap: 6 }}>
              {recentActivity.map((a, i) => {
                ...
                return (
                  ...
                  >
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', minWidth: 70 }}>
...
function EmployeeFile({
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 20 }}>
        ...
        <span
          style={{
            ...
            fontWeight: 600,
            background:
              employee.status === 'active' ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.15)',
      ...
      {/* Tab bar — scrollable */}
      <div
        style={{ display: 'flex', gap: 3, marginBottom: 20, overflowX: 'auto', paddingBottom: 4 }}
      >
        {TABS.map((t) => (
          <button
            key={t.id}
      ...
      {/* Tab content */}
      {tab === 'record_cards' && (
        <RecordCardsView employee={employee} documents={documents} onTabChange={setTab} />
      )}
  ...
  return (
    <div>
      <h3 style={{ fontSize: 16, fontWeight: 700, margin: '0 0 16px' }}>
        Record Cards — {employee.firstName} {employee.lastName}
      </h3>
      <div
        style={{
          display: 'grid',
          ...
          return (
            <div
              ...
              onClick={() => {
                if (
                  card.key === 'disciplina' ||
                  card.key === 'probidad' ||
                  card.key === 'incumplimiento' ||
                  card.key === 'confidencialidad'
                )
                  onTabChange('discipline');
            ...
            >
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                <div
                  style={{ width: 10, height: 10, borderRadius: '50%', background: colors.dot }}
                />
...
function JobDescriptionTab({ employee }: { employee: Employee }) {
  ...
  if (!jd)
    return <EmptyState icon={Briefcase} message="No hay Job Description vinculada a este cargo." />;

  const handleSign = async () => {
    setSigning(true);
    try {
      const hash = await generateSHA256(
        JSON.stringify({ position: jd.positionKey, employee: employee.id, jd })
      );
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <button
          onClick={() => generateJobDescriptionPDF(employee as any)}
        ...
        {!sig && (
          ...
          >
            <FileSignature size={14} /> {signing ? 'Firmando...' : 'Firmar (FES)'}
          </button>
        )}
...
function ExpectationMemoTab({ employee }: { employee: Employee }) {
  ...
  if (!memo)
    return <EmptyState icon={Target} message="No hay Expectation Memo vinculado a este cargo." />;

  const handleSign = async () => {
    setSigning(true);
    try {
      const hash = await generateSHA256(
        JSON.stringify({ position: memo.positionKey, employee: employee.id, memo })
      );
      const ip = await getPublicIP();
      const signature: FESSignature = {
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <button
          onClick={() => generateExpectationMemoPDF(employee as any)}
          style={{ ...btnPrimary, background: '#2563eb' }}
        >
          <Printer size={14} /> PDF
        </button>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        {[
          ...
          return (
            ...
            >
              {completed && <CheckCircle size={14} style={{ marginBottom: 2 }} />} {p.label}{' '}
              {completed ? '— Completado' : active ? '— En curso' : ''}
            </div>
          );
        })}
      </div>
      ...
      <div style={{ background: 'var(--bg-tertiary)', borderRadius: 12, padding: 20 }}>
        <p style={{ fontSize: 12, color: 'var(--text-muted)', fontStyle: 'italic', marginTop: 0 }}>
          {memo.intro}
        </p>
          ...
          <div key={idx} style={{ marginBottom: 16 }}>
            <h4
              style={{
                fontSize: 13,
                fontWeight: 700,
                color: ['#22c55e', '#3b82f6', '#8b5cf6'][idx],
                margin: '0 0 8px',
                display: 'flex',
                alignItems: 'center',
                gap: 6,
            ...
            >
              <div
                style={{
                  ...
                  height: 8,
                  borderRadius: '50%',
                  background: ['#22c55e', '#3b82f6', '#8b5cf6'][idx],
                }}
              />
              {phase.title}
            </h4>
...
function ContratoTab({ employee, afpRate }: { employee: Employee; afpRate: number }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <button
          onClick={() => generateContractPDF(employee as any, afpRate)}
          style={{ ...btnPrimary, background: '#2563eb' }}
        >
      ...
      <div style={{ background: 'var(--bg-tertiary)', borderRadius: 12, padding: 20 }}>
        ...
        <ContractClause
          title="PARTES"
          content={`Empleador: CONNIKU SpA, RUT 78.395.702-7, representada por Cristian Gaete Lazcano.\nTrabajador: ${employee.firstName} ${employee.lastName}, RUT ${employee.rut}.`}
        />
        <ContractClause
          title="CARGO"
          content={`${employee.position} — Departamento de ${employee.department}.`}
        />
        <ContractClause
          title="JORNADA"
          content={`${employee.weeklyHours} horas semanales, lunes a viernes, 09:00 a 18:00 con 1 hora de colacion.`}
        />
        ...
        <ContractClause
          title="DURACION"
          content={`Contrato ${employee.contractType === 'indefinido' ? 'indefinido' : 'a plazo fijo'} desde el ${employee.hireDate}.`}
        />
        <ContractClause
          title="FERIADO"
          content="15 dias habiles anuales (Art. 67 Codigo del Trabajo)."
        />
        <ContractClause
          title="CONFIDENCIALIDAD"
          content="La violacion de la clausula de confidencialidad constituye CAUSAL DE TERMINACION INMEDIATA conforme al Art. 160 N°7 del Codigo del Trabajo, sin derecho a indemnizacion. Subsiste por 2 anos post-termino."
        />
        <ContractClause
          title="DISCIPLINA PROGRESIVA"
          content="El sistema contempla: (i) Amonestacion Verbal, (ii) Escrita 1, (iii) Escrita 2, (iv) Terminacion. Cada etapa documentada en el ERC y firmada electronicamente."
        />
        <ContractClause
          title="EVALUACION DE DESEMPENO"
          content="Dos evaluaciones con promedio inferior a 3.0/5.0 facultan a la empresa a prescindir de la posicion conforme al Art. 161 del Codigo del Trabajo."
        />
        <ContractClause
          title="FALTA DE PROBIDAD"
          content="Art. 160 N°1: Terminacion inmediata ante conducta deshonesta, fraudulenta o contraria a la buena fe."
        />
        <ContractClause
          title="ANEXOS"
          content="1) Job Description  2) Expectation Memo 30/60/90  3) Reglamento Interno  4) ODI  5) Politica de Privacidad"
        />
      </div>
    </div>
  );
...
function DisciplineTab({ employee }: { employee: Employee }) {
  ...
  const [form, setForm] = useState({
    ...
    category: 'disciplina' as RecordCardCategory,
    explanation: '',
    incidentDetails: '',
  });
  const { user } = useAuth();
  ...
  useEffect(() => {
    setRecords(lsGetForEmployee<DisciplineRecord>(KEYS.discipline, employee.id));
  }, [employee.id]);

  const handleSave = () => {
    if (!form.explanation || !form.incidentDetails) {
  ...
  return (
    <div>
            ...
            >
              <div
                style={{
                  fontWeight: 700,
                  fontSize: 18,
                  color: isActive ? dl.color : 'var(--text-muted)',
                }}
              >
                {dl.short}
              </div>
      ...
      {showForm && (
        <div
          className="card"
          style={{ padding: 20, marginBottom: 16, border: '2px solid var(--accent)' }}
        >
          ...
          >
            <div>
              ...
              <select
                value={form.level}
                onChange={(e) => setForm({ ...form, level: e.target.value as DisciplineLevel })}
                style={selectStyle}
              >
                {DISCIPLINE_LEVELS.map((dl) => (
                  <option key={dl.key} value={dl.key}>
                    {dl.label}
                  </option>
                ))}
              </select>
            ...
            <div>
              ...
              <select
                value={form.category}
                onChange={(e) =>
                  setForm({ ...form, category: e.target.value as RecordCardCategory })
                }
                style={selectStyle}
              >
                {RECORD_CARD_CONFIG.map((c) => (
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <textarea
              ...
              placeholder="Describa la infraccion..."
            />
          </div>
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea
              ...
              rows={3}
              style={{ ...inputStyle, resize: 'vertical' } as any}
              placeholder="Fecha, hora, circunstancias..."
            />
          </div>
            ...
            .map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className="card"
                  style={{ padding: 14, borderLeft: `4px solid ${dl.color}` }}
                >
                  ...
                  <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 6 }}>
                    ...
                    {r.acknowledged ? 'Acusado recibo' : 'Pendiente de acuse'}
                  </div>
                </div>
              );
            })}
...
function CoachingTab({ employee }: { employee: Employee }) {
  ...
  useEffect(() => {
    setRecords(lsGetForEmployee<CoachingRecord>(KEYS.coaching, employee.id));
  }, [employee.id]);
      ...
      >
        <div>
          ...
          <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--text-muted)' }}>
            El coaching NO es disciplina y NO requiere aprobacion.
          </p>
        </div>
        <button onClick={() => setShowForm(true)} style={btnPrimary}>
          <MessageSquare size={14} /> Nuevo Coaching
        </button>
      </div>
        ...
        >
          <div style={{ marginBottom: 12 }}>
            ...
            <input
              ...
              style={inputStyle}
              placeholder="Ej: Mejora en comunicacion con clientes"
            />
          </div>
          <div style={{ marginBottom: 12 }}>
            <label
...
function ConversationsTab({ employee }: { employee: Employee }) {
  ...
  useEffect(() => {
    setRecords(lsGetForEmployee<ConversationRecord>(KEYS.conversations, employee.id));
  }, [employee.id]);
    ...
    <div>
      ...
      >
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            onClick={() => setShowForm('library')}
            style={{ ...btnPrimary, background: '#2563eb' }}
          >
            <BookOpen size={14} /> Desde Libreria
          </button>
          <button onClick={() => setShowForm('adhoc')} style={btnPrimary}>
            <MessageSquare size={14} /> Ad-hoc
          </button>
        </div>
      </div>
      ...
      {showForm === 'library' && (
        <div className="card" style={{ padding: 20, marginBottom: 16 }}>
            ...
            {CONVERSATION_LIBRARY.map((c) => (
              <div
                ...
                style={{
                  ...
                  cursor: 'pointer',
                  border:
                    form.title === c.title ? '2px solid var(--accent)' : '2px solid transparent',
                }}
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button onClick={() => setShowForm(null)} style={btnSecondary}>
              Cancelar
            </button>
            <button onClick={() => handleSave('library')} style={btnPrimary}>
              Guardar
            </button>
          </div>
        </div>
        ...
        <div className="card" style={{ padding: 20, marginBottom: 16 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <textarea
              ...
              rows={4}
              style={{ ...inputStyle, resize: 'vertical' } as any}
              placeholder="Resumen de la conversacion..."
            />
          </div>
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <button onClick={() => setShowForm(null)} style={btnSecondary}>
            ...
            <button onClick={() => handleSave('adhoc')} style={btnPrimary}>
              Guardar
            </button>
          </div>
        </div>
          ...
          {records
            ...
            .map((r) => (
              <div
                ...
                style={{
                  padding: 14,
                  borderLeft: `4px solid ${r.type === 'library' ? '#2563eb' : '#8b5cf6'}`,
                }}
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                  <span
                    style={{
                      ...
                      fontWeight: 600,
                      background:
                        r.type === 'library' ? 'rgba(37,99,235,0.1)' : 'rgba(139,92,246,0.1)',
                      color: r.type === 'library' ? '#2563eb' : '#8b5cf6',
                    }}
                  >
...
function AcknowledgementsTab({ employee }: { employee: Employee }) {
  ...
  useEffect(() => {
    setRecords(lsGetForEmployee<AcknowledgementRecord>(KEYS.acknowledgements, employee.id));
  }, [employee.id]);

  const handleSave = () => {
    if (!form.title) {
      alert('Seleccione un reconocimiento.');
      return;
    }
  ...
  return (
    <div>
      ...
      >
        ...
        <button onClick={() => setShowForm(true)} style={{ ...btnPrimary, background: '#f59e0b' }}>
          <Award size={14} /> Emitir Reconocimiento
        </button>
      </div>
      ...
      {showForm && (
        <div className="card" style={{ padding: 20, marginBottom: 16 }}>
            ...
            {ACKNOWLEDGEMENT_LIBRARY.map((a) => (
              <div
                ...
                style={{
                  padding: 12,
                  cursor: 'pointer',
                  border: form.title === a.title ? '2px solid #f59e0b' : '2px solid transparent',
                  textAlign: 'center',
                }}
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            ...
            <button onClick={handleSave} style={{ ...btnPrimary, background: '#f59e0b' }}>
              Emitir
            </button>
          </div>
        </div>
          ...
          {records
            ...
            .map((r) => (
              <div
                key={r.id}
                className="card"
                style={{ padding: 16, textAlign: 'center', background: 'rgba(245,158,11,0.05)' }}
              >
                ...
                <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 8 }}>
                  {new Date(r.date).toLocaleDateString('es-CL')} • {r.issuedBy}
                </div>
              </div>
            ))}
        </div>
...
function PerformanceReviewTab({ employee }: { employee: Employee }) {
  ...
  const [showForm, setShowForm] = useState(false);
  const [formType, setFormType] = useState<'mid_contract' | 'end_contract' | 'additional'>(
    'mid_contract'
  );
  const [dimensions, setDimensions] = useState(
    PERFORMANCE_DIMENSIONS.map((d) => ({ name: d.name, rating: 3 }))
  );
  ...
  useEffect(() => {
    setReviews(lsGetForEmployee<PerformanceReview>(KEYS.reviews, employee.id));
  }, [employee.id]);
  ...
  return (
    <div>
      ...
      >
        ...
        <button onClick={() => setShowForm(true)} style={btnPrimary}>
          <TrendingUp size={14} /> Nueva Evaluacion
        </button>
      </div>
      ...
      {criticalPerformance && (
        ...
        >
          ...
          <strong>
            ALERTA: 2 evaluaciones consecutivas bajo 3.0. Segun contrato, la empresa puede
            prescindir de esta posicion.
          </strong>
        </div>
      )}
      ...
      {showForm && (
        ...
        >
                  ...
                  {[1, 2, 3, 4, 5].map((n) => (
                    <button
                      ...
                      style={{
                        ...
                        cursor: 'pointer',
                        background:
                          dimensions[idx].rating >= n
                            ? (n <= 2 ? '#ef4444' : n === 3 ? '#f59e0b' : '#22c55e') + '20'
                            : 'var(--bg-secondary)',
                        color:
                          dimensions[idx].rating >= n
                            ? n <= 2
          ...
          <div
            style={{
              padding: 12,
              background: overallRating < 3 ? 'rgba(239,68,68,0.1)' : 'rgba(34,197,94,0.1)',
              borderRadius: 10,
              marginBottom: 16,
              textAlign: 'center',
            }}
          >
            ...
            <div
              style={{
                fontSize: 32,
                fontWeight: 800,
                color: overallRating < 3 ? '#ef4444' : '#22c55e',
              }}
            >
              ...
              <div key={r.id} className="card" style={{ padding: 16 }}>
                ...
                >
                  ...
                  <div
                    style={{
                      ...
                      fontSize: 16,
                      background:
                        r.overallRating < 3 ? 'rgba(239,68,68,0.15)' : 'rgba(34,197,94,0.15)',
                  ...
                  {r.dimensions.map((d) => (
                    ...
                    >
                      ...
                      <div
                        style={{
                          fontWeight: 700,
                          color: d.rating < 3 ? '#ef4444' : d.rating >= 4 ? '#22c55e' : '#f59e0b',
                        }}
                      >
                        {d.rating}/5
                      </div>
                ...
                {r.strengths && (
                  <div style={{ fontSize: 12, marginTop: 10 }}>
                    <strong style={{ color: '#22c55e' }}>Fortalezas:</strong> {r.strengths}
                  </div>
                )}
                {r.areasToImprove && (
                  <div style={{ fontSize: 12, marginTop: 4 }}>
                    <strong style={{ color: '#f59e0b' }}>Mejora:</strong> {r.areasToImprove}
                  </div>
                )}
                {r.comments && (
    ...
    <div>
      ...
      >
        ...
        {[
          ...
          {
            type: 'expectation_memo',
            label: 'Expectation Memo 30/60/90 (firmado)',
            required: true,
          },
          ...
          const hasFES = ['contrato', 'job_description', 'expectation_memo'].includes(doc.type)
            ? getSignature(employee.id, doc.type)
            : null;
          ...
          return (
            ...
            >
              {isComplete ? (
                ...
                <AlertTriangle size={15} style={{ color: doc.required ? '#ef4444' : '#f59e0b' }} />
              )}
              <span style={{ flex: 1, fontSize: 12 }}>{doc.label}</span>
              {doc.required && !isComplete && (
                <span style={{ fontSize: 9, color: '#ef4444', fontWeight: 700 }}>OBLIGATORIO</span>
              )}
              {isComplete && (
                <span style={{ fontSize: 9, color: '#22c55e', fontWeight: 700 }}>COMPLETO</span>
              )}
              {hasFES && (
                <span
                  style={{
...
// ═══════════════════════════════════════════════════════════════════
// EMPLOYEE CHAT TAB — Smart Chat about employee documents
// ═══════════════════════════════════════════════════════════════════
...
Documentos: ${documents.map((d) => d.name || d.documentType).join(', ') || 'Sin documentos'}.`;
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (idx >= 0) stored[idx].messages = final;
      else stored.push({ empId: employee.id, messages: final });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: 500 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>
            Pregunta sobre sus documentos, evaluaciones, contrato, etc.
          </div>
        </div>
      </div>
      ...
      >
        {messages.length === 0 && (
          <div
            style={{ textAlign: 'center', color: 'var(--text-muted)', padding: 40, fontSize: 12 }}
          >
...
function AddEmployeeModal({ onClose, onRefresh }: { onClose: () => void; onRefresh: () => void }) {
  ...
  const handleSave = async () => {
    if (!form.rut || !form.firstName || !form.lastName) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div
        ...
        style={{ maxWidth: 800, maxHeight: '90vh', overflow: 'auto', padding: 24 }}
      >
        <div
          style={{
            display: 'flex',
        ...
        <div style={grid2}>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                if (p) {
                  ...
                  if (jd) set('department', jd.department);
                }
              }}
              style={selectStyle}
            >
          ...
          {form.contractType === 'plazo_fijo' && (
            <F label="Fecha Termino" k="endDate" type="date" />
          )}
        ...
        {form.position && form.position !== '__custom__' && getJobDescription(form.position) && (
          ...
          >
            <CheckCircle size={14} /> Job Description, Expectation Memo y Contrato Tipo vinculados
            automaticamente.
          </div>
        )}
    ...
    <div style={{ position: 'relative' }}>
      ...
      {showDetail && (
        ...
        >
          ...
          >
            <Lock size={14} /> Certificado FES
          </h4>
          <div style={{ fontSize: 11, display: 'grid', gap: 4 }}>
            <div>
              <strong>Firmante:</strong> {sig.signerName}
            </div>
            <div>
              <strong>RUT:</strong> {sig.signerRut}
            </div>
            <div>
              <strong>Fecha:</strong> {new Date(sig.timestamp).toLocaleString('es-CL')}
            </div>
            <div>
              <strong>IP:</strong> {sig.ipAddress}
            </div>
            <div
              style={{
...
function ContractClause({ title, content }: { title: string; content: string }) {
  return (
    <div style={{ marginBottom: 12 }}>
      <h4 style={{ fontSize: 12, fontWeight: 700, color: 'var(--accent)', margin: '0 0 4px' }}>
        {title}
      </h4>
