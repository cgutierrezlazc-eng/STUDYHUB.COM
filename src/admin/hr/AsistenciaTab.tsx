...
function generateDemoData(
  employeeList?: { id: string; name: string; position: string; exempt: boolean }[]
) {
    ...
    for (const emp of demoEmployees) {
      ...
      if (overtimeHours > 0) {
        ...
        const overtimeAmount = Math.round(
          overtimeHours * hourlyRate * (1 + CHILE_LABOR.HORAS_EXTRA.recargo)
        );
...
export default function AsistenciaTab() {
  ...
  const [art22Exemptions, setArt22Exemptions] = useState<Record<string, boolean>>(() => {
    ...
    demoData.demoEmployees.forEach((e) => {
      map[e.id] = e.exempt;
    });
    return map;
  });
  ...
  useEffect(() => {
    api
      ...
      .then((data: any) => {
        ...
        setDemoData(newData);
        const map: Record<string, boolean> = {};
        newData.demoEmployees.forEach((emp) => {
          map[emp.id] = emp.exempt;
        });
  ...
  const monthlySummaries = useMemo((): MonthlySummary[] => {
    ...
    return demoData.demoEmployees.map((emp) => {
      ...
      const weeklyAvgHours =
        daysWorked > 0 ? Math.round((totalHours / weeksInMonth) * 100) / 100 : 0;
  ...
  const weeklyChartData = useMemo(() => {
    ...
    return days.map((name, idx) => {
      ...
      const dateStr = date.toISOString().split('T')[0];
      const dayRecords = demoData.records.filter(
        (r) => r.date === dateStr && r.status === 'presente'
      );
    ...
    <div style={{ padding: '24px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      >
        ...
        <StatCard
          ...
          color="#f59e0b"
          subtitle={`Max ${CHILE_LABOR.HORAS_EXTRA.maxDiarias}h/dia (Art. 30)`}
        />
        <StatCard
          icon={Users}
          label="Total Empleados"
          value={demoData.demoEmployees.length}
          color="#3b82f6"
          subtitle="Dotacion registrada"
        />
      </div>
      ...
      {activeTab === 'marcaje' && (
        <div>
          <div
            className="card"
            style={{ padding: 32, textAlign: 'center', maxWidth: 500, margin: '0 auto' }}
          >
            {/* Live clock */}
            <div
              style={{
            ...
            <div
              style={{
                ...
                fontWeight: 600,
                background:
                  clockStatus === 'working' ? 'rgba(34,197,94,0.12)' : 'rgba(100,100,100,0.1)',
            ...
            >
              {clockStatus === 'working' ? (
                <>
                  <CheckCircle size={16} />{' '}
                  {demoData.demoEmployees.find((e) => e.id === selectedWorker)?.name ||
                    'Trabajador'}{' '}
                  — Trabajando desde las {clockEntry}
                </>
              ) : (
                <>
                  <Clock size={16} />{' '}
                  {selectedWorker ? 'Listo para marcar entrada' : 'Selecciona tu nombre arriba'}
                </>
              )}
            </div>
            ...
            <div style={{ marginTop: 16, fontSize: 11, color: 'var(--text-muted)' }}>
              Art. 34 CT: Se descuenta automaticamente 30 min de colacion (no computable como
              jornada)
            </div>
          </div>
          ...
          {attendanceLogs.length > 0 && (
            <div
              className="card"
              style={{ padding: 20, marginTop: 20, maxWidth: 700, margin: '20px auto 0' }}
            >
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                ...
                <button
                  ...
                  style={{ marginLeft: 'auto', ...btnSmall, padding: '4px 10px' }}
                >
                  Actualizar
                </button>
              </div>
                ...
                <tbody>
                  {attendanceLogs.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td style={{ ...tdStyle, textAlign: 'center' }}>
                        <span
                          style={{
                            ...
                            fontWeight: 700,
                            background:
                              r.action === 'in' ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.12)',
                      ...
                      >
                        {new Date(r.timestamp + 'Z').toLocaleTimeString('es-CL', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </td>
      ...
      {activeTab === 'resumen' && (
        <div>
                  ...
                  <tr key={s.employeeId} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    >
                      ...
                      {s.weeklyAvgHours > 45 && !s.isArt22Exempt && (
                        <AlertTriangle
                          size={12}
                          style={{ marginLeft: 4, verticalAlign: 'middle', color: '#ef4444' }}
                        />
                      )}
                    </td>
                    <td style={{ ...tdStyle, textAlign: 'center' }}>
          ...
          >
            ...
            <div>
              <strong>Art. 22 inc. 2 CT:</strong> Quedan excluidos de la limitacion de jornada los
              trabajadores que presten servicios a distintos empleadores, gerentes, administradores,
              apoderados con facultades de administracion y todos aquellos que trabajen sin
              fiscalizacion superior inmediata.
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'horasExtra' && (
        <div>
          ...
          >
            ...
            <span>
              <strong>Art. 32 CT:</strong> El pacto de horas extraordinarias debe constar por
              escrito y tener vigencia maxima de {CHILE_LABOR.HORAS_EXTRA.maxPacto} meses. Max{' '}
              {CHILE_LABOR.HORAS_EXTRA.maxDiarias}h extras por dia.
            </span>
          </div>
                      ...
                      <tr key={r.id} style={{ borderBottom: '1px solid var(--border)' }}>
                        ...
                        <td style={{ ...tdStyle, textAlign: 'center', fontSize: 11 }}>
                          {r.pactoExpiry ? (
                            ...
                            >
                              Hasta{' '}
                              {new Date(r.pactoExpiry + 'T12:00:00').toLocaleDateString('es-CL', {
                                month: 'short',
                                year: 'numeric',
                              })}
                            </span>
      ...
      >
        <AlertCircle size={14} />
        <span>
          <strong>Modo Demo:</strong> Los datos mostrados son de ejemplo. Conectar a endpoints del
          backend cuando esten implementados (POST /hr/attendance/clock-in, GET /hr/attendance,
          etc.)
        </span>
      </div>
    </div>
