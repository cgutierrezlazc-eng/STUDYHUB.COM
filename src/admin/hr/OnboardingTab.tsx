...
interface OffboardingProcess {
  ...
  position: string
  exitDate: string
  exitReason: 'renuncia' | 'despido_necesidades' | 'despido_causal' | 'mutuo_acuerdo' | 'termino_plazo'
  items: ChecklistItem[]
  status: 'en_curso' | 'completado' | 'cancelado'
  createdAt: string
}
...
const DEFAULT_ONBOARDING_ITEMS: { label: string; defaultResponsible: string }[] = [
  { label: 'Contrato firmado (Art. 9 Código del Trabajo — plazo 15 días)', defaultResponsible: 'RRHH' },
  ...
  { label: 'Firma Reglamento Interno de Orden, Higiene y Seguridad (Art. 153 CT)', defaultResponsible: 'RRHH' },
...
const DEFAULT_OFFBOARDING_ITEMS: { label: string; defaultResponsible: string }[] = [
  { label: 'Carta de renuncia / Notificación de despido (Art. 159-163 CT)', defaultResponsible: 'RRHH' },
  ...
  { label: 'Firma de finiquito ante notario/Inspección del Trabajo (Art. 177 CT)', defaultResponsible: 'RRHH' },
...
function ProgressBar({ items }: { items: ChecklistItem[] }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
      ...
      <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>({done}/{items.length})</span>
    </div>
  )
}
...
export default function OnboardingTab() {
  ...
  const [templates, setTemplates] = useState<Template[]>(() =>
    loadFromStorage(STORAGE_KEYS.templates, [
      { id: 'tpl-on-default', name: 'Onboarding Chile (Estándar)', type: 'onboarding', department: null, items: DEFAULT_ONBOARDING_ITEMS, isDefault: true },
      { id: 'tpl-off-default', name: 'Offboarding Chile (Estándar)', type: 'offboarding', department: null, items: DEFAULT_OFFBOARDING_ITEMS, isDefault: true },
    ])
  )

  // Persist on change
  useEffect(() => { saveToStorage(STORAGE_KEYS.onboarding, onboardings) }, [onboardings])
  useEffect(() => { saveToStorage(STORAGE_KEYS.offboarding, offboardings) }, [offboardings])
...
function OnboardingSection({ processes, setProcesses, templates }: {
  ...
  const updateItemField = (processId: string, itemId: string, field: keyof ChecklistItem, value: string) => {
    setProcesses(prev => prev.map(p => {
      if (p.id !== processId) return p
  ...
  const cancelProcess = (processId: string) => {
    setProcesses(prev => prev.map(p => p.id === processId ? { ...p, status: 'cancelado' } : p))
  }
    ...
    <div>
      ...
      {showForm && (
        <div style={{ ...card, border: '1px solid var(--accent)' }}>
          <h4 style={{ margin: '0 0 12px', fontSize: 13, fontWeight: 700, color: 'var(--text-primary)' }}>
            Iniciar Proceso de Onboarding
          </h4>
          <div style={grid2}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Colaborador</label>
              <select style={selectStyle} value={selectedEmpId} onChange={e => setSelectedEmpId(e.target.value)}>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Fecha de ingreso</label>
        ...
        return (
          <div key={proc.id} style={{ ...card, opacity: proc.status === 'cancelado' ? 0.5 : 1 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, cursor: 'pointer' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={badge(
                  proc.status === 'en_curso' ? '#3b82f6' : proc.status === 'completado' ? '#22c55e' : '#ef4444'
                )}>
                  {proc.status === 'en_curso' ? 'En Curso' : proc.status === 'completado' ? 'Completado' : 'Cancelado'}
                </span>
            ...
            {isExpanded && proc.status !== 'cancelado' && (
              <div style={{ marginTop: 12 }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  ...
                  <tbody>
                    {proc.items.map(item => (
                      <tr key={item.id} style={{ borderBottom: '1px solid var(--border)', opacity: item.completed ? 0.6 : 1 }}>
                        <td style={tdStyle}>
                          <input type="checkbox" checked={item.completed}
                            onChange={() => toggleItem(proc.id, item.id)} />
                        </td>
                        <td style={{ ...tdStyle, textDecoration: item.completed ? 'line-through' : 'none', fontSize: 12 }}>
                          {item.label}
                        </td>
                        <td style={tdStyle}>
                          <input style={{ ...inputStyle, padding: '4px 6px', fontSize: 11 }}
                            value={item.responsible}
                            onChange={e => updateItemField(proc.id, item.id, 'responsible', e.target.value)} />
                        </td>
                        <td style={tdStyle}>
                          <input type="date" style={{ ...inputStyle, padding: '4px 6px', fontSize: 11 }}
                            value={item.dueDate}
                            onChange={e => updateItemField(proc.id, item.id, 'dueDate', e.target.value)} />
                        </td>
                        <td style={tdStyle}>
                          <input style={{ ...inputStyle, padding: '4px 6px', fontSize: 11 }}
                            placeholder="Nota..."
                            value={item.notes}
                            onChange={e => updateItemField(proc.id, item.id, 'notes', e.target.value)} />
                        </td>
                      </tr>
                    ))}
                  </tbody>
...
function OffboardingSection({ processes, setProcesses, templates }: {
      ...
      {showForm && (
        <div style={{ ...card, border: '1px solid var(--accent)' }}>
          ...
          <div style={grid2}>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Causal de término</label>
              <select style={selectStyle} value={exitReason} onChange={e => setExitReason(e.target.value as OffboardingProcess['exitReason'])}>
                {EXIT_REASONS.map(r => (
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Fecha de salida</label>
              <input type="date" style={inputStyle} value={exitDate} onChange={e => setExitDate(e.target.value)} />
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Plantilla</label>
              <select style={selectStyle} value={selectedTplId} onChange={e => setSelectedTplId(e.target.value)}>
          ...
          {/* Legal note */}
          <div style={{ marginTop: 12, padding: 10, borderRadius: 8, background: '#f59e0b11', border: '1px solid #f59e0b33', fontSize: 11, color: '#f59e0b' }}>
            <strong>Recordatorio legal:</strong> El finiquito debe ser firmado ante un ministro de fe
            (notario, inspector del trabajo, o presidente del sindicato) dentro de los 10 días hábiles
            siguientes al término del contrato (Art. 177 Código del Trabajo).
          </div>
        </div>
      )}
        ...
        return (
          <div key={proc.id} style={{ ...card, opacity: proc.status === 'cancelado' ? 0.5 : 1 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, cursor: 'pointer' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <span style={badge(proc.status === 'en_curso' ? '#f59e0b' : proc.status === 'completado' ? '#22c55e' : '#ef4444')}>
                  {proc.status === 'en_curso' ? 'En Curso' : proc.status === 'completado' ? 'Finalizado' : 'Cancelado'}
                </span>
                <span style={{ fontSize: 16, color: 'var(--text-muted)' }}>{isExpanded ? '▾' : '▸'}</span>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: 16, fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
              <span>Causal: <strong style={{ color: 'var(--text-primary)' }}>{reasonLabel(proc.exitReason)}</strong></span>
              <span>Fecha salida: <strong style={{ color: 'var(--text-primary)' }}>{proc.exitDate}</strong></span>
            </div>
              ...
              <div style={{ marginTop: 16 }}>
                <div style={{ position: 'relative', paddingLeft: 24 }}>
                  ...
                  {proc.items.map((item, idx) => (
                    <div key={item.id} style={{ position: 'relative', marginBottom: 12, paddingLeft: 16 }}>
                      {/* Dot */}
                      <div style={{
                      ...
                      <div style={{
                        ...
                        <div style={{ display: 'flex', gap: 8, marginTop: 6 }}>
                          <input style={{ ...inputStyle, padding: '3px 6px', fontSize: 10, flex: 1 }}
                            placeholder="Responsable"
                            value={item.responsible}
                            onChange={e => updateItemField(proc.id, item.id, 'responsible', e.target.value)} />
                          <input type="date" style={{ ...inputStyle, padding: '3px 6px', fontSize: 10 }}
                            value={item.dueDate}
                            onChange={e => updateItemField(proc.id, item.id, 'dueDate', e.target.value)} />
                          <input style={{ ...inputStyle, padding: '3px 6px', fontSize: 10, flex: 1 }}
                            placeholder="Nota..."
...
function TemplatesSection({ templates, setTemplates }: {
  templates: Template[]
  ...
  const startFromDefault = (type: 'onboarding' | 'offboarding') => {
    ...
    setTplItems(type === 'onboarding' ? [...DEFAULT_ONBOARDING_ITEMS] : [...DEFAULT_OFFBOARDING_ITEMS])
    setShowForm(true)
  }
  ...
  const handleSave = () => {
    ...
    if (editId) {
      setTemplates(prev => prev.map(t => t.id === editId ? {
        ...t, name: tplName, type: tplType, department: tplDept || null, items: cleanItems,
      } : t))
    } else {
    ...
    <div>
      ...
      {showForm && (
        <div style={{ ...card, border: '1px solid var(--accent)', marginBottom: 16 }}>
          ...
          <div style={grid2}>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Nombre</label>
              <input style={inputStyle} value={tplName} onChange={e => setTplName(e.target.value)}
                placeholder="Nombre de la plantilla" />
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Tipo</label>
              <select style={selectStyle} value={tplType} onChange={e => setTplType(e.target.value as 'onboarding' | 'offboarding')}>
                <option value="onboarding">Onboarding (Ingreso)</option>
                <option value="offboarding">Offboarding (Salida)</option>
              </select>
            </div>
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)' }}>Departamento (opcional)</label>
              <select style={selectStyle} value={tplDept} onChange={e => setTplDept(e.target.value)}>
          ...
          <div style={{ marginTop: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              <button style={btnSmall} onClick={addItem}>+ Agregar ítem</button>
            </div>
            ...
            {tplItems.map((item, idx) => (
              <div key={idx} style={{ display: 'flex', gap: 8, marginBottom: 6, alignItems: 'center' }}>
                ...
                <input style={{ ...inputStyle, flex: 2, padding: '6px 8px', fontSize: 12 }}
                  placeholder="Descripción del ítem"
                  value={item.label}
                  onChange={e => updateItem(idx, 'label', e.target.value)} />
                <input style={{ ...inputStyle, flex: 1, padding: '6px 8px', fontSize: 12 }}
                  placeholder="Responsable"
                  value={item.defaultResponsible}
                  onChange={e => updateItem(idx, 'defaultResponsible', e.target.value)} />
                <button style={{ ...btnSmall, color: '#ef4444', padding: '4px 8px' }}
          ...
          <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
            ...
            <button style={btnSecondary} onClick={resetForm}>Cancelar</button>
          </div>
        </div>
      )}
      ...
      {[
        ...
        <div key={section.label} style={{ marginBottom: 20 }}>
          ...
          {section.list.length === 0 && (
            <div style={{ ...card, textAlign: 'center', color: 'var(--text-muted)', fontSize: 12, padding: 20 }}>
            ...
            <div key={tpl.id} style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button style={btnSmall} onClick={() => startEdit(tpl)}>Editar</button>
                  {!tpl.isDefault && (
                    <button style={{ ...btnSmall, color: '#ef4444', borderColor: '#ef4444' }}
              ...
              <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {tpl.items.slice(0, 5).map((item, i) => (
                  <span key={i} style={{
                    ...
                    background: 'var(--bg-primary)', fontSize: 10, color: 'var(--text-muted)',
                    border: '1px solid var(--border)',
                  }}>
