...
export default function AccessControlTab() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const emps =
        Array.isArray(apiEmployees) && apiEmployees.length > 0 ? apiEmployees : SEED_EMPLOYEES;
      setEmployees(emps);
    } catch {
  ...
  const handleEnableAll = (empId: string) => {
    const perms = getEmployeePermissions(empId);
    Object.keys(perms.moduleAccess).forEach((k) => {
      perms.moduleAccess[k] = true;
    });
    savePermissions(empId, perms);
    setPermissions({ ...permissions, [empId]: perms });
    ...
    return (
      <div className="page-enter" style={{ padding: '20px 0' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div
                style={{
                  ...
                  height: 44,
                  borderRadius: '50%',
                  background: perms.isActive ? 'var(--accent)' : 'var(--text-muted)',
                  display: 'flex',
                  alignItems: 'center',
        ...
        {CATEGORY_ORDER.map((cat) => {
          const catModules = ADMIN_MODULES.filter(
            (m) => m.category === cat && m.status === 'active'
          );
        ...
        >
          <strong>Nota:</strong> Los cambios se aplican inmediatamente. Cuando el backend de
          autenticación esté conectado, los permisos se sincronizarán automáticamente con la cuenta
          del trabajador. Los módulos deshabilitados no aparecerán en el panel del empleado.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="page-enter" style={{ padding: '20px 0' }}>
      ...
      {/* Summary Cards */}
      <div
        style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginTop: 24 }}
      >
        <div
          style={{
            padding: '16px 20px',
        ...
        >
          <div style={{ fontSize: 24, fontWeight: 700, color: '#22c55e' }}>{totalModules}</div>
