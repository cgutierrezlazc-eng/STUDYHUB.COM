...
const CATEGORIES = [
  { key: 'objetivos', label: 'Cumplimiento de objetivos', weight: 0.30 },
  { key: 'calidad', label: 'Calidad del trabajo', weight: 0.20 },
  { key: 'iniciativa', label: 'Iniciativa y proactividad', weight: 0.15 },
  { key: 'equipo', label: 'Trabajo en equipo', weight: 0.15 },
  { key: 'puntualidad', label: 'Puntualidad y asistencia', weight: 0.10 },
  { key: 'comunicacion', label: 'Comunicación', weight: 0.10 },
]
...
const label: React.CSSProperties = {
  ...
  textTransform: 'uppercase' as const, marginBottom: 4, letterSpacing: '0.04em',
}
    ...
    <span style={{ display: 'inline-flex', gap: 2, cursor: onChange ? 'pointer' : 'default' }}>
      {[1, 2, 3, 4, 5].map(s => (
        <span
          ...
          onClick={() => onChange?.(s)}
          style={{
            fontSize: 18, color: s <= value ? '#eab308' : 'var(--text-muted)',
  ...
  return (
    <div style={{ width: '100%', height: 8, borderRadius: 4, background: 'var(--border)' }}>
      <div style={{
        ...
        borderRadius: 4, background: c, transition: 'width 0.3s',
      }} />
    </div>
  )
}
...
export default function DesempenoTab() {
  ...
  const [cycleForm, setCycleForm] = useState<Partial<EvalCycle>>({
    name: '', startDate: '', endDate: '', type: '90', employeeIds: [], status: 'borrador',
  })
  ...
  const [goalForm, setGoalForm] = useState<Partial<Goal>>({
    employeeId: '', name: '', description: '', targetMetric: '',
    deadline: '', weight: 25, progress: 0, status: 'pendiente',
  })
  ...
  // ─── Persist helpers ──────────────────────────────────────────
  function saveCycles(c: EvalCycle[]) { setCycles(c); saveLS(LS_CYCLES, c) }
  function saveEvals(e: Evaluation[]) { setEvals(e); saveLS(LS_EVALS, e) }
  function saveGoals(g: Goal[]) { setGoals(g); saveLS(LS_GOALS, g) }

  // ─── Computed ─────────────────────────────────────────────────
  const overallScore = CATEGORIES.reduce((acc, cat) => acc + (scores[cat.key] || 0) * cat.weight, 0)
  ...
  function handleCreateCycle() {
    if (!cycleForm.name || !cycleForm.startDate || !cycleForm.endDate || !cycleForm.employeeIds?.length) return
    ...
    setCycleForm({ name: '', startDate: '', endDate: '', type: '90', employeeIds: [], status: 'borrador' })
  ...
  function toggleCycleEmployee(empId: string) {
    ...
    setCycleForm({
      ...cycleForm,
      employeeIds: current.includes(empId) ? current.filter(x => x !== empId) : [...current, empId],
    })
  }

  function closeCycle(cycleId: string) {
    saveCycles(cycles.map(c => c.id === cycleId ? { ...c, status: 'cerrado' as CycleStatus } : c))
  }
    ...
    return (
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            <h3 style={{ margin: 0, fontSize: 16, color: 'var(--text-primary)' }}>Ciclos de Evaluacion</h3>
        ...
        {showCycleForm && (
          <div style={{ ...card, border: '1px solid var(--accent)' }}>
            <h4 style={{ margin: '0 0 12px', fontSize: 14, color: 'var(--text-primary)' }}>Crear Ciclo de Evaluacion</h4>
            <div style={grid2}>
              <div>
                <div style={label}>Nombre del ciclo</div>
                <input style={inputStyle} placeholder="Ej: Semestre 1 2026"
                  value={cycleForm.name || ''} onChange={e => setCycleForm({ ...cycleForm, name: e.target.value })} />
              </div>
              <div>
                <div style={label}>Tipo de evaluacion</div>
                <select style={selectStyle} value={cycleForm.type || '90'}
              ...
              <div>
                ...
                <input type="date" style={inputStyle} value={cycleForm.startDate || ''}
                  onChange={e => setCycleForm({ ...cycleForm, startDate: e.target.value })} />
              </div>
              <div>
                <div style={label}>Fecha fin</div>
                <input type="date" style={inputStyle} value={cycleForm.endDate || ''}
            ...
            <div style={{ marginTop: 12 }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 4 }}>
                {SEED_EMPLOYEES.map(emp => {
                  ...
                  return (
                    <button key={emp.id} onClick={() => toggleCycleEmployee(emp.id)}
                      style={{
                        ...btnSmall,
                ...
                <button style={{ ...btnSmall, fontStyle: 'italic' }}
                  onClick={() => setCycleForm({ ...cycleForm, employeeIds: SEED_EMPLOYEES.map(e => e.id) })}>
                  Seleccionar todos
                </button>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              <button style={btnPrimary} onClick={handleCreateCycle}>Crear Ciclo</button>
              <button style={btnSecondary} onClick={() => setShowCycleForm(false)}>Cancelar</button>
            </div>
          </div>
        )}
            ...
            <div key={cycle.id} style={card}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>{cycle.name}</span>
                    <span style={badge(
                      cycle.status === 'activo' ? 'rgba(34,197,94,0.15)' : 'rgba(156,163,175,0.15)',
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  {cycle.status === 'activo' && (
                    <button style={btnSmall} onClick={() => closeCycle(cycle.id)}>Cerrar</button>
                  )}
                  <button style={{ ...btnSmall, color: '#ef4444' }} onClick={() => deleteCycle(cycle.id)}>Eliminar</button>
                </div>
              </div>
              <div style={{ marginTop: 10 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>
                  <span>{prog.done} de {prog.total} evaluaciones completadas</span>
                  <span>{pct}%</span>
                </div>
              ...
              <div style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: 4 }}>
                {cycle.employeeIds.map(eid => {
                  const hasEval = evals.some(e => e.cycleId === cycle.id && e.employeeId === eid && !e.isDraft)
                  return (
                    <span key={eid} style={{
                      ...badge(
  ...
  function handleSubmitEval(isDraft: boolean) {
    ...
    const filtered = evals.filter(e => !(e.cycleId === evalCycleId && e.employeeId === evalEmployeeId))
  ...
  function renderEvaluar() {
    ...
    return (
      <div>
        <h3 style={{ margin: '0 0 16px', fontSize: 16, color: 'var(--text-primary)' }}>Formulario de Evaluacion</h3>

        {activeCycles.length === 0 ? (
          <div style={{ ...card, textAlign: 'center', padding: 40, color: 'var(--text-muted)' }}>
            <p style={{ fontSize: 14 }}>No hay ciclos activos</p>
            <p style={{ fontSize: 12 }}>Crea un ciclo en la pestana "Evaluaciones Activas" primero</p>
          </div>
        ) : (
          <>
            <div style={{ ...card }}>
              <div style={grid2}>
                <div>
                  ...
                  <select style={selectStyle} value={evalCycleId}
                    onChange={e => {
                ...
                <div>
                  ...
                  <select style={selectStyle} value={evalEmployeeId}
                    onChange={e => {
              ...
              <>
                ...
                <div style={card}>
                    ...
                    <tbody>
                      {CATEGORIES.map(cat => (
                        <tr key={cat.key} style={{ borderBottom: '1px solid var(--border)' }}>
                          ...
                          <td style={{ ...tdStyle, textAlign: 'center' }}>{Math.round(cat.weight * 100)}%</td>
                          <td style={{ ...tdStyle, textAlign: 'center' }}>
                            <Stars value={scores[cat.key] || 0}
                  ...
                  <div style={{
                    ...
                    <div>
                      <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 600 }}>PUNTAJE FINAL</div>
                      <div style={{ fontSize: 28, fontWeight: 800, color: classColor(overallClass) }}>
                ...
                <div style={card}>
                  ...
                  <div>
                    ...
                    <textarea style={{ ...inputStyle, minHeight: 70, resize: 'vertical' }}
                      ...
                      value={devPlan} onChange={e => setDevPlan(e.target.value)} />
                  </div>
                </div>
  ...
  function handleCreateGoal() {
    ...
    setGoalForm({ employeeId: '', name: '', description: '', targetMetric: '', deadline: '', weight: 25, progress: 0, status: 'pendiente' })
  ...
  function updateGoalProgress(goalId: string, progress: number) {
    saveGoals(goals.map(g => {
      ...
      const status: GoalStatus = progress >= 100 ? 'completada' : progress > 0 ? 'en_progreso' : 'pendiente'
  ...
  function renderMetas() {
    ...
    const empSummary = SEED_EMPLOYEES.map(emp => {
      ...
      const overdue = empGoals.filter(g => g.deadline < new Date().toISOString().slice(0, 10) && g.status !== 'completada').length
      return { emp, avgProgress, completed, total: empGoals.length, overdue }
    }).filter(Boolean) as { emp: typeof SEED_EMPLOYEES[0]; avgProgress: number; completed: number; total: number; overdue: number }[]
      ...
      <div>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            ...
            <p style={{ margin: '4px 0 0', fontSize: 12, color: 'var(--text-muted)' }}>
              {goals.length} meta(s) registrada(s) | {goals.filter(g => g.status === 'completada').length} completada(s)
            </p>
          </div>
        ...
        {empSummary.length > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10, marginBottom: 16 }}>
            {empSummary.map(({ emp, avgProgress, completed, total, overdue }) => {
              const color = avgProgress >= 75 ? '#22c55e' : avgProgress >= 40 ? '#eab308' : '#ef4444'
              return (
                <div key={emp.id} style={{
                  ...card, marginBottom: 0, cursor: 'pointer',
                  border: goalFilterEmp === emp.id ? '1px solid var(--accent)' : '1px solid var(--border)',
                }} onClick={() => setGoalFilterEmp(goalFilterEmp === emp.id ? '' : emp.id)}>
                  <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-primary)', marginBottom: 6 }}>
                    {emp.firstName} {emp.lastName}
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
                    {completed}/{total} metas | {overdue > 0 ? <span style={{ color: '#ef4444' }}>{overdue} vencida(s)</span> : 'Al dia'}
                  </div>
                  <ProgressBar value={avgProgress} color={color} />
                  <div style={{ fontSize: 11, color, fontWeight: 700, marginTop: 4, textAlign: 'right' }}>
        ...
        {/* Filter */}
        <div style={{ marginBottom: 12 }}>
          <select style={{ ...selectStyle, maxWidth: 280 }} value={goalFilterEmp}
        ...
        {showGoalForm && (
          <div style={{ ...card, border: '1px solid var(--accent)' }}>
            <h4 style={{ margin: '0 0 12px', fontSize: 14, color: 'var(--text-primary)' }}>Nueva Meta</h4>
            <div style={grid2}>
              <div>
                <div style={label}>Empleado</div>
                <select style={selectStyle} value={goalForm.employeeId || ''}
              ...
              <div>
                ...
                <input style={inputStyle} placeholder="Ej: Aumentar ventas 20%"
                  value={goalForm.name || ''} onChange={e => setGoalForm({ ...goalForm, name: e.target.value })} />
              </div>
              <div>
                <div style={label}>Descripcion</div>
                <input style={inputStyle} placeholder="Detalle de la meta..."
              ...
              <div>
                ...
                <input style={inputStyle} placeholder="Ej: $10M en ventas, 95% NPS"
                  value={goalForm.targetMetric || ''} onChange={e => setGoalForm({ ...goalForm, targetMetric: e.target.value })} />
              </div>
              <div>
                <div style={label}>Fecha limite</div>
                <input type="date" style={inputStyle} value={goalForm.deadline || ''}
                  onChange={e => setGoalForm({ ...goalForm, deadline: e.target.value })} />
              </div>
              <div>
                <div style={label}>Peso (%)</div>
                <input type="number" style={inputStyle} min={1} max={100}
                  value={goalForm.weight || 25} onChange={e => setGoalForm({ ...goalForm, weight: Number(e.target.value) })} />
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
              <button style={btnPrimary} onClick={handleCreateGoal}>Crear Meta</button>
              <button style={btnSecondary} onClick={() => setShowGoalForm(false)}>Cancelar</button>
            </div>
          </div>
        )}
        ...
        {filtered.length === 0 && !showGoalForm ? (
          ...
          filtered.map(goal => {
            const isOverdue = goal.deadline < new Date().toISOString().slice(0, 10) && goal.status !== 'completada'
            const statusColor = goal.status === 'completada' ? '#22c55e'
              : goal.status === 'cancelada' ? '#9ca3af'
              : isOverdue ? '#ef4444'
              : goal.status === 'en_progreso' ? '#3b82f6' : '#eab308'
            const statusLabel = goal.status === 'completada' ? 'Completada'
              : goal.status === 'cancelada' ? 'Cancelada'
              : isOverdue ? 'Vencida'
              : goal.status === 'en_progreso' ? 'En progreso' : 'Pendiente'
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      <span style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-primary)' }}>{goal.name}</span>
                      <span style={badge(`${statusColor}20`, statusColor)}>{statusLabel}</span>
                      <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Peso: {goal.weight}%</span>
                    </div>
                    ...
                    {goal.description && (
                      <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 4 }}>{goal.description}</div>
                    )}
                  </div>
                  <button style={{ ...btnSmall, color: '#ef4444' }} onClick={() => deleteGoal(goal.id)}>Eliminar</button>
                </div>
                <div style={{ marginTop: 10 }}>
  ...
  function renderHistorial() {
    ...
    if (sortedEvals.length >= 2) {
      ...
      if (diff > 0.2) { trendLabel = 'Mejorando'; trendColor = '#22c55e' }
      else if (diff < -0.2) { trendLabel = 'Declinando'; trendColor = '#ef4444' }
      else { trendLabel = 'Estable'; trendColor = '#eab308' }
    }

    return (
      <div>
        <h3 style={{ margin: '0 0 16px', fontSize: 16, color: 'var(--text-primary)' }}>Historial de Evaluaciones</h3>

        <div style={{ marginBottom: 16 }}>
          <div style={label}>Seleccionar empleado</div>
          <select style={{ ...selectStyle, maxWidth: 320 }} value={histEmpId}
        ...
        {histEmpId && sortedEvals.length > 0 && (
          <>
            {/* Summary card */}
            <div style={{
              ...card, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16, textAlign: 'center',
            }}>
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 4 }}>EVALUACIONES</div>
                <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-primary)' }}>{sortedEvals.length}</div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 4 }}>ULTIMA NOTA</div>
                <div style={{ fontSize: 24, fontWeight: 800, color: classColor(sortedEvals[0].classification) }}>
                  {sortedEvals[0].overallScore.toFixed(1)}
                </div>
                <div style={{ fontSize: 11, color: classColor(sortedEvals[0].classification), fontWeight: 600 }}>
                  {sortedEvals[0].classification}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 700, marginBottom: 4 }}>TENDENCIA</div>
                {trendLabel ? (
                  <>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Se requieren 2+ evaluaciones</div>
                )}
              </div>
            </div>
                  ...
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      {sortedEvals.slice(0, 3).map((ev, i) => {
                        ...
                        return <th key={i} style={thStyle}>{cycle?.name || `Eval ${i + 1}`}</th>
                      })}
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    <tr style={{ borderTop: '2px solid var(--border)' }}>
                      ...
                      {sortedEvals.slice(0, 3).map((ev, i) => (
                        <td key={i} style={{ ...tdStyle, textAlign: 'center', fontWeight: 800, color: classColor(ev.classification) }}>
                ...
                <div key={ev.id} style={card}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <div style={{
                      ...
                      color: classColor(ev.classification), fontWeight: 700, fontSize: 13,
                    }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 10 }}>
                    {CATEGORIES.map(cat => (
                      <div key={cat.key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12 }}>
                  ...
                  {ev.strengths && (
                    <div style={{ marginBottom: 6 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#22c55e' }}>Fortalezas: </span>
                      <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{ev.strengths}</span>
                    </div>
                  )}
                  {ev.improvements && (
                    <div style={{ marginBottom: 6 }}>
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#f97316' }}>Areas de mejora: </span>
                      <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{ev.improvements}</span>
                    </div>
                  )}
                  {ev.developmentPlan && (
                    <div>
                      <span style={{ fontSize: 11, fontWeight: 700, color: '#3b82f6' }}>Plan de desarrollo: </span>
                      <span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{ev.developmentPlan}</span>
                    </div>
                  )}
                </div>
              )
