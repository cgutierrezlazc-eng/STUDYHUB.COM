...
function calcPlanillaAnual(): number {
  return SEED_EMPLOYEES
    ...
    .reduce((sum, e) => sum + e.grossSalary * 12, 0)
}
...
export default function CapacitacionTab() {
  ...
  const blank: Omit<Training, 'id'> = {
    nombre: '', proveedorOTEC: '', modalidad: 'Presencial',
    duracionHoras: 0, fechaInicio: '', fechaFin: '', costoCLP: 0,
    empleadosInscritos: [], estado: 'Planificada', tipo: 'Técnica',
    codigoSENCE: null, acogidoFranquicia: false, montoImputado: 0,
    certificadoURL: null,
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          {showForm && (
            <div style={{
              ...
              <div style={grid2}>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Nombre del Curso</label>
                  <input style={inputStyle} value={form.nombre}
                    onChange={e => setForm({ ...form, nombre: e.target.value })} placeholder="Nombre del curso" />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Proveedor / OTEC</label>
                  <input style={inputStyle} value={form.proveedorOTEC}
                    onChange={e => setForm({ ...form, proveedorOTEC: e.target.value })} placeholder="Organismo Técnico de Capacitación" />
                </div>
              </div>
              <div style={grid2}>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Modalidad</label>
                ...
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Tipo</label>
              ...
              <div style={grid2}>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Duración (horas)</label>
                  <input style={inputStyle} type="number" value={form.duracionHoras || ''}
                    onChange={e => setForm({ ...form, duracionHoras: Number(e.target.value) })} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Costo (CLP)</label>
                  <input style={inputStyle} type="number" value={form.costoCLP || ''}
                    onChange={e => setForm({ ...form, costoCLP: Number(e.target.value) })} />
                </div>
              </div>
              <div style={grid2}>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Fecha Inicio</label>
                  <input style={inputStyle} type="date" value={form.fechaInicio}
                    onChange={e => setForm({ ...form, fechaInicio: e.target.value })} />
                </div>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Fecha Fin</label>
                  <input style={inputStyle} type="date" value={form.fechaFin}
                    onChange={e => setForm({ ...form, fechaFin: e.target.value })} />
                </div>
              </div>
              <div style={grid2}>
                <div>
                  <label style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Código SENCE (opcional)</label>
                  <input style={inputStyle} value={form.codigoSENCE || ''}
                    onChange={e => setForm({ ...form, codigoSENCE: e.target.value || null })} placeholder="Ej: 1238-0045-2026" />
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, paddingTop: 18 }}>
                  <input type="checkbox" checked={form.acogidoFranquicia}
                    onChange={e => setForm({ ...form, acogidoFranquicia: e.target.checked, montoImputado: e.target.checked ? form.costoCLP : 0 })} />
                  <label style={{ fontSize: 12, color: 'var(--text-primary)' }}>Acogido a Franquicia SENCE</label>
                </div>
              </div>
              ...
              <div>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
                  {SEED_EMPLOYEES.map(e => (
                    <button key={e.id} onClick={() => toggleEmpleado(e.id)} style={{
                      ...btnSmall,
                      background: form.empleadosInscritos.includes(e.id) ? 'var(--accent)' : 'var(--bg-secondary)',
                      color: form.empleadosInscritos.includes(e.id) ? '#fff' : 'var(--text-secondary)',
                      border: form.empleadosInscritos.includes(e.id) ? '1px solid var(--accent)' : '1px solid var(--border)',
                    }}>
              ...
              <button style={{ ...btnPrimary, alignSelf: 'flex-start' }} onClick={handleAdd}
          ...
          {/* Calendar view */}
          <div style={{
            padding: 16, borderRadius: 12, border: '1px solid var(--border)', background: 'var(--bg-secondary)',
          }}>
              ...
              {trainings
                ...
                .map(t => (
                  <div key={t.id} style={{
                    ...
                    <div style={{
                      minWidth: 56, textAlign: 'center', padding: '4px 8px',
                      borderRadius: 8, background: 'var(--accent)', color: '#fff', fontSize: 11, fontWeight: 700,
                    }}>
                      {t.fechaInicio.slice(5)}
                    </div>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)' }}>{t.nombre}</div>
              ...
              <tbody>
                {trainings.map(t => (
                  <tr key={t.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    ...
                    <td style={tdStyle}><span style={tipoBadge(t.tipo)}>{t.tipo}</span></td>
                    <td style={{ ...tdStyle, textAlign: 'center' }}>{t.duracionHoras}</td>
                    <td style={{ ...tdStyle, fontSize: 11 }}>{t.fechaInicio} / {t.fechaFin}</td>
                    ...
                    <td style={tdStyle}><span style={estadoBadge(t.estado)}>{t.estado}</span></td>
                    <td style={tdStyle}>
                      <select style={{ ...selectStyle, padding: '4px 6px', fontSize: 11, width: 'auto' }}
                        value={t.estado} onChange={e => updateEstado(t.id, e.target.value as EstadoCurso)}>
      ...
      {subTab === 'sence' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          {/* Legal reference */}
          <div style={{
            padding: 16, borderRadius: 12, background: '#3b82f610',
            ...
            <div style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.7 }}>
              <p style={{ margin: '0 0 6px' }}>
                <strong>Art. 36:</strong> Las empresas contribuyentes de primera categoría de la Ley de Impuesto a la Renta
                podrán descontar del impuesto a pagar, los gastos efectuados en programas de capacitación de sus trabajadores,
                hasta un monto equivalente al <strong>1% de la planilla anual de remuneraciones imponibles</strong>.
              </p>
              <p style={{ margin: '0 0 6px' }}>
                <strong>Art. 46:</strong> Los cursos deben ser impartidos por Organismos Técnicos de Capacitación (OTEC)
                acreditados por el SENCE y estar registrados con un código SENCE válido.
              </p>
              <p style={{ margin: 0 }}>
                <strong>Art. 10:</strong> Las actividades de capacitación deberán contribuir a elevar la productividad
                de la empresa y ser pertinentes a las funciones del trabajador. El SENCE fiscaliza la adecuada
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            <div style={cardStyle}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Planilla Anual Imponible</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-primary)' }}>${fmt(planillaAnual)}</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                {SEED_EMPLOYEES.filter(e => e.status === 'active').length} trabajadores activos
              </div>
            </div>
            <div style={cardStyle}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Tope Franquicia (1%)</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#3b82f6' }}>${fmt(topeFranquicia)}</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Máximo descontable de impuestos</div>
            </div>
            <div style={cardStyle}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Monto Imputado</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#f59e0b' }}>${fmt(montoUsado)}</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>{pctUsado.toFixed(1)}% utilizado</div>
            </div>
            <div style={cardStyle}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Saldo Disponible</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: saldoFranquicia >= 0 ? '#22c55e' : '#ef4444' }}>
                ${fmt(saldoFranquicia)}
              </div>
          ...
          {pctUsado >= 80 && (
            <div style={{
              ...
              fontSize: 12, color: pctUsado >= 100 ? '#ef4444' : '#f59e0b', fontWeight: 600,
            }}>
              {pctUsado >= 100
          ...
          <div style={{
            ...
            <div style={{ height: 24, borderRadius: 12, background: 'var(--bg-primary)', overflow: 'hidden', position: 'relative' }}>
              <div style={{
                height: '100%', borderRadius: 12, width: `${Math.min(pctUsado, 100)}%`,
          ...
          <div style={{
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tbody>
                {trainings.filter(t => t.acogidoFranquicia).map(t => (
                  <tr key={t.id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ ...tdStyle, fontFamily: 'monospace', fontWeight: 600 }}>{t.codigoSENCE || '—'}</td>
                    <td style={{ ...tdStyle, fontWeight: 600 }}>{t.nombre}</td>
                    <td style={tdStyle}>{t.proveedorOTEC}</td>
                    <td style={{ ...tdStyle, textAlign: 'center' }}>{t.duracionHoras}</td>
                    <td style={{ ...tdStyle, textAlign: 'right' }}>${fmt(t.costoCLP)}</td>
                    <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 600 }}>${fmt(t.montoImputado)}</td>
                    <td style={tdStyle}><span style={estadoBadge(t.estado)}>{t.estado}</span></td>
                  </tr>
                ))}
                {trainings.filter(t => t.acogidoFranquicia).length === 0 && (
                  <tr><td colSpan={7} style={{ ...tdStyle, textAlign: 'center', color: 'var(--text-muted)' }}>
      ...
      {subTab === 'registro' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div>
              <select style={selectStyle} value={filterEmpleado}
          ...
          {(() => {
            const odiTrainings = trainings.filter(t => t.tipo === 'Seguridad laboral (ODI)' && t.estado === 'Completada')
            ...
            if (missing.length > 0) {
              return (
                <div style={{
                  padding: 12, borderRadius: 10, background: '#ef444415',
                  border: '1px solid #ef444440', fontSize: 12, color: '#ef4444',
                }}>
                  <strong>Alerta ODI (Art. 21 DS 40):</strong> Los siguientes trabajadores no tienen capacitación
                  de seguridad completada: {missing.map(id => empName(id)).join(', ')}.
                  La empresa está obligada a informar a los trabajadores sobre los riesgos laborales,
          ...
          {/* Per-employee view */}
          {SEED_EMPLOYEES
            .filter(e => !filterEmpleado || e.id === filterEmpleado)
            .map(emp => {
              ...
              const hasODI = empTrainings.some(t => t.tipo === 'Seguridad laboral (ODI)' && t.estado === 'Completada')
                              ...
                              {t.estado === 'Completada' ? (
                                t.certificadoURL ? (
                                  ...
                                  <button style={btnSmall} onClick={() => {
                                    ...
                                    persist(trainings.map(tr => tr.id === t.id ? { ...tr, certificadoURL: filename } : tr))
                                  }}>
                                    Subir Certificado
                                  </button>
                                )
      ...
      {subTab === 'metricas' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            <div style={cardStyle}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Total Horas Capacitadas</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text-primary)' }}>
                {completadas.reduce((s, t) => s + t.duracionHoras * t.empleadosInscritos.length, 0)}h
              </div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>horas-persona completadas</div>
            </div>
            <div style={cardStyle}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Inversión Total</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#8b5cf6' }}>${fmt(totalInversion)}</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>en capacitaciones activas</div>
            </div>
            <div style={cardStyle}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Franquicia SENCE</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#3b82f6' }}>{pctUsado.toFixed(1)}%</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>del 1% utilizado</div>
            </div>
            <div style={cardStyle}>
              <div style={{ fontSize: 11, color: 'var(--text-muted)', fontWeight: 600 }}>Cursos Completados</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#22c55e' }}>{completadas.length}</div>
              <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>de {trainings.length} totales</div>
            </div>
          </div>
          ...
          <div style={{
            ...
            {SEED_EMPLOYEES.map(emp => {
              ...
              return (
                <div key={emp.id} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  <div style={{ width: 140, fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>
                    {emp.firstName} {emp.lastName}
                  </div>
                  <div style={{ flex: 1, height: 16, borderRadius: 8, background: 'var(--bg-primary)', overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', borderRadius: 8, width: `${Math.min((hrs / maxHrs) * 100, 100)}%`,
                      background: 'var(--accent)',
                    }} />
                  </div>
                  <div style={{ width: 40, fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', textAlign: 'right' }}>
                    {hrs}h
                  </div>
                </div>
              )
          ...
          <div style={{
            ...
            {(() => {
              ...
              trainings.filter(t => t.estado !== 'Cancelada').forEach(t => {
              ...
              return depts.map(([dept, data]) => (
                <div key={dept} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                  <div style={{ width: 120, fontSize: 12, fontWeight: 600, color: 'var(--text-primary)' }}>
                    {dept}
                  </div>
                  <div style={{ flex: 1, height: 16, borderRadius: 8, background: 'var(--bg-primary)', overflow: 'hidden' }}>
                    <div style={{
                      height: '100%', borderRadius: 8, width: `${(data.cost / maxCost) * 100}%`,
                      background: '#8b5cf6',
                    }} />
                  </div>
                  <div style={{ width: 100, fontSize: 12, fontWeight: 600, color: 'var(--text-primary)', textAlign: 'right' }}>
                    ${fmt(data.cost)}
                  </div>
            ...
            <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.7 }}>
              {(() => {
                ...
                if (safetyTrainings.length === 0) {
                  return <span>No hay certificaciones de seguridad completadas. Se recomienda programar capacitación ODI a la brevedad.</span>
                }
                return safetyTrainings.map(t => {
                  ...
                  return (
                    <div key={t.id} style={{
                      ...
                      <div>
                        <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{t.nombre}</div>
                        <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                          Vence: {expiryDate.toISOString().slice(0, 10)} — {t.empleadosInscritos.length} trabajadores
                        </div>
                      </div>
                      <span style={{
                        fontSize: 11, padding: '3px 10px', borderRadius: 20, fontWeight: 700,
          ...
          <div style={{
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
              {TIPOS.map(tipo => {
                const count = trainings.filter(t => t.tipo === tipo && t.estado !== 'Cancelada').length
                ...
                return (
                  <div key={tipo} style={{
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>cursos — {hours}h</div>
                  </div>
                )
              })}
            </div>
