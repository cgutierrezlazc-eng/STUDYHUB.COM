...
function saveVacancies(v: Vacancy[]) {
  localStorage.setItem(LS_VACANCIES, JSON.stringify(v));
}
function saveCandidates(c: Candidate[]) {
  localStorage.setItem(LS_CANDIDATES, JSON.stringify(c));
}
...
export default function ReclutamientoTab() {
  const [activeTab, setActiveTab] = useState<'pipeline' | 'vacantes' | 'candidatos' | 'metricas'>(
    'pipeline'
  );
  ...
  const updateVacancies = (v: Vacancy[]) => {
    setVacancies(v);
    saveVacancies(v);
  };
  const updateCandidates = (c: Candidate[]) => {
    setCandidates(c);
    saveCandidates(c);
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {activeTab === 'pipeline' && (
        <PipelineTab candidates={candidates} vacancies={vacancies} onUpdate={updateCandidates} />
      )}
      {activeTab === 'vacantes' && (
        <VacantesTab vacancies={vacancies} candidates={candidates} onUpdate={updateVacancies} />
      )}
...
function PipelineTab({
  ...
  const filtered =
    filterVacancy === 'all' ? candidates : candidates.filter((c) => c.vacancyId === filterVacancy);
                ...
                <div key={c.id} style={cardStyle}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 6 }}>
                    <span style={{ color: 'var(--text-muted)', fontSize: 10 }}>
                      {daysInStage(c.stageEnteredDate)} día
                      {daysInStage(c.stageEnteredDate) !== 1 ? 's' : ''} en etapa
                    </span>
                  ...
                  <div style={{ display: 'flex', gap: 4 }}>
                    {stage.key !== 'publicada' && (
                      <button
                        style={{ ...btnSmall, flex: 1, justifyContent: 'center' }}
        ...
        >
          ...
          <div style={grid2}>
            ...
            <div>
              ...
              <select
                style={selectStyle}
                value={form.estado}
                onChange={(e) => setForm({ ...form, estado: e.target.value as VacancyStatus })}
              >
          ...
          <div style={{ marginTop: 12 }}>
            ...
            <textarea
              ...
              placeholder="Lista los requisitos del cargo..."
            />
          </div>
          <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
            <button style={btnPrimary} onClick={handleSave}>
              {editId ? 'Guardar Cambios' : 'Crear Vacante'}
            </button>
            <button
              style={btnSecondary}
              onClick={() => {
          ...
          >
            ...
            >
              <div>
                ...
                >
                  ...
                  <span>
                    ${fmt(v.salarioMin)} - ${fmt(v.salarioMax)} CLP
                  </span>
                </div>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <button style={btnSmall} onClick={() => handleEdit(v)}>
                  Editar
                </button>
                <button
...
function CandidatosTab({
  ...
  const handleSave = () => {
    ...
    if (editId) {
      onUpdateCandidates(
        candidates.map((c) => (c.id === editId ? { ...c, ...form, cargoPostulado } : c))
      );
    } else {
      onUpdateCandidates([
        ...candidates,
        { ...form, cargoPostulado, id: genId(), createdAt: today },
      ]);
    }
  ...
  const handleEdit = (c: Candidate) => {
    setForm({
      ...
      email: c.email,
      telefono: c.telefono,
      cargoPostulado: c.cargoPostulado,
      ...
      rating: c.rating,
      stage: c.stage,
      stageEnteredDate: c.stageEnteredDate,
      entrevistaFecha: c.entrevistaFecha,
      entrevistaHora: c.entrevistaHora,
      entrevistador: c.entrevistador,
    });
    setEditId(c.id);
    setShowForm(true);
  };
    ...
    <span style={{ cursor: onChange ? 'pointer' : 'default', letterSpacing: 2 }}>
      {[1, 2, 3, 4, 5].map((i) => (
        <span
          key={i}
          onClick={() => onChange?.(i)}
          style={{ color: i <= rating ? '#f59e0b' : 'var(--text-muted)', fontSize: 14 }}
        >
          {i <= rating ? '★' : '☆'}
        </span>
      ))}
    ...
    <div>
      ...
      {showForm && (
        ...
        >
          ...
          <div style={grid2}>
            ...
            <div>
              ...
              <select
                style={selectStyle}
                value={form.fuente}
                onChange={(e) => setForm({ ...form, fuente: e.target.value as CandidateSource })}
              >
                {SOURCES.map((s) => (
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                <button style={btnSmall} onClick={simulateCVUpload}>
                  Subir CV
                </button>
                {form.cvFileName && (
          ...
          >
            <h5 style={{ margin: '0 0 10px', fontSize: 12, color: 'var(--text-primary)' }}>
              Agendar Entrevista
            </h5>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
              <div>
                ...
                <input
                  style={inputStyle}
                  type="date"
                  value={form.entrevistaFecha || ''}
                  onChange={(e) => setForm({ ...form, entrevistaFecha: e.target.value || null })}
                />
              </div>
              <div>
                <label
                ...
                <input
                  style={inputStyle}
                  type="time"
                  value={form.entrevistaHora || ''}
                  onChange={(e) => setForm({ ...form, entrevistaHora: e.target.value || null })}
                />
              </div>
              <div>
                <label
          ...
          <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
            <button style={btnPrimary} onClick={handleSave}>
              {editId ? 'Guardar Cambios' : 'Agregar Candidato'}
            </button>
            <button
              style={btnSecondary}
              onClick={() => {
              ...
              return (
                <React.Fragment key={c.id}>
                  ...
                  >
                    ...
                    <td style={tdStyle}>
                      {c.cvFileName ? (
                        <span style={{ color: '#10b981', fontSize: 11 }}>Adjunto</span>
                      ) : (
                  ...
                  {isExpanded && (
                    <tr>
                      ...
                      >
                        ...
                        >
                          <div>
                            <strong style={{ color: 'var(--text-secondary)', fontSize: 11 }}>
                              Teléfono
                            </strong>
                          ...
                          <div>
                            ...
                            <div style={{ color: 'var(--text-primary)' }}>
                              {daysInStage(c.stageEnteredDate)} días
                            </div>
                          </div>
                          <div>
                            <strong style={{ color: 'var(--text-secondary)', fontSize: 11 }}>
                          ...
                          <div>
                            ...
                            <div style={{ color: 'var(--text-primary)' }}>
                              {c.entrevistaFecha
                                ? `${c.entrevistaFecha} a las ${c.entrevistaHora || '—'}`
                                : 'No agendada'}
                            </div>
                          </div>
                          <div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
        ...
        {metricCard(
          'Total Candidatos',
          totalCandidates,
          `${hired.length} contratados, ${rejected.length} descartados`
        )}
        {metricCard(
      ...
      >
        <h4 style={{ margin: '0 0 14px', fontSize: 14, color: 'var(--text-primary)' }}>
          ...
          <tbody>
            {sourceStats.map((s) => (
              <tr key={s.value} style={{ borderBottom: '1px solid var(--border)' }}>
                ...
                <td style={{ ...tdStyle, textAlign: 'center' }}>
                  <span style={{ color: '#f59e0b' }}>{'★'.repeat(Math.round(+s.avgRating))}</span>{' '}
                  {s.avgRating}
                </td>
              </tr>
            ))}
            ...
            {vacancies.map((v) => {
              ...
              return (
                <tr key={v.id} style={{ borderBottom: '1px solid var(--border)' }}>
                  ...
                  <td
                    style={{ ...tdStyle, textAlign: 'center', color: '#3b82f6', fontWeight: 600 }}
                  >
                    {vInProcess}
                  </td>
                  <td
                    style={{ ...tdStyle, textAlign: 'center', color: '#10b981', fontWeight: 600 }}
                  >
                    {vHired}
                  </td>
                  <td
                    style={{ ...tdStyle, textAlign: 'center', color: '#ef4444', fontWeight: 600 }}
                  >
                    {vRejected}
                  </td>
                </tr>
