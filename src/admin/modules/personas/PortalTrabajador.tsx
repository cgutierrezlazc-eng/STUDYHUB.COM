...
const DOC_TYPE_ICONS: Record<string, string> = {
  contrato: '📄', liquidacion: '💰', finiquito: '🏁', amonestacion: '⚠️',
  certificado: '🏅', ci: '🪪', afp: '🏦', isapre: '🏥', antecedentes: '🔍',
  academico: '🎓', bancario: '💳', riohs: '📋', etica: '⚖️', equipos: '💻', otro: '📁',
}
...
const FESBlock = ({ doc }: { doc: EmployeeDoc }) => (
  <div style={{
    ...
    <div style={{ background: '#15803d', color: '#fff', padding: '5px 12px', fontWeight: 700, letterSpacing: 1 }}>
    ...
    <div style={{ background: '#f0fdf4', padding: '8px 12px', lineHeight: 1.7 }}>
      ...
      <div><strong>Usuario:</strong> {doc.fes_signer_username}</div>
      <div><strong>N° Empleado:</strong> {doc.fes_employee_number}</div>
      <div><strong>Fecha/hora:</strong> {doc.fes_signed_at ? new Date(doc.fes_signed_at).toLocaleString('es-CL', { timeZone: 'America/Santiago' }) + ' CLT' : '—'}</div>
      <div><strong>Hash SHA-256:</strong> <span style={{ fontSize: 10, color: '#555' }}>Registro interno</span></div>
      {doc.fes_verification_code && (
        <div style={{ marginTop: 4 }}>
          <strong>Verificar en:</strong>{' '}
          <a href={`https://conniku.com/verify/${doc.fes_verification_code}`} target="_blank" rel="noopener noreferrer" style={{ color: '#15803d' }}>
...
export default function PortalTrabajador() {
  ...
  const pendingFes = documents.filter(d => !d.fes_signed && !d.locked && d.document_type === 'contrato')
  const allDocs = documents.filter(d => d.document_type !== 'contrato' || d.fes_signed || d.locked)
    ...
    <div style={{ maxWidth: 840, margin: '0 auto', padding: '24px 20px' }}>
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 13, opacity: 0.8 }}>{employee.position} · {employee.department}</div>
      ...
      {pendingFes.length > 0 && (
        <div style={{
          ...
          <p style={{ fontSize: 13, color: '#78350f', marginBottom: 14 }}>
            Lee el documento completo y firma con tu Firma Electrónica Simple (FES) para que el contrato tenga pleno valor legal.
            Una vez firmado, el documento quedará bloqueado y no podrá ser modificado.
          </p>
          {pendingFes.map(doc => (
      ...
      <Section title="📄 Contrato de Trabajo">
        {contractDocs.length === 0 && (
          <EmptyState msg="No hay contratos en tu carpeta aún." />
        )}
        {contractDocs.map(doc => (
          <DocCard key={doc.id} doc={doc} onSign={() => handleFesSign(doc)} signing={signing === doc.id} />
        ))}
      </Section>

      {/* ─── TODOS MIS DOCUMENTOS ───────────────────────────────────── */}
      <Section title="📁 Mis Documentos" action={
        <label style={{
      ...
      <div style={{
        ...
        <p style={{ marginTop: 6, lineHeight: 1.6 }}>
          Los documentos firmados con <strong>Firma Electrónica Simple (FES)</strong> tienen pleno valor legal en Chile
          conforme a la <strong>Ley 19.799</strong>. Una vez firmado, cada documento queda bloqueado con hash SHA-256,
          identificación del firmante, timestamp y código de verificación.
          Todos los documentos son conservados por mínimo 5 años desde el término del contrato.
        </p>
      </div>
...
function EmptyState({ msg }: { msg: string }) {
  return (
    <div style={{ padding: '20px', textAlign: 'center', color: 'var(--text-muted)', fontSize: 13, background: 'var(--bg-secondary)', borderRadius: 8 }}>
      {msg}
    </div>
  )
}

function DocCard({ doc, onSign, signing }: { doc: EmployeeDoc; onSign: () => void; signing: boolean }) {
          ...
          {!doc.locked && !doc.fes_signed && doc.document_type === 'contrato' && (
            <button
              ...
              style={{
                ...
                cursor: signing ? 'default' : 'pointer', fontWeight: 600,
              }}
            >
              {signing ? '…' : '✍️ Firmar'}
            </button>
