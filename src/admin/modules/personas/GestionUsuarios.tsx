      ...
      >
        ...
        <div style={{ marginTop: 14, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <FesBadge
            ...
            signedAt={contracts.find((d) => d.fes_signed)?.fes_signed_at}
          />
          <span
            style={{
              display: 'inline-flex',
      ...
      <div style={{ padding: '20px 24px', flex: 1 }}>
                ...
                {doc.fes_signed && (
                  ...
                  >
                    <div style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.7 }}>
                      ...
                      <div>
                        <strong>Fecha FES:</strong> {fmtDateTime(doc.fes_signed_at)}
                      </div>
        ...
        <Section title="Acciones CEO">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <button onClick={() => onNavigate(`/admin-panel/hr/accesos`)} style={{ ...actionBtn }}>
          ...
          {!contractSigned && (
            ...
            >
              <strong>Pendiente:</strong> El colaborador no ha firmado su contrato. Debe ingresar a
              Conniku con su usuario corporativo y firmar desde <em>Mi Portal</em>.
            </div>
          )}
        </Section>
      </div>
...
function Row({
  ...
  return (
    <div
      style={{ display: 'flex', alignItems: 'flex-start', gap: 8, marginBottom: 7, fontSize: 12 }}
    >
...
function PlatformUserPanel({ usr, onClose }: { usr: PlatformUser; onClose: () => void }) {
  const fullName =
    [usr.firstName || usr.first_name, usr.lastName || usr.last_name].filter(Boolean).join(' ') ||
    usr.username;
      ...
      <div style={{ padding: '20px 24px', flex: 1 }}>
        ...
        <Section title="Acceso a la plataforma">
          <div style={{ fontSize: 12, color: 'var(--text-muted)', lineHeight: 1.7 }}>
            Para gestionar privilegios de módulos administrativos de este usuario (si es
            colaborador), ve a <strong>Control de Accesos</strong> en la sección HR del panel CEO.
          </div>
        </Section>
      </div>
    </div>
...
export default function GestionUsuarios({ onNavigate }: { onNavigate?: (path: string) => void }) {
  ...
  const loadUsers = useCallback(() => {
    ...
    api
      .adminGetUsers(usersPage, searchUsers || undefined, userFilter || undefined)
      .then((d: any) =>
        setPlatformUsers(Array.isArray(d?.users) ? d.users : Array.isArray(d) ? d : [])
      )
  ...
  const filteredEmps = employees.filter((e) =>
    `${e.firstName} ${e.lastName} ${e.position} ${e.email} ${e.employee_number || ''}`
      .toLowerCase()
      .includes(searchEmps.toLowerCase())
  );
  ...
  const empsSigned = employees.filter(
    (e) =>
      ...
      >
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 12 }}>
          <div style={{ textAlign: 'center' }}>
            ...
            <div
              style={{ fontSize: 10, opacity: 0.7, textTransform: 'uppercase', letterSpacing: 1 }}
            >
              Colaboradores
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginTop: 20 }}>
            ...
            <button
              ...
              style={{
                ...
                border: '1px solid var(--border)',
                background: 'var(--bg-secondary)',
                cursor: filteredUsers.length < 20 ? 'not-allowed' : 'pointer',
                fontSize: 12,
                color: 'var(--text-muted)',
                opacity: filteredUsers.length < 20 ? 0.4 : 1,
              }}
            >
              Siguiente →
            </button>
