...
class LocalErcStore {
  ...
  private save() {
    try {
      localStorage.setItem('conniku_erc_v1', JSON.stringify(this.data));
    } catch {}
  }
...
const PERF_DIMS = [
  {
    ...
    desc: 'Exactitud, prolijidad y nivel de excelencia en entregables',
  },
  {
    key: 'knowledge',
    name: 'Conocimiento del Cargo',
    desc: 'Dominio de habilidades, herramientas y procedimientos necesarios',
  },
  {
    key: 'communication',
    name: 'Comunicación',
    desc: 'Claridad, oportunidad y efectividad al comunicarse con el equipo',
  },
  {
    key: 'teamwork',
    name: 'Trabajo en Equipo',
    desc: 'Colaboración, apoyo mutuo y contribución al ambiente laboral',
  },
  {
    key: 'initiative',
    name: 'Iniciativa y Proactividad',
    desc: 'Capacidad de anticipar problemas y proponer soluciones',
  },
  {
    key: 'attendance',
    name: 'Puntualidad y Asistencia',
    desc: 'Cumplimiento de horarios, asistencia regular y disponibilidad',
  },
  {
    key: 'values',
    name: 'Adherencia a Valores',
    desc: 'Vivencia diaria de los valores corporativos de Conniku',
  },
  {
    key: 'results',
    name: 'Logro de Objetivos',
    desc: 'Cumplimiento de KPIs y metas del período evaluado',
  },
  {
    key: 'adaptability',
    name: 'Adaptabilidad',
    desc: 'Respuesta efectiva ante cambios y situaciones inesperadas',
  },
  {
    key: 'ethics',
    name: 'Ética y Probidad',
    desc: 'Integridad, confidencialidad y conducta profesional',
  },
];

const CONV_LIBRARY = [
  {
    title: 'Feedback de Desempeño',
    desc: 'Conversación sobre rendimiento reciente del trabajador',
  },
  ...
  {
    title: 'Retroalimentación de Proyecto',
    desc: 'Revisión de resultados de un proyecto específico',
  },
  {
    title: 'Expectativas del Período',
    desc: 'Alineamiento de expectativas para el próximo período',
  },
...
function getPositionLevel(pos: string): number {
  ...
  if (p.includes('ceo') || p.includes('gerente general') || p.includes('director general'))
    return 0;
...
function getRecordCardStatus(
  empId: string,
  key: RecordCardKey,
  documents: any[]
): { status: TrafficLight; label: string } {
  const disciplines = getForEmployee<DisciplineRecord>('discipline', empId).filter(
    (d) => d.category === key
  );
  ...
  if (disciplines.some((d) => d.level === 'termination'))
    return { status: 'red', label: 'Causal activa' };
  if (disciplines.some((d) => d.level === 'written_2'))
    return { status: 'red', label: 'Escrita 2 activa' };
  if (disciplines.some((d) => d.level === 'written_1'))
    return { status: 'orange', label: 'Escrita 1 activa' };
  if (disciplines.some((d) => d.level === 'verbal'))
    return { status: 'yellow', label: 'Verbal activa' };
  return { status: 'green', label: 'Sin registros' };
}

async function sendDTNotification(
  employee: Employee,
  ...
  try {
    const levelLabel =
      DISC_LEVELS.find((d) => d.key === discipline.level)?.label || discipline.level;
    ...
    /* DT notification is best-effort */
  }
}
...
function MyWorkView({
  ...
  const activeDisc = allDisc.filter((d) =>
    ['written_1', 'written_2', 'termination'].includes(d.level)
  );
      ...
      <div className="mywork-stats-grid">
        {[
          ...
          },
          {
            ...
            label: 'Por Firmar',
            sub: 'Amonestaciones pendientes',
            color: '#f59e0b',
            icon: AlertTriangle,
          },
          {
            value: activeDisc.length,
          ...
          {
            value: recentAck.slice(-30).length,
          ...
          <div key={s.label} className="mywork-stat">
            ...
            >
              <div className="mywork-stat-number" style={{ color: s.color }}>
                {s.value}
              </div>
              ...
              return (
                <div key={d.id} className="mywork-task-row" onClick={() => onSelectEmployee(emp)}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div className="mywork-task-name">
                      {emp.firstName} {emp.lastName}
                    </div>
                    <div className="mywork-task-meta">
                      {lvl?.label} · {fmtDate(d.date)}
                    </div>
                  </div>
              ...
              return (
                <div key={d.id} className="mywork-task-row" onClick={() => onSelectEmployee(emp)}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className="mywork-task-meta">
                      {d.explanation.slice(0, 50)}
                      {d.explanation.length > 50 ? '…' : ''}
                    </div>
                  </div>
                  <span className={`pe-badge ${lvl?.badge}`}>{lvl?.short}</span>
                  <ChevronRight size={14} style={{ color: 'var(--text-muted)', flexShrink: 0 }} />
                      ...
                      {[1, 2, 3, 4, 5].map((n) => (
                        <div
                          ...
                          style={{
                            ...
                            borderRadius: '50%',
                            background:
                              n <= Math.round(r.overallRating) ? 'var(--accent)' : 'var(--border)',
                          }}
                        />
                      ))}
                    </div>
    ...
    <div>
        ...
        {[
          {
            ...
            color: disciplines.length > 0 ? '#ef4444' : 'var(--accent-green)',
            icon: Flag,
          },
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div className="pe-card">
          <div className="pe-card-header">
            <div className="pe-card-title">
              <Briefcase size={14} /> Datos del Cargo
            </div>
          </div>
          {[
            ...
            { icon: Calendar, label: 'Fecha ingreso', val: fmtDate(employee.hireDate) },
            {
              icon: CreditCard,
              label: 'Contrato',
              val: employee.contractType?.replace('_', ' ') || '—',
            },
        ...
        <div className="pe-card">
          <div className="pe-card-header">
            <div className="pe-card-title">
              <Star size={14} /> Última Evaluación
            </div>
          </div>
          {lastReview ? (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                <div
                  style={{
                    ...
                    fontWeight: 900,
                    color:
                      lastReview.overallRating >= 4
                        ? 'var(--accent-green)'
                        : lastReview.overallRating >= 3
                          ? 'var(--accent)'
                          : '#ef4444',
                    letterSpacing: '-0.05em',
                    lineHeight: 1,
                ...
                <div>
                  <div style={{ fontSize: 12, fontWeight: 600 }}>
                    {lastReview.overallRating >= 4.5
                      ? 'Excepcional'
                      : lastReview.overallRating >= 3.5
                        ? 'Destacado'
                        : lastReview.overallRating >= 2.5
                          ? 'Cumple'
                          : lastReview.overallRating >= 1.5
                            ? 'En Desarrollo'
                            : 'Deficiente'}
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    {fmtDate(lastReview.date)}
                  </div>
                </div>
              </div>
                    ...
                    {[1, 2, 3, 4, 5].map((n) => (
                      <div
                        ...
                        style={{
                          ...
                          height: 5,
                          borderRadius: 3,
                          background: n <= d.rating ? 'var(--accent)' : 'var(--border)',
                        }}
                      />
                    ))}
                  </div>
            ...
            <div className="pe-empty">
              <Star size={28} />
              <div className="pe-empty-title">Sin evaluaciones</div>
              <div className="pe-empty-sub">No se han registrado evaluaciones de desempeño</div>
            </div>
          )}
        </div>
        ...
        {activeDisciplines.length > 0 && (
          <div className="pe-card" style={{ borderColor: 'rgba(239,68,68,0.3)' }}>
            <div className="pe-card-header">
              <div className="pe-card-title">
                <ShieldAlert size={14} color="#ef4444" /> Disciplinas Activas
              </div>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                      {d.explanation.slice(0, 60)}
                      {d.explanation.length > 60 ? '…' : ''}
                    </div>
                  </div>
        ...
        <div className="pe-card">
          ...
          {[
            ...disciplines.map((d) => ({
              date: d.date,
              type: 'amonestacion',
              label: DISC_LEVELS.find((l) => l.key === d.level)?.label || d.level,
              icon: Flag,
              color: '#ef4444',
            })),
            ...coaching.map((c) => ({
            ...
            ...reviews.map((r) => ({
              date: r.date,
              type: 'evaluacion',
              label: `Evaluación · ${r.overallRating.toFixed(1)}/5`,
              icon: Star,
              color: '#0891b2',
            })),
          ]
          ...
          {disciplines.length + coaching.length + acks.length + reviews.length === 0 && (
            <div className="pe-empty">
              <Activity size={24} />
              <div className="pe-empty-title">Sin actividad registrada</div>
            </div>
          )}
        </div>
      </div>
...
function TabAmonestaciones({ employee, issuerName }: { employee: Employee; issuerName: string }) {
  ...
  const [form, setForm] = useState({
    level: 'verbal' as DisciplineLevel,
    category: 'disciplina' as RecordCardKey,
    explanation: '',
    incidentDetails: '',
  });
  ...
  const signRecord = async (rec: DisciplineRecord) => {
    ...
    const hash = await crypto.subtle.digest(
      'SHA-256',
      new TextEncoder().encode(imgData + rec.id + Date.now())
    );
  ...
  return (
    <div>
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            <Flag size={14} /> Historial de Amonestaciones ({records.length})
          </div>
            ...
            return (
              <React.Fragment key={lvl.key}>
                <div style={{ flex: 1, textAlign: 'center' }}>
                  ...
                  >
                    {lvl.label.replace('Amonestación ', '')}
                  </div>
                </div>
                {i < DISC_LEVELS.length - 1 && (
                  <div
                    style={{ width: 20, height: 1, background: 'var(--border)', flexShrink: 0 }}
                  />
                )}
              </React.Fragment>
            );
        ...
        {records.length === 0 ? (
          <div className="pe-empty">
            <Flag size={28} />
            <div className="pe-empty-title">Sin amonestaciones</div>
            <div className="pe-empty-sub">El historial disciplinario está limpio</div>
          </div>
        ) : (
          records
            .slice()
              ...
              return (
                <div key={r.id} className="pe-record-item">
                  <div className="pe-record-item-header">
                    <span className={`pe-badge ${lvl?.badge}`}>
                      {lvl?.short} — {lvl?.label}
                    </span>
                    ...
                    {r.acknowledged ? (
                      <span className="dt-sent-badge">
                        <CheckCircle size={10} /> Firmada · DT Notificada
                      </span>
                    ) : (
                      <button
                        className="pe-btn pe-btn-outline"
                        style={{ fontSize: 10, padding: '3px 8px' }}
                        onClick={() => setSigModal(r)}
                      >
                        <Edit3 size={11} /> Obtener firma
                      </button>
                    )}
                  </div>
                  <div className="pe-record-item-body">
                    ...
                    <br />
                    <strong>Detalles:</strong> {r.incidentDetails}
                  </div>
          ...
          <div className="pe-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pe-modal-header">
              <div className="pe-modal-title">
                <Flag size={15} /> Nueva Amonestación — {employee.firstName} {employee.lastName}
              </div>
              <button
                style={{
                  background: 'none',
            ...
            <div className="pe-modal-body">
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    Basado en historial:{' '}
                    <strong>{DISC_LEVELS.find((l) => l.key === suggestedLevel())?.label}</strong>
                  </div>
                </div>
              </div>
              ...
              <div className="pe-form-grid-2" style={{ marginBottom: 14 }}>
                <div>
                  ...
                  <select
                    ...
                    value={form.level}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, level: e.target.value as DisciplineLevel }))
                    }
                  >
                    {DISC_LEVELS.map((l) => (
                ...
                <div>
                  ...
                  <select
                    ...
                    value={form.category}
                    onChange={(e) =>
                      setForm((f) => ({ ...f, category: e.target.value as RecordCardKey }))
                    }
                  >
                    {RECORD_CARDS.map((c) => (
              ...
              <div style={{ marginBottom: 12 }}>
                <label className="pe-form-label">
                  Explicación de la infracción * (requerido para enviar)
                </label>
                <textarea
                  ...
                  placeholder="Describa la conducta o infracción observada..."
                />
              </div>
              <div>
                <label className="pe-form-label">
                  Detalles del incidente * (requerido para enviar)
                </label>
                <textarea
                  className="pe-textarea"
                  rows={3}
                  ...
                  placeholder="Fecha, hora, lugar, testigos, circunstancias..."
                />
              </div>
            </div>
            <div className="pe-modal-footer">
              <button className="pe-btn pe-btn-ghost" onClick={() => setShowForm(false)}>
                Cancelar
              </button>
              <button
                className="pe-btn pe-btn-primary"
                onClick={save}
                disabled={saving || !form.explanation.trim() || !form.incidentDetails.trim()}
              >
                {saving ? (
                  'Guardando…'
                ) : (
                  <>
                    <Send size={13} /> Guardar y enviar a aprobación
                  </>
                )}
              </button>
            </div>
      ...
      {sigModal && (
        ...
        >
          <div className="pe-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pe-modal-header">
              <div className="pe-modal-title">
                <Edit3 size={15} /> Firma del Trabajador
              </div>
              <button
                style={{
                  background: 'none',
            ...
            <div className="pe-modal-body">
              ...
              >
                Al firmar, el trabajador{' '}
                <strong>
                  {employee.firstName} {employee.lastName}
                </strong>{' '}
                acusa recibo de la{' '}
                <strong>{DISC_LEVELS.find((l) => l.key === sigModal.level)?.label}</strong>. El
                sistema enviará copia a su email y notificará a la Dirección del Trabajo.
              </div>
              <label className="pe-form-label" style={{ marginBottom: 8 }}>
                Firma del trabajador (dibujar con mouse/touch)
              </label>
              <div className={`firma-canvas-wrapper ${hasSig ? 'signed' : ''}`}>
                ...
                />
                <button className="firma-canvas-clear" onClick={clearCanvas}>
                  Limpiar
                </button>
              </div>
...
function TabCoaching({ employee, issuerName }: { employee: Employee; issuerName: string }) {
  ...
  useEffect(
    () => setRecords(getForEmployee<CoachingRecord>('coaching', employee.id)),
    [employee.id]
  );
  ...
  return (
    <div>
      <div className="pe-card">
        ...
        >
          El coaching es retroalimentación de desarrollo.{' '}
          <strong>No constituye disciplina y no requiere aprobación.</strong> Queda en el historial
          del trabajador como registro positivo de acompañamiento.
        </div>
        <div className="pe-card-header">
          <div className="pe-card-title">
            <MessageSquare size={14} /> Sesiones de Coaching ({records.length})
          </div>
          <button className="pe-btn pe-btn-primary" onClick={() => setShowForm(true)}>
        ...
        {records.length === 0 ? (
          <div className="pe-empty">
            ...
            <div className="pe-empty-title">Sin sesiones de coaching</div>
            <div className="pe-empty-sub">
              Registra conversaciones de desarrollo y acompañamiento
            </div>
          </div>
        ) : (
          records
            ...
            .map((r) => (
              <div key={r.id} className="pe-record-item">
                ...
                {r.actionItems && (
                  ...
                  >
                    <strong>Compromisos:</strong> {r.actionItems}
                  </div>
                )}
                <div className="pe-record-item-footer">
                  <span>Por: {r.issuedBy}</span>
                </div>
              </div>
            ))
        )}
      ...
      {showForm && (
        <div className="pe-modal-overlay" onClick={() => setShowForm(false)}>
          <div className="pe-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pe-modal-header">
              <div className="pe-modal-title">
                <MessageSquare size={15} /> Nueva Sesión de Coaching
              </div>
              <button
                style={{
                  background: 'none',
            ...
            <div className="pe-modal-body">
              <div style={{ marginBottom: 12 }}>
                ...
                <input
                  ...
                  placeholder="Ej: Comunicación con el equipo, Gestión del tiempo..."
                />
              </div>
              <div style={{ marginBottom: 12 }}>
                ...
                <textarea
                  ...
                  placeholder="Resumen de la sesión, contexto y observaciones..."
                />
              </div>
              <div>
                ...
                <textarea
                  ...
                  placeholder="Acciones acordadas con el trabajador..."
                />
              </div>
            </div>
            <div className="pe-modal-footer">
              ...
              >
                {saving ? (
                  ...
                  <>
                    <CheckCircle size={13} /> Guardar sesión
                  </>
                )}
              </button>
            </div>
...
function TabEvaluaciones({ employee, issuerName }: { employee: Employee; issuerName: string }) {
  ...
  const [showForm, setShowForm] = useState(false);
  const [reviewType, setReviewType] = useState<'mid_contract' | 'end_contract' | 'additional'>(
    'additional'
  );
  const [dims, setDims] = useState<Record<string, number>>(
    Object.fromEntries(PERF_DIMS.map((d) => [d.key, 0]))
  );
  ...
  useEffect(
    () => setRecords(getForEmployee<PerformanceReview>('reviews', employee.id)),
    [employee.id]
  );
  ...
  return (
    <div>
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            <Star size={14} /> Evaluaciones de Desempeño ({records.length})
          </div>
          <button className="pe-btn pe-btn-primary" onClick={() => setShowForm(true)}>
        ...
        {records.length === 0 ? (
          <div className="pe-empty">
            ...
            <div className="pe-empty-title">Sin evaluaciones</div>
            <div className="pe-empty-sub">
              El sistema programa automáticamente la evaluación a mitad y fin de contrato
            </div>
          </div>
        ) : (
          records
            ...
            .map((r) => {
              ...
              return (
                <div key={r.id} className="pe-record-item">
                  <div
                    className="pe-record-item-header"
                    ...
                    <div style={{ marginTop: 12 }}>
                      ...
                      {r.strengths && (
                        <div style={{ marginBottom: 8, fontSize: 12 }}>
                          <strong>Fortalezas:</strong> {r.strengths}
                        </div>
                      )}
                      {r.areasToImprove && (
                        <div style={{ marginBottom: 8, fontSize: 12 }}>
                          <strong>Áreas de mejora:</strong> {r.areasToImprove}
                        </div>
                      )}
                      {r.comments && (
                        <div style={{ fontSize: 12 }}>
                          <strong>Comentarios:</strong> {r.comments}
                        </div>
                      )}
                      <div className="pe-record-item-footer">
                        <span>Por: {r.issuedBy}</span>
      ...
      {showForm && (
        <div className="pe-modal-overlay" onClick={() => setShowForm(false)}>
          <div className="pe-modal" style={{ maxWidth: 620 }} onClick={(e) => e.stopPropagation()}>
            <div className="pe-modal-header">
              <div className="pe-modal-title">
                <Star size={15} /> Evaluación de Desempeño — {employee.firstName}{' '}
                {employee.lastName}
              </div>
              <button
                style={{
            ...
            <div className="pe-modal-body">
              ...
              <label className="pe-form-label" style={{ marginBottom: 10 }}>
                Dimensiones de evaluación (1 = Deficiente · 5 = Excepcional)
              </label>
              {PERF_DIMS.map((d) => (
                <div key={d.key} className="dim-row" title={d.desc}>
                  ...
                  <div className="dim-stars">
                    {[1, 2, 3, 4, 5].map((n) => (
                      <button
                        key={n}
                        className={`dim-star ${dims[d.key] >= n ? 'filled' : ''}`}
                        onClick={() => setDims((prev) => ({ ...prev, [d.key]: n }))}
                      >
                        {n}
                      </button>
                    ))}
                ...
                <div style={{ textAlign: 'center', padding: '14px 0 6px' }}>
                  <span
                    style={{
                      fontSize: 32,
                      fontWeight: 900,
                      color: ratingLabel(overall()).color,
                      letterSpacing: '-0.04em',
                    }}
                  >
                    {overall().toFixed(1)}
              ...
              <div className="pe-form-grid-2" style={{ marginTop: 14 }}>
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Aspectos positivos del período..."
                  />
                </div>
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Oportunidades de desarrollo..."
                  />
                </div>
              </div>
              <div style={{ marginTop: 12 }}>
                ...
                <textarea
                  ...
                  placeholder="Contexto, situaciones especiales, acuerdos..."
                />
              </div>
            </div>
            <div className="pe-modal-footer">
              ...
              >
                {saving ? (
                  ...
                  <>
                    <Send size={13} /> Emitir evaluación
                  </>
                )}
              </button>
            </div>
...
function TabConversaciones({ employee, issuerName }: { employee: Employee; issuerName: string }) {
  ...
  useEffect(
    () => setRecords(getForEmployee<ConversationRecord>('conversations', employee.id)),
    [employee.id]
  );
  ...
  const save = async () => {
    ...
    const rec: ConversationRecord = {
      ...
      title,
      description:
        mode === 'library' ? CONV_LIBRARY.find((c) => c.title === selected)?.desc || '' : adhocDesc,
      date: new Date().toISOString().split('T')[0],
      issuedBy: issuerName,
      acknowledged: false,
    };
  ...
  return (
    <div>
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            <MessageSquare size={14} /> Conversaciones Registradas ({records.length})
          </div>
          <button className="pe-btn pe-btn-primary" onClick={() => setShowForm(true)}>
        ...
        {records.length === 0 ? (
          <div className="pe-empty">
            <MessageSquare size={28} />
            <div className="pe-empty-title">Sin conversaciones</div>
            <div className="pe-empty-sub">Registra 1:1s, feedback y conversaciones formales</div>
          </div>
        ) : (
          records
            .slice()
            ...
            .map((r) => (
              <div key={r.id} className="pe-record-item">
                <div className="pe-record-item-header">
                  <span
                    className={`pe-badge ${r.type === 'library' ? 'pe-badge-blue' : 'pe-badge-gray'}`}
                  >
                    {r.type === 'library' ? 'Librería' : 'Ad-hoc'}
                  </span>
      ...
      {showForm && (
        <div className="pe-modal-overlay" onClick={() => setShowForm(false)}>
          <div className="pe-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pe-modal-header">
              <div className="pe-modal-title">
                <MessageSquare size={15} /> Registrar Conversación
              </div>
              <button
                style={{
                  background: 'none',
            ...
            <div className="pe-modal-body">
              <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
                {(['library', 'adhoc'] as const).map((m) => (
                  <button
                    key={m}
                    className={`pe-btn ${mode === m ? 'pe-btn-primary' : 'pe-btn-ghost'}`}
                    onClick={() => setMode(m)}
                  >
                  ...
                  {CONV_LIBRARY.map((c) => (
                    <div
                      ...
                      style={{
                        padding: '10px 12px',
                        borderRadius: 8,
                        border: `1px solid ${selected === c.title ? 'var(--accent)' : 'var(--border)'}`,
                        cursor: 'pointer',
                        background: selected === c.title ? 'rgba(37,99,235,0.06)' : 'transparent',
                ...
                <>
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <input
                      ...
                      placeholder="Asunto o tema de la conversación..."
                    />
                  </div>
                  <div>
            ...
            <div className="pe-modal-footer">
              ...
              <button
                className="pe-btn pe-btn-primary"
                onClick={save}
                disabled={saving || (!selected && !adhocTitle)}
              >
                {saving ? (
                  'Guardando…'
                ) : (
...
function TabReconocimientos({ employee, issuerName }: { employee: Employee; issuerName: string }) {
  ...
  useEffect(
    () => setRecords(getForEmployee<AcknowledgementRecord>('acknowledgements', employee.id)),
    [employee.id]
  );
  ...
  return (
    <div>
      <div className="pe-card">
        ...
        >
          Los reconocimientos deben emitirse <strong>en el momento</strong> en que ocurre el logro.
          Quedan registrados en el expediente del trabajador como evidencia positiva de su
          desempeño.
        </div>
        <div className="pe-card-header">
          <div className="pe-card-title">
            <Award size={14} /> Reconocimientos ({records.length})
          </div>
          <button
            className="pe-btn pe-btn-outline"
          ...
          >
            <Award size={13} /> Emitir ahora
          </button>
        </div>
        {records.length === 0 ? (
          <div className="pe-empty">
            <Award size={28} />
            <div className="pe-empty-title">Sin reconocimientos emitidos</div>
            <div className="pe-empty-sub">El historial de logros está vacío</div>
          </div>
        ) : (
          records
            .slice()
            ...
            .map((r) => (
              <div key={r.id} className="pe-record-item">
                ...
                <div className="pe-record-item-body">{r.description}</div>
                <div className="pe-record-item-footer">
                  <span>Emitido por: {r.issuedBy}</span>
                </div>
              </div>
            ))
        )}
      ...
      {showForm && (
        <div className="pe-modal-overlay" onClick={() => setShowForm(false)}>
          <div className="pe-modal" onClick={(e) => e.stopPropagation()}>
            <div className="pe-modal-header">
              <div className="pe-modal-title">
                <Award size={15} /> Emitir Reconocimiento
              </div>
              <button
                style={{
                  background: 'none',
            ...
            <div className="pe-modal-body">
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {ACK_LIBRARY.map((a) => (
                  <div
                    key={a.title}
                    onClick={() => setSelected(a.title)}
                    style={{
                      padding: '10px 12px',
                      borderRadius: 8,
                      border: `1px solid ${selected === a.title ? '#f59e0b' : 'var(--border)'}`,
                      cursor: 'pointer',
                      background: selected === a.title ? 'rgba(245,158,11,0.07)' : 'transparent',
                      display: 'flex',
                      alignItems: 'center',
                      gap: 10,
                      transition: 'all 0.15s',
                  ...
                  >
                    <Award
                      size={16}
                      color={selected === a.title ? '#f59e0b' : 'var(--text-muted)'}
                    />
                    <div>
            ...
            <div className="pe-modal-footer">
              ...
              >
                {saving ? (
                  ...
                  <>
                    <Award size={13} /> Emitir reconocimiento
                  </>
                )}
              </button>
            </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            <FileText size={14} /> Progresión Contractual Conniku
          </div>
          <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
            Código del Trabajo Art. 159 N°4
          </span>
        </div>
        ...
        {stage < 3 && (
          ...
          >
            Próximo hito:{' '}
            {stage === 1
              ? 'Segunda renovación = Contrato Indefinido automático'
              : 'Este contrato debe convertirse en Indefinido'}
          </div>
        )}
      </div>
      ...
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            <Briefcase size={14} /> Condiciones Vigentes
          </div>
        ...
        <div
          style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', marginTop: 4 }}
        >
          {[
            [
              'Tipo',
            ...
            [
              'Salud',
              employee.healthSystem === 'fonasa' ? 'Fonasa' : `Isapre ${employee.isapreName || ''}`,
            ],
            ['Colación', employee.colacion ? `$${employee.colacion.toLocaleString('es-CL')}` : '—'],
            [
              'Movilización',
              employee.movilizacion ? `$${employee.movilizacion.toLocaleString('es-CL')}` : '—',
            ],
      ...
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            <FolderOpen size={14} /> Contratos Firmados ({contractDocs.length})
          </div>
        </div>
        {contractDocs.length === 0 ? (
          <div className="pe-empty">
            ...
            <div className="pe-empty-sub">
              Genera el contrato y fírmalo para que quede registrado aquí
            </div>
          </div>
        ) : (
            ...
            <div key={d.id} className="pe-record-item">
              <div className="pe-record-item-header">
                ...
                <span style={{ fontWeight: 600, fontSize: 13, marginLeft: 6 }}>
                  {d.name || d.documentType || 'Contrato'}
                </span>
                <span style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text-muted)' }}>
                  {fmtDate(d.uploadedAt || d.createdAt)}
                </span>
                {d.signed && (
                  <span className="dt-sent-badge">
...
function TabOnboarding({ employee }: { employee: Employee }) {
  ...
  const [data, setData] = useState<{ ingreso: OBProcess | null; salida: OBProcess | null }>(() => {
    try {
      return (
        JSON.parse(localStorage.getItem(storageKey) || 'null') || { ingreso: null, salida: null }
      );
    } catch {
      return { ingreso: null, salida: null };
    }
  });
  ...
  const initProcess = (type: 'ingreso' | 'salida') => {
    ...
    save({
      ...data,
      [type]: { items, createdAt: new Date().toISOString().split('T')[0], status: 'en_curso' },
    });
  };
  ...
  const toggleItem = (type: 'ingreso' | 'salida', id: string) => {
    ...
    if (!proc) return;
    const items = proc.items.map((i) =>
      i.id === id
        ? {
            ...i,
            done: !i.done,
            doneAt: !i.done ? new Date().toISOString().split('T')[0] : undefined,
          }
        : i
    );
  ...
  const pct = proc
    ? Math.round((proc.items.filter((i) => i.done).length / proc.items.length) * 100)
    : 0;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        {(['ingreso', 'salida'] as const).map((m) => (
          <button
            key={m}
            className={`pe-btn ${mode === m ? 'pe-btn-primary' : 'pe-btn-outline'}`}
            onClick={() => setMode(m)}
          >
            {m === 'ingreso' ? (
              <>
                <UserCheck size={13} /> Onboarding Ingreso
              </>
            ) : (
              <>
                <X size={13} /> Offboarding Salida
              </>
            )}
          </button>
        ))}
      ...
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            {mode === 'ingreso' ? (
              <>
                <UserCheck size={14} /> Checklist de Ingreso
              </>
            ) : (
              <>
                <X size={14} /> Checklist de Salida
              </>
            )}
          </div>
          {!proc && (
          ...
          {proc && (
            <span
              style={{
                ...
                padding: '3px 8px',
                borderRadius: 6,
                background: pct === 100 ? 'rgba(34,197,94,0.1)' : 'rgba(245,158,11,0.1)',
              }}
            >
        ...
        {!proc ? (
          <div className="pe-empty">
            ...
            <div className="pe-empty-title">
              Sin proceso {mode === 'ingreso' ? 'de ingreso' : 'de salida'} iniciado
            </div>
            <div className="pe-empty-sub">
              Inicia el proceso para activar el checklist con{' '}
              {mode === 'ingreso' ? OB_ITEMS_INGRESO.length : OB_ITEMS_SALIDA.length} tareas
            </div>
          </div>
        ) : (
          <>
            ...
            <div style={{ margin: '8px 0 16px' }}>
              ...
              >
                <span>
                  {proc.items.filter((i) => i.done).length} / {proc.items.length} tareas
                </span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {proc.items.map((item) => (
                <div
                  key={item.id}
                  onClick={() => toggleItem(mode, item.id)}
                  style={{
                    display: 'flex',
                ...
                >
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 2 }}>
                      ...
                      {item.doneAt ? ` · Completado: ${fmtDate(item.doneAt)}` : ''}
                    </div>
                  </div>
                </div>
              ))}
...
const CAUSALES_ART159 = [
  ...
  {
    value: 'termino_plazo',
    label: 'Vencimiento plazo fijo (Art. 159 N°4)',
    indemnizacion: false,
    aviso: false,
  },
  {
    value: 'caso_fortuito',
    label: 'Caso fortuito o fuerza mayor (Art. 159 N°6)',
    indemnizacion: false,
    aviso: false,
  },
];
...
const CAUSALES_ART160 = [
  {
    value: 'falta_probidad',
    label: 'Falta de probidad o conducta inmoral (Art. 160 N°1)',
    indemnizacion: false,
    aviso: false,
  },
  {
    value: 'acoso_laboral',
    label: 'Acoso laboral o sexual (Art. 160 N°1 b/c)',
    indemnizacion: false,
    aviso: false,
  },
  {
    value: 'vias_de_hecho',
    label: 'Vías de hecho — agresión (Art. 160 N°2)',
    indemnizacion: false,
    aviso: false,
  },
  {
    value: 'injurias',
    label: 'Injurias al empleador (Art. 160 N°3)',
    indemnizacion: false,
    aviso: false,
  },
  {
    value: 'abandono',
    label: 'Abandono de trabajo (Art. 160 N°4)',
    indemnizacion: false,
    aviso: false,
  },
  {
    value: 'actos_contra',
    label: 'Actos contra los intereses del empleador (Art. 160 N°5)',
    indemnizacion: false,
    aviso: false,
  },
  {
    value: 'incumplimiento',
    label: 'Incumplimiento grave de obligaciones (Art. 160 N°7)',
    indemnizacion: false,
    aviso: false,
  },
];
...
const CAUSALES_ART161 = [
  {
    ...
    label: 'Necesidades de la empresa (Art. 161 inc. 1°)',
    indemnizacion: true,
    aviso: true,
    diasAviso: 30,
  },
  {
    value: 'desahucio',
    label: 'Desahucio — cargo de exclusiva confianza (Art. 161 inc. 2°)',
    indemnizacion: true,
    aviso: true,
    diasAviso: 30,
  },
...
function calcIndemnizacion(
  ...
  hireDate: string,
  exitDate: string
): { years: number; months: number; base: number; capped: number; total: number } {
  ...
  return {
    years: wholeYears,
    months: Math.round((years - wholeYears) * 12),
    base,
    capped: base,
    total: base * countableYears,
  };
}
...
function TabDesvinculacion({ employee, issuerName }: { employee: Employee; issuerName: string }) {
  ...
  const [fechaTermino, setFechaTermino] = useState(
    saved?.fechaTermino || new Date().toISOString().split('T')[0]
  );
  ...
  const indData = selectedCausal?.indemnizacion
    ? calcIndemnizacion(employee.grossSalary, employee.hireDate, fechaTermino)
    : null;
  ...
  const saveData = () => {
    const d = {
      ...
      notas,
      dtNotified: dtSent,
      dtNotifiedAt: saved?.dtNotifiedAt,
    };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      >
        ...
        <span>
          El proceso de desvinculación tiene efectos legales definitivos. Todos los pasos deben
          ejecutarse conforme al Código del Trabajo. Ante dudas, consultar con abogado laboral.
        </span>
      </div>
      ...
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            <AlertCircle size={14} /> Causal de Término
          </div>
        </div>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <select
            className="pe-select"
            value={CAUSALES_ART159.some((c) => c.value === causal) ? causal : ''}
            onChange={(e) => setCausal(e.target.value)}
          >
            <option value="">— Seleccionar —</option>
            {CAUSALES_ART159.map((c) => (
        ...
        <div style={{ marginBottom: 12 }}>
          <label className="pe-form-label">
            Artículo 160 — Causal imputable al trabajador (despido)
          </label>
          <select
            className="pe-select"
            value={CAUSALES_ART160.some((c) => c.value === causal) ? causal : ''}
            onChange={(e) => setCausal(e.target.value)}
          >
            <option value="">— Seleccionar —</option>
            {CAUSALES_ART160.map((c) => (
              <option key={c.value} value={c.value}>
                {c.label}
              </option>
            ))}
        ...
        <div style={{ marginBottom: 16 }}>
          <label className="pe-form-label">
            Artículo 161 — Necesidades de la empresa / desahucio
          </label>
          <select
            className="pe-select"
            value={CAUSALES_ART161.some((c) => c.value === causal) ? causal : ''}
            onChange={(e) => setCausal(e.target.value)}
          >
            <option value="">— Seleccionar —</option>
            {CAUSALES_ART161.map((c) => (
              <option key={c.value} value={c.value}>
                {c.label}
              </option>
            ))}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 12 }}>
          ...
          <div>
            ...
            <input
              ...
              placeholder="Descripción breve del fundamento..."
            />
          </div>
        </div>
        <button
      ...
      {selectedCausal && (
        <div className="pe-card">
          <div className="pe-card-header">
            <div className="pe-card-title">
              <BarChart3 size={14} /> Cálculos Referenciales
            </div>
            <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>
              Valores referenciales — verificar con liquidación oficial
            </span>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 4 }}>
            ...
            >
              ...
              <div style={{ fontSize: 14, fontWeight: 700 }}>
                {Math.floor(calcYearsService(employee.hireDate, fechaTermino))} años{' '}
                {Math.round((calcYearsService(employee.hireDate, fechaTermino) % 1) * 12)} meses
              </div>
            </div>
            <div
              style={{
            ...
            >
              ...
              <div
                style={{ fontSize: 13, fontWeight: 700, color: hasAviso ? '#f59e0b' : '#22c55e' }}
              >
            ...
            <div
              style={{
                padding: '10px 12px',
                borderRadius: 8,
                background: indData ? 'rgba(239,68,68,0.04)' : 'var(--bg-secondary)',
                border: `1px solid ${indData ? 'rgba(239,68,68,0.2)' : 'var(--border)'}`,
              }}
            >
              <div style={{ fontSize: 10, color: 'var(--text-muted)', marginBottom: 4 }}>
              ...
              {indData ? (
                <div style={{ fontSize: 14, fontWeight: 700, color: '#ef4444' }}>
                  ${indData.total.toLocaleString('es-CL')}{' '}
                  <span style={{ fontSize: 10, fontWeight: 400 }}>(ref.)</span>
                </div>
              ) : (
                <div style={{ fontSize: 13, color: 'var(--text-muted)' }}>
                  No corresponde con esta causal
                </div>
              )}
            </div>
            <div
            ...
            >
              ...
              <div style={{ fontSize: 13, fontWeight: 600 }}>
                90 UF = ${(90 * CHILE_LABOR.UF.value).toLocaleString('es-CL')}
              </div>
          ...
          {indData && (
            ...
            >
              Base de cálculo: min(${employee.grossSalary.toLocaleString('es-CL')}, $
              {(90 * CHILE_LABOR.UF.value).toLocaleString('es-CL')}) ×{' '}
              {Math.floor(calcYearsService(employee.hireDate, fechaTermino)) +
                (calcYearsService(employee.hireDate, fechaTermino) % 1 >= 0.5 ? 1 : 0)}{' '}
              años = ${indData.total.toLocaleString('es-CL')}
            </div>
          )}
        </div>
      )}
      ...
      <div className="pe-card">
        <div className="pe-card-header">
          <div className="pe-card-title">
            <Send size={14} /> Notificación Dirección del Trabajo
          </div>
          {dtSent && (
            <span className="dt-sent-badge">
        ...
        <div style={{ fontSize: 12, color: 'var(--text-secondary)', marginBottom: 12 }}>
          Se enviará notificación formal a <strong>inspeccion@dt.gob.cl</strong> con datos del
          término de contrato. Recomendado para Art. 160 y Art. 161.
        </div>
        {saved?.dtNotifiedAt && (
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 8 }}>
            Notificado: {fmtDate(saved.dtNotifiedAt.split('T')[0])}
          </div>
        )}
        <button
          className="pe-btn pe-btn-outline"
          style={{
            borderColor: dtSent ? '#22c55e' : '#ef4444',
            color: dtSent ? '#15803d' : '#b91c1c',
          }}
          onClick={notifyDT}
        ...
        >
          {dtSending ? (
            ...
            <>
              <CheckCircle size={13} /> DT Notificada
            </>
          ) : (
            <>
              <Send size={13} /> Notificar DT
            </>
          )}
        </button>
      </div>
...
function TabVistaTrabajador({ employee, documents }: { employee: Employee; documents: any[] }) {
  ...
  const acks = getForEmployee<AcknowledgementRecord>('acknowledgements', employee.id);
  const convs = getForEmployee<ConversationRecord>('conversations', employee.id);
  const discs = getForEmployee<DisciplineRecord>('discipline', employee.id).filter(
    (d) => !['verbal'].includes(d.level)
  );
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      >
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
          Vista como la verá el trabajador (solo lectura)
        </div>
      ...
      {/* Mi contrato */}
      <div className="pe-card">
        <div className="pe-card-title" style={{ marginBottom: 12 }}>
          <FileText size={14} /> Mi Contrato
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {[
            [
              'Tipo de contrato',
              employee.contractType === 'indefinido' ? 'Indefinido' : 'Plazo Fijo',
            ],
            ['Fecha de ingreso', fmtDate(employee.hireDate)],
            ['Cargo', employee.position],
      ...
      {lastEval && (
        <div className="pe-card">
          <div className="pe-card-title" style={{ marginBottom: 12 }}>
            <Star size={14} /> Mi Última Evaluación
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 8 }}>
            <div style={{ textAlign: 'center' }}>
              <div
                style={{
                  ...
                  fontWeight: 800,
                  color:
                    lastEval.overallRating >= 4
                      ? '#22c55e'
                      : lastEval.overallRating >= 3
                        ? '#f59e0b'
                        : '#ef4444',
                }}
              >
                {lastEval.overallRating.toFixed(1)}
              </div>
            ...
            <div style={{ flex: 1 }}>
              {lastEval.dimensions.map((d) => (
                ...
                >
                  <span style={{ fontSize: 10, width: 140, color: 'var(--text-secondary)' }}>
                    {d.name}
                  </span>
                  <div
                    style={{ flex: 1, height: 5, borderRadius: 3, background: 'var(--bg-primary)' }}
                  >
                    <div
                      style={{
                        width: `${(d.rating / 5) * 100}%`,
                        height: '100%',
                        borderRadius: 3,
                        background:
                          d.rating >= 4 ? '#22c55e' : d.rating >= 3 ? '#f59e0b' : '#ef4444',
                      }}
                    />
                  </div>
          ...
          {lastEval.strengths && (
            ...
            >
              <strong>Fortalezas:</strong> {lastEval.strengths}
            </div>
          )}
          {lastEval.areasToImprove && (
            <div
              style={{
                fontSize: 12,
                padding: '8px 10px',
            ...
            >
              <strong>Oportunidades de mejora:</strong> {lastEval.areasToImprove}
            </div>
          )}
        </div>
      )}
      ...
      {acks.length > 0 && (
        <div className="pe-card">
          <div className="pe-card-title" style={{ marginBottom: 12 }}>
            <Award size={14} /> Mis Reconocimientos ({acks.length})
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {acks
              .slice()
      ...
      {convs.length > 0 && (
        <div className="pe-card">
          <div className="pe-card-title" style={{ marginBottom: 12 }}>
            <MessageSquare size={14} /> Conversaciones Formales ({convs.length})
          </div>
          {convs
            .slice()
            .reverse()
      ...
      {discs.length > 0 && (
        <div className="pe-card">
          <div className="pe-card-title" style={{ marginBottom: 12 }}>
            <AlertTriangle size={14} color="#ef4444" /> Medidas Disciplinarias Activas
          </div>
          {discs
            .slice()
            .reverse()
            .map((d) => (
              <div key={d.id} className="pe-record-item">
                <div className="pe-record-item-header">
                  <span
                    className={`pe-badge ${DISC_LEVELS.find((l) => l.key === d.level)?.badge || 'pe-badge-red'}`}
                  >
                    {DISC_LEVELS.find((l) => l.key === d.level)?.label}
                  </span>
      ...
      {!lastEval && acks.length === 0 && convs.length === 0 && (
        <div className="pe-empty" style={{ marginTop: 20 }}>
          ...
          <div className="pe-empty-sub">
            Aún no hay evaluaciones, reconocimientos ni conversaciones registradas
          </div>
        </div>
      )}
    </div>
...
function ExpedienteDigital({
  ...
  const cardStatuses = RECORD_CARDS.map((rc) => ({
    ...rc,
    ...getRecordCardStatus(employee.id, rc.key, documents),
  }));
  ...
  const TABS: {
    ...
    { id: 'resumen', label: 'Resumen', icon: Activity },
    {
      ...
      label: 'Amonestaciones',
      icon: Flag,
      count: getForEmployee<DisciplineRecord>('discipline', employee.id).length,
    },
    {
      id: 'coaching',
      label: 'Coaching',
      icon: MessageSquare,
      count: getForEmployee<CoachingRecord>('coaching', employee.id).length,
    },
    {
      id: 'evaluaciones',
      label: 'Evaluaciones',
      icon: Star,
      count: getForEmployee<PerformanceReview>('reviews', employee.id).length,
    },
    {
      id: 'conversaciones',
      label: 'Conversaciones',
      icon: MessageSquare,
      count: getForEmployee<ConversationRecord>('conversations', employee.id).length,
    },
    {
      id: 'reconocimientos',
      label: 'Reconocimientos',
      icon: Award,
      count: getForEmployee<AcknowledgementRecord>('acknowledgements', employee.id).length,
    },
    ...
    <div className="expediente-shell">
      ...
      <div className="expediente-header">
        <div className="expediente-worker-row">
          ...
          <div style={{ flex: 1 }}>
            <div className="expediente-name">
              {employee.firstName} {employee.lastName}
            </div>
            <div className="expediente-meta-row">
              <span className="expediente-meta-item">
                <Briefcase size={12} />
                {employee.position}
              </span>
              <span className="expediente-meta-item">
              ...
              <span className="expediente-meta-item">
                <Calendar size={12} />
                Desde {fmtDate(employee.hireDate)}
              </span>
              <span
                className="expediente-status-badge"
                style={{
          ...
          {cardStatuses.map((rc) => {
            ...
            return (
              ...
              >
                ...
                <div className="record-card-name" style={{ color: t.text }}>
                  {rc.label}
                </div>
                <div className="record-card-state" style={{ color: t.text }}>
                  {rc.label}
                </div>
              </div>
            );
      ...
      <div className="expediente-body">
        ...
        {tab === 'amonestaciones' && (
          <TabAmonestaciones employee={employee} issuerName={issuerName} />
        )}
        ...
        {tab === 'conversaciones' && (
          <TabConversaciones employee={employee} issuerName={issuerName} />
        )}
        {tab === 'reconocimientos' && (
          <TabReconocimientos employee={employee} issuerName={issuerName} />
        )}
        ...
        {tab === 'documentos_tab' && (
          <div className="pe-card">
            <div className="pe-card-header">
              <div className="pe-card-title">
                <FolderOpen size={14} /> Documentos ({documents.length})
              </div>
            </div>
            {documents.length === 0 ? (
              <div className="pe-empty">
                <FolderOpen size={28} />
                <div className="pe-empty-title">Sin documentos</div>
              </div>
            ) : (
              documents.map((d: any) => (
                <div key={d.id} className="pe-record-item">
        ...
        {tab === 'desvinculacion' && (
          <TabDesvinculacion employee={employee} issuerName={issuerName} />
        )}
        {tab === 'vista_trabajador' && (
          <TabVistaTrabajador employee={employee} documents={documents} />
        )}
      </div>
    </div>
  );
...
export default function PersonasHub() {
  ...
  useEffect(() => {
    const init = async () => {
      setLoading(true);
      try {
        const [emps] = await Promise.all([api.getEmployees(), globalErcStore.loadAll()]);
        setEmployees(emps || []);
      } catch {
        setEmployees([]);
      }
        ...
        <div className="personas-sidebar">
          <div className="personas-sidebar-header">
            <div className="personas-module-badge">
              <div className="personas-module-icon">
  ...
  const filtered = sorted.filter((e) =>
    `${e.firstName} ${e.lastName} ${e.rut} ${e.position} ${e.department}`
      .toLowerCase()
      ...
      <div className="personas-sidebar">
        ...
        <div className="personas-workers-list">
          {grouped.length === 0 && (
            <div className="pe-empty" style={{ padding: '24px 12px' }}>
              <Users size={24} />
              <div className="pe-empty-title" style={{ fontSize: 12 }}>
                Sin colaboradores
              </div>
            </div>
          )}
            ...
            <div key={label}>
              ...
              {emps.map((emp) => {
                ...
                return (
                  ...
                  >
                    <div
                      className="personas-worker-avatar"
                      style={{
                        background: isSelected ? 'rgba(255,255,255,0.2)' : `${color}20`,
                        color: isSelected ? '#fff' : color,
                        border: `1.5px solid ${isSelected ? 'rgba(255,255,255,0.25)' : color + '40'}`,
                      }}
                    >
                      {initials}
                    </div>
                    <div className="personas-worker-info">
                      <div className="personas-worker-name">
                        {emp.firstName} {emp.lastName}
                      </div>
                      <div className="personas-worker-pos">{emp.position}</div>
                    </div>
                    <div
                      className="personas-online-dot"
                      style={{
                        background: emp.status === 'active' ? '#22c55e' : '#ef4444',
                        boxShadow: `0 0 5px ${emp.status === 'active' ? 'rgba(34,197,94,0.4)' : 'rgba(239,68,68,0.4)'}`,
                      }}
                    />
                  </div>
                );
        ...
        {user?.role === 'owner' && (
          <div style={{ borderTop: '1px solid var(--border)', padding: '8px 10px' }}>
            <button
              onClick={() => {
                setSidePanel(sidePanel === 'accesos' ? 'mywork' : 'accesos');
                setSelected(null);
              }}
              style={{
                width: '100%',
          ...
          <div style={{ padding: 0 }}>
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                  Habilita o deshabilita módulos del Panel Admin por trabajador
                </div>
              </div>
            </div>
            <AccessControlTab />
