...
export type DocumentType = 'factura' | 'factura_exenta' | 'boleta' | 'boleta_honorarios' | 'invoice_internacional' | 'recibo' | 'comprobante_transferencia' | 'sin_documento'
export type PaymentMethod = 'transferencia' | 'tarjeta_debito' | 'tarjeta_credito' | 'efectivo' | 'pago_internacional' | 'debito_automatico' | 'cheque'
...
export const FISCAL_DEADLINES: FiscalDeadline[] = [
  {
    ...
    description: 'Declaracion y pago mensual de IVA (debito - credito fiscal). Incluye PPM si hay ingresos.',
    frequency: 'mensual',
    dayOfMonth: 12,
    month: null,
...
export const KNOWN_RECURRING_EXPENSES: RecurringExpense[] = [
  { description: 'Arriendo oficina + gastos comunes', provider: 'Miss Propiedades / Leasity', category: 'arriendo', currency: 'CLP', estimatedAmount: 697688, frequency: 'mensual', documentType: 'boleta', hasIVA: false, notes: 'Domicilio comercial declarado ante SII' },
  { description: 'Backend hosting (Starter)', provider: 'Render', category: 'hosting', currency: 'USD', estimatedAmount: 7, frequency: 'mensual', documentType: 'invoice_internacional', hasIVA: false, notes: 'FastAPI backend' },
  { description: 'Correo corporativo', provider: 'Zoho Mail', category: 'email_corporativo', currency: 'USD', estimatedAmount: 0, frequency: 'mensual', documentType: 'invoice_internacional', hasIVA: false, notes: 'Plan gratuito actualmente' },
  { description: 'Dominio conniku.com', provider: 'Namecheap', category: 'dominio_ssl', currency: 'USD', estimatedAmount: 12, frequency: 'anual', documentType: 'invoice_internacional', hasIVA: false, notes: '' },
  { description: 'Google Play Console', provider: 'Google', category: 'app_stores', currency: 'USD', estimatedAmount: 25, frequency: 'anual', documentType: 'invoice_internacional', hasIVA: false, notes: 'Pago unico developer account' },
  { description: 'Apple Developer Program', provider: 'Apple', category: 'app_stores', currency: 'USD', estimatedAmount: 99, frequency: 'anual', documentType: 'invoice_internacional', hasIVA: false, notes: 'Developer account iOS' },
  { description: 'API Claude (Anthropic)', provider: 'Anthropic', category: 'api_ia', currency: 'USD', estimatedAmount: 0, frequency: 'mensual', documentType: 'invoice_internacional', hasIVA: false, notes: 'Pago por uso (tokens)' },
  { description: 'Frontend hosting', provider: 'Vercel', category: 'hosting', currency: 'USD', estimatedAmount: 0, frequency: 'mensual', documentType: 'invoice_internacional', hasIVA: false, notes: 'Plan gratuito Hobby' },
]
...
export function getDeadlineStatus(deadline: FiscalDeadline, today: Date = new Date()): { status: DeadlineStatus; nextDate: Date; daysUntil: number } {
  ...
  if (deadline.frequency === 'mensual') {
    ...
    nextDate = thisYear >= today ? thisYear : new Date(currentYear + 1, deadline.month! - 1, deadline.dayOfMonth!)
  } else {
    // semestral
    const thisOccurrence = new Date(currentYear, deadline.month! - 1, deadline.dayOfMonth!)
    nextDate = thisOccurrence >= today ? thisOccurrence : new Date(currentYear + 1, deadline.month! - 1, deadline.dayOfMonth!)
  }
