...
export default function ImpuestosTab() {
  ...
  const periodTx = useMemo(() => getTransactionsByPeriod(month, year), [month, year]);
        ...
        <div>
          ...
          <div className="u-card" style={{ padding: 20, marginBottom: 20 }}>
            ...
            >
              ...
              <a
                href="https://www.sii.cl/servicios_online/1080-1082.html"
                target="_blank"
            ...
            {periodTx.length === 0 ? (
              <div style={{ padding: 30, textAlign: 'center', color: 'var(--text-muted)' }}>
                <p>
                  Sin transacciones para {MONTHS[month - 1]} {year}.
                </p>
                <p style={{ fontSize: 12 }}>
                  Los datos del F29 se calculan automaticamente desde las transacciones.
                </p>
              </div>
            ) : (
              <>
                    ...
                    <table style={{ width: '100%', fontSize: 13 }}>
                      <tbody>
                        ...
                        <tr style={{ borderTop: '2px solid var(--border)', fontWeight: 700 }}>
                          ...
                          <td style={{ textAlign: 'right', paddingTop: 8, color: '#f59e0b' }}>
                            {fmtCLP(summary.ivaDebito)}
                          </td>
                        </tr>
                      </tbody>
                    </table>
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td style={{ textAlign: 'right' }}>
                            {fmtCLP(summary.ivaCreditoRecuperable)}
                          </td>
                        </tr>
                        <tr style={{ borderTop: '2px solid var(--border)', fontWeight: 700 }}>
                          <td style={{ paddingTop: 8 }}>Total Credito IVA</td>
                          <td style={{ textAlign: 'right', paddingTop: 8, color: '#3b82f6' }}>
                            {fmtCLP(summary.ivaCreditoRecuperable)}
                          </td>
                        </tr>
                      </tbody>
                ...
                <div
                  style={{
                    ...
                    marginBottom: 16,
                    background:
                      summary.ivaAPagar > 0 ? 'rgba(239,68,68,0.08)' : 'rgba(34,197,94,0.08)',
                ...
                >
                  <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
                    {summary.ivaAPagar > 0
                      ...
                      : 'Remanente Credito Fiscal a Favor'}
                  </div>
                  <div
                    style={{
                      fontSize: 28,
          ...
          {periodTx.length > 0 && (
            <div className="u-card" style={{ padding: 20 }}>
              <h3 style={{ fontSize: 14, fontWeight: 700, marginBottom: 12 }}>
                Detalle de Transacciones del Periodo
              </h3>
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 8 }}>
                {periodTx.length} transacciones |{' '}
                {periodTx.filter((t) => t.type === 'ingreso').length} ingresos |{' '}
                {periodTx.filter((t) => t.type !== 'ingreso').length} egresos
              </div>
                  ...
                  .map((t) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span style={{ color: 'var(--text-muted)', fontSize: 11, width: 75 }}>
                          {t.date}
                        </span>
        ...
        <div>
          ...
          >
            <MiniCard
              ...
              value={fmtCLP(annualData.totalIngresos)}
              color="#22c55e"
            />
            <MiniCard
              label="Egresos Anuales"
              value={fmtCLP(annualData.totalEgresos)}
              color="#ef4444"
            />
            ...
            <MiniCard
              ...
              value={fmtCLP(annualData.totalDeducible)}
              color="#8b5cf6"
            />
            <MiniCard
              label="Resultado Tributario"
              value={fmtCLP(annualData.resultadoTributario)}
              color={annualData.resultadoTributario >= 0 ? '#22c55e' : '#ef4444'}
            />
          </div>
              ...
              <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: 'var(--bg-secondary)' }}>
                    ...
                    <th style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700 }}>
                      Ingresos
                    </th>
                    <th style={{ padding: '8px 10px', textAlign: 'right', fontWeight: 700 }}>
                      Egresos
                    ...
                    return (
                      ...
                      >
                        ...
                        <td style={{ padding: '8px 10px', textAlign: 'right', color: '#22c55e' }}>
                          {m.ingresos > 0 ? fmtCLP(m.ingresos) : '—'}
                        </td>
                        <td style={{ padding: '8px 10px', textAlign: 'right', color: '#ef4444' }}>
                          {m.egresos > 0 ? fmtCLP(m.egresos) : '—'}
                        </td>
                        <td style={{ padding: '8px 10px', textAlign: 'right', color: '#f59e0b' }}>
                          {m.ivaAPagar > 0 ? fmtCLP(m.ivaAPagar) : '—'}
                        </td>
                        <td style={{ padding: '8px 10px', textAlign: 'right', color: '#3b82f6' }}>
                          {m.ppm > 0 ? fmtCLP(m.ppm) : '—'}
                        </td>
                        <td style={{ padding: '8px 10px', textAlign: 'right', color: '#8b5cf6' }}>
                          {m.deducible > 0 ? fmtCLP(m.deducible) : '—'}
                        </td>
                      </tr>
                    );
                  })}
          ...
          <div className="u-card" style={{ padding: 20 }}>
            <h3 style={{ fontSize: 15, fontWeight: 700, marginBottom: 12 }}>
              Proyeccion Impuesto 1a Categoria
            </h3>
            <table style={{ width: '100%', fontSize: 14, borderCollapse: 'collapse' }}>
              <tbody>
                <tr>
                  <td style={{ padding: '8px 0' }}>Ingresos anuales (percibidos)</td>
                  <td style={{ textAlign: 'right' }}>{fmtCLP(annualData.totalIngresos)}</td>
                </tr>
                <tr style={{ color: '#ef4444' }}>
                  <td style={{ padding: '8px 0' }}>(-) Gastos deducibles (pagados)</td>
                  <td style={{ textAlign: 'right' }}>-{fmtCLP(annualData.totalDeducible)}</td>
                </tr>
                <tr style={{ borderTop: '2px solid var(--border)', fontWeight: 700 }}>
                ...
                <tr>
                  <td style={{ padding: '8px 0' }}>Impuesto 1a Categoria (25%)</td>
                  <td style={{ textAlign: 'right' }}>{fmtCLP(annualData.impuesto1aCat)}</td>
                </tr>
                <tr style={{ color: '#3b82f6' }}>
                  <td style={{ padding: '8px 0' }}>(-) PPM acumulado (credito)</td>
                  <td style={{ textAlign: 'right' }}>-{fmtCLP(annualData.ppmAcumulado)}</td>
                </tr>
                <tr
                  style={{
            ...
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 12 }}>
              * Regimen 14D N°3 ProPyme Transparente — tributacion en base a flujos de caja
              (ingresos percibidos menos gastos efectivamente pagados). La tasa de 25% aplica al
              resultado tributario positivo. Los PPM pagados durante el ano se descuentan como
              credito.
            </p>
          </div>
        </div>
      ...
      {activeView === 'f22' && (
        <div>
          <div style={{ display: 'flex', gap: 12, marginBottom: 20, alignItems: 'center' }}>
            ...
            <div style={{ flex: 1 }} />
            <a
              href="https://www.sii.cl/servicios_online/1080-1083.html"
              target="_blank"
              rel="noopener noreferrer"
              style={{
                display: 'flex',
          ...
          <div
            className="u-card"
            style={{ padding: 20, marginBottom: 20, borderLeft: '4px solid var(--accent)' }}
          >
            <h3 style={{ fontSize: 15, fontWeight: 700, margin: '0 0 8px' }}>Regimen Tributario</h3>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, fontSize: 13 }}>
              <div>
                <p>
                  <strong>Regimen:</strong> Art. 14D N°3 — ProPyme Transparente
                </p>
                <p>
                  <strong>Tasa 1a Categoria:</strong> 25%
                </p>
                <p>
                  <strong>Base:</strong> Flujo de caja (percibido - pagado)
                </p>
              </div>
              <div>
                <p>
                  <strong>Segmento:</strong> {COMPANY.segmento}
                </p>
                <p>
                  <strong>Depreciacion:</strong> Instantanea (activos fijos)
                </p>
                <p>
                  <strong>PPM:</strong> 0,25% sobre ingresos brutos
                </p>
              </div>
            </div>
          </div>
              ...
              <tbody>
                ...
                <tr>
                  <td style={{ padding: '10px 12px' }}>
                    Gastos Deducibles (segun % deducibilidad)
                  </td>
                ...
                <tr>
                  ...
                  <td style={{ padding: '10px 12px', textAlign: 'right' }}>
                    {fmtCLP(annualData.impuesto1aCat)}
                  </td>
                </tr>
                <tr style={{ color: '#3b82f6' }}>
                  <td style={{ padding: '10px 12px' }}>(-) PPM Acumulado (credito)</td>
                  <td style={{ padding: '10px 12px', textAlign: 'right' }}>
                    -{fmtCLP(annualData.ppmAcumulado)}
                  </td>
                </tr>
                <tr
                  style={{
                    ...
                    fontSize: 16,
                    background:
                      annualData.diferencia > 0 ? 'rgba(239,68,68,0.05)' : 'rgba(34,197,94,0.05)',
                  }}
                >
          ...
          <div className="u-card" style={{ padding: 20, borderLeft: '4px solid #f59e0b' }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, margin: '0 0 12px' }}>
              Plazos Declaraciones Anuales
            </h3>
            <div style={{ fontSize: 13, display: 'grid', gap: 8 }}>
              <DeadlineRow
                label="DJ 1887 — Sueldos Pagados"
                date="28 de Marzo"
                url="https://www.sii.cl/servicios_online/1080-1399.html"
              />
              <DeadlineRow
                label="Formulario 22 — Renta Anual"
                date="30 de Abril"
                url="https://www.sii.cl/servicios_online/1080-1083.html"
              />
              <DeadlineRow
                label="Patente Municipal — 1er Semestre"
                date="31 de Enero"
                url="https://www.municipalidadantofagasta.cl/"
              />
              <DeadlineRow
                label="Patente Municipal — 2do Semestre"
                date="31 de Julio"
                url="https://www.municipalidadantofagasta.cl/"
              />
            </div>
