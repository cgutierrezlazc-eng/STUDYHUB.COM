    ...
    <div style={{ padding: 24, maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div className="u-card" style={{ padding: 20, marginBottom: 24 }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <h2 style={{ fontSize: 18, fontWeight: 700, margin: 0 }}>Semaforo Fiscal</h2>
            <button
              onClick={() => {
                ...
                api
                  .checkFiscalAlerts()
                  .then((r: any) =>
                    setAlertStatus(`${r.alerts_sent?.length || 0} alertas enviadas`)
                  )
              ...
              >
                ...
                >
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    <span style={{ fontWeight: 600 }}>Prox:</span> {fmtDate(nextDate)} —{' '}
                    {deadline.platform}
                  </div>
                  <a
                    href={deadline.url}
      ...
      <div className="u-card" style={{ padding: 20, marginBottom: 24 }}>
        ...
        {txCount === 0 ? (
          <div style={{ padding: 30, textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <p style={{ fontSize: 12 }}>
              Ingresa transacciones en <strong>Gastos Operacionales</strong> para ver el resumen
              aqui.
            </p>
          </div>
        ) : (
          <>
            ...
            >
              <SummaryCard
                ...
                value={fmtCLP(summary.totalIngresos)}
                color="#22c55e"
              />
              <SummaryCard
                label="IVA Debito"
                value={fmtCLP(summary.ivaDebito)}
                color="#f59e0b"
                sub="IVA ventas (debes al fisco)"
              />
              <SummaryCard
                label="Total Egresos"
                value={fmtCLP(summary.totalEgresos)}
                color="#ef4444"
              />
              <SummaryCard
                label="IVA Credito Fiscal"
                value={fmtCLP(summary.ivaCreditoRecuperable)}
                color="#3b82f6"
                sub="IVA compras (te devuelven)"
              />
            </div>
            ...
            >
              <h3 style={{ fontSize: 15, fontWeight: 700, margin: '0 0 12px 0' }}>
                Calculo IVA Mensual (F29)
              </h3>
              <table style={{ width: '100%', fontSize: 13, borderCollapse: 'collapse' }}>
                <tbody>
                  <tr>
                    <td style={{ padding: '6px 0' }}>IVA Debito (ventas)</td>
                    <td style={{ textAlign: 'right', fontWeight: 600 }}>
                      {fmtCLP(summary.ivaDebito)}
                    </td>
                  </tr>
                  <tr style={{ color: '#3b82f6' }}>
                    <td style={{ padding: '6px 0' }}>(-) IVA Credito Fiscal (compras)</td>
                  ...
                  >
                    <td style={{ paddingTop: 10 }}>
                      {summary.ivaAPagar > 0 ? 'IVA a Pagar' : 'Remanente CF a Favor'}
                    </td>
                    <td style={{ textAlign: 'right', paddingTop: 10 }}>
                      {summary.ivaAPagar > 0
                        ? fmtCLP(summary.ivaAPagar)
                        : fmtCLP(summary.remanente)}
                    </td>
                  </tr>
                </tbody>
              </table>
      ...
      <div className="u-card" style={{ padding: 20, marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
          Exporta los datos del periodo seleccionado en formato CSV compatible con Excel para tu
          contador.
        </p>
        <div
          style={{
...
function exportLibroTransacciones(month: number, year: number) {
  ...
  const header =
    'Fecha;Tipo;Categoria;Descripcion;Proveedor;RUT Prov;Tipo Doc;N° Doc;Moneda;Monto Original;Tipo Cambio;Monto CLP;Neto;IVA;IVA Recuperable;Retencion;% Deducible;Monto Deducible;Metodo Pago;Recurrente';
  const dataRows: string[] = txs.map((t) =>
    [
      ...
      Math.round(t.retencion),
      t.deductiblePercent,
      Math.round(t.deductibleAmount),
      t.paymentMethod,
      t.recurring ? 'Si' : 'No',
    ].join(';')
  );
  downloadCSV(
    `transacciones_${year}_${String(month).padStart(2, '0')}.csv`,
...
function exportReporteAnual(year: number) {
  ...
  for (let m = 1; m <= 12; m++) {
    ...
    rows.push(
      `${MONTHS_SHORT[m - 1]};${Math.round(s.totalIngresos)};${Math.round(s.totalEgresos)};${Math.round(s.ivaAPagar)};${Math.round(s.ppm)};${Math.round(s.totalDeducible)};${Math.round(s.resultadoTributario)}`
    );
    totalIng += s.totalIngresos;
    totalEgr += s.totalEgresos;
    totalIVA += s.ivaAPagar;
    totalPPM += s.ppm;
    totalDed += s.totalDeducible;
  }
  const resultAnual = totalIng - totalDed;
  rows.push(
    `TOTAL;${Math.round(totalIng)};${Math.round(totalEgr)};${Math.round(totalIVA)};${Math.round(totalPPM)};${Math.round(totalDed)};${Math.round(resultAnual)}`
  );
  rows.push('');
