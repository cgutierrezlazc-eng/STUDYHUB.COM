...
import {
  TrendingUp, Users, BarChart3, Zap, Award, Star,
  PieChart, Target, BookOpen, ArrowUp
...
const sectionTitle: React.CSSProperties = {
  fontSize: 15, fontWeight: 700, marginBottom: 16, marginTop: 32,
...
const kpiCard: React.CSSProperties = {
  background: 'var(--bg-secondary)', borderRadius: 12, padding: '18px 16px',
  border: '1px solid var(--border-color)', position: 'relative', overflow: 'hidden',
}

const kpiValue: React.CSSProperties = {
  fontSize: 28, fontWeight: 800, lineHeight: 1.1, color: 'var(--text-primary)',
}
...
const miniBarContainer: React.CSSProperties = {
  display: 'flex', alignItems: 'flex-end', gap: 3, height: 32, marginTop: 8,
}
...
function SkeletonCard({ height = 100 }: { height?: number }) {
  return (
    <div style={{
      ...cardBase, height, background: 'var(--bg-tertiary)',
    ...
    <div style={miniBarContainer}>
      {data.map((v, i) => (
        <div key={i} style={{
          ...
          background: color, opacity: 0.6 + (i / data.length) * 0.4,
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {segments.map(s => (
          <div key={s.label} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
            ...
            <span style={{ fontWeight: 700, color: 'var(--text-primary)', marginLeft: 'auto' }}>{fmt(s.value)}</span>
          </div>
        ))}
      </div>
    </div>
      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={d.label} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 11, color: 'var(--text-muted)', width: 55, textAlign: 'right', flexShrink: 0 }}>{d.label}</span>
            ...
            <span style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-primary)', width: 30, flexShrink: 0 }}>{d.value}</span>
          </div>
        )
      })}
    </div>
...
// ─── Leaderboard Row ────────────────────────────────────────
function LeaderboardRow({ rank, name, value, unit }: { rank: number; name: string; value: number | string; unit?: string }) {
  ...
  return (
    <div style={tableRow}>
      ...
      <span style={{ fontWeight: 700, color: 'var(--accent)' }}>{value}{unit ? ` ${unit}` : ''}</span>
    </div>
  )
}
...
export default function AnalyticsTab() {
  ...
  // Referral data
  const totalReferrals = safe(fraudReport?.total_referrals, safe(financeDashboard?.referrals?.total))
  const weekReferrals = safe(fraudReport?.new_this_week, safe(weeklyReport?.referrals?.newThisWeek))
  const topReferrers = fraudReport?.top_referrers || []
  const referralConversion = safe(fraudReport?.conversion_rate, totalReferrals > 0 ? ((safe(fraudReport?.active_referred, 0) / totalReferrals) * 100) : 0)
  ...
  const engagementSpark = (field: string) => {
    if (weeklyReport?.engagementHistory?.[field]) return weeklyReport.engagementHistory[field].slice(-8)
    // Generate plausible sparkline from current value
    const current = safe(engagement[field], 5)
    return Array.from({ length: 8 }, (_, i) => Math.max(Math.round(current * (0.6 + Math.random() * 0.8)), 0))
  }
  ...
  if (loading) {
    return (
      <div style={{ padding: 4 }}>
        <SkeletonRow />
        <div style={{ marginTop: 24 }}><SkeletonCard height={220} /></div>
  ...
  return (
    <div style={{ padding: 4 }}>
      ...
      <div style={sectionTitle}>
        {Zap({ size: 18 })} Métricas de Engagement
      </div>
      ...
      <div style={sectionTitle}>
        {Award({ size: 18 })} Contenido Destacado
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 14 }}>
        ...
        <div style={cardBase}>
          ...
          {topCommunities.length > 0 ? topCommunities.slice(0, 5).map((c: any, i: number) => (
            <LeaderboardRow
              key={c.name || i}
              rank={i + 1}
        ...
        <div style={cardBase}>
          ...
          {topSubjects.length > 0 ? topSubjects.slice(0, 5).map((s: any, i: number) => (
            <LeaderboardRow
              key={s.name || i}
              rank={i + 1}
        ...
        <div style={cardBase}>
          ...
          {topStreaks.length > 0 ? topStreaks.map((u: any, i: number) => (
            <LeaderboardRow
      ...
      <div style={sectionTitle}>
        {PieChart({ size: 18 })} Desglose de Ingresos
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        <div style={cardBase}>
          <h4 style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 700 }}>Suscripciones por Tier</h4>
          <DonutChart segments={[
          ...
          <div style={{ marginTop: 16, display: 'flex', gap: 20 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 18, fontWeight: 700 }}>{fmtPct(totalUsers > 0 ? (totalPaid / totalUsers) * 100 : 0)}</div>
            </div>
          </div>
        </div>

        <div style={cardBase}>
          <h4 style={{ margin: '0 0 16px', fontSize: 13, fontWeight: 700 }}>Métricas Financieras</h4>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <MetricRow label="Ingresos brutos (USD)" value={fmtUsd(safe(weeklyReport?.revenue?.grossUsd))} />
            <MetricRow label="Ganancia neta (CLP)" value={`$${fmt(safe(weeklyReport?.revenue?.gananciaNetaClp))} CLP`} />
            <MetricRow label="Usuarios pagos" value={fmt(totalPaid)} />
            <MetricRow label="Tasa de pago" value={fmtPct(totalUsers > 0 ? (totalPaid / totalUsers) * 100 : 0)} />
          </div>
        </div>
      </div>
      ...
      <div style={sectionTitle}>
        {Target({ size: 18 })} Métricas de Referidos
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14, marginBottom: 32 }}>
        <div style={cardBase}>
          <div style={kpiLabel}>Total referidos</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-primary)' }}>{fmt(totalReferrals)}</div>
        </div>
        <div style={cardBase}>
          <div style={kpiLabel}>Referidos esta semana</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-primary)' }}>{fmt(weekReferrals)}</div>
        </div>
        <div style={cardBase}>
          <div style={kpiLabel}>Tasa de conversión</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-primary)' }}>{fmtPct(referralConversion)}</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>referido activo</div>
        </div>

        {/* Top Referrers leaderboard */}
        <div style={{ ...cardBase, gridColumn: topReferrers.length > 0 ? 'span 1' : undefined }}>
          <div style={kpiLabel}>Top Referrers</div>
          {topReferrers.length > 0 ? topReferrers.slice(0, 5).map((r: any, i: number) => (
            <LeaderboardRow
...
function KPICard({ label, value, trend, comparison, color, icon: Icon }: {
  label: string; value: string; trend: number; comparison: string
  color: string; icon: (p?: any) => React.ReactElement
}) {
  const isUp = trend >= 0
  return (
    <div style={kpiCard}>
      <div style={{
        position: 'absolute', top: 12, right: 12, opacity: 0.15,
        color,
      }}>
        {Icon({ size: 28 })}
      </div>
...
function EngagementCard({ label, value, unit, sparkData, color }: {
  label: string; value: number; unit?: string; sparkData: number[]; color: string
}) {
  return (
    <div style={cardBase}>
      <div style={{ fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.5px', color: 'var(--text-muted)' }}>{label}</div>
      <div style={{ fontSize: 24, fontWeight: 800, color: 'var(--text-primary)', marginTop: 4 }}>
        {fmt(value)}{unit ? <span style={{ fontSize: 14, fontWeight: 400, marginLeft: 2 }}>{unit}</span> : null}
      </div>
