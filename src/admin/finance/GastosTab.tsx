...
import {
  Receipt, Calculator, CheckCircle, FileText, Plus, Download, Trash2,
  Search, TrendingDown, TrendingUp, DollarSign, Filter, RefreshCw,
  AlertTriangle, Zap, ArrowUpDown, Edit3, X
} from 'lucide-react'
import {
...
export default function GastosTab() {
  ...
  useEffect(() => {
    fetch('https://mindicador.cl/api/dolar')
      .then(r => r.json())
      .then(d => { if (d?.serie?.[0]?.valor) setUsdRate(Math.round(d.serie[0].valor)) })
      .catch(() => {})
  }, [])
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <div style={{ padding: '4px 10px', background: 'rgba(34,197,94,0.1)', borderRadius: 8, fontSize: 11, color: '#22c55e', fontWeight: 600 }}>
            USD: ${usdRate}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 10, marginBottom: 16 }}>
        <SummaryCard icon={TrendingUp} label="Ingresos" value={summary.totalIngresos} color="#22c55e" sub={`IVA Debito: $${fmt(summary.ivaDebito)}`} />
        <SummaryCard icon={TrendingDown} label="Egresos" value={summary.totalEgresos} color="#ef4444" sub={`IVA Credito: $${fmt(summary.ivaCreditoRecuperable)}`} />
        <SummaryCard icon={Calculator} label="IVA a Pagar" value={summary.ivaAPagar} color={summary.ivaAPagar > 0 ? '#ef4444' : '#22c55e'} sub={summary.remanente > 0 ? `Remanente: $${fmt(summary.remanente)}` : 'Sin remanente'} />
        <SummaryCard icon={CheckCircle} label="Gasto Deducible" value={summary.totalDeducible} color="#3b82f6" sub={`PPM: $${fmt(summary.ppm)}`} />
        <SummaryCard icon={DollarSign} label="Resultado" value={summary.resultadoTributario} color={summary.resultadoTributario >= 0 ? '#22c55e' : '#ef4444'} sub="Ingresos - Gastos deducibles" />
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            style={{ padding: '6px 8px 6px 28px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 12, width: 180 }}
          />
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        {TX_TYPE_OPTIONS.map(t => (
          <FilterPill key={t.value} label={t.label} active={filterType === t.value} onClick={() => setFilterType(t.value)} color={t.color} />
        ))}
        <div style={{ width: 1, background: 'var(--border)', margin: '0 4px' }} />
        <FilterPill label="Todos" active={filterGroup === 'all'} onClick={() => setFilterGroup('all')} />
        {['costo_operacional', 'gasto_admin', 'gasto_ventas', 'remuneracion'].map(g => (
          <FilterPill key={g} label={g.replace(/_/g, ' ')} active={filterGroup === g} onClick={() => setFilterGroup(g)} />
        ))}
      </div>

      {/* Sort controls */}
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, fontSize: 11, color: 'var(--text-muted)', alignItems: 'center' }}>
        <span>{filtered.length} transacciones</span>
        <span>•</span>
        <button onClick={() => { setSortField('date'); setSortDir(d => d === 'asc' ? 'desc' : 'asc') }} style={sortBtn}>
          <ArrowUpDown size={10} /> Fecha {sortField === 'date' ? (sortDir === 'desc' ? '↓' : '↑') : ''}
        </button>
        <button onClick={() => { setSortField('amount'); setSortDir(d => d === 'asc' ? 'desc' : 'asc') }} style={sortBtn}>
          <ArrowUpDown size={10} /> Monto {sortField === 'amount' ? (sortDir === 'desc' ? '↓' : '↑') : ''}
        </button>
      </div>
      ...
      {filtered.length === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 6 }}>
          {filtered.map(tx => {
            ...
            return (
              <div key={tx.id} className="card" style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 10 }}>
                <span style={{ fontSize: 16, width: 28, textAlign: 'center' }}>{cat?.icon || '📋'}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontWeight: 600, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                    {tx.description}
                    {tx.recurring && <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 4, background: 'rgba(59,130,246,0.12)', color: '#3b82f6', fontWeight: 700 }}>REC</span>}
                  </div>
                  <div style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    {tx.date} • {tx.provider} • {cat?.name || tx.category} • {tx.documentType}{tx.documentNumber ? ` #${tx.documentNumber}` : ''}
                  </div>
                </div>
                <div style={{ textAlign: 'right', flexShrink: 0 }}>
                  ...
                  {tx.currency === 'USD' && <div style={{ fontSize: 10, color: 'var(--text-muted)' }}>USD ${tx.amountOriginal}</div>}
                  {tx.iva > 0 && <div style={{ fontSize: 10, color: '#3b82f6' }}>IVA: ${fmt(tx.iva)}</div>}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
                  {tx.deductiblePercent < 100 && tx.deductiblePercent > 0 && (
                    <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 4, background: 'rgba(245,158,11,0.12)', color: '#f59e0b', fontWeight: 700, textAlign: 'center' }}>{tx.deductiblePercent}%</span>
                  )}
                  {tx.taxDeductible && (
                    <span style={{ fontSize: 9, padding: '1px 5px', borderRadius: 4, background: 'rgba(34,197,94,0.12)', color: '#22c55e', fontWeight: 700, textAlign: 'center' }}>DED</span>
                  )}
                </div>
                <button onClick={() => { setEditingTx(tx); setShowAdd(true) }} style={iconBtn}><Edit3 size={13} /></button>
                <button onClick={() => handleDelete(tx.id)} style={iconBtn}><Trash2 size={13} /></button>
              </div>
            )
          })}
        </div>
      ...
      {filtered.length > 0 && (
        <div className="card" style={{ padding: 16, marginTop: 16 }}>
          <h4 style={{ margin: '0 0 10px', fontSize: 13, fontWeight: 700 }}>Resumen por Categoria</h4>
          {Object.entries(summary.byCategory).sort(([, a], [, b]) => (b as number) - (a as number)).map(([catKey, amount]) => {
            const cat = getCatInfo(catKey)
            return (
              <div key={catKey} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '5px 0', borderBottom: '1px solid var(--border)' }}>
                <span style={{ fontSize: 14, width: 24, textAlign: 'center' }}>{cat?.icon || '📋'}</span>
                <span style={{ flex: 1, fontSize: 12 }}>{cat?.name || catKey}</span>
                <span style={{ fontWeight: 700, fontSize: 13 }}>${fmt(amount as number)}</span>
                <span style={{ fontSize: 10, color: 'var(--text-muted)', width: 40, textAlign: 'right' }}>
                  {summary.totalEgresos > 0 ? Math.round(((amount as number) / (summary.totalEgresos || 1)) * 100) : 0}%
                </span>
              </div>
            )
          })}
...
// ═════════════════════════════════════════════════════════════════
function TransactionModal({ editing, month, year, usdRate, onClose, onSave }: {
  editing: Transaction | null
  month: number; year: number; usdRate: number
  onClose: () => void
  ...
  // Calculate amounts
  const amountCLP = form.currency === 'USD' ? Math.round(form.amountOriginal * usdRate) : form.amountOriginal
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 720, maxHeight: '92vh', overflow: 'auto', padding: 24 }}>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
          {TX_TYPE_OPTIONS.map(t => (
            <button key={t.value} onClick={() => setForm({ ...form, type: t.value })} style={{
              flex: 1, padding: '8px 6px', borderRadius: 8, border: form.type === t.value ? `2px solid ${t.color}` : '2px solid transparent',
        ...
        {suggestion && !form.category && (
          <div style={{ padding: '8px 12px', background: 'rgba(37,99,235,0.08)', borderRadius: 8, marginBottom: 12, display: 'flex', alignItems: 'center', gap: 8, fontSize: 12 }}>
            <Zap size={14} style={{ color: '#2563eb' }} />
            <span>Sugerencia: <strong>{suggestion.icon} {suggestion.name}</strong></span>
            <button onClick={() => applySuggestion(suggestion)} style={{ marginLeft: 'auto', padding: '3px 10px', borderRadius: 6, background: '#2563eb', color: '#fff', border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>
              Aplicar
            </button>
          </div>
        )}
        ...
        <div style={grid2}>
          ...
          <div>
            <Label>Fecha</Label>
            <input type="date" value={form.date} onChange={e => setForm({ ...form, date: e.target.value })} style={inputSm} />
          </div>

          {/* Description — triggers smart match */}
          <div>
            <Label>Descripcion *</Label>
            <input value={form.description} onChange={e => setForm({ ...form, description: e.target.value, category: form.category || '' })} placeholder="Ej: Hosting backend Render" style={inputSm} />
          </div>

          {/* Provider */}
          <div>
            <Label>Proveedor</Label>
            <input value={form.provider} onChange={e => setForm({ ...form, provider: e.target.value })} placeholder="Ej: Render, Leasity, Entel" style={inputSm} />
          </div>

          {/* Provider RUT */}
          <div>
            <Label>RUT Proveedor (si es chileno)</Label>
            <input value={form.providerRut} onChange={e => setForm({ ...form, providerRut: e.target.value })} placeholder="76.xxx.xxx-x" style={inputSm} />
          </div>
          ...
          <div>
            ...
            <select value={form.category} onChange={e => {
              ...
              setForm({ ...form, category: e.target.value, hasIVA: cat?.defaultIVA ?? false, currency: cat?.defaultCurrency ?? 'CLP', deductiblePercent: cat?.defaultDeductible ?? 100 })
            }} style={inputSm}>
              <option value="">— Seleccionar —</option>
              {['ingreso', 'costo_operacional', 'gasto_admin', 'gasto_ventas', 'inversion', 'remuneracion', 'prevision', 'impuesto'].map(group => {
          ...
          <div>
            ...
            <input value={form.documentNumber} onChange={e => setForm({ ...form, documentNumber: e.target.value })} placeholder="N° factura, boleta, etc." style={inputSm} />
          </div>

          {/* Currency */}
          <div>
            <Label>Moneda</Label>
            <select value={form.currency} onChange={e => setForm({ ...form, currency: e.target.value })} style={inputSm}>
          ...
          <div>
            ...
            <input type="number" value={form.amountOriginal || ''} onChange={e => setForm({ ...form, amountOriginal: Number(e.target.value) })} placeholder="0" style={inputSm} />
          </div>

          {/* Payment method */}
          <div>
            <Label>Medio de Pago</Label>
            <select value={form.paymentMethod} onChange={e => setForm({ ...form, paymentMethod: e.target.value })} style={inputSm}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <input type="range" min={0} max={100} step={5} value={form.deductiblePercent} onChange={e => setForm({ ...form, deductiblePercent: Number(e.target.value) })} style={{ flex: 1 }} />
              <span style={{ fontSize: 13, fontWeight: 700, minWidth: 40, textAlign: 'right' }}>{form.deductiblePercent}%</span>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 12, flexWrap: 'wrap' }}>
          ...
          {form.recurring && (
            <select value={form.recurringFrequency} onChange={e => setForm({ ...form, recurringFrequency: e.target.value })} style={{ ...inputSm, width: 'auto' }}>
        ...
        <div style={{ marginTop: 12 }}>
          <Label>Notas</Label>
          <input value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} placeholder="Observaciones opcionales..." style={inputSm} />
        </div>

        {/* Calculation preview */}
        <div style={{ marginTop: 16, padding: 12, background: 'var(--bg-tertiary)', borderRadius: 10, fontSize: 12 }}>
          <div style={{ fontWeight: 700, marginBottom: 6, color: 'var(--accent)' }}>Calculo Automatico</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
            {form.currency === 'USD' && <CalcRow label="Conversion" value={`USD $${form.amountOriginal} x $${usdRate} = $${fmt(amountCLP)}`} />}
            ...
            {retencion > 0 && <CalcRow label="Retencion (13.75%)" value={`$${fmt(retencion)}`} color="#f59e0b" />}
            <CalcRow label="Deducible" value={`$${fmt(deductibleAmount)} (${form.deductiblePercent}%)`} color="#22c55e" />
...
async function loadRecurring(month: number, year: number, usdRate: number, reload: () => Promise<void>, existing: Transaction[]) {
  ...
  for (const re of KNOWN_RECURRING_EXPENSES) {
    ...
    if (existing.some(t => t.provider.toLowerCase().includes(re.provider.toLowerCase().split('/')[0].trim()))) continue
...
function exportCSV(txs: Transaction[], month: number, year: number) {
  const headers = ['Fecha', 'Tipo', 'Descripcion', 'Proveedor', 'RUT Proveedor', 'Categoria', 'Documento', 'N° Doc', 'Moneda', 'Monto Original', 'Tasa Cambio', 'Monto CLP', 'Neto', 'IVA', 'IVA Recuperable', 'Retencion', 'Deducible %', 'Monto Deducible', 'Medio Pago', 'Recurrente', 'Notas']
  const rows = txs.map(t => [
    t.date, t.type, `"${t.description}"`, `"${t.provider}"`, t.providerRut || '', t.category, t.documentType, t.documentNumber,
    t.currency, t.amountOriginal, t.exchangeRate, t.amountCLP, t.neto, t.iva, t.ivaRecuperable ? 'SI' : 'NO',
    t.retencion, t.deductiblePercent, t.deductibleAmount, t.paymentMethod, t.recurring ? 'SI' : 'NO', `"${t.notes}"`
  ])
...
function FilterPill({ label, active, onClick, color }: { label: string; active: boolean; onClick: () => void; color?: string }) {
  return (
    <button onClick={onClick} style={{
      padding: '4px 10px', borderRadius: 20, border: 'none', fontSize: 10, fontWeight: 600, cursor: 'pointer',
...
function EmptyState({ month, year, onAdd, onLoadRecurring }: { month: string; year: number; onAdd: () => void; onLoadRecurring: () => void }) {
  return (
    <div className="card" style={{ padding: 32, textAlign: 'center' }}>
      <Receipt size={40} style={{ color: 'var(--text-muted)', marginBottom: 12, opacity: 0.4 }} />
      <h3 style={{ margin: '0 0 4px' }}>Sin transacciones en {month} {year}</h3>
      <p style={{ color: 'var(--text-muted)', fontSize: 13, marginBottom: 16 }}>
        Registra tu primera transaccion o carga los gastos recurrentes conocidos.
      </p>
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center' }}>
        <button onClick={onAdd} style={btnPrimary}><Plus size={14} /> Nueva Transaccion</button>
        <button onClick={onLoadRecurring} style={btnSecondary}><RefreshCw size={14} /> Cargar Recurrentes</button>
      </div>
    </div>
  )
}
...
function CalcRow({ label, value, color, bold }: { label: string; value: string; color?: string; bold?: boolean }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', padding: '2px 0' }}>
      ...
      <span style={{ fontWeight: bold ? 700 : 400, color: color || 'var(--text-primary)' }}>{value}</span>
    </div>
  )
}

function Label({ children }: { children: React.ReactNode }) {
  return <label style={{ fontSize: 11, fontWeight: 600, display: 'block', marginBottom: 3, color: 'var(--text-muted)' }}>{children}</label>
}

// Shared inline styles
const inputSm: React.CSSProperties = { width: '100%', padding: '7px 10px', borderRadius: 7, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 12 }
const selectSm: React.CSSProperties = { padding: '6px 10px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 12 }
const sortBtn: React.CSSProperties = { background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', fontSize: 11, display: 'flex', alignItems: 'center', gap: 3, padding: 0 }
