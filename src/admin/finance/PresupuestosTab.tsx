...
export default function PresupuestosTab() {
  const { user } = useAuth();
  const [activeView, setActiveView] = useState<'presupuesto' | 'varianzas' | 'historico'>(
    'presupuesto'
  );
    ...
    <div style={{ padding: '24px', maxWidth: 1400, margin: '0 auto' }}>
        ...
        {[
          {
            ...
            value: `$${fmt(totals.presupuesto)}`,
            color: 'var(--accent)',
          },
          {
            label: 'Ejecutado',
            value: `$${fmt(totals.ejecutado)}`,
            sub: `${ejecucionPct.toFixed(1)}%`,
            color: '#10b981',
          },
          { label: 'Comprometido', value: `$${fmt(totals.comprometido)}`, color: '#f59e0b' },
          {
            ...
            color: disponible >= 0 ? '#3b82f6' : '#ef4444',
          },
        ].map((card, i) => (
          <div
            key={i}
      ...
      >
        ...
        >
          <div
            style={{
              width: `${Math.min(ejecucionPct, 100)}%`,
              height: '100%',
        ...
        <div style={{ display: 'flex', gap: 16, marginTop: 6, fontSize: 10 }}>
          ...
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span
              style={{ width: 8, height: 8, borderRadius: 2, background: 'rgba(245,158,11,0.4)' }}
            />{' '}
            Comprometido
          </span>
          <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            <span style={{ width: 8, height: 8, borderRadius: 2, background: 'var(--border)' }} />{' '}
            Disponible
          </span>
        </div>
      </div>
        ...
        <div style={{ borderRadius: 12, border: '1px solid var(--border)', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
                  ...
                  <tr key={b.id} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      {editingId === b.id ? (
                        <input
                          ...
                          onBlur={() => {
                            setBudget(
                              budget.map((x) =>
                                x.id === b.id ? { ...x, presupuesto: +editValue } : x
                              )
                            );
                            setEditingId(null);
                          }}
                        ...
                        <span
                          onClick={() => {
                            setEditingId(b.id);
                            setEditValue(b.presupuesto.toString());
                          }}
                    ...
                    <td
                      style={{
                        ...tdStyle,
                        textAlign: 'right',
                        color: b.comprometido > 0 ? '#f59e0b' : 'var(--text-muted)',
                      }}
                    >
                      ${fmt(b.comprometido)}
                    </td>
              ...
              {/* Totals */}
              <tr
                style={{ background: 'var(--bg-secondary)', borderTop: '3px solid var(--border)' }}
              >
                <td style={{ ...tdStyle, fontWeight: 800, fontSize: 13 }}>TOTAL</td>
                <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>
                  ${fmt(totals.presupuesto)}
                </td>
                <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>
                  ${fmt(totals.ejecutado)}
                </td>
                <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700, color: '#f59e0b' }}>
                  ${fmt(totals.comprometido)}
                </td>
                <td
                  style={{
                    ...tdStyle,
                    textAlign: 'right',
                    fontWeight: 800,
                    color: disponible >= 0 ? '#10b981' : '#ef4444',
                  }}
                >
                  ${fmt(disponible)}
                </td>
                <td style={{ ...tdStyle, textAlign: 'right', fontWeight: 700 }}>
                  {ejecucionPct.toFixed(1)}%
                </td>
                <td style={tdStyle}></td>
              </tr>
            </tbody>
      ...
      {activeView === 'varianzas' && (
        <div>
          ...
          >
            ...
            <br />
            <strong>Varianza desfavorable:</strong> El gasto real excede el presupuesto
            (sobrecosto).
          </div>
        </div>
      )}
            ...
            <div style={{ display: 'flex', alignItems: 'flex-end', gap: 16, height: 200 }}>
              {monthlyHistory.map((m, i) => {
                const maxVal = Math.max(
                  ...monthlyHistory.map((h) => Math.max(h.presupuesto, h.ejecutado))
                );
                    ...
                    <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td
                        style={{
                          ...
                          textAlign: 'right',
                          fontWeight: 600,
                          color: varianza >= 0 ? '#10b981' : '#ef4444',
                        }}
                      >
