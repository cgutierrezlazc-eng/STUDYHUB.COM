...
function KPI({
  ...
  const trendColor =
    trend === undefined ? undefined : trend > 0 ? '#22c55e' : trend < 0 ? '#ef4444' : '#94a3b8';
...
const QUICK_LINKS = [
  ...
  { label: 'Workflows Inteligentes', icon: '🤖', route: '/admin-panel/tools/ai-workflows' },
...
export default function CeoOverview({ onNavigate }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([api.getCeoWeeklyReport().catch(() => null), api.getEmployees().catch(() => [])])
      .then(([rpt, emps]) => {
        setReport(rpt);
      ...
      >
        ...
        <div style={{ textAlign: 'right' }}>
          ...
          <div style={{ fontSize: 12, opacity: 0.6, marginTop: 4 }}>
            {r.generatedAt
              ? `Actualizado: ${new Date(r.generatedAt).toLocaleTimeString('es-CL', { hour: '2-digit', minute: '2-digit' })}`
              : 'Sin datos de API'}
          </div>
          {mod.newReports > 0 && (
            <div
            ...
            >
              ⚠️ {mod.newReports} reporte{mod.newReports > 1 ? 's' : ''} pendiente
              {mod.newReports > 1 ? 's' : ''}
            </div>
          )}
        </div>
        ...
        <>
          ...
          <Section title="Ingresos — Semana">
            ...
            >
              <KPI
                label="Ingreso Bruto"
                value={fmtUsd(rev.grossUsd)}
                sub={fmtClp(rev.grossClp)}
                color="#22c55e"
                trend={rev.growthPercent}
              />
              <KPI
                label="Neto tras Comisiones"
                value={fmtClp(rev.netAfterStripeClp)}
                sub="IVA incluido"
                color="#16a34a"
              />
              <KPI
                label="IVA (19%)"
                value={fmtClp(rev.ivaClp)}
                sub="Declarable en F29"
                color="#f59e0b"
              />
              <KPI
                label="Ganancia Neta"
                value={fmtClp(rev.gananciaNetaClp)}
                sub="Libre de impuestos"
                color="#3b82f6"
              />
              <KPI
                label="Pagos Esta Semana"
                value={r.subscriptions?.newPayments || 0}
          ...
          <Section title="Usuarios">
            ...
            >
              <KPI
                ...
                value={(usr.total || 0).toLocaleString('es-CL')}
                sub="Registros acumulados"
              />
              <KPI
                label="Nuevos Esta Semana"
                value={usr.newThisWeek || 0}
                sub={`Semana anterior: ${usr.prevWeekNew || 0}`}
                color="#3b82f6"
                trend={usr.growthPercent}
              />
              <KPI
                label="Activos Esta Semana"
                value={usr.activeThisWeek || 0}
                sub={`Tasa: ${usr.activeRate || 0}%`}
                color="#8b5cf6"
              />
              <KPI
                label="Referidos Esta Semana"
                value={usr.referralsThisWeek || 0}
                sub="Vía código de referido"
                color="#f59e0b"
              />
            </div>
          </Section>
          ...
          <Section title="Engagement">
            ...
            >
              <KPI
                ...
                value={`${eng.studyHours || 0}h`}
                sub={`${health.studyHoursPerUser || 0}h/usuario activo`}
                color="#22c55e"
              />
              <KPI
                label="Quizzes Realizados"
                value={eng.quizzesTaken || 0}
                sub="Esta semana"
                color="#3b82f6"
              />
              <KPI
                label="Posts en Feed"
                value={eng.wallPosts || 0}
                sub={`${health.postsPerUser || 0} posts/usuario`}
                color="#f59e0b"
              />
              <KPI
                label="Posts Comunidades"
                value={eng.communityPosts || 0}
                sub="En grupos y comunidades"
              />
              <KPI
                label="Mensajes"
                value={eng.messages || 0}
                sub="Directos + grupales"
                color="#8b5cf6"
              />
              <KPI
                label="Recompensas Otorgadas"
                value={r.subscriptions?.rewardsGranted || 0}
                sub="Logros + bonos EXP"
                color="#f59e0b"
              />
            </div>
          </Section>
          ...
          <Section title="Recursos Humanos">
            ...
            >
              <KPI
                ...
                sub="Dotación registrada"
                color="#22c55e"
              />
              <KPI
                label="Total en Nómina"
                value={employees.length}
                sub="Incluye todos los estados"
              />
            </div>
          </Section>
            ...
            >
              <div className="card" style={{ padding: '16px 20px' }}>
                ...
                <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 4 }}>
                  Usuarios activos por día (estimado)
                </div>
                <div
                  style={{
                    marginTop: 12,
                ...
                >
                  <div
                    style={{
                      ...
                      borderRadius: 3,
                      background: '#3b82f6',
                      width: `${Math.min(((health.dau_wau || 0) / Math.max(usr.total || 1, 1)) * 100 * 7, 100)}%`,
                    }}
                  />
                </div>
              </div>
              <div className="card" style={{ padding: '16px 20px' }}>
                ...
                <div
                  style={{
                    fontSize: 22,
                    fontWeight: 800,
                    color: mod.newReports > 0 ? '#ef4444' : '#22c55e',
                  }}
                >
