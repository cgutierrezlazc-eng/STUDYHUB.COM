...
| 9 | `src/pages/Start.tsx:452,481,507` | TODOs de cableado de backend (login endpoint) y navegación a rutas que no existen. El onboarding visualmente completo pero funcionalidad de crear cuenta real no cableada. | 🚨 GRAVE — el flujo principal de registro no está conectado al backend en la versión actual de `App.tsx`. |
| 10 | `backend/cleanup_production_db.py` | Script que elimina todos los usuarios excepto el owner, con `print()` extenso. No debería estar en el mismo directorio del backend productivo — riesgo de ejecución accidental. | 🚨 GRAVE — potencial pérdida masiva de datos si se ejecuta por error en producción. |
